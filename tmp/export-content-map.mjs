...
for (const file of walk(contentDir)) {
  ...
  for (const exported of Object.values(mod)) {
    if (
      ...
      if (typeof exported.id === "string") lessonFileById.set(exported.id, rel);
      if (exported.chapter !== undefined) {
        lessonFileBySlugChapter.set(
          `${exported.chapter}/${exported.slug}`,
          rel,
        );
      }
    }
...
const courseMap = new Map(COURSES.map((c) => [c.key, { ...c, chapters: [] }]));
