export default {
  ...
  subtitle: 'When a matrix rotates space, real eigenvectors vanish â€” and algebra forces us into complex numbers that beautifully encode both rotation and scaling.',
  ...
  intuition: {
    prose: [
      'Take the rotation matrix $A = \\begin{bmatrix}0&-1\\\\1&0\\end{bmatrix}$ (90Â° counterclockwise). The characteristic equation: $\\det(A - \\lambda I) = \\lambda^2 + 1 = 0$, so $\\lambda = \\pm i$. No real solutions. Yet the algebra forced a clean answer: eigenvalue $i = 0+1\\cdot i$ has magnitude $|i| = 1$ (no scaling), angle $\\arg(i) = 90Â°$ (rotation by 90Â°). After $k$ applications of $A$: eigenvalue $i^k$ spirals around the unit circle. After 4 steps: $i^4 = 1$ â€” back to the start. The imaginary number $i$ is encoding the rotation you can see geometrically.',
      '**Where you are in the story:** You know how to find eigenvalues from the characteristic equation $\\det(A - \\lambda I) = 0$, and you know how to diagonalize a matrix when it has enough real eigenvectors. But what about a matrix that only rotates â€” like a 90Â° rotation? Geometrically, it pushes every single vector off its original line. There are no real eigenvectors at all.',
      'Yet the characteristic equation is a polynomial â€” and by the Fundamental Theorem of Algebra, every polynomial with real coefficients has roots. They might just be complex numbers. This is not a failure of linear algebra; it is algebra doing exactly what it is supposed to do.',
      ...
      'For the 90Â° rotation matrix, $\\lambda = i = 0 + 1\\cdot i$. The real part is 0 (no stretching), the imaginary part is 1. Magnitude = $\\sqrt{0^2+1^2} = 1$ (no scaling), angle = 90Â° (pure rotation). Exactly what a rotation matrix should do.',
      ...
      '**Why this matters practically.** If you are analyzing a dynamical system â€” anything that evolves over time â€” the eigenvalues of its matrix tell you everything about long-term behavior: do things grow, shrink, or oscillate? Complex eigenvalues with $|\\lambda| > 1$ mean spiral growth. $|\\lambda| < 1$ means spiral decay toward equilibrium. $|\\lambda| = 1$ means perfect sustained oscillation. The eigenvalues are the system\'s fate, written in complex numbers.',
      '**CNC spindle control â€” when complex eigenvalues cause chatter.** A CNC spindle speed controller has a transfer function with a characteristic equation â€” and its roots are eigenvalues. When all roots are real and negative (continuous-time) or inside the unit circle (discrete-time), the spindle holds stable speed. When a root crosses into the right half-plane (or outside the unit circle), the controller becomes unstable and the spindle oscillates â€” this is **chatter**, the ringing vibration that ruins surface finish and tool life. Complex eigenvalues near the stability boundary ($|\\lambda| \\approx 1$) produce lightly-damped oscillations: the spindle "rings" but eventually settles. The frequency of the chatter ring is exactly $\\omega = \\arctan(b/a)$ radians per sample â€” the imaginary part of the eigenvalue.',
    ...
    callouts: [
      {
        type: 'sequencing',
        title: 'Lesson 3 of 7 â€” Eigenvalues & Eigenvectors',
        body: '**Previous (Lesson 2):** Diagonalization â€” rebuilding coordinates in the eigenvector basis.\n**This lesson:** Complex eigenvalues â€” what rotation looks like algebraically, and how $a + bi$ encodes spinning and scaling.\n**Next (Lesson 4):** Jordan Normal Form â€” what to do when a matrix cannot be diagonalized.',
      },
      {
        type: 'insight',
        title: 'Complex Eigenvalue Geometry',
        body: 'For $\\lambda = a + bi$:\n\nâ€¢ $r = |\\lambda| = \\sqrt{a^2 + b^2}$ â†’ stretch factor\nâ€¢ $\\theta = \\arctan(b/a)$ â†’ rotation angle per step\n\n$r > 1$: spirals outward\n$r = 1$: pure rotation (circle)\n$r < 1$: spirals inward (decays)',
      },
      {
        type: 'theorem',
      ...
      },
      {
        ...
        body: 'A matrix that purely rotates space (no fixed directions) has no real eigenvectors. This is not a problem â€” it just means diagonalization over $\\mathbb{R}$ fails. Over $\\mathbb{C}$ (complex numbers), the matrix is still diagonalizable.',
      },
    ],
    visualizations: [
      {
  ...
  math: {
    prose: [
      '**The 90Â° rotation matrix.** The standard counterclockwise 90Â° rotation is:\n\n$A = \\begin{bmatrix}0 & -1 \\\\ 1 & 0\\end{bmatrix}$\n\nThe characteristic equation:\n\n$\\det(A - \\lambda I) = \\det\\begin{bmatrix}-\\lambda & -1 \\\\ 1 & -\\lambda\\end{bmatrix} = \\lambda^2 + 1 = 0$\n\nSolving: $\\lambda^2 = -1 \\Rightarrow \\lambda = \\pm i$. The eigenvalues are $i$ and $-i$ â€” a conjugate pair.',
      '**General rotation by angle $\\theta$.** The counterclockwise rotation matrix is:\n\n$R_\\theta = \\begin{bmatrix}\\cos\\theta & -\\sin\\theta \\\\ \\sin\\theta & \\cos\\theta\\end{bmatrix}$\n\nIts characteristic equation is $\\lambda^2 - 2\\cos\\theta\\,\\lambda + 1 = 0$, giving:\n\n$\\lambda = \\cos\\theta \\pm i\\sin\\theta = e^{\\pm i\\theta}$\n\nThe eigenvalues are points on the unit circle at angle $\\pm\\theta$ in the complex plane. Their magnitude is always 1 â€” pure rotation, no scaling.',
      ...
      '**Euler\'s formula connects everything.** Euler\'s formula states $e^{i\\theta} = \\cos\\theta + i\\sin\\theta$. This means a complex eigenvalue $a + bi$ can be written in polar form as $re^{i\\theta}$, where $r = \\sqrt{a^2+b^2}$ and $\\theta = \\arctan(b/a)$. Repeated application of the eigenvalue is $r^n e^{in\\theta}$ â€” a spiral in the complex plane.',
    ],
    callouts: [
      {
        type: 'definition',
      ...
      },
      {
        ...
        body: 'For a system $\\mathbf{x}_{n+1} = A\\mathbf{x}_n$:\n\nâ€¢ All $|\\lambda_i| < 1$ â†’ stable (converges to origin)\nâ€¢ Any $|\\lambda_i| > 1$ â†’ unstable (blows up)\nâ€¢ All $|\\lambda_i| = 1$ â†’ neutral (oscillates forever)\n\nThis is the eigenvalue stability criterion.',
      },
    ],
    visualizations: [
      {
        ...
        initialProps: {
          initialCells: [
            {
              id: 1,
              cellTitle: 'Complex eigenvalues with eig() â€” magnitude and angle',
              prose: '`[V, D] = eig(A)` works for any matrix, even with complex eigenvalues. Extract real/imaginary parts with `real()` and `imag()`. `abs(lambda)` gives magnitude r. `angle(lambda)` gives angle Î¸ in radians. If |Î»| < 1 â†’ stable; > 1 â†’ unstable; = 1 â†’ neutral.',
...
fprintf('\\n|Î»| < 1 â†’ stable? %d\\n', all(abs(lambda) < 1))`,
            },
            {
              ...
              prose: 'For a pure rotation by Î¸, the eigenvalues are e^(Â±iÎ¸) â€” points on the unit circle. |Î»| = 1 always, so pure rotations are neither stable nor unstable. Varying Î¸ sweeps the eigenvalues around the unit circle. The trace = 2cosÎ¸ tracks the real part.',
              code: `% Sweep rotation angle and watch eigenvalues
angles_deg = 0:30:330;

fprintf('Î¸         eigenvalues                  |Î»|     trace\\n')
fprintf('--------------------------------------------------------\\n')
for theta_deg = angles_deg
    theta = deg2rad(theta_deg);
    R = [cos(theta) -sin(theta); sin(theta) cos(theta)];
    lambda = eig(R);
    fprintf('%3dÂ°     %.3f Â± %.3fi        %.4f   %.4f\\n', ...
        theta_deg, real(lambda(1)), abs(imag(lambda(1))), abs(lambda(1)), trace(R))
            ...
            },
            {
              ...
              prose: 'A discrete-time spindle speed controller has a state matrix whose eigenvalues determine stability. Complex eigenvalues near the unit circle produce lightly-damped oscillation â€” this is chatter. The frequency of chatter oscillation is Ï‰ = angle(Î») radians per sample. For a 1 kHz control loop and angle Î¸, the chatter frequency is Î¸/(2Ï€) Ã— 1000 Hz.',
...
for k = 1:length(lambda)
    lk = lambda(k);
    fprintf('  Î»%d = %.4f + %.4fi\\n', k, real(lk), imag(lk))
    fprintf('     |Î»| = %.6f  (must be < 1 for stability)\\n', abs(lk))
    if abs(imag(lk)) > 1e-6
...
fprintf('\\nController is %s\\n', ternary(stable, 'STABLE', 'UNSTABLE'))`,
      ...
      },
      {
        ...
        mathBridge: 'The complex plane is shown with the unit circle. Each eigenvalue $\\lambda = a + bi$ appears as a point at coordinates $(a, b)$. Drag the eigenvalue point and watch the trajectory of a starting vector as the matrix is applied repeatedly. Points inside the unit circle â†’ spiral in. Outside â†’ spiral out. On the circle â†’ orbit. The angle $\\theta$ from the positive real axis is the rotation angle per step.',
        caption: 'The unit circle separates convergence from divergence.',
      },
      {
        id: 'PythonNotebook',
        title: 'Code: Complex Eigenvalues and Spirals',
        mathBridge: 'np.linalg.eig() returns complex eigenvalues automatically. |Î»| = np.abs(Î»). Angle Î¸ = np.angle(Î»). If |Î»| < 1 â†’ spirals in; |Î»| > 1 â†’ spirals out; |Î»| = 1 â†’ pure rotation.',
...
fig.show()`,
            },
            {
              ...
              difficulty: 'medium',
              prompt: 'For each matrix below, compute its eigenvalues, find |Î»| for each, and classify as: stable (all |Î»| < 1), unstable (any |Î»| > 1), or neutral (all |Î»| = 1). Print your classification and the reason.',
              code: `import numpy as np
...
C = np.array([[0.5, 0.2],
              [0.0, 0.8]])   # triangular â€” eigenvalues are diagonal entries

# For each: compute evals, |Î»|, classify stability
`,
              hint: 'evals, _ = np.linalg.eig(A). magnitudes = np.abs(evals). All < 1 â†’ stable. Any > 1 â†’ unstable. All = 1 â†’ neutral. Use np.allclose(magnitudes, 1) for the neutral check.',
            },
          ]
        }
      },
  ...
  examples: [
    {
      ...
      problem: 'Find the eigenvalues of the 90Â° rotation matrix $A = \\begin{bmatrix}0&-1\\\\1&0\\end{bmatrix}$ and interpret them geometrically.',
      steps: [
        {
        ...
        },
        {
          ...
          annotation: 'No real solutions â€” imaginary roots. This confirms: no real vector stays on its line after a 90Â° rotation.',
          strategyTitle: 'Solve for eigenvalues',
          checkpoint: '',
          hints: [],
        },
        {
          expression: '|\\lambda| = \\sqrt{0^2 + 1^2} = 1, \\quad \\theta = \\arctan(1/0) = 90Â°',
          annotation: 'Magnitude 1: no stretching. Angle 90Â°: rotates each step by exactly 90Â°. The eigenvalue IS the rotation â€” written as a complex number.',
          strategyTitle: 'Interpret geometrically',
          checkpoint: 'What happens after 4 applications of $A$?',
          hints: ['$\\lambda^4 = i^4 = 1$ â€” back to the identity. Four 90Â° rotations = 360Â° = no net rotation. Makes sense geometrically.'],
        },
      ],
      conclusion: 'Eigenvalues $\\pm i$ encode a pure 90Â° rotation with no scaling. Repeated application traces a perfect circle. After 4 steps, every vector returns to its starting position.',
    },
    {
      id: 'la3-003-ex2',
      ...
      steps: [
        {
          ...
          hints: ['$r \\approx 0.707 < 1$. So the transformation shrinks â€” vectors spiral inward.'],
        },
        {
          expression: '\\theta = \\arctan\\!\\left(\\frac{0.5}{0.5}\\right) = \\arctan(1) = 45Â°',
          annotation: 'Each application rotates by 45Â°.',
          strategyTitle: 'Compute rotation angle',
          checkpoint: '',
          hints: [],
        },
        {
          expression: '\\text{After } k \\text{ steps: scale by } r^k = \\left(\\tfrac{1}{\\sqrt{2}}\\right)^k, \\text{ rotate by } 45kÂ°',
          annotation: 'The vector spirals inward (shrinking) while rotating. After 8 steps: $45Â° \\times 8 = 360Â°$ (full circle), but $r^8 = (1/\\sqrt{2})^8 = 1/16$ â€” now 16 times smaller.',
        ...
        },
        {
          ...
          annotation: 'Long-term: the vector spirals toward the origin. The system is stable â€” all vectors converge to zero.',
          strategyTitle: 'Long-term behavior',
          checkpoint: '',
          hints: [],
        },
      ],
      conclusion: '$\\lambda = 0.5 + 0.5i$ produces a stable spiral: each step rotates 45Â° and shrinks by $1/\\sqrt{2} \\approx 0.707$. In the long run, every vector decays to the origin. The system is stable.',
    },
    {
      id: 'la3-003-ex3',
      ...
      steps: [
        ...
        },
        {
          ...
          hints: ['$\\theta = \\arctan(2/3) \\approx 33.7Â°$ per step.'],
        },
      ],
      conclusion: 'For conjugate pair eigenvalues $a \\pm bi$: trace $= 2a$ (real), determinant $= a^2 + b^2 > 0$ (always positive, always invertible). The magnitude $\\sqrt{a^2+b^2}$ determines stability â€” exactly the same as the determinant.',
    },
  ],
  ...
  challenges: [
    {
      ...
      walkthrough: [
        ...
        },
        {
          ...
          annotation: '$r > 1$: vectors spiral outward â€” the system is unstable.',
        },
      ],
      answer: 'Î» = 1 Â± 2i, |Î»| = âˆš5 > 1, spirals outward (unstable)',
    },
    {
      id: 'la3-003-ch2',
      difficulty: 'medium',
      ...
      walkthrough: [
        {
          ...
          annotation: 'Magnitude is 0.8 â€” less than 1.',
        },
        {
          expression: '\\theta = \\arctan(-0.8 / 0) = -90Â°',
          annotation: 'The imaginary axis has $a = 0$, so the angle is $\\pm 90Â°$. Negative imaginary part â†’ $-90Â°$ (clockwise rotation).',
        },
        {
          expression: 'r = 0.8 < 1 \\Rightarrow \\text{spiral inward â€” stable}',
          annotation: 'Each step rotates 90Â° clockwise and shrinks by 0.8. Converges to origin.',
        },
      ],
      answer: 'r = 0.8 < 1, Î¸ = -90Â° (clockwise), spirals inward â€” stable convergence',
    },
    {
      id: 'la3-003-ch3',
      difficulty: 'hard',
      ...
      walkthrough: [
        ...
        },
        {
          ...
          annotation: 'Magnitude 5 â€” each step grows by factor 5. Vectors spiral outward rapidly.',
        },
      ],
      answer: 'trace = 6, det = 25, |Î»| = 5 > 1 â†’ unstable (spirals out)',
    },
  ],
  ...
  semantics: {
    core: [
      ...
      { symbol: 'r = |\\lambda| = \\sqrt{a^2+b^2}', meaning: 'Magnitude â€” the stretch/shrink factor per step' },
      { symbol: '\\theta = \\arctan(b/a)', meaning: 'Angle â€” the rotation per step in radians (or degrees)' },
      { symbol: 'e^{i\\theta} = \\cos\\theta + i\\sin\\theta', meaning: "Euler's formula â€” connects complex exponentials to rotation" },
    ],
    rulesOfThumb: [
      '|Î»| > 1 â†’ unstable (grows/spirals out). |Î»| < 1 â†’ stable (decays/spirals in). |Î»| = 1 â†’ neutral (orbits).',
      ...
      'Pure imaginary eigenvalues (a=0) â†’ pure rotation with no scaling.',
    ],
  },
    ...
    recoveryPoints: [
      {
        ...
        note: 'Complex eigenvalues come from the same $\\det(A - \\lambda I) = 0$ equation â€” just with no real roots. The quadratic formula gives complex solutions when the discriminant $b^2 - 4ac < 0$.',
      },
    ],
    futureLinks: [
      {
        ...
        note: 'Phase 4 moves away from eigenvalues to projections and decompositions. SVD at the end of Phase 4 brings everything together â€” including what to do with non-square matrices that have no eigenvalues at all.',
      },
    ],
  },
  ...
  mentalModel: [
    'No real eigenvectors â†’ characteristic polynomial has complex roots.',
    'Î» = a + bi encodes rotation (b) and scaling (a) in one number.',
    'Magnitude |Î»| = âˆš(aÂ²+bÂ²) tells you if it grows, shrinks, or stays constant.',
    'Real matrices â†’ complex eigenvalues always come in conjugate pairs aÂ±bi.',
    '|Î»| < 1 â†’ stable (decays). |Î»| > 1 â†’ unstable (grows). |Î»| = 1 â†’ oscillates.',
  ],
  ...
  checkpoints: [
    ...
    { id: 'cp-la3-003-2', label: 'Read: follow why real matrices produce conjugate pairs and how they factor through real 2Ã—2 blocks', type: 'read' },
    { id: 'cp-la3-003-3', label: 'Read: understand the stability condition |Î»| < 1 and the Jury/Lyapunov connection', type: 'read' },
    { id: 'cp-la3-003-4', label: 'Run the lab visualization â€” see vectors spiraling under repeated matrix application', type: 'lab' },
    { id: 'cp-la3-003-5', label: 'Run Python/OpenMAT code â€” compute complex eigenvalues and trace the spiral trajectory', type: 'lab' },
    { id: 'cp-la3-003-6', label: 'Complete example 1: find complex eigenvalues of the 90Â° rotation matrix', type: 'example' },
    { id: 'cp-la3-003-7', label: 'Complete example 2: analyze a spiral matrix using |Î»| and arg(Î»)', type: 'example' },
  ...
  quiz: [
    {
      ...
      reviewSection: 'Intuition tab â€” Complex Eigenvalue Geometry',
    },
    {
      ...
      hints: ['Conjugating $A\\mathbf{v} = \\lambda\\mathbf{v}$: since $A$ is real, $\\overline{A\\mathbf{v}} = A\\bar{\\mathbf{v}}$. So $A\\bar{\\mathbf{v}} = \\bar{\\lambda}\\bar{\\mathbf{v}}$ â€” $\\bar{\\lambda}$ is automatically also an eigenvalue.'],
      reviewSection: 'Rigor tab â€” Why Conjugate Pairs?',
    },
    {
      ...
      reviewSection: 'Rigor tab â€” Trace and Determinant Still Apply',
    },
    {
      id: 'complex-eigenvalues-q4',
      type: 'choice',
      text: 'The eigenvalues of the 90Â° rotation matrix are $\\lambda = \\pm i$. What is $\\lambda^4$ (after 4 applications)?',
      options: ['$-1$', '$i$', '$1$', '$0$'],
      answer: '$1$',
      hints: ['$i^4 = i^2 \\cdot i^2 = (-1)(-1) = 1$. Four 90Â° rotations = 360Â° = full circle back to the start.'],
      reviewSection: 'Examples tab â€” 90Â° Rotation Matrix',
    },
    {
      ...
      reviewSection: 'Rigor tab â€” Trace and Determinant',
    },
    {
      ...
      options: [
        'Yes â€” both eigenvalues have negative real part.',
        'No â€” both eigenvalues have magnitude $\\sqrt{5} > 1$.',
        'Yes â€” the discriminant is negative.',
        'Cannot determine without more information.',
      ],
      answer: 'No â€” both eigenvalues have magnitude $\\sqrt{5} > 1$.',
      hints: ['$\\lambda = 1 \\pm 2i$. Magnitude: $|1+2i| = \\sqrt{1+4} = \\sqrt{5} \\approx 2.24 > 1$. In discrete-time systems, stability requires $|\\lambda| < 1$. This system is unstable â€” it oscillates with growing amplitude.'],
      reviewSection: 'Intuition tab â€” long-term behavior',
    },
    {
    ...
    },
    {
      ...
      reviewSection: 'Rigor tab â€” Real Jordan Form',
    },
    {
    ...
    },
    {
      ...
      type: 'choice',
      text: 'A matrix has eigenvalue $\\lambda = e^{i\\pi/6} = \\cos(30Â°) + i\\sin(30Â°)$. After 12 applications, every vector returns exactly to its starting position. Why?',
      options: [
        'Because $|\\lambda| = 1$, so there is no scaling.',
        'Because $\\lambda^{12} = e^{i \\cdot 12\\pi/6} = e^{i2\\pi} = 1$ â€” 12 rotations of 30Â° complete a full circle.',
        'Because eigenvalue 1 means the identity transformation.',
        'Because real and imaginary parts cancel after many applications.',
      ],
      answer: 'Because $\\lambda^{12} = e^{i \\cdot 12\\pi/6} = e^{i2\\pi} = 1$ â€” 12 rotations of 30Â° complete a full circle.',
      ...
      reviewSection: 'Intuition tab â€” repeated application',
    },
  ],
  ...
  misconceptions: [
    {
      ...
      whyStudentsThinkIt: 'Students know eigenvectors must be non-zero, and the complex eigenvectors of a real matrix are complex-valued â€” not real vectors. They conclude there is nothing geometric to say.',
      correctionExample: 'The rotation matrix $A = \\begin{bmatrix}0&-1\\\\1&0\\end{bmatrix}$ has eigenvectors $[1,-i]^\\top$ and $[1,i]^\\top$ (complex). These do not correspond to real directions, but the real-form Jordan block $\\begin{bmatrix}0&-1\\\\1&0\\end{bmatrix}$ (which IS $A$ itself here!) reveals the geometry: 90Â° rotation. The behavior of REAL vectors under $A$ is still fully determined.',
      contrastCase: 'Symmetric matrices have only real eigenvalues and real eigenvectors â€” the Spectral Theorem. Complex eigenvalues arise precisely when the transformation includes a rotation component that cannot be described purely by real stretching.',
    },
    {
      falseBelief: 'Complex eigenvalues always come from "complicated" matrices. Simple-looking matrices have real eigenvalues.',
      whyStudentsThinkIt: 'Students associate complexity of matrix entries with complexity of eigenvalues. A simple 2Ã—2 matrix with small integers seems like it should have simple real eigenvalues.',
  ...
  transferPrompts: [
    {
      ...
      whyThisTechniqueWins: 'The complex roots $s = -1 \\pm i$ reveal everything: real part $-1$ means exponential decay (damping), imaginary part $1$ means oscillation at 1 rad/s. The circuit response is $e^{-t}\\cos(t)$ â€” a damped sinusoid. No simulation needed; the eigenvalue gives the full qualitative picture.',
    },
    {
      situation: 'A robotics engineer needs to verify that a discrete-time control loop (running at 100 Hz) will not oscillate unstably. The state transition matrix $A$ has been computed numerically, and its largest eigenvalue has magnitude $0.98$ and argument $15Â°$.',
      ...
      whyThisTechniqueWins: 'The criterion is immediate: $|\\lambda| = 0.98 < 1$ â†’ stable. The imaginary part (15Â°/step) predicts a damped oscillation at frequency $15Â° \\times 100$ Hz $/ 360Â° \\approx 4.2$ Hz. If the mechanical resonance of the robot arm is near 4.2 Hz, the controller will excite it â€” an eigenvalue check predicts the resonance interaction without simulation.',
    },
  ],
  ...
  debugging: [
    {
      ...
      symptom: 'For a $2 \\times 2$ real matrix, the student finds one complex eigenvalue $3+2i$ and stops. The characteristic equation has degree 2 â€” both roots must be found.',
    ...
    },
    {
      ...
      symptom: 'For $\\lambda = -0.5 + 0.9i$, student says "negative real part â†’ stable" without checking $|\\lambda| = \\sqrt{0.25+0.81} = \\sqrt{1.06} > 1$.',
      ...
      repairStrategy: 'Always identify which type of system before checking: continuous-time â†’ check $\\text{Re}(\\lambda) < 0$; discrete-time â†’ check $|\\lambda| < 1$. In this course, matrix powers use discrete-time criterion.',
    },
  ],

  mastery: {
    targetLevel: 2,
    solveIndependently: 'Find complex eigenvalues of a 2Ã—2 real matrix using the quadratic formula, interpret them geometrically as rotation-scaling, and determine stability ($|\\lambda|$ vs 1).',
    ...
    detectIncorrectApplication: 'Spot when someone applies the continuous-time stability criterion (Re(Î») < 0) to a discrete-time system, or when someone finds only one of the two conjugate eigenvalues.',
