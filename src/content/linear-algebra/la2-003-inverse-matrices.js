export default {
  ...
  intuition: {
    prose: [
      'Take $A = \\begin{bmatrix}2&1\\\\5&3\\end{bmatrix}$. Compute $\\det(A) = 2 \\cdot 3 - 1 \\cdot 5 = 1$. Because $\\det \\neq 0$, the inverse exists: $A^{-1} = \\begin{bmatrix}3&-1\\\\-5&2\\end{bmatrix}$. Verify: $A^{-1}A = \\begin{bmatrix}6-5&3-3\\\\-10+10&-5+6\\end{bmatrix} = \\begin{bmatrix}1&0\\\\0&1\\end{bmatrix} = I$. Now try $B = \\begin{bmatrix}2&4\\\\1&2\\end{bmatrix}$: $\\det(B) = 4-4 = 0$. Column 2 is twice column 1 â€” $B$ squishes the whole plane onto a single line. Once that happens, information is lost forever and no inverse can recover it. The **determinant** is the test: non-zero means reversible, zero means irreversible.',
      '**The inverse machine.** If you apply $A$ then immediately apply $A^{-1}$, the vector must return to where it started. So $A^{-1}A\\mathbf{v} = \\mathbf{v}$ for every vector â€” which means $A^{-1}A = I$, the identity matrix (the "do nothing" transformation). The inverse undoes the warp completely.',
      ...
      '**The fatal case: $\\det(A) = 0$.** This means $A$ squishes the entire 2D plane flat onto a line (or a point). Once space collapses, information is destroyed permanently â€” infinitely many different input vectors land on the same output. You cannot reconstruct the input from the output. No inverse exists.',
      '**The 3D projection example.** When a 3D video game is projected onto your 2D screen, depth information (the Z-coordinate) is permanently discarded. Short character far away = tall character close up â€” you cannot distinguish them from the flat image. This is a linear transformation with $\\det = 0$: the 3D space was collapsed to 2D. The projection matrix has no inverse.',
      '**CNC application: workpiece coordinate frame.** When a CNC machine calibrates its work coordinate system, it measures 3 reference points on the workpiece. These 3 points define three vectors, and the machine computes their determinant. If the determinant is 0 (or near 0), the three points are collinear â€” they all lie on a single line, which is not enough to define a unique flat plane. The calibration fails. A well-set-up fixture keeps the three points spread wide apart, maximizing the determinant and ensuring a stable, invertible coordinate frame.',
    ...
    callouts: [
      {
        type: 'sequencing',
        title: 'Lesson 3 of 12 â€” Matrices & Transformations',
        body: '**Previous:** Matrix multiplication = composing transformations.\n**This lesson:** Determinant = area scaling factor. If det â‰  0, the inverse exists and undoes the transformation.\n**Next:** Null space and column space â€” what happens to vectors when det = 0.',
      },
      {
        type: 'insight',
      ...
      },
      {
        ...
        body: 'In software, NEVER write `x = inv(A) * b` or `x = np.linalg.inv(A) @ b`. It is slower and numerically less stable than `x = A \\ b` (MATLAB) or `x = np.linalg.solve(A, b)` (Python). The backslash operator uses LU factorization internally â€” it is the correct tool for solving systems. Reserve `inv()` only for situations where you genuinely need the entries of $A^{-1}$ for their own sake.',
      },
      {
        type: 'definition',
        title: 'Determinant â€” 2Ã—2 Formula and Meaning',
    ...
    visualizations: [
      ...
      },
      {
        ...
        title: 'A then Aâ»Â¹ = Identity',
        mathBridge: 'Set the first transformation to any invertible matrix A (try a shear or rotation). Then set the second transformation to its inverse Aâ»Â¹ (use the formula: swap main diagonal, negate off-diagonal, scale by 1/det). Press "compose" and watch the combined transformation snap back to the identity â€” the grid returns to its original, undistorted state. This is the meaning of $A^{-1}A = I$: the inverse perfectly cancels the original warp.',
        caption: 'Composing A with its inverse returns the grid exactly to where it started â€” the identity transformation.',
      },
    ],
  },
    ...
    visualizations: [
      {
        ...
        initialProps: {
          initialCells: [
            {
              id: 1,
              cellTitle: 'Determinant â€” area scaling, manual formula, and MATLAB',
              prose: [
                'The determinant tells you how much the transformation scales area. MATLAB computes it with `det(A)`. For 2Ã—2, the formula is ad âˆ’ bc.',
              ],
...
disp('A * inv(A):'); disp(A * A_inv)`,
            ...
            {
              id: 2,
              cellTitle: 'Watching det â†’ 0 as a matrix becomes singular',
              prose: [
                ...
                'In practice, matrices with very small (but nonzero) determinants are near-singular â€” numerically unstable even if technically invertible.',
              ],
...
fprintf('\\nAt (2,2)=1.5: det = %g  (singular!)\\n', det(A_sing))`,
            ...
            {
              id: 3,
              cellTitle: 'Solving Ax=b â€” backslash beats inv()',
              prose: [
...
% Both give same answer â€” but for large systems the difference matters
fprintf('Same result: %d\\n', norm(x_solve - x_inv) < 1e-10)`,
            },
            {
              id: 4,
              cellTitle: 'Application: CNC workpiece coordinate frame â€” is calibration stable?',
              prose: [
                'A CNC machine probes 3 reference points on the workpiece to establish the work coordinate system. These 3 points must NOT be collinear â€” if they are, the determinant of their position matrix is 0 and no unique plane (or coordinate frame) can be defined.',
...
% Put them in a 2Ã—2 matrix (XY plane)
...
fprintf('Condition number (bad):  %.2e  (huge = near-singular)\\n', cond(M_bad))`,
      ...
      },
      {
        ...
        mathBridge: 'np.linalg.det(A) computes det(A) = ad âˆ’ bc. np.linalg.inv(A) computes Aâ»Â¹. Verify: A @ A_inv â‰ˆ I. det = 0 means singular â€” no inverse exists.',
        caption: 'Verify the invertibility conditions and confirm AÂ·Aâ»Â¹ = I numerically.',
        initialProps: {
          initialCells: [
            {
              id: 1,
              cellTitle: 'Determinant â€” the scaling factor',
              prose: [
...
plt.show()`,
            ...
            {
              id: 2,
              cellTitle: 'Computing the inverse and verifying AÂ·Aâ»Â¹ = I',
              prose: [
                '`np.linalg.inv(A)` computes Aâ»Â¹. Multiplying A by its inverse should give the identity matrix I.',
...
plt.show()`,
            },
            {
              ...
              difficulty: 'medium',
              prompt: 'For each matrix below: (1) compute the determinant, (2) state whether it is invertible, (3) for any invertible matrix, compute Aâ»Â¹ and verify A @ Aâ»Â¹ = I. Explain why matrix C is singular.',
              code: `import numpy as np

A = np.array([[4., 2.], [1., 3.]])
B = np.array([[1., 2.], [3., 6.]])   # note: row2 = 3 Ã— row1
C = np.array([[2., -1.], [4., 3.]])

# For each: det, invertible?, and if yes: compute inv and verify
`,
              hint: 'np.linalg.det() for determinant. np.linalg.inv() for inverse. B is singular because its rows are proportional (det = 1Ã—6 - 2Ã—3 = 0).',
            },
          ]
        }
      },
  ...
  rigor: {
    ...
    callouts: [
      ...
      {
        type: 'insight',
        title: 'The 2Ã—2 Inverse Formula Is Cofactors',
      ...
      steps: [
        {
          ...
          hints: ["$\\det = 0$ means the two column vectors land on the same line. Space collapses from 2D to 1D â€” area is permanently destroyed."],
        },
        {
          ...
          annotation: "Column 2 is a scalar multiple of column 1 â€” they are linearly dependent. Both $\\hat{i}$ and $\\hat{j}$ land on the exact same line, so the entire 2D plane collapses onto that line. No information about the perpendicular direction survives.",
  ...
  challenges: [
    ...
    },
    {
      ...
      hint: "Use the 2Ã—2 inverse formula: $K^{-1} = \\frac{1}{ad-bc}\\begin{bmatrix}d & -b \\\\ -c & a\\end{bmatrix}$. For part (c), apply $K^{-1}$ to $\\mathbf{c}$ and check that you get back $\\mathbf{m} = [3, 5]^T$.",
      walkthrough: [
        {
        ...
        },
        {
          ...
          annotation: "$\\det = 1$: the transformation preserves area exactly. This also means the inverse formula has a clean $1/1$ scalar â€” ideal for a key matrix."
        },
        {
  ...
  checkpoints: [
    { id: 'cp-la2-003-1', label: 'Read intuition â€” understand why det = 0 means no inverse', type: 'read' },
    { id: 'cp-la2-003-2', label: 'Read math â€” trace the 2Ã—2 inverse formula by hand', type: 'read' },
    { id: 'cp-la2-003-3', label: 'Read rigor â€” study the Invertible Matrix Theorem (9 equivalences)', type: 'read' },
    { id: 'cp-la2-003-4', label: 'Run OpenMAT cell 1 â€” compute det manually and verify via MATLAB', type: 'lab' },
    { id: 'cp-la2-003-5', label: 'Run OpenMAT cell 3 â€” use backslash to solve Ax=b (not inv)', type: 'lab' },
    { id: 'cp-la2-003-6', label: 'Complete example 1: trace the 2Ã—2 inverse formula step by step', type: 'example' },
    { id: 'cp-la2-003-7', label: 'Complete example 2: recover the input vector by applying Aâ»Â¹', type: 'example' },
    { id: 'cp-la2-003-8', label: 'Attempt challenge 1: compute det by hand using ad âˆ’ bc', type: 'challenge' },
  ...
  quiz: [
    {
      ...
      reviewSection: 'Intuition tab â€” Determinant of 0'
    },
    {
      id: 'la2-003-quiz-2',
      type: 'choice',
      ...
      reviewSection: 'Semantics tab â€” Rules of Thumb'
    },
    {
      id: 'la2-003-quiz-3',
      type: 'choice',
      ...
      reviewSection: 'Math tab â€” Determinant formula'
    },
    {
      id: 'la2-003-quiz-4',
      type: 'choice',
      text: "Why does the 2Ã—2 inverse formula $A^{-1} = \\frac{1}{ad-bc}\\begin{bmatrix}d & -b \\\\ -c & a\\end{bmatrix}$ break down when $\\det(A) = 0$?",
      options: [
        "Division by zero â€” the formula requires scaling by $1/\\det(A)$, and dividing by 0 is undefined.",
      ...
      answer: "Division by zero â€” the formula requires scaling by $1/\\det(A)$, and dividing by 0 is undefined.",
      ...
      reviewSection: 'Math tab â€” Inverse formula'
    },
    {
      id: 'la2-003-quiz-5',
      type: 'choice',
      ...
      options: [
        "Use `A \\\\ b` (MATLAB) or `np.linalg.solve(A, b)` (Python) â€” LU factorization is faster and numerically stabler than computing the full inverse.",
        "Use `A' * b` â€” the transpose avoids numerical issues.",
        "Use `b / A` â€” this is the matrix division operator and avoids forming the inverse.",
        "There is no meaningful difference; `inv(A) * b` is the correct approach."
      ],
      answer: "Use `A \\\\ b` (MATLAB) or `np.linalg.solve(A, b)` (Python) â€” LU factorization is faster and numerically stabler than computing the full inverse.",
      ...
      reviewSection: 'Intuition tab â€” Professional warning callout'
    },
    {
      id: 'la2-003-quiz-6',
      type: 'choice',
      ...
      options: [
        "$-1/3$ â€” because $\\det(A) \\cdot \\det(A^{-1}) = \\det(AA^{-1}) = \\det(I) = 1$.",
        "$3$ â€” the inverse negates the sign and flips the magnitude.",
        "$1/3$ â€” the inverse always has a positive determinant.",
        "$9$ â€” the determinant squares when you invert."
      ],
      answer: "$-1/3$ â€” because $\\det(A) \\cdot \\det(A^{-1}) = \\det(AA^{-1}) = \\det(I) = 1$.",
      hints: ["Use $\\det(AB) = \\det(A)\\det(B)$. Set $B = A^{-1}$: $\\det(A) \\cdot \\det(A^{-1}) = \\det(I) = 1$, so $\\det(A^{-1}) = 1/\\det(A) = 1/(-3) = -1/3$."],
      reviewSection: 'Rigor tab â€” Key properties of the determinant',
    },
    {
      id: 'la2-003-quiz-7',
      type: 'choice',
      ...
      options: [
        '$\\det = 0$; not invertible â€” column 2 is $\\frac{1}{2}$ column 1',
        '$\\det = 12$; invertible',
        '$\\det = -6$; invertible with a negative determinant',
        '$\\det = 24$; invertible because all entries are positive',
      ],
      answer: '$\\det = 0$; not invertible â€” column 2 is $\\frac{1}{2}$ column 1',
    ...
    },
    {
      ...
      reviewSection: 'Example 1 â€” 2Ã—2 inverse formula',
    },
    {
      id: 'la2-003-quiz-9',
      type: 'choice',
      ...
      options: [
        ...
        'Areas are unchanged â€” only lengths change',
      ...
      reviewSection: 'Intuition tab â€” determinant and area',
    },
    {
      id: 'la2-003-quiz-10',
      type: 'choice',
      ...
      options: [
        '$B^{-1}A^{-1}$ â€” reverse the order when inverting a product',
        '$A^{-1}B^{-1}$ â€” invert each factor in the same order',
        '$A^{-1} + B^{-1}$ â€” the inverse of a sum is the sum of inverses',
        '$(A+B)^{-1}$ â€” combine first then invert',
      ],
      answer: '$B^{-1}A^{-1}$ â€” reverse the order when inverting a product',
  ...
  misconceptions: [
    ...
    },
    {
      ...
      correctionExample: '$A = \\begin{bmatrix}1&2\\\\2&4\\end{bmatrix}$ has all nonzero entries but $\\det(A) = 4-4=0$ â€” no inverse. The matrix collapses $\\mathbb{R}^2$ onto the line $y=2x$. Every input on the same line through the origin maps to the same output.',
      contrastCase: 'Even the zero matrix (all zeros) has a determinant of 0 and no inverse. But a matrix need not be zero to be non-invertible â€” it just needs at least one pair of proportional columns.',
    },
  ],
  ...
  transferPrompts: [
    ...
    },
    {
      ...
      whyThisTechniqueWins: 'The view matrix is a rigid rotation/translation ($\\det \\neq 0$, invertible â€” you can reverse the camera placement). The perspective matrix deliberately collapses the $z$-dimension to create depth cues, making $\\det = 0$. You cannot recover 3D depth from a 2D projection â€” the $z$-information is permanently destroyed.',
    },
  ],
  ...
  debugging: [
    ...
    },
    {
      ...
      repairStrategy: 'Use `A \\\\ b` (MATLAB) or `numpy.linalg.solve(A, b)`. These use LU factorization internally â€” they solve the system without materializing $A^{-1}$, which is both faster and numerically stabler.',
    },
  ],
