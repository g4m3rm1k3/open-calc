    ...
    questions: [
      {
        ...
        answer: '$\\nabla f = 2X^\\top(X\\mathbf{w} - \\mathbf{y})$; add $\\lambda\\|\\mathbf{w}\\|^2$',
        hints: ['Apply $\\nabla\\|A\\mathbf{x}-\\mathbf{b}\\|^2 = 2A^\\top(A\\mathbf{x}-\\mathbf{b})$ with $A = X$, $\\mathbf{x} = \\mathbf{w}$, $\\mathbf{b} = \\mathbf{y}$. Ridge adds $\\lambda\\|\\mathbf{w}\\|^2$.'],
      },
      {
        id: 'la2-009-assess-2',
        type: 'choice',
        text: 'For a symmetric matrix $A$, what is $\\nabla_{\\mathbf{w}}(\\mathbf{w}^\\top A \\mathbf{w})$?',
        options: [
          '$2A\\mathbf{w}$ — the matrix-quadratic gradient formula for symmetric $A$',
          '$A^2 \\mathbf{w}$',
          '$\\mathbf{w}^\\top A$',
          '$A + A^\\top$',
        ],
        answer: '$2A\\mathbf{w}$ — the matrix-quadratic gradient formula for symmetric $A$',
        hints: ['For symmetric $A$: $\\nabla(\\mathbf{x}^\\top A \\mathbf{x}) = 2A\\mathbf{x}$. This follows from expanding and differentiating, or from the chain rule applied to the scalar form.'],
      },
      {
        id: 'la2-009-assess-3',
        type: 'choice',
        text: 'What are the dimensions of the Jacobian matrix $J$ for a function $f: \\mathbb{R}^n \\to \\mathbb{R}^m$?',
        options: [
          '$m \\times n$ — one row per output component, one column per input component',
          '$n \\times m$ — one row per input, one column per output',
          '$n \\times n$ — always square',
          '$m \\times m$ — always square in the output space',
        ],
        answer: '$m \\times n$ — one row per output component, one column per input component',
        hints: ['$J_{ij} = \\partial f_i / \\partial x_j$. Rows index the output components ($i = 1,\\ldots,m$), columns index the inputs ($j = 1,\\ldots,n$).'],
      },
      {
        id: 'la2-009-assess-4',
        type: 'choice',
        text: 'Setting $\\nabla_{\\mathbf{w}} \\|X\\mathbf{w} - \\mathbf{y}\\|^2 = \\mathbf{0}$ gives which normal equation?',
        options: [
          '$X^\\top X \\mathbf{w} = X^\\top \\mathbf{y}$ — the least squares normal equation',
          '$X X^\\top \\mathbf{w} = \\mathbf{y}$',
          '$X\\mathbf{w} = \\mathbf{y}$ directly',
          '$\\mathbf{w} = (X^\\top)^{-1} \\mathbf{y}$',
        ],
        answer: '$X^\\top X \\mathbf{w} = X^\\top \\mathbf{y}$ — the least squares normal equation',
        hints: ['$\\nabla = 2X^\\top(X\\mathbf{w} - \\mathbf{y}) = \\mathbf{0}$ gives $X^\\top X \\mathbf{w} = X^\\top \\mathbf{y}$. This is solved by $\\mathbf{w} = (X^\\top X)^{-1} X^\\top \\mathbf{y}$ when $X^\\top X$ is invertible.'],
      },
    ],
  },
