export default {
  ...
  subtitle: 'The central question of linear algebra: given a target, can we reach it â€” and if so, how?',
  ...
  intuition: {
    prose: [
      ...
      '**The Column Picture.** Rewrite the same system as a single vector equation: $x \\begin{bmatrix}2\\\\1\\end{bmatrix} + y \\begin{bmatrix}1\\\\-1\\end{bmatrix} = \\begin{bmatrix}5\\\\1\\end{bmatrix}$. Now you are asking: what scalar combination of the two column vectors reaches the target vector $[5, 1]$? This is a span question â€” and you already know exactly what that means.',
      ...
      '**Gaussian Elimination: the algorithm.** The key insight is that you can do three things to a system of equations without changing the solution: (1) swap two equations, (2) multiply an equation by a non-zero number, (3) add a multiple of one equation to another. These are the three elementary row operations. Gaussian elimination uses them systematically to create zeros below each leading entry, turning the system into a staircase shape â€” called row echelon form â€” where back-substitution becomes trivial.',
      '**Where this is heading:** Once you master systems, the natural next move is to ask: can we represent this whole process as a single object that encodes the system AND the transformation we are performing? Yes â€” that object is a matrix. Phase 2 begins exactly there.',
    ],
    callouts: [
      {
        type: 'sequencing',
        title: 'Lesson 4 of 6 â€” Vectors & Spaces',
        body: '**Previous:** Dot and Cross Products â€” angles and projections between vectors.\n**This lesson:** Systems of Linear Equations â€” Gaussian elimination, pivot variables, solution structure.\n**Next:** Lines and Planes â€” the geometric picture behind every linear constraint.',
      },
      {
        type: 'insight',
        title: 'The Row Picture vs. The Column Picture',
        body: 'Row picture: each equation is a line or plane. Solution = intersection.\n\nColumn picture: can a linear combination of the column vectors reach $\\mathbf{b}$?\n\nSame system. Two completely different â€” and equally valid â€” ways to see it.',
      },
      {
        type: 'definition',
      ...
      },
      {
        ...
        body: `The three valid row operations all require **nonzero** scalars. Multiplying a row by zero is **not** a legal row operation â€” it destroys information and changes the solution set.\n\n**Wrong:** $R_2 \\to 0 \\cdot R_2$ (turns an equation into $0 = 0$, loses a constraint)\n\n**Correct:** $R_2 \\to \\frac{1}{3} R_2$ scales the row but keeps all information.\n\nThe rule: the scalar $c$ in operations 2 and 3 must satisfy $c \\neq 0$.`,
      },
      {
        type: 'insight',
        title: 'When to Use This',
        body: `Use Gaussian elimination (row reduction) whenever you need to:\n\n- **Find the exact solution** of a system of equations â€” always the right tool\n- **Test consistency** before attempting a solution â€” check if a contradiction row appears\n- **Count free variables** â€” each non-pivot column gives one degree of freedom\n- **Find the null space** or column space of a matrix (covered in Chapter 2)\n\nFor 2Ã—2 systems, substitution is faster. For 3Ã—3 and up, row reduction is the only reliable method. In code, use \`np.linalg.solve\` for unique solutions and \`np.linalg.lstsq\` for overdetermined systems.`,
      },
      {
        type: 'warning',
        title: 'Three Possible Outcomes â€” No In-Between',
        body: 'A system of linear equations always has exactly:\nâ€¢ **One solution** (lines/planes intersect at a single point)\nâ€¢ **No solution** (lines/planes are parallel â€” inconsistent)\nâ€¢ **Infinitely many solutions** (lines/planes overlap â€” free variables exist)\n\nThere is no such thing as "exactly two solutions" in a linear system.',
      },
    ],
    visualizations: [
      {
      ...
      },
      {
        ...
        title: 'Linear Systems â€” Concept to CNC',
        mathBridge: 'A five-tab module: Concept reviews the three outcomes and EROs; Canonical walks through a full 3Ã—3 solve step by step; Real World shows how CNC firmware solves a sensor system every millisecond; Interactive lets you perform swap/scale/replace on any system and auto-RREF it; Practice gives five applied problems to work by hand.',
        caption: 'From row operations to real machine positioning â€” one unified module.',
      },
    ],
  },
  ...
  math: {
    prose: [
      ...
      'For a unique solution, we can go further to **Reduced Row Echelon Form (RREF)**: each pivot is 1, and every other entry in the pivot\'s column is 0. Reading off the solution from RREF requires no back-substitution â€” it is immediate.',
      'If a row of the form $[0\\ 0\\ \\cdots\\ 0\\ |\\ c]$ (with $c \\neq 0$) appears, the system is **inconsistent** â€” no solution exists. If a column has no pivot, its corresponding variable is a **free variable** â€” it can take any value, giving infinitely many solutions.',
    ...
    callouts: [
      ...
      },
      {
        ...
        body: 'The system $A\\mathbf{x} = \\mathbf{b}$ is consistent (has at least one solution) if and only if $\\mathbf{b}$ is in the column space of $A$ â€” i.e., $\\text{rank}([A \\mid \\mathbf{b}]) = \\text{rank}(A)$.',
      },
      {
        type: 'insight',
    ...
    visualizations: [
      {
        ...
        mathBridge: 'MATLAB has two main tools for linear systems. The backslash operator A\\b solves Ax=b numerically â€” it is the fastest, most stable option for unique solutions. rref([A b]) performs the full Gauss-Jordan elimination symbolically and shows you the augmented matrix in RREF â€” better for understanding the structure (pivots, free variables, inconsistency). Learn both: use \\ in practice, use rref to see what is actually happening.',
        caption: 'The backslash operator is MATLAB\'s most important single character for linear algebra.',
        initialProps: {
          initialCells: [
            {
              id: 1,
              cellTitle: 'The backslash operator A\\b â€” MATLAB\'s workhorse',
              prose: [
                '`A\\b` is how MATLAB solves Ax = b. It does NOT compute Aâ»Â¹ directly â€” it uses LU factorization internally, which is faster and more numerically stable.',
...
% Solve with backslash â€” the standard MATLAB approach
x = A \\ b
...
inv(A) * b`,
            ...
            {
              id: 2,
              cellTitle: 'rref() â€” seeing Gauss-Jordan elimination',
              prose: [
                ...
                'Reading the RREF: if the last column has a leading 1 in some row â†’ inconsistent (no solution). If a column in A has no pivot â†’ that variable is free â†’ infinitely many solutions.',
              ],
              code: `% Unique solution: full rank
A1 = [2 1; 1 3];  b1 = [8; 9];
disp('=== Unique solution â€” RREF ===')
rref([A1 b1])
% Read: x = ..., y = ...

% Inconsistent: row [0 0 | câ‰ 0] appears
A2 = [1 2; 2 4];  b2 = [3; 7];
disp('=== No solution â€” RREF ===')
rref([A2 b2])
% Second row: [0 0 | 1] â†’ 0 = 1 â†’ contradiction

% Infinite solutions: row [0 0 | 0] appears, free variable
b3 = [3; 6];
disp('=== Infinite solutions â€” RREF ===')
rref([A2 b3])
% Second row: [0 0 | 0] â†’ one equation was redundant`,
            },
            {
              id: 3,
              ...
              prose: [
                ...
                '- rank([A b]) > rank(A) â†’ inconsistent (no solution)\n- rank(A) = number of unknowns â†’ unique solution\n- rank(A) < number of unknowns AND equal to rank([A b]) â†’ infinitely many solutions',
              ],
...
if r_Ab > r_A
    disp('NO SOLUTION â€” inconsistent')
elseif r_A == n
    ...
    fprintf('Number of free variables: %d\\n', n - r_A)
      ...
      },
      {
        ...
        caption: 'Solve systems of equations in one line â€” and visualize the geometry of the solution.',
        initialProps: {
          initialCells: [
            {
              id: 1,
              cellTitle: 'Solve Ax = b â€” unique solution',
              prose: [
                '`np.linalg.solve(A, b)` solves the system in one call. It uses LU factorization â€” faster and more stable than computing Aâ»Â¹ directly.',
...
print(f"Verify A @ x = {A @ x}  (should equal b = {b})")`,
            },
            {
              ...
              prose: [
                'Each equation in the 2Ã—2 system is a line. The solution is where they cross.',
                'Line 1: 2x + y = 8 â†’ y = 8 âˆ’ 2x. Line 2: x + 3y = 9 â†’ y = (9 âˆ’ x)/3.',
              ],
              code: `import numpy as np
...
print("rank unchanged, but rank < n â†’ INFINITE solutions")`,
            },
            {
              ...
              challengeType: 'write',
              challengeNumber: 1,
              challengeTitle: 'Solve a 3Ã—3 system',
              difficulty: 'medium',
              prompt: 'Solve the 3Ã—3 system: x + 2y + z = 9,  2x - y + z = 3,  x + y - z = 1. Use np.linalg.solve(). Then verify with np.allclose(A @ x, b) and print each unknown.',
              code: `import numpy as np
...
`,
  ...
  rigor: {
    prose: [
      'Every elementary row operation is invertible, which means it does not change the solution set. More precisely, row operations produce **row equivalent** matrices â€” matrices that represent different-looking systems with the exact same solution set.',
      'The RREF of a matrix is unique (this is a theorem, and the proof is non-trivial). This means the RREF is a canonical fingerprint of the row space of a matrix â€” two matrices have the same row space if and only if they have the same RREF.',
  ...
  examples: [
    {
      id: 'la1-004-ex1',
      title: 'Unique Solution: 2Ã—2 System',
      ...
      steps: [
        ...
        },
        {
          ...
          annotation: 'Swap rows to get a leading 1 in the top-left position â€” the first pivot. This makes the arithmetic cleaner. Row swaps are legal because they only reorder the equations.',
        ...
        },
        {
          ...
          annotation: 'Back-substitute by adding row 2 to row 1, zeroing out the $-1$ above the second pivot. The matrix is now in RREF â€” identity on the left, solution on the right. Read: $x = 2$, $y = 1$.',
          strategyTitle: 'Back-eliminate to RREF â€” read solution',
      ...
      steps: [
        {
          ...
          annotation: 'Form the augmented matrix $[A \\mid \\mathbf{b}]$. Notice: row 2 of $A$ is exactly 2 times row 1 of $A$ â€” the coefficient rows are proportional. But the right-hand sides $3$ and $9$ are not in that same ratio ($9 \\neq 2 \\times 3 = 6$). This is the warning sign.',
          strategyTitle: 'Augmented matrix â€” spot the proportional coefficients',
        ...
        },
        {
          ...
          annotation: 'Subtract 2 times row 1 from row 2. The left side becomes all zeros â€” the entire equation collapses â€” but the right side becomes $9 - 6 = 3 \\neq 0$. This is the contradiction row.',
          strategyTitle: 'Eliminate â€” contradiction row appears',
        },
        {
          ...
          strategyTitle: 'Identify the contradiction â€” no solution',
          hints: ['The pivot is in the last column, not in a coefficient column. This means the "equation" $0 = 3$ cannot be satisfied.'],
        },
      ],
      conclusion: 'The system is inconsistent â€” no solution exists. Geometrically, the two equations represent parallel lines (both have slope $-1/2$, different $y$-intercepts). Parallel lines never cross.',
    },
    {
      id: 'la1-004-ex3',
      ...
      steps: [
        ...
        },
        {
          ...
          annotation: 'Eliminate column 1: subtract $2 \\times$ row 1 from row 2, and $3 \\times$ row 1 from row 3. Row 2 becomes all zeros â€” this means equation 2 was exactly $2 \\times$ equation 1, carrying no new information.',
          strategyTitle: 'Eliminate column 1 â€” redundant row appears',
          hints: ['An all-zero row signals that one equation was a multiple of another â€” it contained no independent constraint.'],
        },
        {
          expression: '\\xrightarrow{R_2 \\leftrightarrow R_3} \\left[\\begin{array}{ccc|c} 1 & 2 & 1 & 4 \\\\ 0 & 0 & 1 & 2 \\\\ 0 & 0 & 0 & 0 \\end{array}\\right]',
          annotation: 'Swap rows 2 and 3 to move the non-zero row up into the pivot position. Now the matrix is in echelon form. Pivot columns are 1 and 3. Column 2 has no pivot â€” so $y$ is a free variable.',
          strategyTitle: 'Swap to echelon form â€” count pivots',
        },
        {
      ...
      conclusion: 'The system has infinitely many solutions, parameterized as $(2-2t,\\, t,\\, 2)$ for $t \\in \\mathbb{R}$. The solution set is a line in 3D space. One of the three original equations was redundant â€” it carried no new constraint.',
    },
  ],
  ...
  quiz: [
    {
      ...
      hints: ['This row translates to the equation $0x + 0y = 5$, or $0 = 5$ â€” a contradiction.'],
      reviewSection: 'Math tab â€” Consistency Condition',
    },
    {
      id: 'la1-004-quiz-2',
      type: 'choice',
      text: 'A 3Ã—4 system (3 equations, 4 unknowns) has 2 pivot columns. How many free variables are there?',
      options: ['1', '2', '3', '4'],
      answer: '2',
      hints: ['Free variables = total unknowns âˆ’ number of pivots = 4 âˆ’ 2 = 2.'],
      reviewSection: 'Math tab â€” Free Variables',
    },
    {
      id: 'la1-004-quiz-3',
      type: 'choice',
      ...
      hints: ['Multiplying by 0 destroys an equation entirely â€” it loses information and changes the solution set. The three valid operations all require non-zero scalars.'],
      reviewSection: 'Intuition tab â€” Three Elementary Row Operations',
    },
    {
      id: 'la1-004-quiz-4',
      type: 'choice',
      ...
      reviewSection: 'Intuition tab â€” Row vs Column Picture',
    },
    {
      id: 'la1-004-quiz-5',
      type: 'choice',
      ...
      options: [
        'Exactly one',
        'Exactly two',
        'None â€” the system is inconsistent',
        'Infinitely many â€” one free variable',
      ],
      answer: 'Infinitely many â€” one free variable',
      hints: ['Column 3 has no pivot (no leading 1 in that column) â€” so $x_3$ is a free variable. The bottom row reads $0 = 0$, so there is no contradiction. Free variable â†’ infinitely many solutions.'],
      reviewSection: 'Math tab â€” Free Variables',
    },
    {
      id: 'la1-004-quiz-6',
      ...
      options: [
        ...
        'It turns the row into all zeros, destroying the constraint â€” the solution set changes',
        'It is valid as long as you divide back later',
        'Row 2 is not allowed to be scaled',
      ],
      answer: 'It turns the row into all zeros, destroying the constraint â€” the solution set changes',
      hints: ['The three legal row operations all require a nonzero scalar. Multiplying by 0 replaces an equation with $0 = 0$ â€” an empty constraint â€” which allows solutions that the original system would reject.'],
      reviewSection: 'Intuition tab â€” warning callout',
    },
  ],

  // â”€â”€ Misconceptions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  misconceptions: [
    {
      falseBelief: 'If I get a row of all zeros, the system has no solution.',
      whyStudentsThinkIt: 'Students associate "something went wrong" with a zero row, confusing it with the contradiction row [0 0 | câ‰ 0].',
      correctionExample: 'A zero row [0 0 0 | 0] means one equation was redundant â€” it adds no new constraint. The system may still be consistent (with a free variable). Only a pivot in the augmented column [0 0 | 5] means no solution.',
      contrastCase: 'Contradiction row [0 0 | 5] â†’ no solution. Zero row [0 0 | 0] â†’ free variable, infinitely many solutions.',
    },
    {
      falseBelief: 'A system of n equations in n unknowns always has exactly one solution.',
      whyStudentsThinkIt: 'Square systems look "balanced" â€” same number of equations and unknowns â€” so students expect a unique answer.',
      correctionExample: 'The system x + y = 3, 2x + 2y = 6 is 2Ã—2 but has infinitely many solutions (the two equations are the same line). The system x + y = 3, 2x + 2y = 7 is also 2Ã—2 but has no solution (parallel lines).',
    ...
    },
    {
      ...
      correctionExample: 'Row operations on the augmented matrix [A|b] transform both sides simultaneously, preserving the solution set. The RREF matrix represents a DIFFERENT but EQUIVALENT system â€” one with the same solutions but easier to read.',
      contrastCase: 'Compare: 2x+y=5 and the equivalent x+0y=2 (from RREF). Both have x=2 as part of the solution â€” the system changed form but not substance.',
    },
  ],

  // â”€â”€ Transfer Prompts â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  transferPrompts: [
    {
      situation: 'You are given a 5Ã—3 overdetermined system (more equations than unknowns) representing sensor measurements with noise. Which technique applies?',
      competingTechniques: ['Gaussian elimination (exact solution)', 'Least squares (np.linalg.lstsq)', 'Ignoring extra equations'],
      whyThisTechniqueWins: 'Gaussian elimination finds exact solutions when they exist, but overdetermined systems usually have no exact solution. Least squares minimizes the residual ||Axâˆ’b||Â² and is the right tool for noisy data. Gaussian elimination remains essential as a subroutine inside the least-squares solver.',
    },
    {
      situation: 'You need to check whether three vectors in â„Â³ are linearly independent â€” without computing a determinant.',
      competingTechniques: ['Row-reduce the matrix formed by the three column vectors', 'Compute the determinant', 'Check by inspection'],
      whyThisTechniqueWins: 'Form the 3Ã—3 matrix with the vectors as columns and row-reduce. If you get 3 pivot columns (rank 3), they are independent. If rank < 3, they are dependent. This generalizes to any size â€” determinants only work for square matrices and give less insight into which vectors are redundant.',
    },
  ],

  // â”€â”€ Debugging â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  debugging: [
    {
      ...
      repairStrategy: 'Check np.linalg.matrix_rank(A) vs np.linalg.matrix_rank(np.column_stack([A, b])). If ranks differ â†’ no solution. If rank(A) < n â†’ free variables. Use np.linalg.lstsq for a minimum-norm solution in the infinite-solutions case.',
    },
    {
      commonError: 'Arithmetic error during row reduction gives the wrong pivot.',
      symptom: 'The RREF looks wrong â€” pivot is not 1, or zeros are not where expected.',
  ...
  // â”€â”€ Mastery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  mastery: {
    targetLevel: 3,
    solveIndependently: 'Row-reduce a 3Ã—3 augmented matrix to RREF by hand, identify pivot and free variables, and write the complete general solution with parameters.',
    explainVerbally: 'Explain why multiplying a row by zero is illegal, and why a pivot in the augmented column means no solution. Articulate the row picture vs the column picture using a specific 2Ã—2 example.',
