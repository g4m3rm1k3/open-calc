...
const LESSON_GEO_2_3 = {
  ...
  subtitle: 'What a proof actually is, why it matters, and how to write one from scratch.',
  sequential: true,
...
**The remedy for all of these:** Write slowly, one step at a time, and demand a specific justification for each step before moving on.`,
    ...
    // ── Challenges ────────────────────────────────────────────────────────────
    {
      ...
      options: [
        { label: 'A', text: 'No errors — if two angles are equal, ASA applies.' },
        { label: 'B', text: 'Two errors: (1) ASA requires two angles AND the included side to be equal — the student has given no side. (2) Even knowing two pairs of equal angles, you need a side to use ASA or AAS. The step jumps to congruence without establishing a side.' },
        { label: 'C', text: 'One error: the student should have used AA instead of ASA.' },
        { label: 'D', text: 'One error: 90° angles should use the HL criterion, not ASA.' },
      ],
      check: (label) => label === 'B',
      successMessage: 'Correct. ASA requires: two angles AND the included side between them, for both triangles, matching up as corresponding pairs. The student has: one pair of equal angles (∠A = ∠B = 90°). That is one condition out of three for ASA. The included side between what angles? There is no second angle stated, no side stated. AA congruence does not exist — AA gives similarity, not congruence. HL applies only when you already know you have right triangles with equal hypotenuses and a leg.',
...

    {
      ...
      options: [
        { label: 'A', text: 'Direct proof — assume equal alternate interior angles, immediately conclude the lines are parallel.' },
        { label: 'B', text: 'Proof by contradiction — assume the lines are NOT parallel (so they intersect), then show this forces the alternate interior angles to be unequal — contradicting the given. This is the standard proof of the Alternate Interior Angles Converse.' },
        { label: 'C', text: 'Proof by cases — consider acute and obtuse alternate interior angles separately.' },
        { label: 'D', text: 'Proof by induction — the result holds for one transversal, then the next.' },
      ],
      check: (label) => label === 'B',
      successMessage: 'Correct. The direct proof is difficult: "equal angles → parallel" is hard to establish directly, because parallelism (lines never meeting) is a negative, universal claim that\'s hard to prove positively. Contradiction works perfectly: assume NOT parallel → the lines meet at some point P, forming a triangle → the exterior angle theorem gives the alternate interior angles as unequal → contradicts the given. Contradiction is the natural strategy for "these lines have a certain property everywhere" when you can easily derive a contradiction from the opposite assumption.',
...

    {
      ...
      options: [
        { label: 'A', text: 'Step 1: Given. Step 2: Looks equal in diagram. Step 3: SSS.' },
        { label: 'B', text: 'Step 1: Reflexive Property of Equality. Step 2: Definition of midpoint. Step 3: Cannot conclude congruence yet — need a third pair of equal quantities (AB = AC is not given).' },
        { label: 'C', text: 'Step 1: Reflexive Property. Step 2: Definition of midpoint. Step 3: SSS (AM=AM, BM=MC, AB=AC).' },
        { label: 'D', text: 'Step 1: Transitive Property. Step 2: Given. Step 3: SAS.' },
      ],
      check: (label) => label === 'B',
      successMessage: 'Correct. Step 1: AM = AM by the Reflexive Property — any quantity equals itself. Step 2: BM = MC by the Definition of Midpoint — a midpoint divides a segment into two equal halves. Step 3: We cannot yet conclude △ABM ≅ △ACM. We have AM = AM (one side) and BM = MC (one side), but we don\'t know AB = AC — that would require the triangle to be isosceles, which is not given. The proof needs more information before claiming congruence.',
...
export default {
  ...
  title: 'Introduction to Proofs',
  subtitle: 'What a proof actually is, why it matters, and how to write one from scratch.',
  tags: ['geometry', 'proofs', 'logic', 'direct-proof', 'contradiction', 'contrapositive', 'justification', 'two-column-proof'],
  hook: {
    question: 'How do you prove something that "looks" obvious — and why isn\'t checking examples enough?',
    realWorldContext: 'Every theorem in mathematics, every verified algorithm in computer science, and every safety-critical system specification rests on proof. Understanding what makes an argument a proof — and what the common errors are — is the most transferable skill in formal mathematics.',
