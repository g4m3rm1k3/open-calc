...
const LESSON_GEO_2_1_LOGIC = {
  ...
  subtitle: 'The grammar of mathematical certainty — how geometry converts informal observations into airtight arguments.',
  sequential: true,
...
**Example:** The converse of "If AB ∥ CD and AB = CD, then ABCD is a parallelogram" would require proving that ABCD is a parallelogram implies both AB ∥ CD and AB = CD. Negating the conclusion gives ¬(AB ∥ CD ∧ AB = CD) = ¬(AB ∥ CD) ∨ ¬(AB = CD). A counterexample must show that at least one of the two conditions fails.`,
    ...
    // ── Challenges ────────────────────────────────────────────────────────────
    {
      ...
      options: [
        { label: 'A', text: 'Converse: "If diagonals are perpendicular, it is a square." (FALSE — a rhombus also has perpendicular diagonals). Contrapositive: "If diagonals are not perpendicular, it is not a square." (TRUE — equivalent to original). All four are true.' },
        { label: 'B', text: 'Original: TRUE. Converse: FALSE (rhombus has perpendicular diagonals but is not a square). Contrapositive: TRUE (equivalent to original). Inverse: FALSE (equivalent to converse).' },
        { label: 'C', text: 'All four are true because squares have special properties.' },
        { label: 'D', text: 'The converse and original are always the same for geometric theorems.' },
      ],
      check: (label) => label === 'B',
      successMessage: 'Correct. Original (P→Q): TRUE. A square\'s diagonals are perpendicular. Converse (Q→P): FALSE. A rhombus has perpendicular diagonals but is not a square (its angles aren\'t necessarily 90°). Contrapositive (¬Q→¬P): TRUE — always equivalent to the original. Inverse (¬P→¬Q): FALSE — equivalent to the converse. The rhombus is the key counterexample: it disproves both the converse and the inverse.',
...

    {
      ...
      options: [
        { label: 'A', text: 'The reasoning is valid — failure to find a counterexample is strong evidence for truth.' },
        { label: 'B', text: 'The reasoning is invalid. To prove a universal statement requires a proof, not failure to find a counterexample. To disprove it requires one counterexample. Consider an isosceles triangle with angles 80°-80°-20° — no 60° angle. This counterexample disproves the statement.' },
        { label: 'C', text: 'The reasoning is invalid because all triangles have 60° angles.' },
        { label: 'D', text: 'The reasoning is valid for geometric statements specifically.' },
      ],
      check: (label) => label === 'B',
      successMessage: 'Correct. The student commits a fundamental logical error: absence of a counterexample is not proof of truth. There are infinitely many isosceles triangles, and the student hasn\'t checked them all. A specific counterexample: an isosceles triangle with legs meeting at 20° has base angles of (180°-20°)/2 = 80°. Its angles are 80°, 80°, 20° — none equal to 60°. One counterexample is sufficient to disprove the universal claim.',
...

    {
      ...
      options: [
        ...
        { label: 'B', text: 'Substitution and Subtraction Property of Equality: from m∠A + m∠B = 180° and m∠B + m∠C = 180°, set the left sides equal (both = 180°), giving m∠A + m∠B = m∠B + m∠C. Subtract m∠B from both sides: m∠A = m∠C.' },
        { label: 'C', text: 'Definition of supplementary angles.' },
        { label: 'D', text: 'Vertical Angles Theorem.' },
      ],
      check: (label) => label === 'B',
      successMessage: 'Correct. The chain of reasoning: (1) m∠A + m∠B = 180° and m∠B + m∠C = 180° (given/definition of supplementary). (2) m∠A + m∠B = m∠B + m∠C (Substitution — both equal 180°, so they equal each other). (3) m∠A = m∠C (Subtraction Property of Equality — subtract m∠B from both sides). This is the standard proof that "supplements of the same angle are equal."',
...
export default {
  ...
  title: 'Logic and Statements',
  subtitle: 'The grammar of mathematical certainty — how geometry converts informal observations into airtight arguments.',
  tags: ['geometry', 'logic', 'conditional-statements', 'propositions', 'contrapositive', 'converse', 'quantifiers', 'definitions'],
  hook: {
    question: 'How does geometry convert "it looks obvious" into "it must be true"?',
    realWorldContext: 'Every theorem in geometry is a conditional statement with precise logical structure. Understanding conditionals, their converses, and quantifiers is what separates geometric proof from geometric observation.',
