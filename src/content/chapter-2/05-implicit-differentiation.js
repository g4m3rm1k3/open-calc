      ...
      steps: [
        {
          ...
          strategyTitle: 'Differentiate both sides with respect to x',
          checkpoint: 'Why is it valid to differentiate both sides of an equation simultaneously?',
          hints: [
            'Apply d/dx to the entire left side and the entire right side as separate operations.',
            'If two expressions are equal, their derivatives with respect to the same variable are also equal — this is a basic property of equality that licenses differentiating both sides.',
            'This is the same principle used in Ch2 Lesson 1 whenever we differentiate an identity like d/dx[x²] = 2x — we are implicitly differentiating both sides of y = x².',
          ],
        },
        {
          ...
          strategyTitle: 'Chain rule on y²: d/dx[y²] = 2y·(dy/dx)',
          checkpoint: 'Why does d/dx[y²] = 2y·(dy/dx) rather than just 2y?',
          hints: [
            'Treat y as the inner function and apply the chain rule: differentiate the outer function u² to get 2u = 2y, then multiply by the derivative of the inner function y with respect to x, which is dy/dx.',
            'Chain rule (Ch2 Lesson 2): d/dx[f(g(x))] = f\'(g(x))·g\'(x). Here f(u)=u² and g(x)=y(x), so d/dx[y²] = 2y·(dy/dx). Without this factor, you would be treating y as a constant rather than as a function of x.',
            'Every y-term in an implicit equation picks up a ·(dy/dx) factor for exactly this reason. This is the core mechanic of implicit differentiation — the chain rule applied to an unknown inner function y(x).',
          ],
        },
        {
          expression: "2y\\frac{dy}{dx} = -2x",
          annotation: 'Move the 2x term to the right side by subtracting it from both sides.',
          strategyTitle: 'Isolate dy/dx terms on one side',
          checkpoint: 'After moving terms, which side should contain all dy/dx expressions?',
          hints: [
            'Subtract 2x from both sides to move all non-dy/dx terms to the right, leaving only the dy/dx term on the left.',
            'The goal is to collect every term that contains the factor dy/dx on one side and everything else on the other — this sets up the factoring step that isolates dy/dx.',
            'This algebraic isolation mirrors the strategy in any related-rates problem (a future application of implicit differentiation): separate what you are solving for from everything else before dividing.',
          ],
        },
        {
          expression: "\\frac{dy}{dx} = \\frac{-2x}{2y} = -\\frac{x}{y}",
          annotation: 'Divide both sides by 2y to isolate dy/dx.',
          strategyTitle: 'Isolate dy/dx algebraically',
          checkpoint: 'After isolating dy/dx, what variables should the answer be expressed in terms of?',
          hints: [
            'Divide both sides by the coefficient of dy/dx, which is 2y, to leave dy/dx alone on the left.',
            'Because the original equation is in two variables x and y, the derivative dy/dx naturally involves both — this is normal and expected for implicit curves.',
            'The result dy/dx = −x/y matches the general formula dy/dx = −F_x/F_y for F(x,y) = x² + y² − 25: F_x = 2x and F_y = 2y, so −F_x/F_y = −x/y.',
          ],
        },
        {
          ...
          strategyTitle: 'Substitute the specific point (3, 4)',
          checkpoint: 'Before substituting, should you verify that (3, 4) actually lies on the curve x² + y² = 25?',
          hints: [
            'Plug x = 3 and y = 4 directly into the formula dy/dx = −x/y to evaluate the slope at this specific point.',
            'You must substitute both coordinates because dy/dx depends on both x and y — the slope of an implicit curve is determined by position, not just by x alone.',
            'Always verify the point satisfies the original equation first: 3² + 4² = 9 + 16 = 25 ✓. If a point does not lie on the curve, the derivative formula has no meaning there.',
          ],
        },
        {
          expression: "y - 4 = -\\frac{3}{4}(x - 3)",
          annotation: 'Use point-slope form with point (3, 4) and slope -3/4.',
          strategyTitle: 'Write point-slope form of tangent line',
          checkpoint: 'What two pieces of information are needed to write the equation of any line?',
          hints: [
            'Use the point-slope formula y − y₀ = m(x − x₀) with the known point (3, 4) and the slope m = −3/4 just computed.',
            'Point-slope form requires exactly one point on the line and the slope — both of which are now known. This is identical to writing a tangent line for an explicit function (from Ch2 Lesson 1).',
            'The tangent-line construction is the same whether the curve was defined explicitly or implicitly. Implicit differentiation only changes how you find the slope; everything after that is identical.',
          ],
        },
        {
          expression: "y = -\\frac{3}{4}x + \\frac{9}{4} + 4 = -\\frac{3}{4}x + \\frac{25}{4}",
          annotation: 'Simplify: 9/4 + 4 = 9/4 + 16/4 = 25/4.',
          strategyTitle: 'Simplify to slope-intercept form',
          checkpoint: 'Can you verify this answer geometrically — does the tangent line at (3, 4) look perpendicular to the radius at that point?',
          hints: [
            'Expand the right side: −(3/4)(−3) = 9/4, then add 4 = 16/4 to get the y-intercept 25/4.',
            'Converting from point-slope to slope-intercept is pure arithmetic. Keeping a common denominator (fourths here) avoids fraction errors.',
            'Geometric check: the radius from the origin to (3, 4) has slope 4/3. The tangent slope is −3/4. Since (4/3)(−3/4) = −1, they are negative reciprocals — confirming the tangent is perpendicular to the radius, as it must be for a circle.',
          ],
        },
      ],
      ...
      steps: [
        {
          ...
          strategyTitle: 'Differentiate both sides with respect to x',
          checkpoint: 'Before differentiating, identify every term that contains y — how many terms will require the chain rule?',
          hints: [
            'Apply d/dx to the full left side and the full right side. On the left you have two terms; on the right one product requiring the product rule.',
            'Equality is preserved under differentiation: if two expressions are equal, their derivatives are equal. This licenses differentiating the entire equation at once.',
            'This is the same opening move as in ex1 (circle). The difference is that the right side is now a product of x and y, which will require both the product rule and the chain rule.',
          ],
        },
        {
          ...
          strategyTitle: 'Chain rule on y³: d/dx[y³] = 3y²·(dy/dx)',
          checkpoint: 'What is the outer function and what is the inner function when applying the chain rule to y³?',
          hints: [
            'Differentiate d/dx[x³] = 3x² as usual. For d/dx[y³], treat y as the inner function: outer is u³, inner is y(x), so result is 3y²·(dy/dx).',
            'Chain rule (Ch2 Lesson 2): d/dx[u^n] = n·u^(n−1)·(du/dx). Here u = y(x), so d/dx[y³] = 3y²·(dy/dx). The pattern is the same as for y² in ex1, just with a higher power.',
            'Every y-term picks up a ·(dy/dx) factor — this is the universal rule of implicit differentiation. The right side 6xy needs the product rule next because it is a product of two functions of x.',
          ],
        },
        {
          ...
          strategyTitle: 'Product rule on xy: both x and y are functions of x',
          checkpoint: 'In the product rule applied to xy, why does the y term not get a ·(dy/dx) factor while the derivative of y does?',
          hints: [
            'Product rule: d/dx[u·v] = u\'·v + u·v\'. With u = x and v = y: u\' = 1, v\' = dy/dx. So d/dx[xy] = 1·y + x·(dy/dx) = y + x(dy/dx).',
            'Product rule (Ch2 Lesson 1) combined with chain rule (Ch2 Lesson 2): x differentiates to 1 (plain power rule), and y differentiates to dy/dx (chain rule because y is a function of x). Both rules act together.',
            'This combined product-plus-chain-rule situation is called the "product rule chain trap" in ch2 lesson 04b. Recognizing that y is a function of x is what prevents the classic mistake of treating the derivative of y as just 1.',
          ],
        },
        {
          expression: "3x^2 + 3y^2\\frac{dy}{dx} = 6y + 6x\\frac{dy}{dx}",
          annotation: 'Substitute the product rule result into the equation.',
          strategyTitle: 'Substitute the product rule result to get the full differentiated equation',
          checkpoint: 'After substitution, how many distinct terms contain dy/dx, and on which sides of the equation are they?',
          hints: [
            'Replace d/dx[xy] with y + x(dy/dx) in the right side, then distribute the factor of 6 to get 6y + 6x(dy/dx).',
            'Substitution keeps the equation in the form: (everything from the left) = (everything from the right), making it easy to spot which terms have dy/dx and which do not.',
            'At this stage, dy/dx appears on both sides — 3y²(dy/dx) on the left and 6x(dy/dx) on the right. The next step collects them all on one side, exactly as in the circle example.',
          ],
        },
        {
          ...
          strategyTitle: 'Collect all dy/dx terms on one side',
          checkpoint: 'Why must all dy/dx terms be gathered on the same side before factoring?',
          hints: [
            'Subtract 6x(dy/dx) from both sides (to move the right-side dy/dx term to the left) and subtract 3x² from both sides (to move the non-dy/dx left term to the right).',
            'Factoring dy/dx is only possible when all terms containing it are on the same side. This is the same algebraic principle as collecting like terms before solving a linear equation.',
            'This "collect and factor" move is the heart of the implicit differentiation procedure and applies to every implicit problem, no matter how complex: gather all dy/dx terms, factor, divide.',
          ],
        },
        {
          expression: "(3y^2 - 6x)\\frac{dy}{dx} = 6y - 3x^2",
          annotation: 'Factor dy/dx from the left side.',
          strategyTitle: 'Factor dy/dx from the collected terms',
          checkpoint: 'After factoring, what expression multiplies dy/dx, and what happens if that expression equals zero?',
          hints: [
            'Factor dy/dx out of 3y²(dy/dx) − 6x(dy/dx) to get (3y² − 6x)(dy/dx). The right side 6y − 3x² remains unchanged.',
            'Factoring dy/dx is like factoring any common variable from a sum: a·z + b·z = (a+b)·z. Here z = dy/dx, a = 3y², b = −6x.',
            'If the coefficient (3y² − 6x) equals zero, we cannot divide to isolate dy/dx — this corresponds to a vertical tangent on the folium, where dy/dx is undefined. This is exactly the F_y = 0 condition from the Implicit Function Theorem.',
          ],
        },
        {
          ...
          strategyTitle: 'Divide and simplify to isolate dy/dx',
          checkpoint: 'After simplifying by dividing by 3, can you connect the result to the general formula dy/dx = −F_x/F_y?',
          hints: [
            'Divide both sides by (3y² − 6x), then cancel the common factor of 3 from all four terms (numerator and denominator) to reach the simplified form (2y − x²)/(y² − 2x).',
            'The division step is valid wherever 3y² − 6x ≠ 0, i.e., y² ≠ 2x. Those are exactly the points of vertical tangency on the folium — the formula breaks down there.',
            'Check against the general formula: F(x,y) = x³ + y³ − 6xy, so F_x = 3x² − 6y and F_y = 3y² − 6x. Then −F_x/F_y = −(3x² − 6y)/(3y² − 6x) = (6y − 3x²)/(3y² − 6x) = (2y − x²)/(y² − 2x) ✓',
          ],
        },
      ],
