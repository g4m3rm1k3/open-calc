const lesson = {
  ...
  quiz: [
    {
      ...
      answer: "config.txt is renamed to settings.txt (and if settings.txt existed, it's overwritten)",
      hints: [
    ...
    },
    {
      ...
      answer: "cp requires the `-r` flag to copy directories recursively",
      hints: [
  ...
  definitions: [
    { term: "cp", definition: "Copies a file: `cp source dest`. The original is unchanged. Use `cp -r` for directories. Silently overwrites the destination if it already exists." },
    { term: "mv", definition: "Moves or renames a file. `mv old new` in the same directory is a rename. `mv file dir/` moves the file into a directory. The original path is gone after mv." },
    { term: "glob pattern", definition: "A wildcard pattern the shell expands before passing to a command. `*.js` matches all files ending in `.js`. `cp *.txt docs/` copies all txt files." },
  ],
};
