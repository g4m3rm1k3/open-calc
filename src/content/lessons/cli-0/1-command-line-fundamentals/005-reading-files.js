const lesson = {
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: "Updates its last-modified timestamp without changing contents",
      hints: [
  ...
  definitions: [
    { term: "touch", definition: "Creates an empty file if it doesn't exist. If the file already exists, updates its modification timestamp without changing contents." },
    { term: "cat", definition: "Prints a file's contents to the terminal. Originally 'concatenate' — `cat f1 f2` prints both in sequence. For long files, use `head` or `tail` instead." },
    { term: "head / tail", definition: "`head -n N file` prints the first N lines. `tail -n N file` prints the last N lines. Default is 10 lines when `-n` is omitted. Used to read log files without loading everything." },
    { term: "wc", definition: "Word count — measures file length. `wc -l` counts lines, `wc -w` counts words, `wc -c` counts bytes. Useful before reading an unknown file to gauge its size." },
  ],
};
