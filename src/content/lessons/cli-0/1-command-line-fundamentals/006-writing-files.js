const lesson = {
  ...
  rigor: {
    prose: [
      ...
      "**Process substitution and here-docs.** Beyond `>` and `>>`, shells support `<<EOF ... EOF` (here-document) to write multi-line content inline. Single-quoted `'EOF'` prevents variable expansion inside. This is a common pattern for writing configuration files in deploy scripts.",
    ],
    callouts: [
      {
        type: "warning",
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: "`grep 'ERROR' app.log > errors.txt`",
      hints: [
  ...
  definitions: [
    { term: "output redirection (>)", definition: "Sends a command's stdout into a file, creating it if it doesn't exist and overwriting it if it does. `echo 'text' > file.txt` replaces file.txt with 'text'." },
    { term: "append (>>)", definition: "Sends a command's stdout to the end of a file without removing existing content. `echo 'line' >> file.txt` adds a new line each time it's run." },
    { term: "stderr", definition: "Standard error — file descriptor 2. Error messages go here by default, separately from stdout. Redirect with `2>`. This is why errors still appear when you use `>`." },
    { term: "/dev/null", definition: "A special file that discards all data written to it. `command > /dev/null` silences stdout. `command 2>/dev/null` silences error messages." },
  ],
};
