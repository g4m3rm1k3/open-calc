const lesson = {
  ...
  rigor: {
    prose: [
      ...
      "**grep vs. find — different searches.** `grep` searches file contents (what's inside). `find` searches the filesystem (names and paths). They're complementary: `find . -name '*.log' | xargs grep 'ERROR'` uses find to locate all log files, then pipes their names to grep to search inside them.",
    ],
    callouts: [
      {
        type: "tip",
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: "Finds all files (not directories) with names ending in .test.js",
      hints: [
  ...
  definitions: [
    { term: "grep", definition: "Searches file contents for lines matching a pattern. `grep -rn 'term' .` searches all files recursively with line numbers. The pattern is a regular expression." },
    { term: "find", definition: "Searches for files and directories by name or type. `find . -name '*.js'` finds all JS files. `find . -type d` finds only directories." },
    { term: "which", definition: "Locates a command's executable in $PATH. `which node` returns the full path to the node binary. Useful when multiple versions are installed." },
    { term: "regex", definition: "Regular expression — a pattern language used by grep and other tools. `^` anchors to line start, `$` to line end, `.` matches any character, `*` means zero or more." },
  ],
};
