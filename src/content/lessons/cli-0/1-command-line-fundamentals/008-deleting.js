const lesson = {
  ...
  quiz: [
    {
      ...
      answer: "It's removed from the filesystem immediately with no built-in recovery",
      hints: [
    ...
    },
    {
      ...
      answer: "Suppresses error messages for missing files",
      hints: [
  ...
  definitions: [
    { term: "rm", definition: "Removes a file permanently with no Trash or undo. `rm file.txt` deletes the file immediately. No confirmation by default." },
    { term: "rm -r", definition: "Removes a directory and all its contents recursively. The `-r` flag is required for directories — without it, rm refuses to delete them." },
    { term: "rm -rf", definition: "Recursive force delete: removes a directory tree and suppresses errors for missing files. Used for build artifacts (rm -rf dist/); dangerous when the path is wrong." },
  ],
};
