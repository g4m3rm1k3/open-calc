const lesson = {
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: "rmdir only works on empty directories — this is a safety feature",
      hints: [
        "`rmdir` is intentionally designed to only remove empty directories. This prevents you from accidentally deleting an entire directory tree with a typo. To remove a directory and its contents, you must deliberately use `rm -r`.",
      ],
    },
  ],

  definitions: [
    { term: "mkdir", definition: "Creates a directory. `mkdir name` creates a single directory in the current location. The parent directory must already exist." },
    { term: "mkdir -p", definition: "Creates a directory and all missing parent directories in one command: `mkdir -p a/b/c` creates `a/`, `a/b/`, and `a/b/c/` if they don't exist." },
    { term: "rmdir", definition: "Removes an empty directory. Fails if the directory contains files — a deliberate safety feature. Use `rm -r` to remove a directory with contents." },
  ],
};
