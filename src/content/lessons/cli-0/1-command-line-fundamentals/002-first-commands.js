const lesson = {
  ...
  quiz: [
    {
      ...
      answer: "Lists files in long format (showing permissions, size, and date)",
      hints: [
    ...
    },
    {
      ...
      answer: "`echo hello world` passes two arguments; `echo 'hello world'` passes one",
      hints: [
  ...
  definitions: [
    { term: "flag", definition: "A modifier to a command that changes its behavior. Short flags use a single dash: `-l`, `-a`. Long flags use double dash: `--verbose`. Short flags can be combined: `-la` = `-l -a`." },
    { term: "argument", definition: "The thing a command acts on — a file, directory, or value passed after the command name and flags. `cat README.md` has one argument: `README.md`." },
    { term: "exit code", definition: "An integer a command exits with: 0 means success, non-zero means failure. Accessible via `$?`. Shell scripts use exit codes to detect and handle errors." },
    { term: "man page", definition: "The built-in manual for any Unix command. `man ls` opens the full reference for `ls`. Navigate with arrow keys, search with `/pattern`, quit with `q`." },
  ],
};
