export default {
  ...
  subtitle:
    "Observational studies vs. experiments, and every way data collection can go wrong.",
  tags: [
    "study design",
    "observational study",
    "experiment",
    ...
    visualizations: [
      {
        ...
        type: "flowchart",
        purpose:
          "Gives a repeatable decision process for choosing causal vs associational wording.",
        misconceptionAddressed:
          "Any strong correlation can be written as a causal claim.",
        invariant:
          "Without random assignment (or strong causal identification), conclusions must remain associational.",
      },
      {
        id: "stat1-003-viz-2",
        ...
        type: "diagram",
        purpose:
          "Shows where each bias enters: who gets in, who responds, how outcomes are measured, and what confounds remain.",
        misconceptionAddressed:
          "Bias is one generic error instead of multiple distinct failure modes.",
        invariant:
          "More data reduces variance, but it does not remove systematic bias introduced upstream.",
      },
    ],
  },
    ...
    cells: [
      {
        ...
        type: "python",
        cellTitle:
          "Simulate confounding: lurking variable distorts a relationship",
        code: `import random
random.seed(42)
...
`,
        instructions:
          "The correlation is strong (~0.8) even though ice cream has zero direct effect on drowning. To detect confounding in a real study, you would need to hold temperature constant (control for it) or use random assignment. This is why observational data alone cannot establish causation.",
      },
      {
        id: "stat1-003-cell-2",
        type: "python",
...
`,
        instructions:
          "The observational estimate is distorted by age (older patients both receive the drug more AND have higher baseline BP). Random assignment breaks this link — the treated and control groups have similar age distributions by chance, so the estimate is close to the true −8 point effect.",
      },
    ],
  },
  ...
  examples: [
    {
      ...
      difficulty: "easy",
      problem:
        "Researchers track 10,000 adults for 15 years. They record coffee consumption at baseline and track cardiovascular events. Coffee drinkers have 20% fewer heart attacks than non-drinkers. (a) What type of study is this? (b) What conclusion is valid? (c) What is the key limitation?",
      steps: [
        {
          expression: "\\text{Prospective cohort study — observational}",
          annotation:
            "A group (cohort) is followed forward in time. Researchers observe but do not assign coffee consumption. No random assignment → observational study.",
          strategyTitle: "Step 1: Identify study type",
        },
        {
          expression:
            '\\text{Valid: "Coffee consumption is associated with a 20\\% lower rate of heart attacks in this cohort."}',
          annotation:
            'Associational language is correct: "associated with," not "causes." We observed a correlation, not a causal effect.',
          strategyTitle: "Step 2: Valid conclusion",
        },
        {
          expression:
            "\\text{Limitation: unmeasured confounders. Coffee drinkers may also exercise more, smoke less, etc.}",
          annotation:
            "The key limitation of all observational studies: variables not measured or adjusted for could explain the association. The study cannot establish causality — only that coffee drinking and lower heart attack rates co-occur.",
          strategyTitle: "Step 3: Key limitation",
        },
      ],
    },
    {
      ...
      difficulty: "medium",
      problem:
        'A company compares sales performance between employees who attended a 2-day training program and those who did not. Trained employees sell 15% more on average. HR concludes "the training causes a 15% sales improvement." Identify the flaw and propose a valid study design.',
      steps: [
        {
          expression: "\\text{Flaw: self-selection confounding}",
          annotation:
            "Employees who choose to attend training (or are selected for it) likely differ from non-attendees in motivation, tenure, ability, or manager support. Any of these could explain higher sales, independent of the training itself.",
          strategyTitle: "Step 1: Identify the confounder",
        },
        {
          expression:
            '\\text{Valid conclusion: "Employees who attended training sold 15\\% more than those who did not."}',
          annotation:
            "This is a descriptive associational claim. It does not imply causation. The training group and comparison group are not comparable on baseline characteristics.",
          strategyTitle: "Step 2: Correct language",
        },
        {
          expression:
            "\\text{Valid design: randomly assign eligible employees to training vs. waitlist control}",
          annotation:
            "Randomization makes the two groups comparable at baseline. Any difference in sales after the training period can then be attributed to the training itself — not to pre-existing differences between groups.",
          strategyTitle: "Step 3: Propose valid design",
        },
      ],
    },
    {
      ...
      difficulty: "hard",
      problem:
        "Hospital A treats 800 patients: 600 are low-risk, 200 are high-risk. Hospital A's overall survival rate is 85%. Hospital B treats 1,000 patients: 200 are low-risk, 800 are high-risk. Hospital B's overall survival rate is 75%. A health reporter concludes \"Hospital A is better.\" But: Low-risk patients in Hospital A survive at 90%; in Hospital B, also 90%. High-risk patients: Hospital A 70%, Hospital B 72%. Explain the paradox.",
      steps: [
        {
          expression:
            "\\text{Hospital A overall: } \\frac{600(0.90) + 200(0.70)}{800} = \\frac{540+140}{800} = \\frac{680}{800} = 85\\%",
          annotation:
            "Hospital A treats mostly low-risk patients (600/800 = 75%). Their high survival rate is driven by case mix, not quality.",
          strategyTitle: "Step 1: Verify Hospital A",
        },
        {
          expression:
            "\\text{Hospital B overall: } \\frac{200(0.90) + 800(0.72)}{1000} = \\frac{180+576}{1000} = \\frac{756}{1000} = 75.6\\%",
          annotation:
            "Hospital B treats mostly high-risk patients (800/1000 = 80%). Within each risk group, Hospital B matches or beats Hospital A.",
          strategyTitle: "Step 2: Verify Hospital B",
        },
        {
          expression:
            "\\text{Within each stratum: } \\underbrace{90\\% = 90\\%}_{\\text{low-risk}} \\text{ and } \\underbrace{70\\% < 72\\%}_{\\text{high-risk, B is better}}",
          annotation:
            "Within both risk groups, Hospital B is at least as good. The aggregate comparison is misleading because patient risk level (the confounder) is distributed very differently across hospitals.",
          strategyTitle: "Step 3: The reversal explained",
        },
        {
          expression:
            "\\text{Correct comparison: control for patient risk level}",
          annotation:
            "This is Simpson's Paradox in action. The confounder (risk level) is associated with both hospital choice (high-risk patients go to specialized centers) and outcome (higher risk → lower survival). Always condition on relevant confounders before comparing groups.",
          strategyTitle: "Step 4: Resolution",
        },
      ],
    },
  ],
  ...
  challenges: [
    {
      ...
      difficulty: "medium",
      problem:
        "A mobile app company wants to test whether a new onboarding flow (version B) increases 7-day retention compared to the current flow (version A). They have 50,000 new users per week. Design a valid experiment: state the unit of randomization, how to assign users, what the outcome is, and what conclusion is valid if B's 7-day retention is 2 percentage points higher.",
      walkthrough: [
        {
          expression:
            "\\text{Unit of randomization: individual user (not day-of-week or device type)}",
          annotation:
            "Randomize at the user level so that each new user is independently assigned to A or B with probability 0.5. This prevents day-of-week confounding (which would occur if you showed A on Monday and B on Tuesday).",
        },
        {
          expression:
            "P(T=B) = 0.50 \\text{ for each user, independent of all other users}",
          annotation:
            "Each new user is assigned to version B with 50% probability using a random number draw. This is the random assignment that creates comparable groups.",
        },
        {
          expression:
            "\\text{Outcome: } Y_i = \\mathbf{1}[\\text{user } i \\text{ opens the app on day 7}]",
          annotation:
            "7-day retention is a binary outcome (returned or not on day 7). Measure this for every user in both groups over the same 1-week period.",
        },
        {
          expression:
            '\\hat{\\Delta} = \\bar{Y}_B - \\bar{Y}_A = 0.02 \\Rightarrow \\text{"Version B causes a 2pp increase in 7-day retention"}',
          annotation:
            "Because assignment was random, the 2 percentage point difference is attributable to the onboarding flow, not to pre-existing user differences. This is a valid causal claim within the study population.",
        },
      ],
      answer:
        "Randomize at the user level with 50/50 split. Measure 7-day binary retention for both groups simultaneously. A 2pp difference under random assignment is a valid causal estimate of the effect of onboarding B on retention.",
    },
    {
      id: "stat1-003-ch2",
      ...
      walkthrough: [
        ...
        {
          expression:
            "\\text{2. Response bias (social desirability): self-reported exercise is overstated}",
          annotation:
            "When people self-report exercise hours, they tend to round up (reporting 4 hours when they actually did 3) because exercise is socially desirable. This also pushes measured exercise hours upward.",
        },
        {
          expression:
            "\\text{3. Confounding: health-conscious people both exercise more AND eat better, sleep better, etc.}",
          annotation:
            'If the study concludes "exercise causes better health," the conclusion is confounded: health-conscious lifestyle (diet, sleep, stress management) is associated with both exercise frequency and health outcomes. The exercise-health correlation may partly reflect this broader lifestyle difference.',
        },
        {
          expression:
            "\\text{Overall direction of bias: exercise-health correlation is likely overestimated}",
          annotation:
            "All three biases point in the same direction: the sample exercises more than the general population, reports even more than they actually do, and is healthier for confounded reasons. The study overstates how much and how strongly exercise predicts health in the general population.",
        },
      ],
      answer:
        "Three biases: (1) selection — app users are not representative, exercise hours overestimated; (2) response — social desirability inflates self-reported exercise; (3) confounding — overall healthy lifestyle explains both exercise and health outcomes.",
    },
    {
      id: "stat1-003-ch3",
      ...
      difficulty: "hard",
      problem:
        "A randomized controlled trial tests a new anxiety treatment. 200 patients with severe anxiety disorder at a single academic medical center are randomly assigned to the new treatment (n=100) or standard care (n=100). At 12 weeks, the treatment group shows a 30-point reduction in anxiety scores vs. 15-point reduction in the control group. (a) Is internal validity strong? (b) Is external validity strong? (c) What populations and settings can this conclusion be generalized to?",
      walkthrough: [
        {
          expression:
            "\\text{Internal validity: strong — random assignment, controlled setting}",
          annotation:
            "Random assignment of 200 patients to treatment and control ensures the two groups are comparable at baseline. The 15-point difference in improvement is attributable to the treatment, not to baseline differences. Internal validity is strong.",
        },
        {
          expression:
            "\\text{External validity: limited — single center, severe anxiety, academic medical setting}",
          annotation:
            "The study population: patients with severe anxiety disorder at one academic medical center. This excludes mild/moderate anxiety, patients who would not seek treatment at an academic center, patients with comorbidities that lead to exclusion, and patients in different healthcare systems.",
        },
        {
          expression:
            "\\text{Generalize to: adults with severe anxiety disorder in similar academic settings who resemble the enrolled patients}",
          annotation:
            'The finding "this treatment reduces anxiety by 15 points more than standard care, in patients with severe anxiety at an academic medical center over 12 weeks" is the valid generalized claim. Further generalization requires replication in community settings, different severity levels, and diverse populations.',
        },
      ],
      answer:
        "Strong internal validity (random assignment). Limited external validity: results apply to patients with severe anxiety at academic medical centers. Generalization to other settings and severity levels requires additional studies.",
    },
  ],
  ...
  semantics: {
    core: [
      {
        symbol: "\\text{Bias}(\\hat{\\theta})",
        meaning:
          "Systematic error in estimation: E[θ̂] − θ. Positive = overestimation, negative = underestimation. More data does not fix bias.",
      },
      {
        symbol: "T \\perp\\!\\!\\!\\perp \\mathbf{X}",
        meaning:
          "Treatment assignment is independent of all covariates — guaranteed by randomization in an experiment",
      },
      {
        symbol: "Y_i(t)",
        meaning:
          "Potential outcome for unit i under treatment t — the outcome that would be observed if unit i were assigned to condition t",
      },
      {
        symbol: "\\text{ATE}",
        meaning:
          "Average Treatment Effect: E[Y(1) − Y(0)] — the expected causal effect of treatment across the population",
      },
      {
        symbol: "\\rho_{XY \\cdot Z}",
        meaning:
          "Partial correlation between X and Y after controlling for confounder Z — the association remaining after adjustment",
      },
    ],
    rulesOfThumb: [
    ...
    futureLinks: [
      {
        ...
        note: "Confidence intervals and hypothesis tests assume no bias — they measure precision, not accuracy. A biased study with perfect precision is worse than an imprecise unbiased one.",
      },
      {
        lessonId: "stat7-001",
        label: "Chi-square Tests",
        note: "Chi-square tests for independence in contingency tables are one way to detect and quantify confounding in observational studies.",
      },
      {
        lessonId: "stat8-001",
        label: "Linear Regression",
        note: 'Multiple regression is used to "control for" confounders in observational studies by including them as covariates — though this only works for measured confounders.',
      },
    ],
  },
  ...
  checkpoints: [
    {
      id: "cp-stat1-003-1",
      label:
        "Read: distinguish observational study vs. experiment with one example each",
      type: "read",
    },
    {
      id: "cp-stat1-003-2",
      label:
        "Read: define confounding and explain why it prevents causal conclusions",
      type: "read",
    },
    {
      id: "cp-stat1-003-3",
      label: "Read: name the four bias types and give one example of each",
      type: "read",
    },
    {
      id: "cp-stat1-003-4",
      label:
        "Apply the evaluation procedure to example 2 before reading the solution",
      type: "example",
    },
    {
      id: "cp-stat1-003-5",
      label:
        "Complete example 3: trace through Simpson's Paradox arithmetic step by step",
      type: "example",
    },
    {
      id: "cp-stat1-003-6",
      label:
        "Attempt challenge 2: identify all three bias types in the fitness app survey",
      type: "challenge",
    },
    {
      id: "cp-stat1-003-7",
      label:
        "Read: explain the difference between bias and variance using the MSE formula",
      type: "read",
    },
    {
      id: "cp-stat1-003-8",
      label:
        "Read: state the two conditions that distinguish internal from external validity",
      type: "read",
    },
  ],
    ...
    questions: [
      {
        ...
        instructions:
          "The key feature is random assignment to treatment conditions by the researchers.",
      },
    ],
  },
  ...
  quiz: [
    {
      ...
      answer:
        "A variable associated with both the exposure and the outcome, distorting the apparent relationship",
      hints: [
        "A confounder affects both sides of the relationship you are studying.",
        "Coffee drinkers also tend to have other lifestyle differences — which of these is a confounder?",
      ],
    ...
    },
    {
      ...
      answer:
        "Bias is a systematic error that does not average out with more data",
      hints: [
        "A miscalibrated scale reads 2 kg heavy. Does weighing 10,000 objects fix the calibration?",
        "Bias is not random — it consistently shifts estimates in one direction.",
      ],
    ...
    },
    {
      ...
      answer:
        "Causality cannot be established because temporal precedence is unknown and confounders exist",
      hints: [
        "What type of study is cross-sectional — observational or experimental?",
        "Can a cross-sectional study tell you what came first?",
      ],
    ...
    },
    {
      ...
      hints: [
        "Single-blind = patient doesn't know. Double-blind adds who else?",
        "The purpose is to prevent both placebo effects and assessment bias.",
      ],
      reviewSection:
        'Intuition → "Blinding removes response and measurement bias" paragraph',
    },
    {
      id: "stat1-003-quiz-5",
      type: "choice",
      ...
      answer:
        "The causal conclusion is valid within the study, but may not generalize to other populations or settings",
      hints: [
        "Internal = causal validity within the study. External = generalizability beyond the study.",
        "These are separate and can be high or low independently.",
      ],
    ...
    },
    {
      ...
      answer:
        "An aggregate comparison gives the opposite conclusion from comparisons within subgroups, due to a confounder",
      hints: [
        "Recall the hospital example: overall Hospital A looked better, but within each risk group Hospital B matched or exceeded Hospital A.",
  ...
  misconceptions: [
    {
      ...
      whyStudentsThinkIt:
        "Students (and sometimes journalists) conflate statistical power (larger studies detect smaller effects) with study validity (the design actually measures what it claims to measure).",
      correctionExample:
        "Literary Digest's 2.4-million-response poll was enormous but biased. The size made the editors more confident in a wrong answer. Size amplifies whatever the study is measuring — including systematic error.",
      contrastCase:
        "A 200-person double-blind RCT has much higher internal validity than a 2-million-person observational study on the same question, if the observational study has unmeasured confounding.",
    },
    {
      falseBelief:
        "Controlling for a variable in regression removes all confounding.",
      whyStudentsThinkIt:
        'Students learn that including a confounder as a covariate in regression "adjusts" for it, and incorrectly generalize this to mean all confounding is resolved.',
      correctionExample:
        "Regression can only control for variables that were measured and included. Unmeasured confounders are not controlled. If you study the effect of income on health but do not measure diet (which correlates with both income and health), the income coefficient is still confounded.",
      contrastCase:
        "In an RCT, randomization controls for all confounders — measured and unmeasured — by making the treatment groups exchangeable at baseline. This is why RCTs are more credible than adjusted observational studies for causal questions.",
    },
    {
      falseBelief: "Correlation between X and Y means X causes Y.",
      whyStudentsThinkIt:
        "The association is visible in data and easy to interpret; the causal direction and confounding are invisible without additional reasoning.",
      correctionExample:
        "Ice cream sales and drowning deaths are correlated — both increase in summer. The confounder is temperature (hot weather → people swim more → more drownings; hot weather → people buy ice cream). Ice cream does not cause drownings.",
      contrastCase:
        "Smoking and lung cancer are also correlated. Unlike ice cream/drownings, decades of evidence including dose-response relationships, biological mechanisms, and natural experiments support causality. Correlation alone never proves causation, but additional evidence can accumulate to make causation highly credible.",
    },
  ],
  ...
  debugging: [
    {
      commonError:
        'Using language like "X leads to Y" or "X improves Y" for an observational finding.',
      symptom:
        'A report says "employees who exercised during work hours showed improved productivity" and concludes the exercise program caused the improvement.',
      whyItHappened:
        "Causal language is more compelling than associational language, so writers unconsciously use it even when the study design does not support it.",
      repairStrategy:
        'In observational studies, restrict language to: "is associated with," "is correlated with," "was observed alongside," "predicts" (in a statistical, not causal, sense). Reserve "causes," "leads to," "improves," "reduces" for randomized experiments or studies with explicit causal identification strategies (e.g., instrumental variables, regression discontinuity).',
    },
    {
      commonError: "Ignoring non-response when reporting survey results.",
      symptom:
        'A survey was sent to 1,000 customers; 200 responded. The researcher reports the mean satisfaction score of the 200 respondents as "customer satisfaction."',
      whyItHappened:
        "The responding sample is easy to analyze; the non-respondents left no data. It is easy to forget they exist.",
      repairStrategy:
        "Always report the response rate and acknowledge non-response bias. If the 800 non-respondents are systematically different (e.g., dissatisfied customers ignore surveys), the 200-respondent estimate is biased. Strategies to mitigate: follow up with non-respondents, compare early vs. late responders (late responders resemble non-responders more), use weighting if demographic data on non-respondents is available.",
    },
  ],
  ...
  mastery: {
    targetLevel: 3,
    solveIndependently:
      "Given a study, determine whether causal or associational language is appropriate, identify at least two confounders or bias sources, and propose a design change that would address the most serious threat.",
    explainVerbally:
      "Explain Simpson's Paradox using the hospital example: why does Hospital A look better overall but Hospital B is better within every patient risk stratum?",
    detectIncorrectApplication:
      "Identify when causal language is used for an observational finding and explain what additional evidence would be needed to support a causal conclusion.",
    transferToUnfamiliar:
      "Given a novel study you have not seen (e.g., a business A/B test, a public health cohort, a school intervention), classify it, evaluate its validity, and state what conclusions are and are not supported by the design.",
  },
};
