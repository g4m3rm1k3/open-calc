export default {
  ...
  aliases:
    "simple random sample stratified sampling cluster sampling systematic sampling convenience sample probability sample",
  timeToComplete: 30,
  coreConcept:
    "A probability sample gives every unit in the population a known, nonzero chance of selection. The sampling method you choose determines whether your estimates are valid and how efficiently you can achieve a target margin of error.",
  prerequisites: ["stat1-001"],
  nextLesson: "stat1-003",

  hook: {
    question:
      "Can a sample of 1,000 voters accurately represent 250 million? Yes — but only if you choose them correctly.",
    realWorldContext:
      "In 1936, Literary Digest magazine mailed 10 million ballots and received 2.4 million responses. They predicted Alf Landon would beat Franklin Roosevelt by a landslide. Roosevelt won by one of the largest margins in history. The problem was not the sample size — 2.4 million is enormous. The problem was how the sample was chosen: from telephone directories and car registration lists, which in 1936 overrepresented wealthy, Republican-leaning households. A Gallup poll of 50,000 using proper random selection predicted the correct winner. This is the central lesson of sampling: method matters more than size.",
  },
    ...
    visualizations: [
      {
        ...
        type: "table",
        purpose:
          "Helps beginners separate method, cost, and inference validity in one view.",
        misconceptionAddressed:
          "Larger n automatically guarantees valid inference, regardless of sampling design.",
        invariant:
          "Only probability samples support classical confidence intervals for population claims.",
      },
      {
        id: "stat1-002-viz-2",
        ...
        type: "diagram",
        purpose:
          "Shows why many similar observations in one cluster add less information than the same count spread across clusters.",
        misconceptionAddressed:
          "Measuring more units always increases effective sample size one-for-one.",
        invariant:
          "When intracluster correlation rho increases, effective sample size decreases for fixed n.",
      },
    ],
  },
...
`,
        instructions:
          "Run the cell. The convenience sample mean is consistently higher than the population mean because we are sampling from the top of a sorted list. Change the sorting to reverse=False (low GPA first) — the bias flips. SRS error is small and random, not systematic.",
      },
      {
        id: "stat1-002-cell-2",
        type: "python",
...
`,
        instructions:
          "Stratified sampling has lower standard deviation of estimates — more precise. This advantage is largest when strata have very different means (like freshmen vs. seniors here). If all strata had the same mean, stratified and SRS would be equally precise.",
      },
    ],
  },
  ...
  examples: [
    {
      ...
      difficulty: "easy",
      problem:
        "A company has 400 employees (numbered 1–400) and wants to survey 40 about job satisfaction. Describe the SRS procedure and compute the selection probability for each employee.",
      steps: [
        {
          expression: "N = 400, \\quad n = 40",
          annotation: "Identify population size N and desired sample size n.",
          strategyTitle: "Step 1: State N and n",
        },
        {
          expression: "\\pi_i = \\frac{n}{N} = \\frac{40}{400} = 0.10",
          annotation:
            "Every employee has a 10% chance of being selected. This equal probability is the defining property of SRS.",
          strategyTitle: "Step 2: Selection probability",
        },
        {
          expression:
            "\\text{Procedure: generate 40 distinct random integers from } [1, 400]",
          annotation:
            "Use a random number generator (Python: random.sample(range(1,401), 40), Excel: RANDBETWEEN or RAND). Survey those 40 employees. This guarantees equal probability and no duplication.",
          strategyTitle: "Step 3: Execute the draw",
        },
      ],
    },
    {
      ...
      steps: [
        {
          expression:
            "W_1 = \\frac{2400}{8000} = 0.30, \\quad W_2 = 0.30, \\quad W_3 = 0.20, \\quad W_4 = 0.20",
          annotation:
            "Compute stratum weights. Freshmen and sophomores are each 30% of the population; juniors and seniors are each 20%.",
          strategyTitle: "Step 1: Stratum weights",
        },
        {
          expression:
            "n_1 = 0.30 \\times 200 = 60, \\quad n_2 = 60, \\quad n_3 = 40, \\quad n_4 = 40",
          annotation:
            "Proportional allocation: multiply each weight by the total sample size. Allocate 60 freshmen, 60 sophomores, 40 juniors, 40 seniors.",
          strategyTitle: "Step 2: Proportional allocation",
        },
        {
          expression:
            "\\bar{y}_{st} = 0.30 \\bar{y}_1 + 0.30 \\bar{y}_2 + 0.20 \\bar{y}_3 + 0.20 \\bar{y}_4",
          annotation:
            "After collecting data, the stratified estimator is the weighted average of stratum sample means.",
          strategyTitle: "Step 3: Estimator formula",
        },
        {
          expression:
            "\\text{More precise than SRS because: between-stratum variation is controlled}",
          annotation:
            "The four year groups have different means (20, 16, 14, 12 hrs). An SRS might by chance draw too many freshmen (raising the estimate) or too few seniors. Stratified sampling guarantees the right proportions, so the only remaining variance is within-stratum variation.",
          strategyTitle: "Step 4: Precision argument",
        },
      ],
    },
    {
      ...
      difficulty: "medium",
      problem:
        "For each study, identify the sampling method used and state one key risk:\n(a) A quality-control manager inspects every 20th product off an assembly line.\n(b) A health researcher divides a city into 50 neighborhoods, randomly selects 8 neighborhoods, then surveys all households in those 8 neighborhoods.\n(c) A psychology professor recruits study participants from her own intro class.",
      steps: [
        {
          expression: "\\text{(a) Every 20th item: systematic sampling}",
          annotation:
            "Risk: if there is a periodic defect pattern with period 20 (e.g., a machine calibration drift that repeats every 20 units), the sample will either always catch it or always miss it. Check whether the sampling interval could align with any production cycle.",
          strategyTitle: "(a) Systematic — period risk",
        },
        {
          expression:
            "\\text{(b) 8 random neighborhoods, all households: cluster sampling}",
          annotation:
            "Risk: households within a neighborhood are more similar to each other than to random households city-wide (shared income, ethnicity, housing type). The intracluster correlation $\\rho > 0$ reduces the effective sample size below the total households surveyed.",
          strategyTitle: "(b) Cluster — intracluster correlation",
        },
        {
          expression: "\\text{(c) Own intro class: convenience sample}",
          annotation:
            "Risk: intro psychology students are not representative of the general population (younger, more educated, WEIRD — Western, Educated, Industrialized, Rich, Democratic). Results cannot be validly generalized beyond this specific population. Not a probability sample.",
          strategyTitle: "(c) Convenience — generalizability",
        },
      ],
    },
  ],
  ...
  challenges: [
    {
      ...
      difficulty: "medium",
      problem:
        "You are designing a poll to estimate the proportion of registered voters in a state who support a ballot measure. The state has three regions: Urban (1.2 million voters), Suburban (1.8 million voters), Rural (0.6 million voters). Support levels are expected to differ substantially by region. Your total budget allows n = 600 interviews. (a) Identify why stratified sampling is preferred over SRS here. (b) Compute proportional allocation. (c) State the stratified estimator formula.",
      walkthrough: [
        {
          expression:
            "N = 1{,}200{,}000 + 1{,}800{,}000 + 600{,}000 = 3{,}600{,}000",
          annotation: "Total registered voter population. Identify N first.",
        },
        {
          expression:
            "W_{\\text{Urban}} = \\frac{1.2M}{3.6M} = 0.333 \\quad W_{\\text{Suburban}} = 0.500 \\quad W_{\\text{Rural}} = 0.167",
          annotation:
            "Stratum weights: Urban = 33.3%, Suburban = 50%, Rural = 16.7%.",
        },
        {
          expression:
            "n_{\\text{Urban}} = 0.333 \\times 600 = 200 \\quad n_{\\text{Suburban}} = 300 \\quad n_{\\text{Rural}} = 100",
          annotation:
            "Proportional allocation. Each stratum contributes its exact population proportion of the total sample.",
        },
        {
          expression:
            "\\hat{p}_{st} = 0.333 \\hat{p}_{\\text{Urban}} + 0.500 \\hat{p}_{\\text{Sub}} + 0.167 \\hat{p}_{\\text{Rural}}",
          annotation:
            "Weighted average of stratum sample proportions. This is design-unbiased for the true population proportion p.",
        },
        {
          expression:
            "\\text{Preferred over SRS: support rates differ by region; SRS risks under-representing the smaller Rural stratum}",
          annotation:
            "With SRS, the 600 interviews might yield only ~97 rural voters by chance (16.7% × 600). Stratified sampling guarantees exactly 100, eliminating that source of variability.",
        },
      ],
      answer:
        "Stratified sampling with proportional allocation: 200 Urban, 300 Suburban, 100 Rural. Estimator: weighted average of stratum proportions with weights 0.333, 0.500, 0.167.",
    },
    {
      id: "stat1-002-ch2",
      ...
      difficulty: "hard",
      problem:
        "A city wants to know the average number of potholes per block. The city has 2,000 blocks. A researcher picks a random starting block, then surveys every 40th block: blocks 7, 47, 87, 127, … for a total of 50 blocks. She finds the average is 3.2 potholes/block. Later, someone discovers that the city's grid has a repeating pattern: corner blocks (which occur every 40 blocks along main arteries) have been recently repaved and have 0 potholes. (a) Identify the sampling method. (b) Explain the bias. (c) Suggest a fix.",
      walkthrough: [
        {
          expression: "\\text{Sampling interval: } N/n = 2000/50 = 40",
          annotation:
            "Systematic sample with interval 40. The researcher correctly computed the interval.",
        },
        {
          expression:
            "\\text{Period of repaving cycle} = 40 = \\text{sampling interval} \\Rightarrow \\text{every sample block is a corner block}",
          annotation:
            "The sampling interval exactly matches the repaving period. Every selected block is recently repaved, with 0 potholes. The estimate 3.2 potholes/block is not representative of the full population.",
        },
        {
          expression:
            "\\text{Bias} = \\bar{y}_{\\text{sample}} - \\mu = 3.2 - \\mu_{\\text{true}}",
          annotation:
            "We cannot compute the exact bias without knowing the true mean, but the direction is clear: the sample underestimates potholes because it selects the best-maintained blocks.",
        },
        {
          expression:
            "\\text{Fix: use SRS, or change interval to a non-divisor of 40 (e.g., 39 or 41), or use stratified sampling}",
          annotation:
            "The cleanest fix is SRS: draw 50 blocks at random from the list of 2,000. Alternatively, use an interval that does not align with any known cycle. If block type (corner vs. interior) is known, stratify on it.",
        },
      ],
      answer:
        "Systematic sampling with period 40 that coincidentally matches the repaving cycle, causing all selected blocks to be recently repaved. Fix: use SRS or change the interval to avoid alignment with any periodic pattern in the population.",
    },
    {
      id: "stat1-002-ch3",
      ...
      difficulty: "hard",
      problem:
        "A company has three departments. HR (80 employees, sampled 16, mean salary $52,000, s=$8,000), Engineering (200 employees, sampled 40, mean $95,000, s=$15,000), Sales (120 employees, sampled 24, mean $68,000, s=$11,000). Compute the stratified estimate of the company-wide mean salary.",
      walkthrough: [
        {
          expression: "N = 80 + 200 + 120 = 400 \\text{ total employees}",
          annotation: "Total population size.",
        },
        {
          expression:
            "W_{HR} = 80/400 = 0.20 \\quad W_{Eng} = 200/400 = 0.50 \\quad W_{Sales} = 120/400 = 0.30",
          annotation:
            "Stratum weights must sum to 1: 0.20 + 0.50 + 0.30 = 1.00. ✓",
        },
        {
          expression:
            "\\bar{y}_{st} = 0.20(52{,}000) + 0.50(95{,}000) + 0.30(68{,}000)",
          annotation: "Weighted average of stratum means.",
        },
        {
          expression: "= 10{,}400 + 47{,}500 + 20{,}400 = \\$78{,}300",
          annotation:
            "The stratified estimate of the company-wide mean salary is $78,300.",
        },
      ],
      answer:
        "$78,300. The stratified estimator weights each department's sample mean by that department's share of the total workforce.",
    },
  ],
    ...
    core: [
      {
        symbol: "N",
        meaning:
          "Population size — total number of units in the target population",
      },
      { symbol: "n", meaning: "Sample size — total number of units measured" },
      {
        symbol: "\\pi_i",
        meaning:
          "Inclusion probability of unit i — the probability that unit i is selected in the sample",
      },
      {
        symbol: "W_h",
        meaning:
          "Stratum weight — N_h/N, the proportion of the population in stratum h",
      },
      {
        symbol: "\\bar{y}_{st}",
        meaning:
          "Stratified estimator of the population mean — weighted average of stratum sample means",
      },
      {
        symbol: "\\rho",
        meaning:
          "Intracluster correlation coefficient — similarity of units within the same cluster (0 = independent, 1 = identical)",
      },
    ],
    rulesOfThumb: [
  ...
  spiral: {
    ...
    futureLinks: [
      {
        lessonId: "stat1-003",
        label: "Study Design and Bias",
        note: "Sampling method is one source of bias. stat1-003 covers selection bias, response bias, and measurement bias systematically.",
      },
      {
        lessonId: "stat6-001",
        label: "Statistical Inference",
        note: "Confidence intervals and hypothesis tests assume probability sampling. The formulas for margin of error assume SRS unless adjusted for the design.",
      },
      {
        lessonId: "stat6-002",
        label: "Confidence Intervals",
        note: "The variance formula used to compute confidence intervals changes depending on the sampling design (SRS vs stratified vs cluster).",
      },
    ],
  },
  ...
  checkpoints: [
    {
      id: "cp-stat1-002-1",
      label:
        "Read: define probability vs. non-probability sample with one example each",
      type: "read",
    },
    {
      id: "cp-stat1-002-2",
      label: "Read: explain the sampling frame and what coverage error is",
      type: "read",
    },
    {
      id: "cp-stat1-002-3",
      label: "Read: name the five sampling methods and the key risk of each",
      type: "read",
    },
    {
      id: "cp-stat1-002-4",
      label:
        "Apply the classification procedure callout to example 3 before reading the solutions",
      type: "example",
    },
    {
      id: "cp-stat1-002-5",
      label:
        "Complete example 2: compute proportional allocation for the four-stratum university study",
      type: "example",
    },
    {
      id: "cp-stat1-002-6",
      label:
        "Attempt challenge 3: compute the stratified mean salary from scratch",
      type: "challenge",
    },
    {
      id: "cp-stat1-002-7",
      label:
        "Read: explain why the Literary Digest poll failed despite its enormous sample size",
      type: "read",
    },
    {
      id: "cp-stat1-002-8",
      label:
        "Read: state when the finite population correction can be ignored and why",
      type: "read",
    },
  ],
    ...
    questions: [
      {
        ...
        instructions:
          "The city is divided into groups (zip codes), some groups are selected at random, and all units within the selected groups are measured.",
      },
    ],
  },
  ...
  quiz: [
    {
      ...
      options: [
        "Surveying volunteers who respond to a social media post",
        "Interviewing everyone who enters a coffee shop on Monday morning",
        "Drawing names from a numbered list using a random number generator",
        "Asking your friends and colleagues to complete a survey",
      ],
      answer:
        "Drawing names from a numbered list using a random number generator",
      hints: [
        "Probability sampling requires that every unit has a known, non-zero chance of selection.",
        "Only one option uses a random selection mechanism.",
      ],
      reviewSection:
        'Intuition → "The key distinction: probability vs. non-probability" paragraph',
    },
    {
      id: "stat1-002-quiz-2",
      type: "choice",
    ...
    },
    {
      ...
      hints: [
        "Stratification controls between-stratum variation.",
        "If all strata have the same mean, stratification adds no precision benefit.",
      ],
    ...
    },
    {
      ...
      answer:
        "Units within a cluster are correlated, reducing effective sample size",
      hints: [
        "Think about neighbors sharing similar characteristics.",
        "The intracluster correlation coefficient ρ measures this.",
      ],
      reviewSection:
        'Intuition → "Cluster Sampling" paragraph and the intracluster correlation callout',
    },
    {
      id: "stat1-002-quiz-5",
      type: "choice",
      ...
      answer:
        "Literary Digest used a biased, non-probability sample; Gallup used proper random selection",
      hints: [
        "Think about who was on the mailing list in 1936.",
        "Telephone directories and car registration lists overrepresented which demographic?",
      ],
    ...
    },
    {
      ...
      hints: [
        "When n is tiny relative to N, does it matter whether you sample with or without replacement?",
  ...
  misconceptions: [
    {
      ...
      whyStudentsThinkIt:
        "Students correctly learn that larger samples give smaller margins of error. They incorrectly extend this to conclude that size alone guarantees quality.",
      correctionExample:
        "Literary Digest (2.4 million responses, biased sample) vs. Gallup (50,000, probability sample). The smaller, unbiased sample was far more accurate. A biased sample of any size converges to the wrong answer.",
      contrastCase:
        "Two SRS samples from the same population: n=100 vs n=1000. Here bigger is better. But a convenience sample of 1,000 vs an SRS of 100 — the SRS may be more accurate.",
    },
    {
      falseBelief: "Stratified sampling always gives better results than SRS.",
      whyStudentsThinkIt:
        'Students learn that stratification "controls" for subgroup differences and assume it always improves precision.',
      correctionExample:
        "If the strata have identical means on the outcome variable (e.g., you stratify by eye color when studying income, and mean income is the same across eye colors), stratification provides zero precision gain over SRS. It only helps when strata differ on the outcome.",
      contrastCase:
        "Stratifying by age group when estimating age — by definition improves precision enormously. Stratifying by shoe size when estimating political views — likely provides no benefit.",
    },
    {
      falseBelief: "A random sample means a haphazard or casual selection.",
      whyStudentsThinkIt:
        'In everyday language, "random" means arbitrary or unplanned. In statistics, it means sampled according to a known probability mechanism.',
      correctionExample:
        "Asking the first 50 people you see is not a random sample — it is a convenience sample. Drawing 50 names from a hat containing all population members is a random sample (SRS), even though it seems simple.",
      contrastCase:
        'The word "random" in statistics always requires specifying the probability distribution: "Each unit is selected with probability n/N, independently." Without that specification, a selection process is not statistically random even if it feels arbitrary.',
    },
  ],
  ...
  transferPrompts: [
    {
      situation:
        "A hospital wants to estimate the average length of stay for patients discharged last year. It has a complete list of all 12,000 discharges.",
      competingTechniques: [
        "Survey all 12,000 (census)",
        "Draw an SRS of 400 from the list",
        "Sample every 30th discharge in chronological order",
      ],
      whyThisTechniqueWins:
        "With a complete list, SRS of 400 gives a valid estimate with a predictable margin of error at low cost. A census is unnecessary unless subgroup-level precision is required. Systematic sampling (every 30th) is practical but risks bias if discharge timing has a weekly cycle (e.g., more elective procedures on Mondays that affect LOS).",
    },
    {
      situation:
        "A national education study wants to measure reading scores across all public schools in a country with 50,000 schools in 500 districts.",
      competingTechniques: [
        "SRS of 500 schools",
        "Cluster sample: random sample of 50 districts, test all schools in those districts",
        "Stratified sample by district size and region",
      ],
      whyThisTechniqueWins:
        "Cluster sampling is far more practical — testing all 500 schools in 50 districts requires sending test administrators to only 50 locations. SRS of 500 schools scattered across the country requires 500 separate site visits. For national estimates, the efficiency gain of clustering usually outweighs the precision loss, especially if the study also wants district-level estimates.",
    },
  ],
  ...
  debugging: [
    {
      commonError:
        "Computing a stratified estimate using simple arithmetic mean instead of weighted mean.",
      symptom:
        "Average of the three stratum sample means gives a different (usually wrong) answer than the weighted formula.",
      whyItHappened:
        "If stratum sample sizes are proportional to stratum population sizes, the unweighted average of sample means happens to equal the weighted estimate. But if they differ (e.g., you over-sampled a small stratum), the unweighted average is biased.",
      repairStrategy:
        "Always use $\\bar{y}_{st} = \\sum_h W_h \\bar{y}_h$ where $W_h = N_h / N$. The weights come from population sizes, not sample sizes.",
    },
    {
      commonError:
        "Treating a self-selected online survey as if it were a probability sample.",
      symptom:
        "Researcher reports confidence intervals and p-values from a survey promoted on social media, implying the results generalize to the population.",
      whyItHappened:
        "The confidence interval formula is mechanically applicable to any data — Python and R will compute it without checking whether the data came from a probability sample. Students apply the formula without checking the assumption.",
      repairStrategy:
        'Before applying inferential formulas, verify: "Was every unit in the population given a known, nonzero chance of selection?" If not, confidence intervals and p-values are not valid for inference about the population. Report descriptive statistics only and explicitly state the convenience sample limitation.',
    },
  ],
  ...
  mastery: {
    targetLevel: 3,
    solveIndependently:
      "Given a population with known stratum sizes, compute proportional stratified allocation, compute the stratified mean estimate, and identify the sampling method from a study description.",
    explainVerbally:
      "Explain why the 1936 Literary Digest poll failed despite having 2.4 million responses, using the specific concepts of probability sampling and coverage error.",
    detectIncorrectApplication:
      "Identify when a researcher uses a convenience sample but reports it as if it were a probability sample and explain exactly what inferential claims are no longer valid.",
    transferToUnfamiliar:
      "Given a novel sampling scenario you have not seen (e.g., sampling hospital patients, sampling tweets from a social media platform), identify which probability sampling method is most appropriate and justify the choice.",
  },
};
