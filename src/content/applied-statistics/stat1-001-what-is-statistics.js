export default {
  ...
  subtitle: 'Descriptive vs inferential reasoning, populations vs samples, and the four types of data.',
  tags: ['statistics', 'descriptive', 'inferential', 'population', 'sample', 'data types'],
  aliases: 'introduction to statistics descriptive inferential population sample nominal ordinal interval ratio',
  timeToComplete: 25,
  coreConcept: 'Statistics is the science of learning from data under uncertainty. Descriptive statistics summarizes what you have; inferential statistics uses a sample to draw conclusions about a population you cannot fully observe.',
  prerequisites: [],
  nextLesson: 'stat1-002',

  // ── Hook ──────────────────────────────────────────────────────
  hook: {
    question: 'How can testing 1,000 people tell you anything reliable about 330 million?',
    realWorldContext: 'Before every U.S. election, polling organizations survey roughly 1,000 randomly selected voters and predict outcomes for the entire country — within ±3 percentage points, 95% of the time. A pharmaceutical company tests a new drug on 400 patients, not 40 million, and the FDA uses that data to decide whether everyone can take it. A quality-control engineer measures 50 bolts from a production run of 500,000 and decides whether the entire batch ships. In every case, the pattern is identical: collect a small, carefully chosen slice of reality, apply statistical reasoning, and make confident statements about the whole. That is statistics.',
  },
  ...
  examples: [
    {
      ...
      difficulty: 'easy',
      problem: 'Five patients report the following resting heart rates (beats per minute): 72, 65, 80, 58, 75. (a) What measurement scale is heart rate? (b) Is this a population or a sample? (c) Compute a descriptive summary: the sample mean $\\bar{x}$.',
      steps: [
        { expression: '\\text{Heart rate (bpm): ratio scale}', annotation: 'Heart rate has a true zero (0 bpm = no heartbeat), equal gaps (70 bpm vs 80 bpm differs the same as 80 vs 90), and valid ratios (80 bpm is twice 40 bpm).', strategyTitle: 'Step 1: Identify the measurement scale' },
        { expression: '\\text{Five patients} \\Rightarrow \\text{sample}', annotation: 'Unless these five patients ARE the entire population of interest (e.g., "five patients in this specific trial"), they are a sample from a larger population such as "all adult patients in this clinic." Context determines this — here we treat them as a sample.', strategyTitle: 'Step 2: Population or sample?' },
        { expression: '\\bar{x} = \\frac{72 + 65 + 80 + 58 + 75}{5} = \\frac{350}{5} = 70', annotation: 'Sum all five values: 72+65=137, 137+80=217, 217+58=275, 275+75=350. Divide by n=5: 350/5 = 70 bpm. This is the sample mean — a descriptive statistic about these five patients.', strategyTitle: 'Step 3: Compute the sample mean' },
      ],
    },
    {
      id: 'stat1-001-ex2',
      ...
      difficulty: 'medium',
      problem: 'A company surveys 200 of its 5,400 employees and finds that 148 of the 200 (74%) prefer remote work. (a) State one purely descriptive claim. (b) State one inferential claim. (c) Identify the population parameter and the sample statistic.',
      steps: [
        { expression: '\\hat{p}_{\\text{sample}} = \\frac{148}{200} = 0.74', annotation: 'The proportion $\\hat{p}$ (p-hat) = 74% is a sample statistic — a fact about the 200 surveyed employees. Computing it is a descriptive act.', strategyTitle: 'Step 1: Compute the sample statistic' },
        { expression: '\\text{Descriptive: "74\\% of the 200 surveyed employees prefer remote work."}', annotation: 'No uncertainty language needed. This is a direct numerical fact about the data collected. No inference required.', strategyTitle: 'Step 2: Descriptive claim' },
        { expression: '\\text{Inferential: "We estimate that approximately 74\\% of all 5,400 employees prefer remote work (±6\\%}\\text{, 95\\% CI)."}', annotation: 'This claim goes beyond the data. The "±6%" (margin of error) acknowledges that the true population proportion p might differ from the sample 74%. Computing the margin of error formally requires the methods of stat1-004 (confidence intervals).', strategyTitle: 'Step 3: Inferential claim with uncertainty' },
        { expression: 'p = \\text{true proportion among all 5,400 (unknown parameter)} \\quad \\hat{p} = 0.74 \\text{ (sample statistic)}', annotation: 'Parameter p (Greek / Roman here since proportions use p) is unknown. The hat notation $\\hat{p}$ signals "this is an estimate of the parameter p computed from sample data."', strategyTitle: 'Step 4: Label parameter vs. statistic' },
      ],
    },
    {
      id: 'stat1-001-ex3',
      ...
      difficulty: 'hard',
      problem: 'A psychology report contains the following three statements. For each, identify the measurement scale of the variable involved and state whether the statistical operation used is valid for that scale:\n(a) "The average political affiliation score was 3.2 (where 1=Far Left, 3=Center, 5=Far Right)."\n(b) "The GDP of Country A ($800B) is four times the GDP of Country B ($200B)."\n(c) "The mean year of birth in our sample was 1989, so the typical participant was born in 1989."',
      steps: [
        { expression: '\\text{(a) Political affiliation: ordinal scale}', annotation: '"Far Left=1 … Far Right=5" imposes a number but not a metric — the distance between "Center" and "Center-Right" may not equal the distance between "Center-Right" and "Right." Computing a mean on ordinal data is technically invalid (the mean requires equal gaps). The summary "average = 3.2" is widely done in practice and often useful as an approximation, but it should be reported with awareness that equal-interval assumptions may not hold.', strategyTitle: 'Step 1: Political affiliation' },
        { expression: '\\text{(b) GDP: ratio scale} \\Rightarrow \\text{ratio valid}', annotation: 'GDP in dollars has a true zero (zero dollars = no economic output). Ratio statements ("four times as large") are valid. $800B / $200B = 4 is a meaningful claim.', strategyTitle: 'Step 2: GDP' },
        { expression: '\\text{(c) Birth year: interval scale} \\Rightarrow \\text{mean valid, ratio invalid}', annotation: 'Calendar year is interval scale — equal gaps (1989 to 1990 = 1990 to 1991) but no true zero (year 0 is a historical convention, not the absence of time). The mean birth year 1989 is valid because the mean only requires equal gaps, not a true zero. But "participant born in 1989 is twice as old as..." would be invalid as a year ratio. The mean here is a useful descriptive summary of center.', strategyTitle: 'Step 3: Birth year' },
      ],
    },
  ],
  ...
  challenges: [
    {
      ...
      difficulty: 'medium',
      problem: 'You want to estimate the average daily screen time of all high school students in your state (population size: ~500,000 students). Describe: (a) the population, (b) a feasible sample, (c) one variable and its measurement scale, (d) one descriptive claim you can make after data collection, and (e) one inferential claim with a margin-of-error statement.',
      walkthrough: [
        { expression: '\\text{Population: all } \\sim 500{,}000 \\text{ high school students in the state}', annotation: 'The population includes every unit you want to draw conclusions about. Be precise: "high school students" = enrolled in grades 9–12 in public and private schools within the state.' },
        { expression: 'n = 400 \\text{ students, randomly selected from 20 schools}', annotation: 'A sample of 400 gives a margin of error of roughly ±5% for proportions, or ±(1.96 × s/√400) for means. Selecting from 20 schools adds geographic spread.' },
        { expression: '\\text{Daily screen time (hours): ratio scale}', annotation: 'Screen time in hours has a true zero (0 hours = no screen use). Equal gaps. Ratios valid: 6 hours is twice 3 hours.' },
        { expression: '\\bar{x}_{400} = 5.3 \\text{ hours (descriptive — fact about the 400 sampled)}', annotation: 'State this without uncertainty language. It describes the data you have.' },
        { expression: '\\hat{\\mu} = 5.3 \\pm 0.4 \\text{ hours (95\\% CI for all 500,000 students)}', annotation: 'The ±0.4 margin quantifies your uncertainty about the population parameter μ. You cannot know μ exactly — the CI is what inferential statistics provides.' },
      ],
      answer: 'A valid study specifies a well-defined population, a random sample with a stated size, identifies the measurement scale of key variables, and separates descriptive facts (computed from the sample) from inferential estimates (claims about the population, always with uncertainty).',
    },
    {
      id: 'stat1-001-ch2',
      title: 'Detect and fix a scale violation',
      difficulty: 'medium',
      problem: 'A report states: "On a 1–5 customer satisfaction scale, Group A averaged 4.2 and Group B averaged 2.1. Therefore Group A is exactly twice as satisfied as Group B." Identify the scale of the satisfaction variable, state whether the conclusion is valid, and explain what claim would be valid.',
      walkthrough: [
        { expression: '\\text{Satisfaction 1–5: ordinal scale}', annotation: 'The numbers 1–5 impose order but not a metric. The gap between "1 = very dissatisfied" and "2 = dissatisfied" is not guaranteed to equal the gap between "4 = satisfied" and "5 = very satisfied." Ordinal scale.' },
        { expression: '\\frac{4.2}{2.1} = 2 \\Rightarrow \\text{ratio claim: INVALID on ordinal data}', annotation: 'Ratio operations (division, "twice as much") require ratio-scale data with a true zero. A 1–5 satisfaction scale does not have a true zero — there is no "zero satisfaction" defined. Even if the means were computed validly (debatable for ordinal), the ratio is meaningless.' },
        { expression: '\\text{Valid claim: Group A\'s average rating (4.2) is higher than Group B\'s (2.1) by 2.1 points on the 1-5 scale.}', annotation: 'Stating the difference in points is safer than a ratio. Even better: use the median and compare distributions visually. For ordinal data, the median is the most appropriate center measure.' },
      ],
      answer: 'Satisfaction on a 1–5 scale is ordinal. Ratios are invalid on ordinal data. A valid claim is: Group A\'s median rating was higher than Group B\'s; or, the average score differed by 2.1 points on the scale (without the "twice as" language).',
    },
    {
      id: 'stat1-001-ch3',
      title: 'Margin of error and sample size',
      difficulty: 'hard',
      problem: 'A polling firm wants to estimate the proportion of voters who support a ballot measure. They want their 95% confidence interval to be no wider than ±3 percentage points. Using the formula $n \\geq \\left(\\frac{1.96}{2 \\times 0.03}\\right)^2$ (which assumes maximum variance when $p = 0.5$), compute the minimum sample size required. Then explain qualitatively what happens to the required sample size if the target margin is tightened to ±1.5%.',
      walkthrough: [
        { expression: 'n \\geq \\left(\\frac{1.96}{2 \\times 0.03}\\right)^2 = \\left(\\frac{1.96}{0.06}\\right)^2 = (32.67)^2 \\approx 1{,}068', annotation: 'The formula comes from the confidence interval for a proportion: width = 2 × 1.96 × √(p(1−p)/n). Setting this ≤ 2×0.03 and using p(1−p) ≤ 0.25 (maximum at p=0.5) gives n ≥ (1.96/0.06)². Round up to 1,068.' },
        { expression: '\\text{Margin } \\pm 3\\% \\Rightarrow n \\approx 1{,}068 \\quad \\text{Margin } \\pm 1.5\\% \\Rightarrow n \\approx 4{,}268', annotation: 'Halving the margin requires quadrupling n: (1.96 / (2×0.015))² ≈ 4,268. This is the 1/√n law: to halve the error, multiply sample size by 4.' },
      ],
      answer: 'Minimum n ≈ 1,068 for ±3% margin. Tightening to ±1.5% requires n ≈ 4,268 — four times as many — because margin of error scales as 1/√n.',
    },
  ],
  ...
  semantics: {
    core: [
      { symbol: '\\mu', meaning: 'Population mean — the average over all N units in the population; usually unknown' },
      { symbol: '\\bar{x}', meaning: 'Sample mean — the arithmetic average computed from the n sampled values; an estimator of μ' },
      { symbol: 'N', meaning: 'Population size — the total number of units the study wants to draw conclusions about' },
      { symbol: 'n', meaning: 'Sample size — the number of units actually measured; n ≪ N in most real studies' },
      { symbol: '\\hat{p}', meaning: 'Sample proportion — the fraction of sampled units with a given attribute; estimator of the population proportion p' },
    ],
    rulesOfThumb: [
  ...
  spiral: {
    recoveryPoints: [],
    futureLinks: [
      { lessonId: 'stat1-002', label: 'Descriptive Statistics', note: 'This lesson defined descriptive statistics as a concept. stat1-002 teaches the specific tools: mean, median, variance, IQR, and how to choose between them based on scale and distribution shape.' },
      { lessonId: 'stat1-004', label: 'Statistical Inference', note: 'The confidence interval notation (x̄ ± margin) used here is formalized in stat1-004 using the Central Limit Theorem and the t-distribution.' },
      { lessonId: 'stat1-005', label: 'Regression Analysis', note: 'Measurement scale determines which regression model is valid. Ratio and interval outcomes support linear regression; ordinal outcomes require ordinal regression; nominal outcomes require logistic regression.' },
    ],
  },
  ...
  quiz: [
    {
      ...
      answer: 'Using sample data to make estimates or decisions about a population',
      hints: ['Inferential statistics always involves going beyond the data you have.', 'The key word is "population" — you are making a claim about units you did not observe.'],
    ...
    },
    {
      ...
      hints: ['The key word is "respondents" — these are the 500 people actually surveyed, not the full population.', 'A descriptive claim states a fact about the data collected, with no uncertainty needed.'],
      reviewSection: 'Intuition → "Descriptive vs. Inferential — The Two Jobs" callout',
    },
    {
      id: 'stat1-001-quiz-3',
      type: 'choice',
      ...
      options: ['Nominal', 'Ordinal', 'Interval', 'Ratio'],
      answer: 'Interval',
      hints: ['Equal gaps between degrees exist, but is 0°F the "absence of temperature"?', '0°F does not mean "no heat" — it is an arbitrary reference point.'],
    ...
    },
    {
      ...
      answer: 'It halves',
      hints: ['Margin of error is proportional to 1/√n.', '√400 = 20 and √100 = 10 — a factor of 2 in the denominator.'],
      reviewSection: 'Intuition → "Larger samples shrink uncertainty" paragraph',
    },
    {
      id: 'stat1-001-quiz-5',
      type: 'choice',
      ...
      hints: ['The mean requires equal-interval spacing between values.', 'Ordinal data has order but not guaranteed equal gaps.'],
      reviewSection: 'Intuition → "Four types of data" and "Why the scale matters" paragraphs',
    },
    {
      id: 'stat1-001-quiz-6',
      type: 'choice',
      ...
      options: ['x̄', 's', 'μ', 'r'],
      answer: 'μ',
      hints: ['Population parameters use Greek letters.', 'μ (mu) is the population mean.'],
      reviewSection: 'Intuition → "Parameter vs. Statistic — a Notation Convention" callout',
    },
  ],
  ...
  misconceptions: [
    {
      falseBelief: 'If data are stored as numbers, they must be interval or ratio scale.',
      whyStudentsThinkIt: 'Computers store everything as numbers; students assume that because pain ratings (1–10) or zip codes (90210) are numbers, arithmetic on them is valid.',
      correctionExample: 'ZIP code 90210 minus ZIP code 10001 = 80,209 — a meaningless number. Finishing position 1 and position 3 are numbers, but "average finishing position = 2.0" is not meaningful when the time gaps are unequal.',
      contrastCase: 'Height in centimeters is also stored as a number, and averaging it is valid (ratio scale). The question is not "is it a number?" but "do equal differences represent equal quantities?"',
    },
    {
      falseBelief: 'A larger sample is always better — if I have all the data, I need no statistics.',
      whyStudentsThinkIt: 'Students conflate "more data = more accurate" with "complete population = no uncertainty." If you have the entire population, parameters are known exactly, so no inference is needed.',
      correctionExample: 'A census (surveying all 330 million Americans) gives you the exact population mean with zero uncertainty — no confidence interval needed. But if you only surveyed 1,000, you need inferential statistics to estimate the population mean.',
      contrastCase: 'If your "population" is next year\'s customers (unobserved), even a full historical dataset is still a sample from that conceptual population, and inference remains necessary.',
    },
    {
      falseBelief: 'The margin of error tells you the maximum possible error.',
      whyStudentsThinkIt: 'Students read "±3%" as a guarantee that the true value is within 3% of the estimate.',
      correctionExample: 'A 95% confidence interval of 62 ± 8 minutes means: if we repeated this sampling procedure 100 times, about 95 of the 100 intervals would contain the true mean. It does not mean the true mean is definitely within ±8 of 62 — there is a 5% chance the interval misses entirely.',
      contrastCase: 'A 99% CI would be wider (±10 minutes) and miss less often. A 90% CI would be narrower (±7 minutes) but miss more often. Confidence level and width trade off.',
    },
  ],
  ...
  transferPrompts: [
    {
      situation: 'A hospital administrator has records for all 4,200 patients treated last year and wants to compute the average length of stay.',
      competingTechniques: ['Compute the sample mean from a subset', 'Compute the population mean from all records', 'Report the median instead'],
      whyThisTechniqueWins: 'Since all 4,200 records are available, this is a census — compute the population mean μ directly. No sampling or confidence intervals are needed; there is no uncertainty. Using a sample here would artificially introduce uncertainty that does not exist.',
    },
    {
      situation: 'A marketing team has satisfaction scores (1–5 stars) from 8,000 customers and wants to report "the average satisfaction."',
      competingTechniques: ['Compute arithmetic mean of the 1–5 scores', 'Report the median star rating', 'Count how many gave each rating (frequency table)'],
      whyThisTechniqueWins: 'Star ratings are ordinal — the mean is technically invalid but widely used. The safest choice is to report the median (the middle value when sorted) and also show the frequency distribution (how many gave 1, 2, 3, 4, 5 stars). This gives more information than a single number and does not require equal-interval assumptions.',
    },
  ],
  ...
  debugging: [
    {
      ...
      symptom: 'The mean of "country codes" or "survey question IDs" produces a number that seems plausible but has no meaning.',
      whyItHappened: 'When nominal categories are encoded as integers (1=USA, 2=Canada, 3=Mexico), pandas and numpy will happily compute the mean — there is no automatic type check.',
      repairStrategy: 'Before analyzing any column, print its dtype and ask: "Does averaging this make sense?" For categorical columns, use value_counts() or mode(), not mean(). In pandas, explicitly cast nominal columns to Categorical dtype to prevent accidental arithmetic.',
    },
    {
      commonError: 'Reporting only the sample statistic without the margin of error.',
      symptom: 'A result like "our sample shows 74% preference" is treated as a definitive population fact without uncertainty.',
      whyItHappened: 'The computation of the sample proportion is straightforward; computing and reporting the confidence interval requires an extra step that students skip.',
      repairStrategy: 'Make it a rule: every inferential claim needs an uncertainty statement. In Python, use scipy.stats.proportion_confint(count, nobs, alpha=0.05) to compute the 95% CI automatically alongside the proportion.',
    },
  ],
  ...
  mastery: {
    ...
    transferToUnfamiliar: 'Given a study you have never seen (e.g., a biology field survey, a marketing A/B test), correctly label all variables by scale, separate descriptive from inferential claims, and flag any scale violations in the reported statistics.',
  },
};
