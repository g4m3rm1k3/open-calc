const sim2_009 = {
  ...
  subtitle: 'Add a numerical derivative, a moveable tangent line, and an automatic zero-crossing scanner — turning the static plotter into a lightweight calculus tool.',
  tags: ['derivative', 'tangent line', 'zero crossings', 'numerical methods', 'central difference', 'canvas'],
  timeToComplete: 40,
  coreConcept: 'Numerical differentiation (central difference) approximates f\'(x) ≈ (f(x+h) − f(x−h)) / (2h). A tangent line is drawn by evaluating f and f\' at one x position and extending the resulting line across the viewport. Zero crossings are found by scanning for sign changes in the function\'s output.',

  hook: {
    question: 'A CAS system shows you a tangent line that moves as you drag a slider. Where does it get the slope from — and how does it find the zeros without solving the equation symbolically?',
    realWorldContext: 'Root-finding algorithms (Newton\'s method, bisection) power everything from circuit simulators to machine-learning optimisers. The sign-change scan you write here is the first step of bisection — the numerical backbone used when closed-form solutions don\'t exist. Every scientific computing library has these tools; this lesson shows you how they work at the pixel level.',
  },
    ...
    visualizations: [
      {
        ...
        mathBridge: 'Work through the four cells in order. Cell 1 implements the numerical derivative and displays f\'(x). Cell 2 adds the tangent line with a draggable slider. Cell 3 scans for zero crossings. Cell 4 combines all three tools.',
        initialProps: {
          initialCells: [
            {
              id: 1,
...
redraw()`,
            ...
            // ── Challenges ──────────────────────────────────────────────────────
            {
              ...
              difficulty: 'easy',
              prompt: 'The `H_STEP` constant controls how far apart the two sample points are in the central-difference formula. Change it from 0.001 to 1.0, then to 0.000001, and observe how the displayed slope changes. At what value does the approximation become noticeably wrong?',
...
redraw()`,
            },
            {
              ...
              difficulty: 'medium',
              prompt: 'The plotter draws the curve but doesn\'t yet highlight the zeros. Fill in the `findZeros` scan loop: walk x from xMin to xMax in 400 steps, detect sign changes, refine each root with linear interpolation, and draw a colored dot at each zero on the x-axis.',
...
redraw()`,
            },
            {
              ...
              difficulty: 'hard',
              prompt: 'Add a "Show Area" checkbox. When checked, fill N rectangles under the curve between x=−3 and x=3 (midpoint Riemann sum) and display the total area. Use a `<input type="range">` slider to control N (from 4 to 200 rectangles).',
