const sim2_010 = {
  ...
  subtitle: 'Assemble everything — dynamic viewport, multiple colour-coded traces, analysis overlays, an animated time parameter, and a presets library — into a polished, self-contained graphing calculator.',
  tags: ['capstone', 'plotter', 'animation', 'presets', 'multiple functions', 'viewport', 'analysis', 'canvas'],
  timeToComplete: 50,
  coreConcept: 'The capstone plotter is a composition of independent tools: the viewport engine (Lesson 8) provides w2c and pan/zoom; the trace loop (Lesson 7) draws multiple expressions; the analysis layer (Lesson 9) adds derivative and zeros; and the rAF loop (Lesson 6) animates a time parameter t in any expression.',

  hook: {
    question: 'Every graphing calculator — from a Ti-84 to Desmos — is built from the same handful of primitives you have written over the past nine lessons. What does it take to assemble them into a cohesive tool?',
    realWorldContext: 'The architecture you finalise here is identical to production plotting tools: a state object (functions, viewport, settings), a pure `render(state)` function that redraws everything, and event handlers that mutate state and call `render`. Desmos, GeoGebra, and matplotlib all share this shape. Learning to recognise it means you can read and extend any of them.',
  },
    ...
    visualizations: [
      {
        ...
        mathBridge: 'Each cell adds one layer to the final plotter. Cell 1: core plotter. Cell 2: analysis overlays. Cell 3: animated t parameter. Cell 4: presets library. By Cell 4 you have a complete graphing calculator in ~200 lines.',
        initialProps: {
          initialCells: [
            {
              id: 1,
...
app.querySelector('#home').addEventListener('click',()=>{state.vp={...DEFAULT};if(!state.running)render()})`,
            ...
            // ── Challenges ──────────────────────────────────────────────────────
            {
              ...
              difficulty: 'easy',
              prompt: 'Add a sixth preset called "Lissajous" to the PRESETS array. It should show three traces: `sin(x)`, `sin(2*x)`, and `sin(3*x)` in three different colors, with viewport x ∈ [−7, 7], y ∈ [−1.5, 1.5].',
...
render()`,
            },
            {
              ...
              difficulty: 'medium',
              prompt: 'Add an "Export PNG" button. When clicked, call `cv.toDataURL(\'image/png\')` to get a base64 PNG, create a temporary `<a>` element with `href` set to that URL and `download="plot.png"`, click it programmatically, then remove it. The browser will save the canvas as a PNG file.',
...
render()`,
            },
            {
              ...
              difficulty: 'hard',
              prompt: 'Add a "Parametric" mode. In this mode, the user enters two expressions: `x(t)` and `y(t)`. The plotter samples t from 0 to 2π in small steps and draws a curve through the resulting (x, y) points — tracing a path in 2D space rather than a function graph. Wire up a checkbox that switches between function mode and parametric mode.',
