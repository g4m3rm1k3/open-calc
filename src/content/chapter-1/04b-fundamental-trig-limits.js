  ...
  examples: [
    {
      ...
      steps: [
        { expression: '\\frac{\\sin(7x)}{3x} = \\frac{7}{3} \\cdot \\frac{\\sin(7x)}{7x}', annotation: 'Multiply by 7/7 style scaling so denominator matches the sine input (7x), unlocking pillar form.', hints: ['To apply lim sin(u)/u = 1, make the denominator u = 7x.', 'Multiply numerator and denominator by 7.'] },
        { expression: '\\lim_{x\\to0} \\frac{\\sin(7x)}{3x} = \\frac{7}{3} \\cdot 1', annotation: 'Now the first factor is constant and second is exact pillar lim sin(u)/u.', hints: ['The limit of a constant times a function is constant times limit.', 'lim sin(7x)/(7x) = 1.'] },
        { expression: '= \\frac{7}{3}', annotation: '', hints: ['The standard limit contributes 1, so only the coefficient 7/3 remains.', 'Multiply 7/3 by 1 to finish the evaluation.'] },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: '\\text{Strategy trigger: at least one recognizable pillar pattern first}', annotation: 'When you see 0/0, do not substitute; first rewrite into known limit blueprints.', hints: ['Identify the form that matches a known limit.', 'Rewrite to use lim (1-cos u)/u = 0.'] },
        { expression: '\\frac{1-\\cos(5x)}{x} = 5 \\cdot \\frac{1-\\cos(5x)}{5x}', annotation: 'Dress denominator as 5x so second pillar applies directly.', hints: ['Multiply numerator and denominator by 5.', 'Make the denominator 5x to match the argument.'] },
        { expression: '\\lim_{x\\to0} \\frac{1-\\cos(5x)}{x} = 5 \\cdot 0', annotation: 'Apply second pillar with u=5x.', hints: ['lim (1-cos(5x))/(5x) = 0.', 'Constant times 0 is 0.'] },
        { expression: '= 0', annotation: '', hints: ['The second-pillar limit is 0, so the product becomes 5 · 0.', 'Anything times 0 is 0.'] },
      ],
      conclusion: 'Answer: 0.',
    },
    {
      ...
      steps: [
        { expression: '\\frac{1-\\cos(5x)}{x^2} = 25 \\cdot \\frac{1-\\cos(5x)}{(5x)^2}', annotation: 'Scale denominator to (u)^2.', hints: ['Multiply numerator and denominator by 25.', 'Make denominator (5x)^2.'] },
        { expression: '\\lim_{u\\to0} \\frac{1-\\cos u}{u^2} = \\frac{1}{2}', annotation: 'Known derived limit.', hints: ['Recall lim (1-cos u)/u^2 = 1/2.', 'Substitute u = 5x.'] },
        { expression: '\\lim_{x\\to0} \\frac{1-\\cos(5x)}{x^2} = 25 \\cdot \\frac{1}{2} = \\frac{25}{2}', annotation: '', hints: ['Multiply the coefficient 25 by the derived limit 1/2.', 'The product is 25/2.'] },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: '\\tan x - \\sin x = \\frac{\\sin x}{\\cos x} - \\sin x', annotation: 'Translate to basics: tan is too opaque; rewrite in sin/cos form first.', hints: ['Write tan x as sin x / cos x.', 'Subtract sin x.'] },
        { expression: '= \\sin x\\left(\\frac{1}{\\cos x}-1\\right)', annotation: 'Factoring trick exposes a shared sin(x) factor.', hints: ['Factor out sin x.', 'Common factor is sin x.'] },
        { expression: '= \\sin x\\cdot\\frac{1-\\cos x}{\\cos x}', annotation: 'Common denominator creates the 1-cos(x) blueprint needed for the second pillar chain.', hints: ['Combine fractions.', 'Get common denominator cos x.'] },
        { expression: '\\frac{\\tan x-\\sin x}{x^3}=\\frac{\\sin x}{x}\\cdot\\frac{1-\\cos x}{x^2}\\cdot\\frac{1}{\\cos x}', annotation: 'Split x^3 as x*x^2 so each factor becomes a known limit component.', hints: ['Write numerator over x^3.', 'Split into product of limits.'] },
        { expression: '\\to 1\\cdot\\frac{1}{2}\\cdot 1=\\frac{1}{2}', annotation: 'First pillar, derived second-pillar limit, then cos continuity at 0.', hints: ['Apply limits: sin x / x -> 1, (1-cos x)/x^2 -> 1/2, 1/cos x -> 1.', 'Product is 1/2.'] },
      ],
  ...
  challenges: [
    {
      ...
      walkthrough: [
        { expression: '\\frac{\\sin(9x)}{2x} = \\frac{9}{2}\\cdot\\frac{\\sin(9x)}{9x}', annotation: '', hints: ['Multiply numerator and denominator by 9/9 so the denominator becomes 9x.', 'Then factor out 9/2 to match the standard sin(u)/u form.'] },
        { expression: '\\to \\frac{9}{2}', annotation: '', hints: ['Apply lim sin(9x)/(9x) = 1.', '9/2 * 1 = 9/2.'] },
      ],
      answer: '9/2',
    },
    {
      ...
      walkthrough: [
        { expression: '\\frac{1-\\cos(3x)}{x^2}=9\\cdot\\frac{1-\\cos(3x)}{(3x)^2}', annotation: '', hints: ['Multiply numerator and denominator by 9.', 'Make denominator (3x)^2.'] },
        { expression: '\\to 9\\cdot\\frac{1}{2}=\\frac{9}{2}', annotation: '', hints: ['lim (1-cos u)/u^2 = 1/2 with u=3x.', '9 * 1/2 = 9/2.'] },
      ],
      answer: '9/2',
    },
    {
      ...
      walkthrough: [
        { expression: '\\frac{\\sin x - x}{x}=\\frac{\\sin x}{x}-1', annotation: '', hints: ['Split the fraction.', 'Write as (sin x / x) - 1.'] },
        { expression: '\\lim_{x\\to0}\\left(\\frac{\\sin x}{x}-1\\right)=1-1=0', annotation: '', hints: ['lim (sin x / x) = 1.', '1 - 1 = 0.'] },
      ],
      answer: '0',
    },
  ],
