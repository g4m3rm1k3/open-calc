export default {
  ...
  hook: {
    question: 'What happens to a function as you get infinitely close to a point — but never quite reach it?',
  ...
  math: {
    ...
    keyRules: [
      { name: 'Sum Rule', formula: '\\lim_{x \\to a} [f(x) + g(x)] = \\lim f + \\lim g' },
      { name: 'Product Rule', formula: '\\lim_{x \\to a} [f(x) \\cdot g(x)] = \\lim f \\cdot \\lim g' },
      { name: 'Quotient Rule', formula: '\\lim_{x \\to a} \\frac{f(x)}{g(x)} = \\frac{\\lim f}{\\lim g}, \\quad \\lim g \\neq 0' },
  ...
  rigor: {
    ...
    proofSteps: [
      { expression: '\\lim_{x \\to 1} (2x + 1) = 3', annotation: 'Claim: the limit is 3. We must PROVE it using ε-δ.' },
      { expression: '|f(x) - L| = |(2x+1) - 3| = |2x - 2| = 2|x-1|', annotation: 'Simplify |f(x) - L|. We need this < ε.' },
      { expression: '2|x - 1| < \\varepsilon \\iff |x-1| < \\frac{\\varepsilon}{2}', annotation: 'So if we choose δ = ε/2, we\'re done.' },
      { expression: '\\text{Choose } \\delta = \\frac{\\varepsilon}{2}', annotation: 'For ANY ε > 0, this δ works. The limit is proven.' },
    ],
  },
  ...
  examples: [
    {
      ...
      steps: [
        { expression: '\\frac{x^2 - 9}{x - 3}', annotation: 'Direct substitution gives 0/0 — indeterminate form.' },
        { expression: '= \\frac{(x-3)(x+3)}{x-3}', annotation: 'Factor the numerator using difference of squares.' },
        { expression: '= x + 3 \\quad (x \\neq 3)', annotation: 'Cancel the (x-3) terms. Valid since x≠3 in a limit.' },
        { expression: '\\lim_{x \\to 3} (x+3) = 6', annotation: 'Now substitute x=3 directly. Limit is 6.' },
      ],
    },
    {
      id: 'ch1-10-ex2',
      ...
      steps: [
        { expression: '\\text{Left: } \\lim_{x \\to 0^-} \\frac{|x|}{x} = \\frac{-x}{x} = -1', annotation: 'For x < 0, |x| = -x. Limit from the left is -1.' },
        { expression: '\\text{Right: } \\lim_{x \\to 0^+} \\frac{|x|}{x} = \\frac{x}{x} = 1', annotation: 'For x > 0, |x| = x. Limit from the right is +1.' },
        { expression: '-1 \\neq 1 \\implies \\text{limit DNE}', annotation: 'Left ≠ Right, so the two-sided limit does NOT exist.' },
      ],
    },
    {
      id: 'ch1-10-ex3',
      ...
      steps: [
        { expression: '\\cos x \\leq \\frac{\\sin x}{x} \\leq 1 \\quad (x \\text{ near } 0)', annotation: 'Geometric proof gives this squeeze inequality.' },
        { expression: '\\lim_{x \\to 0} \\cos x = 1, \\quad \\lim_{x \\to 0} 1 = 1', annotation: 'Both outer functions approach 1.' },
        { expression: '\\therefore \\lim_{x \\to 0} \\frac{\\sin x}{x} = 1', annotation: 'By the Squeeze Theorem, the middle must too.' },
      ],
    },
  ],
  ...
  challenges: [
    {
      ...
      walkthrough: [
        { expression: 'x^3 - 8 = (x-2)(x^2+2x+4)', annotation: 'Difference of cubes factoring.' },
        { expression: '\\lim_{x \\to 2} (x^2 + 2x + 4)', annotation: 'Cancel (x-2), then substitute.' },
    ...
    },
    {
      ...
      walkthrough: [
        { expression: 'f(1) = 2(1) - 1 = 1', annotation: 'f(1) is defined and equals 1.' },
        { expression: '\\lim_{x \\to 1^-} x^2 = 1, \\quad \\lim_{x \\to 1^+}(2x-1) = 1', annotation: 'Both one-sided limits equal 1.' },
        { expression: '\\lim = f(1) = 1 \\implies \\text{continuous}', annotation: 'All three conditions met. Continuous at x=1.' },
      ],
