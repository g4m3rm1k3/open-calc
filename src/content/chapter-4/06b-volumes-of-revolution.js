...
export default {
  ...
  intuition: {
    ...
    callouts: [
      ...
      },
      {
        ...
        body: 'The volume of a solid of revolution = 2π × (distance from centroid to axis) × (area of the region). For a ring (annulus) of inner radius a, outer radius b: centroid is at r̄ = (a+b)/2, area = π(b²−a²). Volume of the donut-shape (torus) = 2π × (a+b)/2 × π(b−a)²/2 = ... This gives the torus formula V = 2π²Rr² where R is the axis distance and r is the tube radius.',
      },
      {
        type: 'misconception',
  ...
  examples: [
    ...
    },
    {
      ...
      steps: [
        { expression: '\\text{Outer radius: } R = 4 - x^2 \\text{ (distance from } y=x^2 \\text{ up to } y=4\\text{)}', annotation: 'The curve y=x² is below y=4. The disk radius is measured vertically from the curve to the axis y=4.' },
        { expression: '\\text{Inner radius: } r = 0 \\text{ (rotating a single curve, no hole — it is a disk)}', annotation: 'No inner boundary — the region touches the axis of rotation at no point (the axis y=4 is above the curve). Actually we are rotating the AREA between y=x² and y=4.' },
