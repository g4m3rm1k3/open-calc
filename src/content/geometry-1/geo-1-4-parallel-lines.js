...
const LESSON_GEO_1_4 = {
  ...
  subtitle: 'How a single crossing line reveals the hidden geometry of parallel lines — and why it all depends on one postulate.',
  sequential: true,
...
The converses are how parallelism is detected and verified in practice — in construction, drafting, engineering, and the proofs that follow in this course.`,
    ...
    // ── Challenges ─────────────────────────────────────────────────────────────
    {
      ...
      options: [
        { label: 'A', text: '112° — co-interior angles of parallel lines are equal.' },
        { label: 'B', text: '68° — Co-interior Angles Theorem: co-interior angles between parallel lines are supplementary (sum to 180°), so 180° − 112° = 68°.' },
        { label: 'C', text: '90° — all angles formed by parallel lines and a transversal are right angles.' },
        { label: 'D', text: '44° — co-interior angles are complementary.' },
      ],
      check: (label) => label === 'B',
      successMessage: 'Correct. The Co-interior Angles Theorem: co-interior (same-side interior) angles between parallel lines are supplementary — they sum to 180°. So 180° − 112° = 68°. This contrasts with alternate interior angles, which are EQUAL. The difference: alternate angles are on opposite sides (Z-shape), co-interior are on the same side (C-shape).',
      failMessage: 'Co-interior angles (same side of the transversal, between the parallel lines) are supplementary — they sum to 180°. This is the Co-interior Angles Theorem. So 180° − 112° = 68°. Do not confuse with alternate interior angles, which are EQUAL. The shape memory: alternate = Z-shape → equal; co-interior = C-shape → supplementary.',
...

    {
      ...
      options: [
        { label: 'A', text: 'The classmate — measurements are local and can\'t guarantee behavior at infinity.' },
        { label: 'B', text: 'The student — the Converse of the Alternate Interior Angles Theorem is a proven theorem, not just a local observation. Equal alternate interior angles logically guarantee the lines are parallel everywhere.' },
        { label: 'C', text: 'Neither — you need to measure at multiple transversals.' },
        { label: 'D', text: 'The student, but only if the measurement is exact to many decimal places.' },
      ],
      check: (label) => label === 'B',
      successMessage: 'Correct. This is precisely the point of proof over measurement. The converse theorem is a logical consequence of the Parallel Postulate. It doesn\'t say "the lines look parallel near this transversal." It says IF alternate interior angles are equal, THEN the lines are parallel — everywhere, forever. One measurement (if exact) gives logical certainty. That is what makes mathematics different from science.',
...
export default {
  ...
  title: 'Parallel Lines and Transversals',
  subtitle: 'How a single crossing line reveals the hidden geometry of parallel lines — and why it all depends on one postulate.',
  tags: ['geometry', 'parallel-lines', 'transversal', 'alternate-interior-angles', 'corresponding-angles', 'co-interior-angles', 'parallel-postulate'],
  hook: {
    question: 'How can you prove two lines are truly parallel — with certainty, not just appearance?',
    realWorldContext: 'Every angle relationship formed by a transversal cutting parallel lines is a theorem derived from Euclid\'s Fifth Postulate. The converses let you prove parallelism from angle measurement — the test used in construction and navigation for centuries.',
