export default {
  ...
  hook: {
    question:
      'A 5 kg bowling ball rolls at 3 m/s. A 0.05 kg tennis ball flies at 60 m/s. ' +
      'Both have the same kinetic energy (check: ½(5)(9) = 22.5 J; ½(0.05)(3600) = 90 J — actually they don\'t). ' +
      'Revised question: the bowling ball and tennis ball both hit you. Which is harder to stop? ' +
      'What quantity captures "difficulty to stop" better than speed alone?',
    realWorldContext:
      'Momentum is what makes collisions dangerous. ' +
      'A truck at 5 m/s has far more momentum than a bicycle at 5 m/s — ' +
      'the truck is almost impossible to stop quickly, while the bicycle can brake in meters. ' +
      'Air bags, crumple zones, and safety padding all work by extending the time of collision ' +
      '(increasing \\(\\Delta t\\)) to reduce the force — a direct consequence of the impulse-momentum theorem.',
    previewVisualizationId: 'VectorKinematicsLab',
  },

  intuition: {
    prose: [
      '**The answer:** The bowling ball. Even though the tennis ball is faster, the bowling ball has more mass. ' +
        'To stop something, you need to change its velocity — but a massive object resists change more stubbornly. ' +
        'The quantity that captures this "inertia in motion" is **momentum**: \\(\\vec{p} = m\\vec{v}\\).',

      '**Momentum is a vector:** This is crucial. Two cars at the same speed but opposite directions have momenta that add to zero — not 2mv. ' +
        'Two cars moving in the same direction add: total momentum = sum of individual momenta. ' +
        'Direction is encoded by sign in 1D, and by components in 2D/3D.',

      '**Newton\'s Second Law is really about momentum:** Newton didn\'t write \\(F = ma\\). ' +
        'He wrote \\(F = dp/dt\\) — force is the rate of change of momentum. ' +
        'For constant mass, \\(dp/dt = m(dv/dt) = ma\\), so \\(F = ma\\) is a special case. ' +
        'The more fundamental form handles variable-mass systems (rockets, falling chains) and explains how collision forces work.',

      '**Why momentum is conserved in collisions:** When two objects collide, the force one exerts on the other (Newton\'s Third Law) ' +
        'is equal and opposite. So \\(dp_1/dt = -dp_2/dt\\), meaning \\(d(p_1 + p_2)/dt = 0\\) — total momentum is constant. ' +
        'This holds whether the collision is gentle or violent, elastic or inelastic. It is one of the most powerful conservation laws in physics.',
    ],
    callouts: [
      {
        type: 'sequencing',
        ...
        body:
          ...
          '**This chapter (Momentum):** p = mv — a vector. Conserved whenever net external force = 0. ' +
          'Handles impacts and collisions naturally.\n' +
      ...
      },
      {
        ...
        body:
          '\\text{If } \\vec{F}_{\\text{net, external}} = 0: \\quad \\vec{p}_{\\text{total}} = \\text{constant}\\\\' +
      ...
      },
      {
        ...
        body:
          'Both involve mass and velocity, but they behave differently. ' +
          'Momentum is a vector (direction matters); KE is a scalar. ' +
          'Momentum is always conserved in a closed system; KE is conserved only in elastic collisions. ' +
          'Never mix up conservation of momentum with conservation of energy — they apply to different situations.',
      },
      {
        type: 'connection',
        title: 'Calculus connection: F = dp/dt is the derivative definition',
        body:
          '\\(\\vec{F} = \\dfrac{d\\vec{p}}{dt}\\). For constant mass, \\(\\dfrac{d(m\\vec{v})}{dt} = m\\dfrac{d\\vec{v}}{dt} = m\\vec{a}\\). ' +
          'For variable mass (rocket): \\(F = \\dot{m}v + m\\dot{v}\\) — the product rule applied to momentum.',
      },
    ],
    visualizations: [
      {
        ...
        mathBridge:
          'Set two objects with the same speed but different masses. Watch how momentum differs. ' +
          'Then reverse one velocity: momentum adds as vectors, not magnitudes.',
      ...
      },
      {
        ...
        caption:
          'Arrow length = |p| = mv. Arrow direction = direction of motion. ' +
          'Doubling mass doubles the arrow. Doubling speed doubles it again. ' +
          'Reversing velocity reverses the arrow — momentum flips sign.',
      },
    ],
  },
  ...
  math: {
    prose: [
      ...
      'From the Work-Energy Theorem, KE = ½mv² = p²/(2m). ' +
        'Two objects with the same momentum can have different kinetic energies if they have different masses.',
    ...
    callouts: [
      {
        ...
        body:
          'Equal mass \\(m\\), different speeds \\(v_1 < v_2\\):\\\\' +
          '\\(p_1/p_2 = v_1/v_2\\) (momentum ratio = speed ratio).\\\\' +
          '\\(KE_1/KE_2 = v_1^2/v_2^2\\) (energy ratio = speed ratio squared).\\\\' +
          'Energy is more sensitive to speed. A 2× speed increase doubles momentum but quadruples KE.',
      },
      {
        type: 'theorem',
    ...
    visualizations: [
      {
        ...
        caption:
          'Before collision: two arrows (p₁ and p₂). After: two different arrows. ' +
          'The vector sum is identical before and after — that is conservation.',
      },
    ],
  },
  ...
  examples: [
    ...
    },
    {
      ...
      problem:
        '\\text{A 3 kg ball at (4, 0) m/s collides with a stationary 2 kg ball. ' +
        'After: 3 kg ball moves at (2, 1) m/s. Find the 2 kg ball\'s velocity.}',
      steps: [
        {
  ...
  challenges: [
    ...
    },
    {
      ...
      difficulty: 'medium',
      problem:
        '\\text{A 1500 kg car moves north at 20 m/s and a 1000 kg car moves south at 15 m/s. ' +
        'Find the total momentum of the system (define north as positive).}',
    ...
    },
    {
      ...
      difficulty: 'hard',
      problem:
        '\\text{A 0.1 kg bullet at 400 m/s embeds in a 4.9 kg stationary block (inelastic collision). ' +
        'Find the block+bullet speed. Then calculate the kinetic energy before and after — what fraction is lost?}',
