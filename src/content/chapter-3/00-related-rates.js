  ...
  examples: [
    {
      ...
      steps: [
        { expression: 'x^2 + y^2 = 100', annotation: 'The Pythagorean theorem: base x, height y, and the fixed 10-ft hypotenuse. This holds for all t.' },
        { expression: '2x\\,\\frac{dx}{dt} + 2y\\,\\frac{dy}{dt} = 0', annotation: 'Differentiate both sides with respect to t. The right side is 0 because 100 is constant. The chain rule gives the factor dx/dt and dy/dt on each term.' },
        { expression: 'x = 6 \\Rightarrow y = \\sqrt{100 - 36} = \\sqrt{64} = 8', annotation: 'Find y at the instant x = 6 using the original equation. We must know the current position of BOTH variables before substituting rates.' },
        { expression: '2(6)(2) + 2(8)\\,\\frac{dy}{dt} = 0', annotation: 'Substitute x = 6, y = 8, and dx/dt = 2 into the rate equation.' },
        { expression: '24 + 16\\,\\frac{dy}{dt} = 0', annotation: 'Simplify the left side.' },
        { expression: '\\frac{dy}{dt} = -\\frac{24}{16} = -\\frac{3}{2} \\text{ ft/s}', annotation: 'Solve for dy/dt. The negative sign means y is decreasing — the top slides DOWN. At this moment the top drops at 3/2 ft/s while the base slides out at 2 ft/s — the top moves slower than the base because the ladder is more horizontal than vertical (x > y).' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: 'V = \\frac{4}{3}\\pi r^3', annotation: 'Volume of a sphere. r and V are both functions of t.' },
        { expression: '\\frac{dV}{dt} = 4\\pi r^2 \\,\\frac{dr}{dt}', annotation: 'Differentiate both sides with respect to t. d/dt[(4/3)πr³] = (4/3)π·3r²·(dr/dt) = 4πr²·(dr/dt). The factor 4πr² is the surface area — new volume is added in a thin surface layer.' },
        { expression: '100 = 4\\pi(5)^2 \\cdot \\frac{dr}{dt}', annotation: 'Substitute dV/dt = 100 and r = 5.' },
        { expression: '100 = 4\\pi(25)\\,\\frac{dr}{dt} = 100\\pi\\,\\frac{dr}{dt}', annotation: 'Simplify: 4π·25 = 100π.' },
        { expression: '\\frac{dr}{dt} = \\frac{100}{100\\pi} = \\frac{1}{\\pi} \\approx 0.318 \\text{ cm/s}', annotation: 'Solve for dr/dt. The radius grows at 1/π cm/s at this moment.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: '\\frac{r}{h} = \\frac{3}{4} \\Rightarrow r = \\frac{3h}{4}', annotation: 'Similar triangles: the tank has radius 3 when height is 4. At any water height h, the water surface radius r satisfies r/h = 3/4. This is the key geometric constraint that reduces two variables to one.' },
        { expression: 'V = \\frac{1}{3}\\pi r^2 h = \\frac{1}{3}\\pi \\left(\\frac{3h}{4}\\right)^2 h = \\frac{3\\pi h^3}{16}', annotation: 'Substitute r = 3h/4 into the cone volume formula. Now V is a function of h alone — a single-variable relationship.' },
        { expression: '\\frac{dV}{dt} = \\frac{9\\pi h^2}{16}\\,\\frac{dh}{dt}', annotation: 'Differentiate V = 3πh³/16 with respect to t. d/dt[3πh³/16] = (9πh²/16)·(dh/dt).' },
        { expression: '-2 = \\frac{9\\pi(2)^2}{16}\\,\\frac{dh}{dt} = \\frac{9\\pi}{4}\\,\\frac{dh}{dt}', annotation: 'Substitute dV/dt = -2 (negative because draining) and h = 2. Simplify: 9π·4/16 = 9π/4.' },
        { expression: '\\frac{dh}{dt} = \\frac{-2 \\cdot 4}{9\\pi} = \\frac{-8}{9\\pi} \\approx -0.283 \\text{ m/min}', annotation: 'Solve for dh/dt. The water level drops at 8/(9π) m/min.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: '\\text{Let } x = \\text{person\'s distance from base, } s = \\text{shadow length}', annotation: 'Define variables. Both x and s are functions of t. The tip of the shadow is at distance x + s from the base.' },
        { expression: '\\frac{20}{x + s} = \\frac{6}{s}', annotation: 'Similar triangles: the streetlight, the tip of the shadow, and the person\'s top form similar triangles. The full triangle (from base to shadow tip, height 20) is similar to the small triangle (from person to shadow tip, height 6).' },
        { expression: '20s = 6(x + s) \\Rightarrow 20s = 6x + 6s \\Rightarrow 14s = 6x', annotation: 'Cross-multiply and simplify. This gives the constraint between s and x.' },
        { expression: 's = \\frac{3x}{7}', annotation: 'Solve for s in terms of x.' },
        { expression: '\\frac{ds}{dt} = \\frac{3}{7}\\,\\frac{dx}{dt} = \\frac{3}{7}(5) = \\frac{15}{7} \\approx 2.14 \\text{ ft/s}', annotation: 'Differentiate both sides with respect to t. Since 3/7 is constant, ds/dt = (3/7)·(dx/dt). The shadow length grows at a constant 15/7 ft/s.' },
        { expression: '\\text{Shadow tip speed} = \\frac{d}{dt}(x + s) = \\frac{dx}{dt} + \\frac{ds}{dt} = 5 + \\frac{15}{7} = \\frac{50}{7} \\approx 7.14 \\text{ ft/s}', annotation: 'The tip moves at the person\'s speed plus the shadow growth rate. The shadow tip outpaces the person.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: '\\tan(\\theta) = \\frac{h}{500}', annotation: 'The angle of elevation θ satisfies tan(θ) = opposite/adjacent = h/500. Both θ and h vary with t.' },
        { expression: '\\sec^2(\\theta)\\,\\frac{d\\theta}{dt} = \\frac{1}{500}\\,\\frac{dh}{dt}', annotation: 'Differentiate both sides with respect to t. d/dt[tan(θ)] = sec²(θ)·(dθ/dt) by the chain rule. The right side: d/dt[h/500] = (1/500)·(dh/dt).' },
        { expression: 'h = 500 \\Rightarrow \\tan(\\theta) = 1 \\Rightarrow \\theta = \\frac{\\pi}{4}', annotation: 'At h = 500 ft, tan(θ) = 500/500 = 1, so θ = π/4 (45°).' },
        { expression: '\\sec^2\\!\\left(\\frac{\\pi}{4}\\right) = \\frac{1}{\\cos^2(\\pi/4)} = \\frac{1}{(1/\\sqrt{2})^2} = 2', annotation: 'Compute sec²(π/4). cos(π/4) = 1/√2, so cos²(π/4) = 1/2, so sec²(π/4) = 2.' },
        { expression: '2\\,\\frac{d\\theta}{dt} = \\frac{10}{500} = \\frac{1}{50}', annotation: 'Substitute sec²(θ) = 2 and dh/dt = 10 into the rate equation.' },
        { expression: '\\frac{d\\theta}{dt} = \\frac{1}{100} \\text{ rad/s}', annotation: 'Solve for dθ/dt. The angle increases at 1/100 radian per second at this moment — about 0.57°/s.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: 'z^2 = x^2 + y^2', annotation: 'Let x = Car B\'s distance from intersection, y = Car A\'s distance, z = distance between cars. The cars are on perpendicular roads, so the Pythagorean theorem applies.' },
        { expression: '2z\\,\\frac{dz}{dt} = 2x\\,\\frac{dx}{dt} + 2y\\,\\frac{dy}{dt}', annotation: 'Differentiate both sides with respect to t.' },
        { expression: 'x = 12, \\; y = 5 \\Rightarrow z = \\sqrt{144 + 25} = \\sqrt{169} = 13 \\text{ mi}', annotation: 'Compute z using the Pythagorean theorem. The 5-12-13 right triangle gives z = 13.' },
        { expression: '\\frac{dx}{dt} = -80, \\quad \\frac{dy}{dt} = -60', annotation: 'Both cars approach the intersection, so their distances decrease: dx/dt = -80 mph (B moves east toward intersection), dy/dt = -60 mph (A moves north toward intersection).' },
        { expression: '2(13)\\,\\frac{dz}{dt} = 2(12)(-80) + 2(5)(-60)', annotation: 'Substitute all values into the rate equation.' },
        { expression: '26\\,\\frac{dz}{dt} = -1920 - 600 = -2520', annotation: 'Simplify the right side.' },
        { expression: '\\frac{dz}{dt} = \\frac{-2520}{26} = -\\frac{1260}{13} \\approx -96.9 \\text{ mph}', annotation: 'The negative sign means the cars are getting closer. They approach each other at about 96.9 mph.' },
      ],
    ...
    },
    {
      ...
      steps: [
        ...
        { expression: '50 = 2\\pi(10)\\,\\frac{dr}{dt} = 20\\pi\\,\\frac{dr}{dt}', annotation: 'Substitute dA/dt = 50 and r = 10.' },
        { expression: '\\frac{dr}{dt} = \\frac{50}{20\\pi} = \\frac{5}{2\\pi} \\approx 0.796 \\text{ m/hr}', annotation: 'Solve for dr/dt.' },
      ],
  ...
  challenges: [
    {
      ...
      walkthrough: [
        { expression: 'x^2 + 100^2 = L^2', annotation: 'The kite string, horizontal distance, and fixed height form a right triangle. L is the hypotenuse (string length).' },
        { expression: '2x\\,\\frac{dx}{dt} = 2L\\,\\frac{dL}{dt}', annotation: 'Differentiate with respect to t. The height 100 is constant, so d/dt[100²] = 0.' },
        { expression: 'L = 200 \\Rightarrow x = \\sqrt{200^2 - 100^2} = \\sqrt{40000 - 10000} = \\sqrt{30000} = 100\\sqrt{3}', annotation: 'Find x when L = 200 using the Pythagorean theorem.' },
        { expression: '2(100\\sqrt{3})\\,\\frac{dx}{dt} = 2(200)(5)', annotation: 'Substitute x = 100√3, L = 200, dL/dt = 5.' },
        { expression: '\\frac{dx}{dt} = \\frac{200 \\cdot 5}{100\\sqrt{3}} = \\frac{1000}{100\\sqrt{3}} = \\frac{10}{\\sqrt{3}} = \\frac{10\\sqrt{3}}{3} \\approx 5.77 \\text{ m/s}', annotation: 'Solve for dx/dt and rationalize.' },
      ],
    ...
    },
    {
      ...
      walkthrough: [
        { expression: 'r = \\frac{h}{2}', annotation: 'h = 2r means r = h/2. Use this to eliminate r.' },
        { expression: 'V = \\frac{1}{3}\\pi r^2 h = \\frac{1}{3}\\pi \\left(\\frac{h}{2}\\right)^2 h = \\frac{\\pi h^3}{12}', annotation: 'Substitute r = h/2 into the cone volume formula.' },
        { expression: '\\frac{dV}{dt} = \\frac{\\pi h^2}{4}\\,\\frac{dh}{dt}', annotation: 'Differentiate V = πh³/12 with respect to t: (π/12)·3h²·(dh/dt) = (πh²/4)·(dh/dt).' },
        { expression: '30 = \\frac{\\pi(10)^2}{4}\\,\\frac{dh}{dt} = \\frac{100\\pi}{4}\\,\\frac{dh}{dt} = 25\\pi\\,\\frac{dh}{dt}', annotation: 'Substitute dV/dt = 30 and h = 10.' },
        { expression: '\\frac{dh}{dt} = \\frac{30}{25\\pi} = \\frac{6}{5\\pi} \\approx 0.382 \\text{ ft/min}', annotation: 'Solve for dh/dt.' },
      ],
    ...
    },
    {
      ...
      walkthrough: [
        { expression: 'x^2 + 144 = L^2', annotation: 'The pulley is 12 ft above water (constant). x is the boat\'s horizontal distance from below the pulley, L is the rope length. Pythagorean theorem.' },
        { expression: '2x\\,\\frac{dx}{dt} = 2L\\,\\frac{dL}{dt}', annotation: 'Differentiate with respect to t.' },
        { expression: 'L = 15 \\Rightarrow x = \\sqrt{225 - 144} = \\sqrt{81} = 9 \\text{ ft}', annotation: 'Find x when L = 15.' },
        { expression: '\\frac{dL}{dt} = -3 \\text{ ft/s}', annotation: 'The rope is being pulled IN, so L decreases: dL/dt = -3.' },
        { expression: '2(9)\\,\\frac{dx}{dt} = 2(15)(-3) \\Rightarrow 18\\,\\frac{dx}{dt} = -90', annotation: 'Substitute into the rate equation.' },
        { expression: '\\frac{dx}{dt} = -5 \\text{ ft/s}', annotation: 'The boat moves toward the dock at 5 ft/s. The negative sign confirms x is decreasing.' },
      ],
