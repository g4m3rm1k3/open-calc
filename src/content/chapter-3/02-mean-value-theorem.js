...
export default {
  ...
  examples: [
    {
      ...
      steps: [
        { expression: '\\frac{f(3)-f(1)}{3-1} = \\frac{9-1}{2} = 4', annotation: 'Compute the average rate of change (slope of the secant from (1,1) to (3,9)).' },
        { expression: "f'(x) = 2x", annotation: 'Compute the derivative.' },
        { expression: "f'(c) = 4 \\Rightarrow 2c = 4 \\Rightarrow c = 2", annotation: 'Set f\'(c) equal to the average rate and solve. c = 2 ∈ (1, 3) ✓.' },
        { expression: '\\text{The tangent at } c=2 \\text{ has slope } f\'(2) = 4 = \\text{slope of secant}', annotation: 'Verify: f\'(2) = 4 matches the secant slope. The MVT is satisfied with c = 2.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: '\\frac{f(1)-f(-1)}{1-(-1)} = \\frac{1-(-1)}{2} = \\frac{2}{2} = 1', annotation: 'Compute the average rate of change.' },
        { expression: "f'(x) = 3x^2", annotation: 'Derivative of x³.' },
        { expression: "f'(c) = 1 \\Rightarrow 3c^2 = 1 \\Rightarrow c^2 = \\frac{1}{3} \\Rightarrow c = \\pm\\frac{1}{\\sqrt{3}} \\approx \\pm 0.577", annotation: 'Solve for c. Two solutions: c = ±1/√3.' },
        { expression: 'c = \\frac{1}{\\sqrt{3}} \\in (-1, 1) \\checkmark, \\quad c = -\\frac{1}{\\sqrt{3}} \\in (-1, 1) \\checkmark', annotation: 'Both values are in the open interval (-1, 1), confirming the MVT. This illustrates that the MVT can have multiple c values.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: 'f(t) = \\sin(t), \\quad f\'(t) = \\cos(t)', annotation: 'Apply the MVT to f(t) = sin(t) on the interval between x and y (WLOG x < y).' },
        { expression: '\\sin(x) - \\sin(y) = \\cos(c)(x - y) \\text{ for some } c \\in (y, x)', annotation: 'By the MVT: f(x) - f(y) = f\'(c)(x-y).' },
        { expression: '|\\sin(x) - \\sin(y)| = |\\cos(c)| \\cdot |x - y|', annotation: 'Take absolute values of both sides. Use |ab| = |a|·|b|.' },
        { expression: '|\\cos(c)| \\leq 1 \\text{ for all } c', annotation: 'The cosine function satisfies |cos(c)| ≤ 1 everywhere.' },
        { expression: '\\therefore |\\sin(x) - \\sin(y)| \\leq 1 \\cdot |x - y| = |x - y|', annotation: 'Multiply the inequality by |x-y| ≥ 0. ∎' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: 's(t) = \\text{position (miles), } s(0) = 0, s(3) = 200', annotation: 'Set up: s(t) is the position function. s is continuous (the car moves continuously) and differentiable (it has a velocity) on (0,3).' },
        { expression: '\\frac{s(3) - s(0)}{3 - 0} = \\frac{200}{3} \\approx 66.7 \\text{ mph}', annotation: 'Compute the average velocity over the 3-hour trip.' },
        { expression: '\\text{By MVT: } \\exists\\, c \\in (0, 3) \\text{ such that } s\'(c) = \\frac{200}{3} \\approx 66.7 \\text{ mph}', annotation: 'Apply the MVT: since s satisfies the hypotheses, there exists c with s\'(c) = 200/3.' },
        { expression: 's\'(c) = \\frac{200}{3} > 60 \\text{ mph}', annotation: 'The instantaneous speed at time c exceeds 60 mph. ∎' },
      ],
  ...
  challenges: [
    ...
    },
    {
      ...
      walkthrough: [
        { expression: 'g(x) = f(x) \\cdot e^{-x}', annotation: 'Define the helper function g. We will show g is constant.' },
        { expression: "g'(x) = f'(x)e^{-x} + f(x)(-e^{-x}) = [f'(x) - f(x)]e^{-x}", annotation: 'Differentiate g using the product rule.' },
        { expression: "f'(x) = f(x) \\Rightarrow f'(x) - f(x) = 0 \\Rightarrow g'(x) = 0", annotation: 'Since f\'(x) = f(x), the bracket is zero, so g\'(x) = 0 for all x.' },
        { expression: "g'(x) = 0 \\text{ everywhere} \\Rightarrow g \\text{ is constant (by MVT corollary)}", annotation: 'By Corollary 1 of the MVT, g is constant.' },
        { expression: 'g(0) = f(0) \\cdot e^0 = 0 \\cdot 1 = 0', annotation: 'Evaluate g at 0 using f(0) = 0.' },
        { expression: 'g(x) = 0 \\text{ for all } x \\Rightarrow f(x) \\cdot e^{-x} = 0 \\Rightarrow f(x) = 0', annotation: 'Since g is the constant 0 and e^{-x} ≠ 0, we conclude f(x) = 0. ∎' },
      ],
    ...
    },
    {
      ...
      walkthrough: [
        { expression: 'h(x) = f(x)[g(b)-g(a)] - g(x)[f(b)-f(a)]', annotation: 'Define the helper function h. It is a linear combination of f and g, so h is continuous on [a,b] and differentiable on (a,b).' },
        { expression: 'h(a) = f(a)[g(b)-g(a)] - g(a)[f(b)-f(a)]', annotation: 'Evaluate h at a.' },
        { expression: 'h(b) = f(b)[g(b)-g(a)] - g(b)[f(b)-f(a)]', annotation: 'Evaluate h at b.' },
        { expression: 'h(b) - h(a) = [f(b)-f(a)][g(b)-g(a)] - [g(b)-g(a)][f(b)-f(a)] = 0', annotation: 'Compute h(b) - h(a). It factors as [f(b)-f(a)][g(b)-g(a)] - [g(b)-g(a)][f(b)-f(a)] = 0 (the two terms are equal). So h(a) = h(b).' },
        { expression: "\\text{By Rolle's Theorem: } \\exists\\, c \\in (a,b) \\text{ with } h'(c) = 0", annotation: 'h satisfies Rolle\'s hypotheses (continuous on [a,b], differentiable on (a,b), h(a) = h(b)).' },
        { expression: "h'(x) = f'(x)[g(b)-g(a)] - g'(x)[f(b)-f(a)]", annotation: 'Differentiate h.' },
        { expression: "h'(c) = 0 \\Rightarrow f'(c)[g(b)-g(a)] = g'(c)[f(b)-f(a)] \\quad \\square", annotation: 'Set h\'(c) = 0 and rearrange. This is Cauchy\'s MVT. ∎' },
      ],
