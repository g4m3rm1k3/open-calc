  ...
  examples: [
    {
      ...
      steps: [
        { expression: 'f(x) = \\sqrt{x}, \\quad f(25) = 5', annotation: 'Evaluate the function at the base point a = 25.' },
        { expression: "f'(x) = \\frac{1}{2\\sqrt{x}}, \\quad f'(25) = \\frac{1}{10}", annotation: 'Compute the derivative and evaluate at a = 25.' },
        { expression: 'L(x) = 5 + \\frac{1}{10}(x - 25)', annotation: 'Write the linearization L(x) = f(a) + f\'(a)(x-a).' },
        { expression: 'L(26) = 5 + \\frac{1}{10}(1) = 5.1 \\quad (\\text{actual: } \\sqrt{26} \\approx 5.0990)', annotation: 'Estimate √26. Error ≈ 0.001.' },
        { expression: 'L(24) = 5 + \\frac{1}{10}(-1) = 4.9 \\quad (\\text{actual: } \\sqrt{24} \\approx 4.8990)', annotation: 'Estimate √24. Error ≈ 0.001.' },
        { expression: 'L(25.1) = 5 + \\frac{1}{10}(0.1) = 5.01 \\quad (\\text{actual: } \\sqrt{25.1} \\approx 5.0100)', annotation: 'Estimate √25.1. Error < 0.0001 — much better for smaller displacement.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: 'f(0) = \\sin(0) = 0, \\quad f\'(0) = \\cos(0) = 1', annotation: 'Evaluate f and f\' at a = 0.' },
        { expression: 'L(x) = 0 + 1 \\cdot (x - 0) = x', annotation: 'The linearization of sin(x) at 0 is simply L(x) = x. This is the famous "small-angle approximation."' },
        { expression: '\\sin(0.1) \\approx L(0.1) = 0.1 \\quad (\\text{actual: } 0.09983...)', annotation: 'Estimate sin(0.1). Error ≈ 0.00017 — excellent for 0.1 radians (≈5.7°).' },
        { expression: '\\text{For } \\sin(\\pi/2 + 0.1), \\text{ use } a = \\pi/2', annotation: 'Choose a base point closer to π/2 + 0.1 for better accuracy.' },
        { expression: 'f(\\pi/2) = 1, \\quad f\'(\\pi/2) = \\cos(\\pi/2) = 0', annotation: 'Evaluate at the new base point a = π/2.' },
        { expression: 'L(x) = 1 + 0 \\cdot (x - \\pi/2) = 1', annotation: 'The linearization at a = π/2 is just the constant 1.' },
        { expression: '\\sin(\\pi/2 + 0.1) \\approx 1 \\quad (\\text{actual: } \\cos(0.1) \\approx 0.99500)', annotation: 'Estimate: 1. Actual: ≈ 0.995. Error ≈ 0.005. The zero-slope tangent is less accurate here.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: 'f(x) = e^x, \\quad f(0) = 1, \\quad f\'(0) = e^0 = 1', annotation: 'The exponential function equals its own derivative everywhere. At a = 0: f(0) = 1 and f\'(0) = 1.' },
        { expression: 'L(x) = 1 + x', annotation: 'Linearization of e^x at 0 is L(x) = 1 + x. This is one of the most important approximations in mathematics.' },
        { expression: 'e^{0.2} \\approx 1 + 0.2 = 1.2 \\quad (\\text{actual: } e^{0.2} \\approx 1.2214)', annotation: 'Estimate e^0.2. Error ≈ 0.021 (1.7% relative error). Decent for x = 0.2.' },
        { expression: 'e^{0.01} \\approx 1 + 0.01 = 1.01 \\quad (\\text{actual: } e^{0.01} \\approx 1.01005)', annotation: 'Estimate e^0.01. Error ≈ 0.00005 (0.005% relative error). Excellent for x = 0.01.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: "y' = 3x^2 - 2", annotation: 'Differentiate y with respect to x.' },
        { expression: "y'(2) = 3(4) - 2 = 10", annotation: 'Evaluate the derivative at x = 2.' },
        { expression: 'dy = y\'(2) \\cdot dx = 10 \\cdot 0.1 = 1.0', annotation: 'The differential dy = f\'(x)·dx gives the change along the tangent line.' },
        { expression: 'y(2) = 8 - 4 + 1 = 5', annotation: 'Current function value at x = 2.' },
        { expression: 'y(2.1) = (2.1)^3 - 2(2.1) + 1 = 9.261 - 4.2 + 1 = 6.061', annotation: 'Exact function value at x = 2.1.' },
        { expression: '\\Delta y = y(2.1) - y(2) = 6.061 - 5 = 1.061', annotation: 'The actual change in y.' },
        { expression: '\\text{Error} = \\Delta y - dy = 1.061 - 1.0 = 0.061', annotation: 'The differential underestimates the actual change. The error is (dx)² times f\'\'(x)/2 ≈ 6·(0.1)²/2 = 0.03 (slightly off because we\'re not exactly at x=2 for the second derivative).' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: 'V = \\frac{4}{3}\\pi r^3', annotation: 'Volume of a sphere.' },
        { expression: '\\frac{dV}{dr} = 4\\pi r^2', annotation: 'Derivative of V with respect to r (surface area).' },
        { expression: 'dV = 4\\pi r^2 \\cdot dr', annotation: 'The differential dV gives the approximate error in V due to error dr in r.' },
        { expression: 'dV \\approx 4\\pi(10)^2(\\pm 0.05) = \\pm 4\\pi(100)(0.05) = \\pm 20\\pi \\approx \\pm 62.8 \\text{ cm}^3', annotation: 'Substitute r = 10 and dr = ±0.05.' },
        { expression: '\\text{Relative error} = \\frac{|dV|}{V} = \\frac{20\\pi}{\\frac{4}{3}\\pi(1000)} = \\frac{20}{\\frac{4000}{3}} = \\frac{60}{4000} = 1.5\\%', annotation: 'The relative error in V is 3 times the relative error in r (0.5%). This factor of 3 comes from the exponent 3 in V = (4/3)πr³.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: 'f(1) = \\ln(1) = 0, \\quad f\'(x) = \\frac{1}{x}, \\quad f\'(1) = 1', annotation: 'Evaluate at a = 1.' },
        { expression: 'L(x) = 0 + 1 \\cdot (x - 1) = x - 1', annotation: 'Linearization: L(x) = x - 1. Equivalently, ln(x) ≈ x - 1 near x = 1, or ln(1+u) ≈ u near u = 0.' },
        { expression: '\\ln(1.1) \\approx L(1.1) = 0.1 \\quad (\\text{actual: } 0.09531...)', annotation: 'Estimate ln(1.1). Error ≈ 0.0047 — about 5% relative error.' },
        { expression: '\\ln(0.9) \\approx L(0.9) = -0.1 \\quad (\\text{actual: } -0.10536...)', annotation: 'Estimate ln(0.9). Error ≈ 0.005 — about 5% relative error.' },
      ],
    ...
    },
    {
      ...
      steps: [
        { expression: '30° = \\frac{\\pi}{6} \\text{ rad}, \\quad 2° = \\frac{\\pi}{90} \\text{ rad} \\approx 0.03491 \\text{ rad}', annotation: 'Convert to radians. Calculus must be done in radians — the derivative formulas for trig functions only hold in radian measure.' },
        { expression: 'f(\\theta) = \\sin(\\theta), \\quad f\'(\\theta) = \\cos(\\theta)', annotation: 'The relevant derivative.' },
        { expression: 'd(\\sin\\theta) = \\cos(\\theta)\\,d\\theta', annotation: 'Write the differential.' },
        { expression: '\\text{Error} \\approx |\\cos(\\pi/6)| \\cdot (\\pi/90) = \\frac{\\sqrt{3}}{2} \\cdot \\frac{\\pi}{90} \\approx 0.8660 \\cdot 0.03491 \\approx \\pm 0.03023', annotation: 'Substitute θ = π/6 and dθ = π/90.' },
        { expression: '\\text{Relative error} = \\frac{0.03023}{\\sin(\\pi/6)} = \\frac{0.03023}{0.5} \\approx 6.05\\%', annotation: 'A 2° error (6.67% relative error in angle) produces about 6% relative error in the sine value.' },
      ],
  ...
  challenges: [
    {
      ...
      walkthrough: [
        { expression: 'f(x) = (1+x)^{1000}, \\quad f\'(x) = 1000(1+x)^{999}', annotation: 'Identify f and compute f\'.' },
        { expression: 'L(x) = f(0) + f\'(0) \\cdot x = 1 + 1000x', annotation: 'Linearization at a = 0: f(0) = 1, f\'(0) = 1000.' },
        { expression: '(1.001)^{1000} \\approx L(0.001) = 1 + 1000(0.001) = 1 + 1 = 2', annotation: 'Apply the linearization with x = 0.001.' },
        ...
        { expression: '(1 + 1/n)^n \\to e \\text{ as } n \\to \\infty', annotation: 'This is the definition of e. With n = 1000, (1+1/1000)^1000 ≈ e^1 = 2.718... The linear approximation misses this because it only captures the first-order term; (1+x)^n for large n requires higher-order terms.' },
      ],
    ...
    },
    {
      ...
      walkthrough: [
        { expression: 'f(x) = f(a) + f\'(a)(x-a) + \\varepsilon(x)(x-a) \\text{ where } \\varepsilon(x) \\to 0 \\text{ as } x \\to a', annotation: 'This is the definition of differentiability at a: the error in the linear approximation goes to 0 faster than |x-a|.' },
        { expression: '\\lim_{x \\to a} f(x) = \\lim_{x \\to a} \\left[f(a) + f\'(a)(x-a) + \\varepsilon(x)(x-a)\\right]', annotation: 'Take the limit of both sides as x → a.' },
        { expression: '= f(a) + f\'(a) \\cdot \\lim_{x \\to a}(x-a) + \\lim_{x \\to a}[\\varepsilon(x)(x-a)]', annotation: 'Split the limit (using limit laws, valid since each piece converges).' },
        { expression: '\\lim_{x \\to a}(x-a) = 0', annotation: 'The factor (x-a) → 0 as x → a.' },
        { expression: '\\left|\\varepsilon(x)(x-a)\\right| \\leq |\\varepsilon(x)| \\cdot |x-a| \\to 0 \\cdot 0 = 0', annotation: 'Since ε(x) → 0 and (x-a) → 0, their product → 0.' },
        { expression: '\\therefore \\lim_{x \\to a} f(x) = f(a) + 0 + 0 = f(a)', annotation: 'The limit equals f(a) — precisely the definition of continuity at a.' },
      ],
    ...
    },
    {
      ...
      walkthrough: [
        { expression: 'T = 2\\pi\\sqrt{\\frac{L}{g}} = \\frac{2\\pi}{\\sqrt{g}} \\cdot L^{1/2}', annotation: 'Write T as a function of L (g is fixed).' },
        { expression: '\\frac{dT}{dL} = \\frac{2\\pi}{\\sqrt{g}} \\cdot \\frac{1}{2\\sqrt{L}} = \\frac{\\pi}{\\sqrt{gL}}', annotation: 'Differentiate T with respect to L.' },
        { expression: 'dT = \\frac{\\pi}{\\sqrt{gL}} \\cdot dL', annotation: 'Write the differential.' },
        { expression: 'T(1) = 2\\pi\\sqrt{1/9.8} \\approx 2\\pi(0.3194) \\approx 2.007 \\text{ s}', annotation: 'Compute the nominal period at L = 1 m.' },
        { expression: 'dT = \\frac{\\pi}{\\sqrt{9.8 \\cdot 1}} \\cdot (\\pm 0.01) = \\frac{\\pi}{\\sqrt{9.8}} \\cdot 0.01 \\approx \\frac{3.1416}{3.130} \\cdot 0.01 \\approx \\pm 0.01004 \\text{ s}', annotation: 'Substitute L = 1, g = 9.8, dL = ±0.01.' },
        { expression: '\\text{Relative error} = \\frac{|dT|}{T} \\approx \\frac{0.01004}{2.007} \\approx 0.5\\%', annotation: 'The relative error in T equals half the relative error in L (because T ∝ L^{1/2}: the exponent 1/2 scales the relative error).' },
      ],
