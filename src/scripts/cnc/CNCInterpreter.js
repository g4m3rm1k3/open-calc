...
export class CNCInterpreter {
  ...
  loadProgram(code) {
    ...
    this.blocks.forEach((b, idx) => {
      ...
      if (oTok) {
        this.labels.set(oTok.value, idx);
        this.labels.set(Math.floor(oTok.value), idx);
      }
  ...
  reset() {
    ...
    for (let i = 1; i <= 48; i++)
      offsets[`EWO_P${i}`] = { X: 0, Y: 0, Z: 0, A: 0, B: 0, C: 0 };
    // Siemens G500 (machine, always zero) and G505-G599 (user frames)
    offsets["G500"] = { X: 0, Y: 0, Z: 0, A: 0, B: 0, C: 0 };
    for (let i = 505; i <= 599; i++)
      offsets[`G${i}`] = { X: 0, Y: 0, Z: 0, A: 0, B: 0, C: 0 };
  ...
  _executeBlock(block) {
    ...
    if (gtoAbs) {
      this._gotoLabel(parseFloat(gtoAbs[1]));
      return;
    }
    const gtoNamed = line.match(/^GOTO[FB]\s+(\S+)/i);
    if (gtoNamed) {
      this._gotoLabel(gtoNamed[1]);
      return;
    }
  ...
  _processWords(tokens, rawLine) {
      ...
      switch (t.word) {
        case "G": {
          if ([0, 1, 2, 3].includes(v)) {
            ...
            /* G04 dwell — no-op for simulation */
          } else if (v === 10) {
            hasG10 = true;
            ...
            this.state.activeOffset = "G500"; // Siemens: machine coords
          else if (v >= 505 && v <= 599)
            this.state.activeOffset = `G${Math.floor(v)}`;
          else if (v >= 80 && v <= 89)
            this.state.cycleMode = "G" + Math.floor(v);
        ...
        case "T":
          this.state.activeT = Math.floor(v);
          break;
        case "S":
          this.state.spindleRPM = v;
          break;
        case "F":
          this.state.feedrate = v;
          break;
        case "H":
          this.state.activeH = Math.floor(v);
          break;
        case "D":
          this.state.activeD = Math.floor(v);
          break;

        case "X":
          tX = this._resolveAxisVal(rawLine, "X", v);
          break;
        case "Y":
          tY = this._resolveAxisVal(rawLine, "Y", v);
          break;
        case "Z":
          tZ = this._resolveAxisVal(rawLine, "Z", v);
          break;
        case "I":
          tI = v;
          break;
        ...
        case "R": {
          if (this.dialect !== "siemens") {
            ...
            const hasCycle = tokens.some(
              (t2) => t2.word === "G" && t2.value >= 81 && t2.value <= 89,
            );
        ...
        case "P": {
          ...
          const g541 = tokens.find(
            (t2) => t2.word === "G" && Math.abs(t2.value - 54.1) < 0.01,
          );
          if (g541) {
    ...
    if (hasG10) {
      this._handleG10(tokens, rawLine);
      return;
    }
  ...
  _updateMachine() {
    const off = this.state.offsets[this.state.activeOffset] ?? {
      X: 0,
      Y: 0,
      Z: 0,
    };
    ...
    if (this.state.activeH > 0) {
      ...
      let tlLen = tl ? (tl.len ?? 0) : 0;
      // Tools are stored in mm; convert to program units when running in inch mode
      if (this.state.units === "inch") tlLen = tlLen / 25.4;
  ...
  _handleG10(tokens, rawLine) {
    ...
    if (L === 2) {
      ...
      if (key && this.state.offsets[key]) {
        if (xv)
          this.state.offsets[key].X = this._resolveAxisVal(
            rawLine,
            "X",
            xv.value,
          );
        if (yv)
          this.state.offsets[key].Y = this._resolveAxisVal(
            rawLine,
            "Y",
            yv.value,
          );
        if (zv)
          this.state.offsets[key].Z = this._resolveAxisVal(
            rawLine,
            "Z",
            zv.value,
          );
      ...
      if (rv)
        this.toolTable[P] = { ...(this.toolTable[P] ?? {}), len: rv.value };
    } else if (L === 12 || L === 13) {
      // G10 L12/13 Pn R — cutter radius comp geometry/wear
      if (rv)
        this.toolTable[P] = { ...(this.toolTable[P] ?? {}), dia: rv.value * 2 };
    }
  }
  ...
  _tryAssignment(line) {
    ...
    const numM = clean.match(
      /^(?:#(\d+)|R(\d+)|V(\d+)|VC\[(\d+)\])\s*=\s*(.+)$/i,
    );
    if (numM) {
    ...
    if (defM) {
      if (defM[2])
        this.state.namedVars.set(
          defM[1].toUpperCase(),
          this._evalExpr(defM[2]),
        );
      return true;
    }
  ...
  _setNamedVar(name, val) {
    ...
    const sysNum =
      FANUC_NAMED[name] ??
      FANUC_NAMED[name.startsWith("_") ? name : "_" + name];
    if (sysNum !== undefined) {
  ...
  _resolveAxisVal(rawLine, axis, tokenNum) {
    ...
    const eqRef = rawLine.match(
      new RegExp("\\b" + axis + "\\s*=\\s*([^\\s,;]+)", "i"),
    );
    if (eqRef) {
      const s = eqRef[1];
  ...
  _handleM98(tokens) {
    ...
    this.callStack.push({
      returnPtr: this.state.programPointer + 1,
