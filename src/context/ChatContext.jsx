...
export function ChatProvider({ children }) {
  ...
  const [blockedUsers, setBlockedUsers] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("oc-blocked-peers") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const blockPeer = useCallback((peerId, username) => {
    setBlockedUsers((prev) => {
      ...
      const next = [
        ...prev,
        { peerId, username: username || peerId.slice(0, 8) },
      ];
  ...
  const sendMessage = useCallback(
    (text, roomType, isLovelace = false) => {
      ...
      const roomId =
        roomType === "global"
          ? "global"
          : `lesson-${lessonIdFromPath(location.pathname) ?? "unknown"}`;

      if (roomType === "global") {
        try {
          sendGlobalRef.current?.(payload);
        } catch {}
        setGlobalMessages((prev) => mergeMessages(prev, [msg]));
      } else {
        try {
          sendLessonRef.current?.(payload);
        } catch {}
        setLessonMessages((prev) => mergeMessages(prev, [msg]));
      }
  ...
  const sendLovelaceQuery = useCallback(
    (queryId, text, recentMessages, roomType) => {
