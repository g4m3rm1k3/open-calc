  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const distSq =
        positions[idx] * positions[idx] +
        positions[idx + 1] * positions[idx + 1];
      if (distSq > 3600 || Math.random() < 0.005) {
        // Reset at dist 60 or randomly
      ...
      if (dist < 2) {
        colors[idx] = 1;
        colors[idx + 1] = 1;
        colors[idx + 2] = 1; // Bright nucleus
      } else if (dist < 20) {
        colors[idx] = 0.98 * fade;
        colors[idx + 1] = 0.45 * fade;
        colors[idx + 2] = 0.08 * fade; // Red/Orange
      } else {
        colors[idx] = 0.1 * fade;
        colors[idx + 1] = 0.4 * fade;
        colors[idx + 2] = 1.0 * fade; // Blue Ion
      }
    }
...
function MeteorSystem() {
  ...
  useFrame((state) => {
    ...
    if (rand < 0.00001 && meteors.length < 1) {
      const shower = Array.from({ length: 15 }).map((_, i) => ({
        id: Date.now() + i,
        type: "shower",
      }));
...
export default function DynamicBackground({ mode, config }) {
  ...
  return (
    <div
      data-bg
      className="fixed inset-0 z-[-1] pointer-events-none overflow-hidden bg-slate-100 dark:bg-[#020617]"
    >
