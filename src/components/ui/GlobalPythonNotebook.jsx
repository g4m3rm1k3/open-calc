...
import { X, Play, Plus, Trash2, Terminal, Cpu, Database, Save, Activity } from 'lucide-react';
...
export default function GlobalPythonNotebook({ isOpen, onClose }) {
  ...
  const [cells, setCells] = useState([
    { id: 1, code: '# Welcome to the Global Python Notebook\nimport numpy as np\nimport math\n\n# Variables persist across cells\nx = np.linspace(0, 10, 100)\ny = np.sin(x)\nprint(f"Computed {len(x)} points of sin(x)")\n"Ready to go!"', output: '', status: 'idle' }
  ]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const stopShortcuts = (e) => {
        ...
        const isInput = activeTag === 'INPUT' || activeTag === 'TEXTAREA' || document.activeElement?.classList.contains('monaco-editor');
        
        // If it's one of OpenCalc's single-key shortcuts, stop it from bubbling if notebook is open
        if (!isInput && ['g', '3', 'x', 's', 'c'].includes(e.key.toLowerCase())) {
           e.stopPropagation();
        }
      };
          ...
          >
            ...
            <header className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800 shrink-0">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800 dark:text-white">Python Sandbox</h2>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className={`w-2 h-2 rounded-full ${runtimeStatus === 'ready' ? 'bg-teal-500 animate-pulse' : 'bg-amber-500'}`} />
                    <span className="text-[10px] uppercase tracking-wider font-bold text-slate-500">
                      {runtimeStatus === 'loading' ? 'Initializing Core...' : 
                       runtimeStatus === 'installing' ? 'Hydrating Libraries (NumPy, SciPy, SymPy)...' : 
              ...
              {runtimeStatus === 'ready' ? (
                <>
                  <div className="grid grid-cols-3 gap-3 mb-2">
                    <div className="p-3 rounded-2xl bg-teal-50 dark:bg-teal-500/10 border border-teal-100 dark:border-teal-500/20 text-center">
                      <Cpu size={14} className="mx-auto mb-1 text-teal-600 dark:text-teal-400" />
                      <span className="block text-[10px] font-bold text-teal-700 dark:text-teal-300">NumPy Loaded</span>
                    </div>
                    <div className="p-3 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 text-center">
                      <Activity size={14} className="mx-auto mb-1 text-indigo-600 dark:text-indigo-400" />
                      <span className="block text-[10px] font-bold text-indigo-700 dark:text-indigo-300">SciPy Ready</span>
                    </div>
                    <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20 text-center">
                      <Database size={14} className="mx-auto mb-1 text-amber-600 dark:text-amber-400" />
                      <span className="block text-[10px] font-bold text-amber-700 dark:text-amber-300">SymPy Ready</span>
                    </div>
                  </div>

                  {cells.map((cell) => (
                    <div key={cell.id} className="group bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-md transition-all">
                      <div className="flex items-center justify-between px-4 py-2 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">In [{cell.id}]</span>
                        <div className="flex items-center gap-2 opacity-100 group-hover:opacity-100 transition-opacity">
                          ...
                          >
                            {cell.status === 'running' ? <><span className="w-2 h-2 rounded-full border border-white border-t-transparent animate-spin" /> RUNNING</> : <><Play size={10} fill="currentColor" /> RUN</>}
                          </button>
                          <button 
                      ...
                      {cell.output && (
                        <div className="px-5 py-4 bg-slate-900 border-t border-slate-800 font-mono text-[12px] leading-relaxed">
                          <div className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mb-2">Result [{cell.id}]</div>
                          <pre className={`whitespace-pre-wrap ${cell.status === 'error' ? 'text-rose-400' : 'text-slate-300'}`}>
                            {cell.output}
                          </pre>
                        </div>
                      )}
                ...
                <div className="h-full flex flex-col items-center justify-center space-y-6 text-center">
                   ...
                   <div className="space-y-2">
                     <h3 className="text-xl font-bold text-slate-800 dark:text-white">Setting up Python...</h3>
                     <p className="text-sm text-slate-500 dark:text-slate-400 max-w-xs">
                        Downloading high-performance math libraries. This happens once per session.
                     </p>
                   </div>
                   {runtimeStatus === 'error' && (
                     <div className="p-4 bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-900/30 rounded-2xl text-rose-600 dark:text-rose-400 text-xs font-medium">
                        Network Error: Could not download Pyodide. Check your connection.
                     </div>
                   )}
                </div>
              )}
