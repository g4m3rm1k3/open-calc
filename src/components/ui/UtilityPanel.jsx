...
export default function UtilityPanel() {
  ...
  const [width, setWidth] = useState(() =>
    parseInt(localStorage.getItem(STORAGE_KEY_WIDTH) || "320"),
  );
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex-shrink-0 bg-slate-50/50 dark:bg-white/5">
          <div className="flex items-center gap-1 bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-xl">
            ...
            >
              ...
              {pins.length > 0 && (
                <span className="bg-brand-500 text-white text-[9px] px-1.5 rounded-full">
                  {pins.length}
                </span>
              )}
            </button>
            ...
            >
              ...
              {sortedNotes.length > 0 && (
                <span className="bg-yellow-500 text-white text-[9px] px-1.5 rounded-full">
                  {sortedNotes.length}
                </span>
              )}
            </button>
              ...
              {pins.length === 0 ? (
                <div className="px-6 py-12 text-center">
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Hover over visualizations in lessons and click the pin icon
                    to save them here.
                  </p>
                </div>
              ) : (
                pins.map((pin) => (
                  <div
                    key={pin.id}
                    className="group p-4 hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors relative"
                  >
                    <button
                    ...
                    >
                      ...
                      <div className="text-sm font-bold text-slate-800 dark:text-slate-100 leading-snug group-hover:text-brand-500 transition-colors">
                        {pin.title}
                      </div>
                    </button>
                    <button
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {sortedNotes.length === 0 ? (
                <div className="px-6 py-12 text-center">
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Clip a section of any lesson to keep your thoughts
                    organized.
                  </p>
                </div>
              ) : (
                sortedNotes.map(([noteId, note]) => {
                  ...
                  return (
                    ...
                    >
                      <button
                        onClick={() => handleNoteAction(noteId)}
