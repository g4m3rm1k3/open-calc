      ...
      {phaseOrder.map((phase, idx) => {
        ...
        return (
          <div key={phase} className="flex items-center flex-1 min-w-0">
            <div className={`flex-1 flex items-center gap-1.5 px-3 py-2 rounded-lg transition-all
              ${isActive ? `${meta.bg} ${meta.border} border` : ''}
              ${!isActive && isReached ? 'opacity-60' : ''}
              ${!isActive && !isReached ? 'opacity-30' : ''}
            `}>
              <span className={`flex-shrink-0 text-[9px] font-black uppercase tracking-[0.15em] leading-tight
            ...
            {!isLast && (
              <svg className="flex-shrink-0 w-4 h-4 text-slate-300 dark:text-slate-600 mx-0.5" viewBox="0 0 16 16" fill="currentColor">
...
function PhaseTransition({ phase }) {
  ...
  return (
    <div className={`my-4 px-5 py-3 rounded-xl border flex items-center gap-3 ${meta.dividerBg}`}>
      ...
      <div>
        <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${meta.dividerText}`}>{meta.label}</p>
        <p className={`text-xs font-medium ${meta.dividerText} opacity-80`}>{meta.tagline}</p>
      </div>
    </div>
  )
}
  ...
  return (
    ...
    `}>
      ...
      >
        ...
        {/* Phase badge */}
        <span className={`flex-shrink-0 text-[9px] font-black uppercase tracking-[0.15em] px-1.5 py-0.5 rounded
...
export default function FirstPrinciplesLesson({ discovery }) {
  ...
  for (let idx = 0; idx < steps.length; idx++) {
    ...
    if (step.phase !== lastPhase) {
      renderedItems.push(
        <PhaseTransition key={`phase-${step.phase}-${idx}`} phase={step.phase} />
      )
