...
export default function PinsNotesPopup({ initialTab = 'pins', onClose }) {
  ...
  return (
    <div
      ref={ref}
      className="fixed bottom-11 right-2 z-[1900] w-[340px] max-h-[72vh] flex flex-col rounded-2xl overflow-hidden shadow-2xl border border-black/10 dark:border-white/[0.1] bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl"
    >
      {/* Header */}
      <div className="flex-shrink-0 flex items-center justify-between px-3 py-2.5 border-b border-black/[0.08] dark:border-white/[0.07]">
        <div className="flex gap-1">
          ...
          >
            ...
            Pins
            {pins.length > 0 && (
              <span className={`text-[9px] px-1.5 rounded-full ${tab === 'pins' ? 'bg-white/30 text-white' : 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400'}`}>
                {pins.length}
              </span>
            )}
          </button>
          ...
          >
            ...
            {sortedNotes.length > 0 && (
              <span className={`text-[9px] px-1.5 rounded-full ${tab === 'notes' ? 'bg-white/30 text-white' : 'bg-amber-100 dark:bg-amber-900/50 text-amber-600 dark:text-amber-400'}`}>
                {sortedNotes.length}
              </span>
            )}
          </button>
        ...
        <div className="flex items-center gap-0.5">
          {tab === 'notes' && (
            <>
              <button onClick={exportNotes} title="Export notes" className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                <Download className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => importRef.current?.click()} title="Import notes" className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
        ...
        {tab === 'pins' ? (
          pins.length === 0 ? (
            <div className="py-12 text-center px-6">
              <Pin className="w-6 h-6 text-slate-300 dark:text-slate-600 mx-auto mb-3" />
              <p className="text-xs text-slate-400 dark:text-slate-500">No pins yet. Click the pin icon on any visualization to save it here.</p>
            </div>
          ) : (
            <div className="divide-y divide-black/[0.04] dark:divide-white/[0.05]">
              {pins.map(pin => (
                <div key={pin.id} className="group flex items-start gap-2 p-3 hover:bg-slate-50 dark:hover:bg-white/[0.04] transition-colors">
                  <button onClick={() => handlePinClick(pin)} className="flex-1 text-left min-w-0">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5">{pin.subtitle || 'Visualization'}</p>
                    <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 leading-snug truncate">{pin.title}</p>
                  </button>
                  <button
                    onClick={() => removePin(pin.id)}
                    className="flex-shrink-0 p-1 opacity-0 group-hover:opacity-100 text-slate-300 hover:text-red-500 transition-all"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
          ...
          sortedNotes.length === 0 ? (
            <div className="py-12 text-center px-6">
              <PenLine className="w-6 h-6 text-slate-300 dark:text-slate-600 mx-auto mb-3" />
              <p className="text-xs text-slate-400 dark:text-slate-500">No notes yet. Clip a section of any lesson to save it here.</p>
            </div>
          ) : (
            <div className="divide-y divide-black/[0.04] dark:divide-white/[0.05]">
              {sortedNotes.map(([noteId, note]) => {
                ...
                return (
                  <div key={noteId} className="group flex items-start gap-2 p-3 hover:bg-slate-50 dark:hover:bg-white/[0.04] transition-colors">
                    <button onClick={() => handleNoteClick(noteId)} className="flex-1 text-left min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                        <span style={{ background: dot }} className="w-2 h-2 rounded-full flex-shrink-0" />
                        <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">{section || 'Note'}</span>
                      </div>
                      <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 leading-snug mb-0.5 truncate">{meta?.title || lessonId}</p>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed line-clamp-2">{note.text}</p>
                    </button>
                    <button
