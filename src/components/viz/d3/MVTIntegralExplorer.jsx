...
export default function MVTIntegralExplorer() {
  ...
  // --- Main UI ---
  return (
    <div style={{ fontFamily: 'var(--font-sans)', padding: '4px 0', maxWidth: 800, margin: '0 auto' }}>
      <style>{`@keyframes sd{from{opacity:0;transform:translateY(-5px)}to{opacity:1;transform:translateY(0)}}`}</style>
      <h2 style={{ fontSize: 28, fontWeight: 700, textAlign: 'center', marginBottom: 8 }}>Mean Value Theorem for Integrals</h2>
      ...
      {section === 'explorer' && (
        <div style={{ animation: 'sd .2s ease-out' }}>
          <div style={{ marginBottom: 10, textAlign: 'center', color: '#334155', fontSize: 16 }}>
          ...
          <svg width={W} height={H} style={{ overflow: 'visible', background: '#fff', borderRadius: 8, border: '1px solid #e2e8f0' }}>
            {/* ...existing SVG code... */}
            {/* Gridlines */}
            {xSc.ticks(7).map((t, i) => (
            ...
            />
            <text x={xSc(c)} y={ySc(fn(c)) - 12} textAnchor="middle" fontSize={12} fill="#10b981">c</text>
            {/* Labels */}
            <text x={W - margin.right - 4} y={margin.top + 2} textAnchor="end" fontSize={13} fill="#6470f1" fontWeight="bold">{PRESETS[fnIdx].fnTex}</text>
            <text x={xSc(a)} y={ySc(0) + 18} textAnchor="middle" fontSize={12} fill="#f59e0b" fontWeight="bold">a</text>
            <text x={xSc(b)} y={ySc(0) + 18} textAnchor="middle" fontSize={12} fill="#f59e0b" fontWeight="bold">b</text>
            <text x={(xSc(a) + xSc(b)) / 2} y={ySc(avgValue) - 8} textAnchor="middle" fontSize={13} fill="#f59e0b">Area = f(c)·(b-a)</text>
            <text x={(xSc(a) + xSc(b)) / 2} y={H - 12} textAnchor="middle" fontSize={12} fill="#64748b">
      ...
      {section === 'trap' && (
        <div style={{ animation: 'sd .2s ease-out' }}>
          <div style={{ background: '#FCEBEB', borderLeft: '3px solid #ef4444', borderRadius: 8, padding: 18, marginBottom: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 600, color: '#501313', marginBottom: 8 }}>The trap: confusing total area with average value</div>
            <div style={{ fontSize: 14, color: '#791F1F', marginBottom: 8 }}>
      ...
      {section === 'proof' && (
        <div style={{ animation: 'sd .2s ease-out' }}>
          <div style={{ background: '#eef2ff', borderLeft: '3px solid #6366f1', borderRadius: 8, padding: 18, marginBottom: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 600, color: '#4338ca', marginBottom: 8 }}>Proof Sketch</div>
            <ol style={{ marginLeft: 18, color: '#334155', fontSize: 14, lineHeight: 1.7 }}>
      ...
      {section === 'intuition' && (
        <div style={{ animation: 'sd .2s ease-out' }}>
          <div style={{ background: '#ecfdf5', borderLeft: '3px solid #059669', borderRadius: 8, padding: 18, marginBottom: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 600, color: '#047857', marginBottom: 8 }}>Intuition</div>
            <div style={{ fontSize: 14, color: '#0F6E56', marginBottom: 8 }}>
      ...
      {section === 'real' && (
        <div style={{ animation: 'sd .2s ease-out' }}>
          <div style={{ background: '#e1f5ee', borderLeft: '3px solid #10b981', borderRadius: 8, padding: 18, marginBottom: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 600, color: '#10b981', marginBottom: 8 }}>Real-World Context</div>
            <div style={{ fontSize: 14, color: '#0F6E56', marginBottom: 8 }}>
      ...
      {section === 'examples' && (
        <div style={{ animation: 'sd .2s ease-out' }}>
          <div style={{ background: '#e0f2fe', borderLeft: '3px solid #0ea5e9', borderRadius: 8, padding: 18, marginBottom: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 600, color: '#0ea5e9', marginBottom: 8 }}>Examples</div>
      ...
      <div style={{ background: '#fef9c3', borderLeft: '3px solid #facc15', borderRadius: 8, padding: 18, marginTop: 24 }}>
        <b>Why is this useful?</b> This theorem lets you replace a complicated area with a simple rectangle — making average value calculations and estimates much easier. It's used in physics, engineering, and probability whenever you want to know if a function actually attains its average value.
      </div>
    </div>
  );
}
