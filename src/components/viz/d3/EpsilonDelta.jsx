...
export default function EpsilonDelta({ params }) {
  ...
  const isStepMode = currentStep !== undefined && currentStep !== -1;
  const [interactiveEpsilon, setInteractiveEpsilon] = useState(targetEpsilon)

  const epsilon = isStepMode ? targetEpsilon : interactiveEpsilon;
  ...
  useEffect(() => {
    ...
    // Base properties based on Step Progress
    const showEpsilon = !isStepMode || currentStep >= 1;
    const showIntersections = !isStepMode || currentStep >= 3;
    const showDelta = !isStepMode || currentStep >= 6; // usually the final steps
    ...
    // Intersections (dots)
    if (showIntersections && showEpsilon) {
      if (typeof getDelta === 'string' || getDelta) {
         try {
             const xLeft = c - delta;
             const xRight = c + delta; // This is a tiny simplification, measuring final delta boundary
             svg.append('circle').attr('cx', xSc({c})).attr('cy', ySc(L+epsilon)).attr('r', 5).attr('fill', '#ef4444')
             svg.append('circle').attr('cx', xSc(xRight)).attr('cy', ySc(f(xRight))).attr('r', 5).attr('fill', '#ef4444')
             svg.append('circle').attr('cx', xSc(xLeft)).attr('cy', ySc(f(xLeft))).attr('r', 5).attr('fill', '#ef4444')
         } catch(e) {}
      }
    }
  ...
  }, [fn, c, L, epsilon, delta, currentStep, isStepMode])
    ...
    <div>
      ...
      {!isStepMode && (
         <div className="px-4 mt-2">
           <SliderControl label="ε (epsilon)" min={0.05} max={2} step={0.05} value={interactiveEpsilon} onChange={setInteractiveEpsilon} />
         </div>
      )}
      <p className="text-xs text-center text-slate-500 dark:text-slate-400 mt-2 italic">
        {isStepMode ? "Graph strictly syncs with your math step progress to reveal the hidden geometry of limits." : "Yellow band = ε-tolerance on output. Purple band = δ-tolerance on input. When x is within δ of c (purple), f(x) is within ε of L (yellow)."}
      </p>
    </div>
  )
}
