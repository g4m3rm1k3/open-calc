...
export default function G4_4_CrossSections({ params = {} }) {
  ...
  return (
    <div style={{ fontFamily: "var(--font-sans)", padding: ".5rem 0", maxWidth: 740 }}>
      ...
      <div style={FP}>
        {FH("Story", storyTagStyle, "Cross-sections and Cavalieri's principle")}
        <div style={{ ...FB, fontSize: 14, lineHeight: 1.8, color: "var(--color-text-primary)" }}>
          <p style={{ marginBottom: 12 }}><span style={{ color: isDark ? "#c084fc" : "#6b21a8", fontWeight: 500 }}>Albert</span> sliced through all three solids — sphere, cylinder, cone — at the same height. <em style={{ color: "var(--color-text-secondary)" }}>"Look at the cross-sections. The sphere's disc has area πr²−πz². The cylinder's annulus (ring) minus the cone's disc also gives πr²−πz². They're identical at every height."</em></p>
          <p style={{ marginBottom: 0 }}><span style={{ color: isDark ? "#60a5fa" : "#1e40af", fontWeight: 500 }}>Mic</span>: <em style={{ color: "var(--color-text-secondary)" }}>"So by Cavalieri, sphere volume = cylinder volume minus cone volume."</em> <em style={{ color: "var(--color-text-secondary)" }}>"Exactly. Archimedes knew this 2200 years ago. We're calling it Cavalieri's principle. He called it 'the method of exhaustion.' We call it integration."</em></p>
        </div>
      </div>
      <div style={FP}>
        {FH("Discovery", storyTagStyle, "At every height z: sphere cross-section = cylinder − cone")}
        <div style={FB}>
