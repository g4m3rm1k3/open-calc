...
export default function PLCScanCycleViz() {
  ...
  function computeOIT(iitState) {
    ...
    const conveyor = motorFwd;
    const alarm = !iitState['LIMIT_SW_2'] ? 1 : 0;
    return { 'MOTOR_FWD': motorFwd, 'CONVEYOR': conveyor, 'ALARM_LIGHT': alarm };
  }
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', background: '#0a0f1e', color: '#e2e8f0', borderRadius: 12, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '10px 16px', background: '#111827', borderBottom: '1px solid #1e293b', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
          ...
          <span style={{ fontSize: 10, color: '#475569' }}>scan #{scanCount} · {scanTimeMs}ms/scan</span>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={() => { setMode(m => m === 'auto' ? 'step' : 'auto'); setStepIdx(0); }}
            style={{ padding: '4px 12px', borderRadius: 16, fontSize: 11, fontWeight: 700, cursor: 'pointer', border: '1px solid #334155', background: mode === 'step' ? '#1e293b' : 'transparent', color: '#94a3b8' }}>
            {mode === 'auto' ? '⏩ Auto' : '👆 Step'}
          </button>
          {mode === 'auto' && (
          ...
          {mode === 'step' && (
            <button onClick={stepForward}
              style={{ padding: '4px 16px', borderRadius: 16, fontSize: 11, fontWeight: 700, cursor: 'pointer', border: '1px solid #6366f1', background: '#1e1b4b', color: '#a5b4fc' }}>
              Next Step →
            </button>
          )}
        </div>
      ...
      {/* Phase timeline */}
      <div style={{ display: 'flex', padding: '8px 16px', gap: 4, background: '#111827', borderBottom: '1px solid #1e293b' }}>
        {PHASES.map((p, i) => (
          <div key={p.id} style={{ flex: p.duration, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
            <div style={{
              ...
              background: i < phaseIdx ? p.color : i === phaseIdx ? p.color : '#1e293b',
              opacity: i < phaseIdx ? 0.4 : 1,
              transition: 'background 0.3s',
            }} />
            <span style={{ fontSize: 9, color: i === phaseIdx ? p.color : '#475569', fontWeight: i === phaseIdx ? 700 : 400, textAlign: 'center', whiteSpace: 'nowrap' }}>{p.name}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
        ...
        <div style={{ padding: 16, borderRight: '1px solid #1e293b' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <span style={{ fontSize: 24 }}>{phase.icon}</span>
            <div>
              <div style={{ fontWeight: 700, fontSize: 14, color: phase.color }}>{phase.name}</div>
              <div style={{ fontSize: 10, color: '#475569' }}>{phase.detail}</div>
            </div>
          </div>
          <p style={{ fontSize: 11, color: '#94a3b8', lineHeight: 1.5, marginBottom: 12 }}>{phase.desc}</p>
          <div style={{ background: '#0f172a', borderRadius: 8, padding: 12 }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>Execution Log</div>
            {phase.steps.map((s, i) => (
              <div key={i} style={{
                fontSize: 10, fontFamily: 'monospace', padding: '3px 6px', borderRadius: 4, marginBottom: 3,
                color: mode === 'step' ? (i <= stepIdx ? phase.color : '#334155') : phase.color,
        ...
        <div style={{ padding: 16 }}>
          <div style={{ marginBottom: 14 }}>
            ...
            {INPUT_SENSORS.map(s => (
              <button key={s.name} onClick={() => toggleInput(s.name)}
                ...
                <div style={{ width: 14, height: 14, borderRadius: 3, border: `2px solid ${inputs[s.name] ? '#6366f1' : '#334155'}`, background: inputs[s.name] ? '#6366f1' : 'transparent', flexShrink: 0 }} />
                <span style={{ fontSize: 10, fontFamily: 'monospace', color: '#94a3b8', minWidth: 60 }}>{s.addr}</span>
                <span style={{ fontSize: 11, color: inputs[s.name] ? '#a5b4fc' : '#475569' }}>{s.label}</span>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {INPUT_SENSORS.map(s => {
                ...
                return (
                  <div key={s.name} style={{ background: '#111827', borderRadius: 6, padding: '4px 8px', border: `1px solid ${val ? '#6366f133' : '#1e293b'}` }}>
                    <div style={{ fontSize: 9, fontFamily: 'monospace', color: '#475569' }}>{s.addr}</div>
                    <div style={{ fontFamily: 'monospace', fontSize: 12, fontWeight: 700, color: val ? '#6366f1' : '#334155' }}>{val ? '1' : '0'}</div>
                  </div>
                );
              })}
            </div>
            {phaseIdx >= 1 && <div style={{ fontSize: 9, color: '#475569', marginTop: 4, fontStyle: 'italic' }}>Frozen at scan start — won't change until next input scan</div>}
          </div>
          ...
          <div>
            ...
            {OUTPUT_ACTUATORS.map(a => {
              ...
              const physVal = phaseIdx >= 2 && !!oit[a.name];
              return (
                <div key={a.name} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                  <div style={{ width: 14, height: 14, borderRadius: 3, background: oitVal ? '#f59e0b40' : '#1e293b', border: `1px solid ${oitVal ? '#f59e0b' : '#334155'}`, flexShrink: 0 }} />
                  <span style={{ fontSize: 10, fontFamily: 'monospace', color: '#94a3b8', minWidth: 60 }}>{a.addr}</span>
                  <span style={{ fontSize: 11, color: oitVal ? '#fde68a' : '#475569' }}>{a.label}</span>
                  <div style={{ marginLeft: 'auto', display: 'flex', gap: 6, alignItems: 'center' }}>
                    <span style={{ fontSize: 9, color: '#475569' }}>OIT</span>
                    <span style={{ fontFamily: 'monospace', fontSize: 10, fontWeight: 700, color: oitVal ? '#f59e0b' : '#334155' }}>{oitVal ? '1' : '0'}</span>
                    <span style={{ fontSize: 9, color: '#475569' }}>→ HW</span>
                    <span style={{ fontFamily: 'monospace', fontSize: 10, fontWeight: 700, color: physVal ? '#10b981' : '#334155' }}>{physVal ? '1' : '0'}</span>
                  </div>
                </div>
              );
            })}
            {phaseIdx < 2 && <div style={{ fontSize: 9, color: '#475569', marginTop: 4, fontStyle: 'italic' }}>Physical outputs not updated until Output Scan completes</div>}
          </div>
        </div>
      </div>

      {/* Footer */}
      <div style={{ padding: '8px 16px', background: '#111827', borderTop: '1px solid #1e293b', fontSize: 10, color: '#475569', display: 'flex', gap: 16 }}>
