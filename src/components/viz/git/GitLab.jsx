  ...
  const renderDAG = () => (
    <div className="p-8 space-y-10 animate-in fade-in duration-700">
      ...
      <div className="relative">
        <div className="absolute left-[7px] top-2 bottom-2 w-0.5 bg-blue-50 dark:bg-blue-900/300/10" />
        <div className="space-y-6">
          {git.getHistory().reverse().map((c) => (
            <div key={c.hash} className="pl-8 relative group cursor-pointer" onClick={() => { git.checkout(c.hash); setCorruptionActive(false); setLostWorkWarning(null); forceUpdate() }}>
              <div className={`absolute left-0 top-1.5 w-4 h-4 rounded-full transition-all border-4 shadow-xl ${git.head === c.hash ? 'bg-blue-50 dark:bg-blue-900/300 border-blue-500/20 scale-150 z-10' : 'bg-slate-300 dark:bg-slate-700 border-transparent hover:scale-110'}`} />
              <div className={`p-4 rounded-[2rem] border-2 transition-all ${git.head === c.hash ? 'border-blue-500/40 shadow-2xl scale-[1.03] ring-1 ring-blue-500/10' : 'border-black/5 dark:border-white/5 opacity-40 hover:opacity-100 hover:border-blue-500/10'}`} style={{ background: C.surface, color: C.text }}>
                <div className="flex items-center justify-between mb-2">
                   ...
                   {git.head === c.hash && <div className="text-[8px] font-black bg-blue-50 dark:bg-blue-900/300 text-white px-2 py-0.5 rounded-full uppercase scale-90">Current State</div>}
                </div>
      ...
      {Array.from(git.workingDir.entries()).map(([path, content]) => (
        <div key={path} className="p-5 rounded-[2rem] border-2 transition-all group cursor-pointer hover:border-blue-500/40 shadow-sm hover:shadow-xl" 
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase ${corruptionActive ? 'bg-red-50 dark:bg-red-900/300 text-white' : 'bg-slate-100 dark:bg-slate-800 opacity-40'}`}>
        ...
        <div className="flex items-center gap-8">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
               <div className="w-2 h-2 rounded-full bg-emerald-50 dark:bg-emerald-900/300 animate-pulse" />
      ...
      {mission && (
        <div className="px-12 py-5 bg-brand-500/10 border-b-4 border-brand-500/20 flex items-center justify-between">
           ...
           {scenario === 'corruptable' && !corruptionActive && (
             <button onClick={() => simulateCorruption('story.txt')} className="px-4 py-2 bg-red-600/20 hover:bg-red-600 text-red-600 dark:text-red-400 hover:text-white border-2 border-red-600/20 text-[10px] font-black rounded-xl uppercase transition-all shadow-lg active:scale-95">
                Simulate Accident
             </button>
           )}
        </div>
        ...
        <div className="flex-1 flex flex-col relative overflow-hidden" style={{ background: C.console }}>
          {editingFile ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-20 text-center">
               ...
               {lostWorkWarning && (
                 <div className="mb-12 p-8 bg-red-50 dark:bg-red-900/300/10 border-4 border-red-500/30 rounded-[3rem] max-w-lg mb-10 animate-bounce">
               ...
               {snapshotCount < MAX_SNAPSHOTS ? (
                 <div className="space-y-12">
                   ...
                   <button 
                     ...
                     className="px-16 py-8 bg-white dark:bg-slate-900 hover:bg-brand-500 hover:text-white text-slate-950 text-base font-black uppercase tracking-[0.4em] rounded-[2.5rem] shadow-2xl transform active:scale-95 transition-all outline outline-offset-8 outline-white/10"
                   >
                     Take Snapshot
                   </button>
                 </div>
