...
export const mkInsight = (col = "#6366f1") => ({ padding: "12px 14px", borderLeft: `3px solid ${col}`, background: "var(--color-background-secondary)", borderRadius: "0 8px 8px 0", fontSize: 13, color: "var(--color-text-secondary)", lineHeight: 1.65, margin: "10px 0" });
export const mkBadge = (book, ch) => (<div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "4px 12px", borderRadius: 20, background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", fontSize: 11, fontWeight: 500, color: "var(--color-text-tertiary)", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 16 }}>{`Book ${book} · Chapter ${ch}`}</div>);
...
export function StepBlock({ num, label, tex, note, ready, col = "#6366f1" }) {
  const ref = useRef(null);
