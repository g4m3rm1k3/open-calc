...
const BED_TYPES = {
  ...
  slant:     { label:"Slant Bed",     icon:"◤", desc:"45° / 60° slant bed — improved chip evacuation" },
  slant30:   { label:"30° Slant",     icon:"◤", desc:"30° slant bed — gang tooling common" },
  vertical:  { label:"Vertical",      icon:"▮", desc:"Vertical turning center (VTC/VTL)" },
  inverted:  { label:"Inverted",      icon:"▾", desc:"Inverted vertical spindle — gravity chip fall" },
};

const TURRET_TYPES = {
  disc:      { label:"Disc Turret",   cap:12, desc:"Standard VDI disc turret" },
  drum:      { label:"Drum Turret",   cap:16, desc:"BMT drum turret, high rigidity" },
  gang:      { label:"Gang Tool",     cap:8,  desc:"Gang-style tooling (Swiss/flat bed)" },
...
const buildMachCfg = (opts) => {
  const isLathe = opts.class === "lathe" || opts.class === "millturn" || opts.class === "swiss";
...
const MACHINE_PRESETS = {
  vmc3:     buildMachCfg({ class:"mill",     label:"VMC 3-Axis",   bedType:"flat",   turret:"ats",  yAxis:false, bAxis:false, cAxis:false, liveTools:false, subSpindle:false, turret2:false, maxRPM:12000, maxSpindleRPM:0,    spindleKW:22 }),
  vmc5:     buildMachCfg({ class:"mill",     label:"VMC 5-Axis",   bedType:"flat",   turret:"ats",  yAxis:false, bAxis:true,  cAxis:true,  liveTools:false, subSpindle:false, turret2:false, maxRPM:12000, maxSpindleRPM:0,    spindleKW:22 }),
  lathe2:   buildMachCfg({ class:"lathe",    label:"Lathe 2-Axis", bedType:"slant",  turret:"disc", yAxis:false, bAxis:false, cAxis:false, liveTools:false, subSpindle:false, turret2:false, maxRPM:3000,  maxSpindleRPM:3000, spindleKW:15 }),
  latheY:   buildMachCfg({ class:"lathe",    label:"Lathe Y-Axis", bedType:"slant",  turret:"drum", yAxis:true,  bAxis:false, cAxis:false, liveTools:true,  subSpindle:false, turret2:false, maxRPM:6000,  maxSpindleRPM:6000, spindleKW:18 }),
  latheSub: buildMachCfg({ class:"lathe",    label:"Lathe Sub-Sp", bedType:"slant",  turret:"disc", yAxis:false, bAxis:false, cAxis:false, liveTools:true,  subSpindle:true,  turret2:true,  maxRPM:4000,  maxSpindleRPM:4000, spindleKW:15 }),
  millturn: buildMachCfg({ class:"millturn", label:"Mill-Turn",    bedType:"slant",  turret:"disc", yAxis:true,  bAxis:true,  cAxis:true,  liveTools:true,  subSpindle:false, turret2:false, maxRPM:8000,  maxSpindleRPM:5000, spindleKW:30 }),
  swiss:    buildMachCfg({ class:"swiss",    label:"Swiss Turn",   bedType:"flat",   turret:"gang", yAxis:false, bAxis:false, cAxis:true,  liveTools:true,  subSpindle:true,  turret2:false, maxRPM:12000, maxSpindleRPM:8000, spindleKW:7  }),
  hmc:      buildMachCfg({ class:"mill",     label:"HMC 4-Axis",   bedType:"flat",   turret:"ats",  yAxis:false, bAxis:true,  cAxis:false, liveTools:false, subSpindle:false, turret2:false, maxRPM:12000, maxSpindleRPM:0,    spindleKW:22 }),
};

// Tool classes
const MILL_TOOLS   = ["End Mill","Ball Mill","Bull Nose","Face Mill","Drill","Center Drill","Spot Drill","Reamer","Tap","Chamfer","T-Slot","Probe"];
const LATHE_TOOLS  = ["OD Turning","ID Boring","Facing","Grooving","Parting","Threading","OD Profiling","Knurling"];
...
const initMS = () => ({
  ...
  mpos:{X:0,Y:0,Z:0},
  feed:0, rpm:0, dir:"", coolant:false,
  activeT:1, activeH:1,
  motion:"G00", posMode:"G90", plane:"G17", units:"mm",
  wcs:"G54", cycle:"G80",
  done:false, error:false, msg:"",
  ptr:0,
  offsets:{
  ...
  geoms.forEach(g=>{
    if(lathing){
      if(g.type==="line"&&g.pts?.length>=2){
        ...
        lines.push(`${N()} G01 X${(p1.x*2).toFixed(3)} Z${p1.y.toFixed(3)} F${feed}`);
      }else if(g.type==="arc"){
        const sx=g.cx+g.r*Math.cos(g.a0),sy=g.cy+g.r*Math.sin(g.a0);
        const ex=g.cx+g.r*Math.cos(g.a1),ey=g.cy+g.r*Math.sin(g.a1);
        const I=g.cx-sx,K=g.cy-sy;
        const cmd=g.a1>g.a0?"G02":"G03";
        lines.push(`${N()} G00 X${(sx*2).toFixed(3)} Z${sy.toFixed(3)}`);
        lines.push(`${N()} ${cmd} X${(ex*2).toFixed(3)} Z${ey.toFixed(3)} I${I.toFixed(3)} K${K.toFixed(3)} F${feed}`);
      ...
      if(g.type==="rect"){
        ...
        lines.push(`${N()} G01 Z${(-depth).toFixed(3)} F${Math.round(feed*0.3)}`);
        ...
        if(r>0){
          lines.push(`${N()} G00 X${(g.cx+r).toFixed(3)} Y${g.cy.toFixed(3)}`);
          lines.push(`${N()} G01 Z${(-depth).toFixed(3)} F${Math.round(feed*0.3)}`);
          lines.push(`${N()} G02 I${(-r).toFixed(3)} J0. F${feed}`);
          lines.push(`${N()} G00 Z5.`);
        }
      }else if(g.type==="line"&&g.pts?.length>=2){
        lines.push(`${N()} G00 X${g.pts[0].x.toFixed(3)} Y${g.pts[0].y.toFixed(3)}`);
        lines.push(`${N()} G01 Z${(-depth).toFixed(3)} F${Math.round(feed*0.3)}`);
        for(let i=1;i<g.pts.length;i++)
          lines.push(`${N()} G01 X${g.pts[i].x.toFixed(3)} Y${g.pts[i].y.toFixed(3)} F${feed}`);
        if(g.closed&&g.pts.length>2)lines.push(`${N()} G01 X${g.pts[0].x.toFixed(3)} Y${g.pts[0].y.toFixed(3)}`);
        lines.push(`${N()} G00 Z5.`);
      }else if(g.type==="arc"){
        const sx=g.cx+g.r*Math.cos(g.a0),sy=g.cy+g.r*Math.sin(g.a0);
        const ex=g.cx+g.r*Math.cos(g.a1),ey=g.cy+g.r*Math.sin(g.a1);
        ...
        lines.push(`${N()} G01 Z${(-depth).toFixed(3)} F${Math.round(feed*0.3)}`);
        lines.push(`${N()} G02 X${ex.toFixed(3)} Y${ey.toFixed(3)} I${(g.cx-sx).toFixed(3)} J${(g.cy-sy).toFixed(3)} F${feed}`);
        lines.push(`${N()} G00 Z5.`);
      }
    }
  });
...
const PROG_LIB = {
  mill:[
    {id:"O0001",name:"Square Pocket",desc:"G01 linear, G43 TLO, 3-axis",code:`O0001 (SQUARE POCKET)
...
G00 Z50.
M09 M05 M30`},
    {id:"O0002",name:"Bolt Circle — WHILE",desc:"#vars WHILE DO SIN COS, 8 holes Ø80",code:`O0002 (BOLT CIRCLE)
G21 G90 G17 G40 G49 G80
T2 M06 G43 H2
S2000 M03 M08
#100 = 0
...
G00 Z50.
M05 M30`},
    {id:"O0003",name:"Circular Pocket",desc:"G02 spiral arc pocket",code:`O0003 (CIRCULAR POCKET)
G21 G90 G17
T1 M06 G43 H1
S1800 M03 M08
G00 X0 Y0 Z5.
...
G00 Z50.
M09 M05 M30`},
    {id:"O0004",name:"Subroutine Demo",desc:"M98/M99 drill sub at 4 corners",code:`O0004 (SUBROUTINE DEMO)
G21 G90
T2 M06 G43 H2
S2000 M03
G00 X10. Y10.
...
M99`},
  ...
  lathe:[
    {id:"O1001",name:"OD Turning",desc:"Roughing + finish, G96 CSS, facing",code:`O1001 (OD TURNING)
G21 G90 G18 G40
...
G00 X100. Z50.
M09 M05 M30`},
    {id:"O1002",name:"Taper + Shoulder",desc:"Face, OD taper, shoulder, undercut",code:`O1002 (TAPER AND SHOULDER)
G21 G90 G18 G40
T0404 (FACING INSERT)
G96 S180 M03
G50 S4000
...
G00 Z50.
M09 M05 M30`},
    {id:"O1003",name:"OD Groove + Part-Off",desc:"Multiple grooves then parting",code:`O1003 (GROOVE AND PART)
G21 G90 G18 G40
T0505 (3mm GROOVING)
G97 S800 M03
M08
...
G00 X100. Z100.
M09 M05 M30`},
    {id:"O1004",name:"Threading G76",desc:"M40×1.5 two-pass threading cycle",code:`O1004 (THREADING M40x1.5)
G21 G90 G18 G40
...
G00 X100. Z100.
M09 M05 M30`},
    {id:"O1005",name:"Live Tool Cross-Drill",desc:"C-axis index + live tool drilling",code:`O1005 (LIVE TOOL CROSS DRILL)
G21 G90 G18 G40
...
M05 M09 M30`},
  ...
  millturn:[
    {id:"O2001",name:"Mill-Turn Combo",desc:"OD turn + B-axis mill feature",code:`O2001 (MILL-TURN)
G21 G90 G18 G40
...
export default function CNCSimPro() {
  ...
  const [layers, setLayers] = useState({stock:true,grid:true,tool:true,removal:true});
  ...
  const [coordInput, setCoordInput] = useState({visible:false,x:"0",y:"0",z:"0"});
  ...
  const autoRun = useCallback(()=>{
    if(!playingRef.current||doneRef.current){setIsPlaying(false);return;}
    const blks=blksRef.current;
    if(ptrRef.current>=blks.length){setIsPlaying(false);doneRef.current=true;return;}
    if(speedMode==="max"){
      for(let i=0;i<30&&!doneRef.current&&ptrRef.current<blks.length;i++)step();
      if(!doneRef.current&&playingRef.current)playRef.current=requestAnimationFrame(autoRun);
      else setIsPlaying(false);
    }else{
      step();
      if(doneRef.current||!playingRef.current){setIsPlaying(false);return;}
      if(sbk){setIsPlaying(false);return;}
      const f=msRef.current.feed||200, fovr=feedOvr/100;
      const mul=speedMode==="rt"?1:custSpeed/100;
      const delay=Math.max(1,Math.min(2000,2/(f*fovr/60/1000*mul)));
      playRef.current=setTimeout(autoRun,delay);
    }
  },[speedMode,feedOvr,custSpeed,sbk,step]);

  const toggleCycle=useCallback(()=>{
    if(doneRef.current){reload();return;}
    const n=!playingRef.current;
    playingRef.current=n; setIsPlaying(n);
    if(n)autoRun(); else{clearTimeout(playRef.current);cancelAnimationFrame(playRef.current);}
  },[autoRun,reload]);
  ...
  const draw = useCallback(()=>{
    ...
    if(isLathe){
      ...
      const toS=(z,xr)=>({ // xr = radius (positive = above CL)
        sx: originX + z*sc,
        sy: originY - xr*sc
      });
      ...
      if(layers.stock){
        ...
        const sz=stock.z||0, sx2=stock.x||0;
        ...
        ctx.fillText(`Ø${stock.diameter||80} × ${stock.length||150}mm`,tl.sx+4,tl.sy-4);
      }
      ...
      if(layers.removal&&matRemoval.length>0){
        ...
        if(profile.size>0){
          ...
          const se=toS(lastZ,R),sf=toS(firstZ,R);
          ctx.lineTo(se.sx,se.sy);ctx.lineTo(sf.sx,sf.sy);
          ctx.closePath();ctx.fill();
          // Draw machined profile line
          ctx.strokeStyle=C.rapid; ctx.lineWidth=1.5;
          ctx.beginPath();
          zs.forEach((z,i)=>{
          ...
          ctx.stroke();
          // Mirror
          ctx.strokeStyle="rgba(255,110,46,0.3)"; ctx.lineWidth=1; ctx.setLineDash([3,2]);
          ctx.beginPath();
          zs.forEach((z,i)=>{
            const p=toS(z,-profile.get(z));
            i===0?ctx.moveTo(p.sx,p.sy):ctx.lineTo(p.sx,p.sy);
      ...
      // Axes arrows
      const O=toS(0,0);
      [["Z",60,0,C.red],["X",0,40,C.green]].forEach(([l,dz,dx,col])=>{
        const P=toS(dz,dx);
        ...
        ctx.lineTo(P.sx-8*Math.cos(ang-.4),P.sy-8*Math.sin(ang-.4));
        ctx.lineTo(P.sx-8*Math.cos(ang+.4),P.sy-8*Math.sin(ang+.4));
        ctx.closePath();ctx.fill();
        ctx.font="bold 10px system-ui";ctx.fillText(l,P.sx+4,P.sy-3);
      });
      // Origin cross
      ...
      geoms.forEach((g,gi)=>{
        ...
        ctx.strokeStyle=sel?C.blue:"#4a9eff90"; ctx.lineWidth=sel?2:1.2;
        ctx.fillStyle=sel?"#58a6ff15":"transparent";
        if(g.type==="line"&&g.pts?.length>=2){
          const a=toS(g.pts[0].x,g.pts[0].y),b=toS(g.pts[1].x,g.pts[1].y);
          ctx.beginPath();ctx.moveTo(a.sx,a.sy);ctx.lineTo(b.sx,b.sy);ctx.stroke();
        }else if(g.type==="arc"){
          const c=toS(g.cx,g.cy);
          const sp=toS(g.cx+g.r*Math.cos(g.a0),g.cy+g.r*Math.sin(g.a0));
          const r2d=Math.sqrt((sp.sx-c.sx)**2+(sp.sy-c.sy)**2);
          ctx.beginPath();ctx.arc(c.sx,c.sy,r2d,g.a0,g.a1,g.ccw);ctx.stroke();
        }else if(g.type==="circle"){
          const c=toS(g.cx,g.cy),ep=toS(g.cx+g.r,g.cy);
          const r2d=Math.sqrt((ep.sx-c.sx)**2+(ep.sy-c.sy)**2);
          ctx.beginPath();ctx.arc(c.sx,c.sy,r2d,0,Math.PI*2);ctx.stroke();ctx.fill();
        }
      });
      ...
      if(layers.tool){
        ...
        if(t){
          const tp=toS(msRef.current.pos.Z, Math.abs(msRef.current.pos.X)/2);
          ctx.strokeStyle=C.amber; ctx.fillStyle=C.amber+"30"; ctx.lineWidth=2;
          if(t.type==="OD Turning"||t.type==="OD Profiling"){
            const isz=8*cam.zoom;
            ctx.beginPath();ctx.moveTo(tp.sx,tp.sy);
            ctx.lineTo(tp.sx+isz,tp.sy-isz);ctx.lineTo(tp.sx+isz*2,tp.sy-isz);
            ctx.lineTo(tp.sx+isz*2,tp.sy+isz*0.5);ctx.closePath();
            ctx.fill();ctx.stroke();
            ctx.strokeStyle=C.amber+"60";ctx.lineWidth=1;
            ctx.strokeRect(tp.sx+isz*2,tp.sy-isz,(t.hlen||80)*sc,isz*1.5);
          }else if(t.type==="Grooving"||t.type==="Parting"){
            const w=Math.max(3,t.dia||3)*sc*0.5;
            ctx.fillRect(tp.sx-w/2,tp.sy-12,w,12);ctx.strokeRect(tp.sx-w/2,tp.sy-12,w,12);
            ctx.strokeStyle=C.amber+"60";ctx.strokeRect(tp.sx-8,tp.sy-40,16,28);
          }else if(t.type==="Facing"){
            ctx.beginPath();ctx.moveTo(tp.sx,tp.sy);
            ctx.lineTo(tp.sx+12,tp.sy-8);ctx.lineTo(tp.sx+12,tp.sy-20);
            ctx.lineTo(tp.sx,tp.sy-12);ctx.closePath();ctx.fill();ctx.stroke();
          }else if(t.type==="ID Boring"){
            ctx.beginPath();ctx.moveTo(tp.sx,tp.sy);
            ctx.lineTo(tp.sx-10,tp.sy-8);ctx.lineTo(tp.sx-10,tp.sy-20);
            ctx.lineTo(tp.sx,tp.sy-12);ctx.closePath();ctx.fill();ctx.stroke();
          }else if(t.type?.startsWith("Live")){
            const r=Math.max(3,t.dia/2*sc*0.5);
            ctx.beginPath();ctx.arc(tp.sx,tp.sy,r,0,Math.PI*2);ctx.stroke();ctx.fill();
            ctx.fillStyle=C.amber;ctx.font="7px system-ui";ctx.fillText("LT",tp.sx-5,tp.sy+3);
          }else{
            ctx.beginPath();ctx.arc(tp.sx,tp.sy,6,0,Math.PI*2);ctx.stroke();
          }
        }
      }
      ...
      if(drawTool!=="select"&&snapIndicator){
        ...
        ctx.strokeStyle=C.blue; ctx.lineWidth=1; ctx.setLineDash([3,3]);
        ctx.beginPath();ctx.moveTo(sp.sx-12,sp.sy);ctx.lineTo(sp.sx+12,sp.sy);ctx.stroke();
        ctx.beginPath();ctx.moveTo(sp.sx,sp.sy-12);ctx.lineTo(sp.sx,sp.sy+12);ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle=C.blue; ctx.font="9px monospace";
        ctx.fillText(`Z${snapIndicator.wx.toFixed(2)} X${(snapIndicator.wy*2).toFixed(2)}`,sp.sx+10,sp.sy-5);
      }
      ...
      if(drawActive&&drawPts.length>0){
        ...
        ctx.beginPath();
        drawPts.forEach((p,i)=>{const s=toS(p.x,p.y);i===0?ctx.moveTo(s.sx,s.sy):ctx.lineTo(s.sx,s.sy);});
        ctx.stroke(); ctx.setLineDash([]);
        drawPts.forEach(p=>{const s=toS(p.x,p.y);ctx.strokeStyle=C.blue;ctx.fillStyle=C.blue+"40";ctx.beginPath();ctx.arc(s.sx,s.sy,4,0,Math.PI*2);ctx.fill();ctx.stroke();});
      }
      ...
      if(layers.grid){
        ...
        for(let i=-300;i<=300;i+=25){
          ...
          ctx.beginPath();ctx.moveTo(a.sx,a.sy);ctx.lineTo(b.sx,b.sy);ctx.stroke();
          const c=proj(-300,i,0),d=proj(300,i,0);
      ...
      // Axes
      const O=proj(0,0,0);
      [["X",70,0,0,C.red],["Y",0,70,0,C.green],["Z",0,0,70,C.blue]].forEach(([l,x,y,z,col])=>{
        const P=proj(x,y,z);
        ctx.strokeStyle=col;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(O.sx,O.sy);ctx.lineTo(P.sx,P.sy);ctx.stroke();
        const ang=Math.atan2(P.sy-O.sy,P.sx-O.sx);
        ctx.fillStyle=col;ctx.beginPath();ctx.moveTo(P.sx,P.sy);ctx.lineTo(P.sx-8*Math.cos(ang-.4),P.sy-8*Math.sin(ang-.4));ctx.lineTo(P.sx-8*Math.cos(ang+.4),P.sy-8*Math.sin(ang+.4));ctx.closePath();ctx.fill();
        ctx.font="bold 10px system-ui";ctx.fillText(l,P.sx+4,P.sy-3);
      });
      ctx.strokeStyle=C.green;ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(O.sx,O.sy,4,0,Math.PI*2);ctx.stroke();
      ...
      geoms.forEach((g,gi)=>{
        const sel=gi===selGeom;
        ctx.strokeStyle=sel?C.blue:"#4a9eff90";ctx.lineWidth=sel?2:1.2;ctx.fillStyle=sel?"#58a6ff15":"transparent";
        if(g.type==="line"&&g.pts?.length>=2){
          const a=proj(g.pts[0].x,g.pts[0].y,0),b=proj(g.pts[1].x,g.pts[1].y,0);
          ctx.beginPath();ctx.moveTo(a.sx,a.sy);ctx.lineTo(b.sx,b.sy);ctx.stroke();
          g.pts.slice(2).forEach((pt,i)=>{const np=proj(pt.x,pt.y,0);ctx.lineTo(np.sx,np.sy);});
          ctx.stroke();
        }else if(g.type==="rect"){
          const ps=[proj(g.x,g.y,0),proj(g.x+g.w,g.y,0),proj(g.x+g.w,g.y+g.h,0),proj(g.x,g.y+g.h,0)];
          ctx.beginPath();ctx.moveTo(ps[0].sx,ps[0].sy);ps.slice(1).forEach(p=>ctx.lineTo(p.sx,p.sy));ctx.closePath();ctx.stroke();ctx.fill();
        }else if(g.type==="circle"){
          const c=proj(g.cx,g.cy,0),e=proj(g.cx+g.r,g.cy,0);
          const r2=Math.sqrt((e.sx-c.sx)**2+(e.sy-c.sy)**2);
          ctx.beginPath();ctx.arc(c.sx,c.sy,r2,0,Math.PI*2);ctx.stroke();ctx.fill();
        }else if(g.type==="arc"){
          const c=proj(g.cx,g.cy,0),sp=proj(g.cx+g.r*Math.cos(g.a0),g.cy+g.r*Math.sin(g.a0),0);
          const r2=Math.sqrt((sp.sx-c.sx)**2+(sp.sy-c.sy)**2);
          ctx.beginPath();ctx.arc(c.sx,c.sy,r2,g.a0,g.a1,g.ccw);ctx.stroke();
        }
      });
      ...
      if(layers.tool){
        const t=tools[msRef.current.activeT];
        if(t&&t.cls!=="lathe"){
          const tp=proj(msRef.current.pos.X,msRef.current.pos.Y,msRef.current.pos.Z);
          ...
          ctx.fillStyle=C.amber+"15";ctx.fill();
          const sr=Math.max(1,t.shank/2*sc2);
          const ft=proj(msRef.current.pos.X,msRef.current.pos.Y,msRef.current.pos.Z+(t.lc||22));
          ctx.strokeStyle=C.amber+"60";ctx.lineWidth=1;
          ctx.beginPath();ctx.moveTo(tp.sx-r,tp.sy);ctx.lineTo(ft.sx-sr,ft.sy);ctx.moveTo(tp.sx+r,tp.sy);ctx.lineTo(ft.sx+sr,ft.sy);ctx.stroke();
          if(t.type?.startsWith("Live")){
            ctx.fillStyle=C.teal;ctx.font="7px system-ui";ctx.fillText("LT",tp.sx-5,tp.sy+3);
          }
        }
      }
      ...
      if(drawActive&&drawPts.length>0){
        ctx.strokeStyle=C.blue;ctx.lineWidth=1.5;ctx.setLineDash([5,3]);
        ctx.beginPath();
        drawPts.forEach((p,i)=>{const s=proj(p.x,p.y,0);i===0?ctx.moveTo(s.sx,s.sy):ctx.lineTo(s.sx,s.sy);});
        ctx.stroke();ctx.setLineDash([]);
        drawPts.forEach(p=>{const s=proj(p.x,p.y,0);ctx.strokeStyle=C.blue;ctx.fillStyle=C.blue+"40";ctx.beginPath();ctx.arc(s.sx,s.sy,4,0,Math.PI*2);ctx.fill();ctx.stroke();});
      }
      // Snap crosshair
      if(drawTool!=="select"&&snapIndicator){
        const sp=proj(snapIndicator.wx,snapIndicator.wy,0);
        ctx.strokeStyle=C.blue;ctx.lineWidth=1;ctx.setLineDash([3,3]);
        ctx.beginPath();ctx.moveTo(sp.sx-14,sp.sy);ctx.lineTo(sp.sx+14,sp.sy);ctx.stroke();
        ctx.beginPath();ctx.moveTo(sp.sx,sp.sy-14);ctx.lineTo(sp.sx,sp.sy+14);ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle=C.blue;ctx.font="9px monospace";
        ctx.fillText(`X${snapIndicator.wx.toFixed(2)} Y${snapIndicator.wy.toFixed(2)}`,sp.sx+10,sp.sy-5);
      }
    }
  },[mach,layers,stock,fixtures,geoms,drawPts,drawActive,drawTool,selGeom,snapIndicator,matRemoval,curPt,tools]);

  // ─── Resize observer ───────────────────────────────────────────
  useEffect(()=>{
    const cvs=cvsRef.current,vp=vpRef.current;if(!cvs||!vp)return;
    const ro=new ResizeObserver(()=>{cvs.width=vp.clientWidth;cvs.height=vp.clientHeight;draw();});
    ro.observe(vp);
    cvs.width=vp.clientWidth;cvs.height=vp.clientHeight;
    return()=>ro.disconnect();
  },[draw]);

  useEffect(()=>{draw();},[draw,ms,curPt,geoms,drawPts,drawActive,layers,stock,matRemoval]);

  // ─── Screen → World coord conversion ──────────────────────────
  const s2w = useCallback((sx,sy)=>{
    const cvs=cvsRef.current;if(!cvs)return{x:0,y:0};
    const cam=camRef.current,W=cvs.width,H=cvs.height;
    if(mach.isLathe){
      const sc=cam.zoom*Math.min(W,H)/300;
  ...
  const snapWorld = useCallback((wx,wy)=>{
    ...
    if(snapMode.points){
      ...
      let bestD=15,bestX=sx,bestY=sy; // 15px threshold
      const cvs=cvsRef.current;if(!cvs)return{x:sx,y:sy};
      const cam=camRef.current,W=cvs.width,H=cvs.height;
      const sc=mach.isLathe?cam.zoom*Math.min(W,H)/300:cam.zoom*Math.min(W,H)/280;
      ...
      geoms.forEach(g=>{
        ...
        if(g.cx!=null){check(g.cx,g.cy);check(g.cx+g.r,g.cy);check(g.cx-g.r,g.cy);}
        if(g.x!=null){check(g.x,g.y);check(g.x+g.w,g.y);check(g.x,g.y+g.h);check(g.x+g.w,g.y+g.h);}
      });
      // Also snap to draw points
      drawPts.forEach(p=>{
        const d=Math.sqrt((wx-p.x)**2+(wy-p.y)**2);
        if(d<15/sc){bestX=p.x;bestY=p.y;}
      });
      if(bestX!==sx||bestY!==sy){sx=bestX;sy=bestY;}
    }
    return{x:sx,y:sy};
  ...
  const onMouseMove=useCallback((e)=>{
    ...
    if(d.on){
      ...
      if(d.btn===1){
        if(mach.isLathe){
          camRef.current.px+=dx; camRef.current.py+=dy;
        }else{
          camRef.current.az-=dx*0.007;
          camRef.current.el=Math.max(-1.5,Math.min(1.5,camRef.current.el+dy*0.007));
        }
      }else if(d.btn===2||d.btn===4){
        const W=cvsRef.current?.width||800,H=cvsRef.current?.height||600;
        const sc=mach.isLathe?camRef.current.zoom*Math.min(W,H)/300:camRef.current.zoom*Math.min(W,H)/280;
        camRef.current.px+=dx/sc; camRef.current.py-=dy/sc;
      }
    ...
    if(drawTool!=="select"){
      ...
      setSnapIndicator({wx:snapped.x,wy:snapped.y,sx:e.clientX-rect.left,sy:e.clientY-rect.top});
    }
  ...
  const onMouseDown=useCallback((e)=>{
    ...
    if(drawTool==="line"){
      if(!drawActive){setDrawPts([pt]);setDrawActive(true);}
      else{setGeoms(g=>[...g,{type:"line",pts:[drawPts[0],pt]}]);setDrawPts([]);setDrawActive(false);}
    }else if(drawTool==="rect"){
      if(!drawActive){setDrawPts([pt]);setDrawActive(true);}
      else{
        const p0=drawPts[0];
        setGeoms(g=>[...g,{type:"rect",x:Math.min(p0.x,pt.x),y:Math.min(p0.y,pt.y),w:Math.abs(pt.x-p0.x),h:Math.abs(pt.y-p0.y)}]);
        setDrawPts([]);setDrawActive(false);
      }
    }else if(drawTool==="circle"){
      if(!drawActive){setDrawPts([pt]);setDrawActive(true);}
      else{
        const p0=drawPts[0],r=Math.sqrt((pt.x-p0.x)**2+(pt.y-p0.y)**2);
      ...
      const np=[...drawPts,pt];
      if(np.length===1){setDrawPts(np);setDrawActive(true);}
      else if(np.length===2){setDrawPts(np);}
      else if(np.length===3){
        // 3 points → find circumcircle
        const[p1,p2,p3]=np;
        const ax2=p1.x,ay2=p1.y,bx=p2.x,by=p2.y,cx2=p3.x,cy2=p3.y;
        const D=2*(ax2*(by-cy2)+bx*(cy2-ay2)+cx2*(ay2-by));
        if(Math.abs(D)<0.001){setDrawPts([]);setDrawActive(false);return;}
        const ux=((ax2**2+ay2**2)*(by-cy2)+(bx**2+by**2)*(cy2-ay2)+(cx2**2+cy2**2)*(ay2-by))/D;
        const uy=((ax2**2+ay2**2)*(cx2-bx)+(bx**2+by**2)*(ax2-cx2)+(cx2**2+cy2**2)*(bx-ax2))/D;
        ...
        let ccw=false;
        let da=amid-a0; if(da<0)da+=Math.PI*2;
        let da1=a1-a0; if(da1<0)da1+=Math.PI*2;
        ccw=da<da1;
        setGeoms(g=>[...g,{type:"arc",cx:ux,cy:uy,r,a0,a1,ccw}]);
        setDrawPts([]);setDrawActive(false);
      }else{setDrawPts(np);}
    }else if(drawTool==="contour"){
      const np=[...drawPts,pt];
      setDrawPts(np);setDrawActive(true);
    }
  ...
  const onWheel=useCallback((e)=>{
    e.preventDefault();
    camRef.current.zoom=Math.max(0.05,Math.min(20,camRef.current.zoom*(e.deltaY>0?.88:1.14)));
    draw();
  },[draw]);
  ...
  const onContextMenu=useCallback((e)=>{
    ...
    if(drawTool==="contour"&&drawActive&&drawPts.length>1){
      setGeoms(g=>[...g,{type:"line",pts:[...drawPts],closed:true}]);
    }
    setDrawPts([]);setDrawActive(false);
  },[drawTool,drawActive,drawPts]);

  const setVPView=useCallback((v)=>{
    setVpView(v);camRef.current.px=0;camRef.current.py=0;camRef.current.zoom=1;
    if(v==="top"){camRef.current.az=0;camRef.current.el=Math.PI/2-.01;}
    else if(v==="front"){camRef.current.az=0;camRef.current.el=0;}
    else if(v==="side"){camRef.current.az=Math.PI/2;camRef.current.el=0;}
    else if(v==="lathe"){camRef.current.az=0;camRef.current.el=0;}
    else{camRef.current.az=-.65;camRef.current.el=.48;}
    draw();
  },[draw]);
  ...
  useEffect(()=>{
    const cvs=toolCvsRef.current;if(!cvs)return;
    cvs.width=cvs.parentElement?.clientWidth||240;cvs.height=110;
    const ctx=cvs.getContext("2d");
    ctx.fillStyle=C.bg;ctx.fillRect(0,0,cvs.width,cvs.height);
    const t=editTool,W=cvs.width,H=cvs.height;
    const isLatheTool=["OD Turning","ID Boring","Facing","Grooving","Parting","Threading","OD Profiling","Knurling"].includes(t.type)||t.cls==="lathe";
    ctx.strokeStyle=C.amber;ctx.lineWidth=1.5;ctx.fillStyle=C.amber+"20";
    const cx=W/2,bot=H-10;
    if(isLatheTool){
      if(t.type==="OD Turning"||t.type==="OD Profiling"){
        const s=22;
        ctx.beginPath();ctx.moveTo(cx,bot);ctx.lineTo(cx+s,bot-s);ctx.lineTo(cx,bot-s*1.8);ctx.lineTo(cx-s,bot-s);ctx.closePath();ctx.fill();ctx.stroke();
        ctx.strokeStyle=C.amber+"50";ctx.lineWidth=1;ctx.strokeRect(cx,bot-s*1.8,50,28);
        ctx.fillStyle=C.amber;ctx.font="8px system-ui";ctx.fillText(`CNMG ${t.iAngle||80}° CR${t.cr}`,4,12);
      }else if(t.type==="Grooving"||t.type==="Parting"){
        const w=Math.max(12,(t.dia||3)*2.5);
        ctx.fillRect(cx-w/2,bot-16,w,16);ctx.strokeRect(cx-w/2,bot-16,w,16);
        ctx.strokeStyle=C.amber+"50";ctx.lineWidth=1;ctx.strokeRect(cx-10,bot-44,20,30);
        ctx.fillStyle=C.amber;ctx.font="8px system-ui";ctx.fillText(`Width ${t.dia||3}mm`,4,12);
      }else if(t.type==="Threading"){
        ctx.beginPath();ctx.moveTo(cx-10,bot);ctx.lineTo(cx,bot-18);ctx.lineTo(cx+10,bot);ctx.closePath();ctx.fill();ctx.stroke();
      }else if(t.type==="Facing"){
        ctx.beginPath();ctx.moveTo(cx-5,bot);ctx.lineTo(cx+18,bot-10);ctx.lineTo(cx+18,bot-26);ctx.lineTo(cx-5,bot-16);ctx.closePath();ctx.fill();ctx.stroke();
      }else if(t.type==="ID Boring"){
        ctx.beginPath();ctx.moveTo(cx+5,bot);ctx.lineTo(cx-18,bot-10);ctx.lineTo(cx-18,bot-26);ctx.lineTo(cx+5,bot-16);ctx.closePath();ctx.fill();ctx.stroke();
      }else{ctx.beginPath();ctx.arc(cx,bot-20,15,0,Math.PI*2);ctx.fill();ctx.stroke();}
      if(t.type?.startsWith("Live")){
        ctx.strokeStyle=C.teal;ctx.lineWidth=2;ctx.beginPath();ctx.arc(cx,bot-20,10,0,Math.PI*2);ctx.stroke();
        ctx.fillStyle=C.teal;ctx.font="bold 8px system-ui";ctx.fillText("LIVE",cx-10,bot-16);
      }
    }else{
      // Mill profile
      const totalLen=(t.lt||75)+(t.hlen||50);
      const sc=Math.min((H-16)/totalLen,(W/2-8)/((t.hdia||32)/2));
      const px=d=>cx+d*sc,py=z=>bot-z*sc;
      const r=t.dia/2||5;
      ctx.beginPath();
      ctx.moveTo(px(-r),py(0));
      if(t.cr>0){ctx.lineTo(px(-(r-t.cr)),py(0));ctx.arc(px(-(r-t.cr)),py(t.cr),t.cr*sc,Math.PI/2,Math.PI);}
      ctx.lineTo(px(-r),py(t.lc||22));
      ctx.lineTo(px(-(t.shank/2||5)),py(t.lt||75));
      const hr=(t.hdia||32)/2;
      ctx.lineTo(px(-hr),py(t.lt||75));ctx.lineTo(px(-hr),py((t.lt||75)+(t.hlen||50)));
      ctx.lineTo(px(hr),py((t.lt||75)+(t.hlen||50)));ctx.lineTo(px(hr),py(t.lt||75));
      ctx.lineTo(px(t.shank/2||5),py(t.lt||75));ctx.lineTo(px(r),py(t.lc||22));
      if(t.cr>0){ctx.arc(px(r-t.cr),py(t.cr),t.cr*sc,0,-Math.PI/2,true);}
      ctx.lineTo(px(r),py(0));ctx.closePath();ctx.fill();ctx.stroke();
      ctx.strokeStyle=C.blue+"40";ctx.lineWidth=0.5;ctx.setLineDash([3,3]);
      ctx.beginPath();ctx.moveTo(cx,bot);ctx.lineTo(cx,bot-totalLen*sc-5);ctx.stroke();ctx.setLineDash([]);
      ctx.fillStyle=C.txt3;ctx.font="8px system-ui";ctx.fillText(`Ø${t.dia} TLO:${t.tlo} LC:${t.lc}`,4,10);
    }
  },[editTool]);
  ...
  useEffect(()=>{
    const cvs=stockCvsRef.current;if(!cvs)return;
    cvs.width=cvs.parentElement?.clientWidth||220;cvs.height=76;
    const ctx=cvs.getContext("2d");
    ctx.fillStyle=C.bg;ctx.fillRect(0,0,cvs.width,cvs.height);
    if(stock.shape==="cyl"){
      ...
      ctx.fillStyle=C.blue;ctx.font="bold 8px system-ui";ctx.fillText(`Ø${stock.diameter||80} × ${stock.length||150}mm`,4,10);
    }else{
      const W=stock.width||100,H2=stock.height||80,D=stock.depth||40;
      const sk=.35,sc=Math.min((cvs.width-30)/(W+D*sk),(cvs.height-14)/(H2+D*sk))*.85;
      const ox=10,oy=cvs.height-8;
  ...
  useEffect(()=>{
    const h=e=>{
      ...
      if(e.code==="Space"){e.preventDefault();toggleCycle();}
      else if(e.code==="KeyS")step();
      else if(e.code==="KeyR")resetProg();
      else if(e.code==="Escape"){setDrawPts([]);setDrawActive(false);setDrawTool("select");}
  ...
  const jog=useCallback((dir)=>{
    const d=dir*jogStep;
    setJogPos(prev=>{const n={...prev};n[jogAxis]=+(n[jogAxis]+d).toFixed(4);return n;});
    setMs(m=>({...m,pos:{...m.pos,[jogAxis]:(m.pos[jogAxis]||0)+d}}));
    draw();
  ...
  const visibleTools = useMemo(()=>{
    return Object.entries(tools).filter(([,t])=>{
      if(mach.isLathe&&!mach.isMill) return t.cls==="lathe"||(t.cls==="live"&&mach.liveTools);
      if(!mach.isLathe) return t.cls==="mill"||(t.cls==="live"&&mach.liveTools);
  ...
  return(<>
    ...
    <div className="sim">
      ...
      <div className="topbar">
        <div className="brand">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7" stroke={C.blue} strokeWidth="1.5"/><circle cx="8" cy="8" r="3" fill={C.blue}/></svg>
          CNC·SIM <span style={{fontSize:8,color:C.txt3,letterSpacing:2}}>PRO v4</span>
        </div>
        <div className="tseg">
          <div className={`run-dot${isPlaying?" run":doneRef.current?" done":""}`}/>
        ...
        <div className="tseg">
          ...
          <span className="tlbl" style={{marginLeft:6}}>TLO</span>
          <span className="bdg bdg-bl">H{String(ms.activeH).padStart(2,"0")} +{activeT?.tlo||0}</span>
        </div>
        <div className="tseg">
          <span className="tlbl">S</span>
          <span className="tval" style={{color:C.blue2,minWidth:65}}>{ms.rpm} RPM</span>
          <span className="tlbl" style={{marginLeft:5}}>F</span>
          <span className="tval" style={{color:C.amber2,minWidth:80}}>{ms.feed} {ms.units==="inch"?"ipm":"mm/min"}</span>
        </div>
        <div className="tseg" style={{gap:4}}>
          <span className="bdg bdg-bl">{ms.wcs}</span>
          <span className="bdg bdg-mt">{ms.motion}</span>
          <span className="bdg bdg-mt">{ms.plane}</span>
          <span className="bdg bdg-mt">{ms.units==="inch"?"G20":"G21"}</span>
        ...
        <button
          ...
          title="Exit CNC Sim"
          style={{marginLeft:8,marginRight:8,padding:"4px 12px",background:"transparent",border:`1px solid ${C.bd}`,borderRadius:5,color:C.txt3,fontSize:11,fontWeight:700,letterSpacing:1,cursor:"pointer",flexShrink:0}}
      ...
      <div className="main" style={{gridTemplateColumns:`260px minmax(320px,1fr) 6px ${rightPanelWidth}px`}}>
        ...
        <div className="panel">
          <div className="tabrow">
            {["dro","mach","wcs","jog","vars"].map(t=>(
              <div key={t} className={`tab${leftTab===t?" on":""}`} onClick={()=>setLeftTab(t)}>{t.toUpperCase()}</div>
            ))}
          </div>
          <div className="pscroll">
            ...
            {leftTab==="dro"&&<>
              ...
              {mach.axes.map(ax=>(
                <div className="dro" key={ax}>
                  ...
                  <span className="dro-num" style={{color:ax==="Z"?C.blue2:C.txt}}>{(ms.pos[ax]||0).toFixed(4)}</span>
                  <span className="dro-unit">{mach.rotary.includes(ax)?"°":"mm"}</span>
                </div>
              ))}
              <div className="sec">Machine (G53)</div>
              {["X","Y","Z"].filter(ax=>mach.axes.includes(ax)).map(ax=>(
                <div className="mini" key={ax}><span className="mini-l">M{ax}</span><span className="mini-v">{(ms.mpos[ax]||0).toFixed(4)}</span></div>
              ))}
              {mach.subSpindle&&<div className="mini"><span className="mini-l">SS-Z</span><span className="mini-v">0.0000</span></div>}
              <div className="sec">Spindle / Feed</div>
              <div className="sgrid">
                <div className="sbox"><div className="sbox-l">Spindle</div><div className="sbox-v" style={{color:C.blue2}}>{ms.rpm} RPM</div></div>
                <div className="sbox"><div className="sbox-l">Dir</div><div className="sbox-v" style={{color:ms.dir==="CW"?C.green2:ms.dir==="CCW"?C.amber2:C.txt3}}>{ms.dir||"OFF"}</div></div>
                <div className="sbox"><div className="sbox-l">Feed</div><div className="sbox-v" style={{color:C.amber2}}>{ms.feed}</div></div>
                <div className="sbox"><div className="sbox-l">Coolant</div><div className="sbox-v" style={{color:ms.coolant?C.teal:C.txt3}}>{ms.coolant?"ON":"OFF"}</div></div>
              </div>
              {mach.liveTools&&<>
                <div className="sec">Live Tool Status</div>
                <div className="sgrid">
                  <div className="sbox"><div className="sbox-l">LT RPM</div><div className="sbox-v" style={{color:C.teal}}>—</div></div>
                  <div className="sbox"><div className="sbox-l">LT Dir</div><div className="sbox-v" style={{color:C.txt3}}>—</div></div>
                </div>
              </>}
              <div className="sec">Progress</div>
              <div className="progbar"><div className="progfill" style={{width:`${blocks.length?Math.round(pointer/blocks.length*100):0}%`}}/></div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:C.txt3,marginTop:3}}>
                <span>BLK {pointer} / {blocks.length}</span>
                <span>{blocks.length?Math.round(pointer/blocks.length*100):0}%</span>
              </div>
              <div className="sec">Diagnostics</div>
              <div className="mvar"><span className="mvar-k">Rapid</span><span className="mvar-v">{pStats.rapid}</span></div>
              <div className="mvar"><span className="mvar-k">Linear cut</span><span className="mvar-v">{pStats.cut}</span></div>
              <div className="mvar"><span className="mvar-k">Arc moves</span><span className="mvar-v">{pStats.arc}</span></div>
              <div className="mvar"><span className="mvar-k">Est. time</span><span className="mvar-v">{pStats.time}s</span></div>
              <div className="mvar"><span className="mvar-k">Path length</span><span className="mvar-v">{pStats.dist}mm</span></div>
            </>}
            ...
            {leftTab==="mach"&&<>
              ...
              */}
              ...
              <div className="mcfg-row"><span className="mcfg-lbl">Class</span>
                <select value={customMach.class||"lathe"} onChange={e=>setCustomMach(m=>({...m,class:e.target.value}))} style={{width:110}}>
                  <option value="mill">Mill</option><option value="lathe">Lathe</option>
                  <option value="millturn">Mill-Turn</option><option value="swiss">Swiss</option>
                </select>
              </div>
              <div className="mcfg-row"><span className="mcfg-lbl">Bed Type</span>
                <select value={customMach.bedType||"slant"} onChange={e=>setCustomMach(m=>({...m,bedType:e.target.value}))} style={{width:110}}>
                  {Object.entries(BED_TYPES).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}
                </select>
              </div>
              <div className="mcfg-row"><span className="mcfg-lbl">Turret</span>
                <select value={customMach.turret||"disc"} onChange={e=>setCustomMach(m=>({...m,turret:e.target.value}))} style={{width:110}}>
                  {Object.entries(TURRET_TYPES).map(([k,v])=><option key={k} value={k}>{v.label} ({v.cap})</option>)}
                </select>
              </div>
              {["yAxis","bAxis","cAxis","liveTools","subSpindle","turret2"].map(opt=>(
                <div className="mcfg-row" key={opt}>
                  <span className="mcfg-lbl">{{yAxis:"Y Axis",bAxis:"B Axis",cAxis:"C Axis / Spindle",liveTools:"Live Tooling",subSpindle:"Sub-Spindle",turret2:"2nd Turret"}[opt]}</span>
                  <div className="toggle">
                    <input type="checkbox" checked={!!customMach[opt]} onChange={e=>setCustomMach(m=>({...m,[opt]:e.target.checked}))}/>
                    <span style={{fontSize:9,color:customMach[opt]?C.green2:C.txt3}}>{customMach[opt]?"ON":"OFF"}</span>
                  </div>
                </div>
              ))}
              <div className="frow" style={{marginTop:6}}>
                <div className="field"><div className="lbl">Max RPM</div><input type="number" value={customMach.maxSpindleRPM||3000} onChange={e=>setCustomMach(m=>({...m,maxSpindleRPM:+e.target.value}))}/></div>
                <div className="field"><div className="lbl">kW</div><input type="number" value={customMach.spindleKW||15} onChange={e=>setCustomMach(m=>({...m,spindleKW:+e.target.value}))}/></div>
              </div>
              <div className="field"><div className="lbl">Label</div><input type="text" value={customMach.label||""} onChange={e=>setCustomMach(m=>({...m,label:e.target.value}))}/></div>
              <button className="btn btn-gr full lg" onClick={applyCustomMach} style={{marginTop:6}}>✓ Apply Custom Config</button>
              <div className="div"/>
              <div className="sec">Active Config</div>
              <div style={{fontSize:9,color:C.txt3,lineHeight:1.9,fontFamily:"monospace"}}>
              ...
              <div className="btnrow">
                <button className={`btn${stock.shape==="rect"?" btn-bl":""}`} style={{flex:1}} onClick={()=>setStock(s=>({...s,shape:"rect"}))}>Rect</button>
                <button className={`btn${stock.shape==="cyl"?" btn-bl":""}`} style={{flex:1}} onClick={()=>setStock(s=>({...s,shape:"cyl"}))}>Cylinder</button>
              </div>
              {stock.shape==="rect"?(
                <div className="frow" style={{marginTop:5}}>
                  <div className="field"><div className="lbl">W</div><input type="number" value={stock.width||100} onChange={e=>setStock(s=>({...s,width:+e.target.value}))}/></div>
                  <div className="field"><div className="lbl">H</div><input type="number" value={stock.height||80} onChange={e=>setStock(s=>({...s,height:+e.target.value}))}/></div>
                  <div className="field"><div className="lbl">D</div><input type="number" value={stock.depth||40} onChange={e=>setStock(s=>({...s,depth:+e.target.value}))}/></div>
                </div>
              ):(
                <div className="frow" style={{marginTop:5}}>
                  <div className="field"><div className="lbl">Ø dia</div><input type="number" value={stock.diameter||80} onChange={e=>setStock(s=>({...s,diameter:+e.target.value}))}/></div>
                  <div className="field"><div className="lbl">Length</div><input type="number" value={stock.length||150} onChange={e=>setStock(s=>({...s,length:+e.target.value}))}/></div>
                </div>
              )}
              <div className="frow" style={{marginTop:4}}>
                <div className="field"><div className="lbl">Table X</div><input type="number" value={stock.x||0} step={0.001} onChange={e=>setStock(s=>({...s,x:+e.target.value}))}/></div>
                <div className="field"><div className="lbl">Table Y</div><input type="number" value={stock.y||0} step={0.001} onChange={e=>setStock(s=>({...s,y:+e.target.value}))}/></div>
                <div className="field"><div className="lbl">Table Z</div><input type="number" value={stock.z||0} step={0.001} onChange={e=>setStock(s=>({...s,z:+e.target.value}))}/></div>
              </div>
              <canvas className="stock-cvs" ref={stockCvsRef}/>
              <div className="div"/>
              <div className="sec">Fixtures</div>
              <button className="btn btn-gr full" onClick={()=>setFixtures(p=>[...p,{id:Date.now(),name:`Fix ${p.length+1}`,x:120,y:0,z:0,w:60,h:50,d:25}])}>+ Add Fixture</button>
              {fixtures.map((f,fi)=>(
                <div key={f.id} style={{background:C.bg,border:`1px solid ${C.bd}`,borderRadius:4,padding:"6px 8px",marginTop:4}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                    <span style={{fontWeight:600,fontSize:10}}>{f.name}</span>
                    <button className="btn btn-rd" style={{fontSize:8,padding:"1px 5px"}} onClick={()=>setFixtures(p=>p.filter((_,i)=>i!==fi))}>✕</button>
                  </div>
                  <div className="frow">
                    {["x","y","z"].map(k=><div key={k} className="field"><div className="lbl">{k.toUpperCase()}</div><input type="number" value={f[k]} step={0.001} onChange={e=>setFixtures(p=>p.map((x,i)=>i===fi?{...x,[k]:+e.target.value}:x))}/></div>)}
                  </div>
                  <div className="frow">
                    {["w","h","d"].map(k=><div key={k} className="field"><div className="lbl">{k.toUpperCase()}</div><input type="number" value={f[k]} onChange={e=>setFixtures(p=>p.map((x,i)=>i===fi?{...x,[k]:+e.target.value}:x))}/></div>)}
                  </div>
                </div>
              ))}
            </>}
            ...
            {leftTab==="wcs"&&<>
              ...
              {Object.entries(ms.offsets).map(([k,v])=>(
                <div key={k} className={`wcs-item${ms.wcs===k?" on":""}`} onClick={()=>setMs(m=>({...m,wcs:k}))}>
                  <div>
                    <div style={{fontFamily:"monospace",fontWeight:700,fontSize:10,color:ms.wcs===k?C.blue2:C.txt}}>{k} {ms.wcs===k&&<span style={{fontSize:7,background:C.blue,color:"#000",padding:"1px 3px",borderRadius:1}}>ACT</span>}</div>
                    <div style={{display:"flex",gap:8,marginTop:2}}>
                      {["X","Y","Z",...mach.rotary].map(ax=><span key={ax} style={{fontSize:8,color:C.txt3,fontFamily:"monospace"}}>{ax}:{(v[ax]||0).toFixed(2)}</span>)}
                    </div>
                  </div>
                </div>
              ))}
              ...
              {["X","Y","Z",...mach.rotary].map(ax=>(
                <div className="frow" key={ax} style={{marginBottom:4,alignItems:"center"}}>
                  <span style={{width:16,fontWeight:700,fontSize:11,color:axisColor(ax),flexShrink:0}}>{ax}</span>
                  <input type="number" step={0.001} value={(ms.offsets[ms.wcs]?.[ax]||0).toFixed(3)}
                    onChange={e=>setMs(m=>({...m,offsets:{...m.offsets,[m.wcs]:{...m.offsets[m.wcs],[ax]:parseFloat(e.target.value)||0}}}))}/>
                  <button className="btn" style={{padding:"4px 6px",fontSize:9,flexShrink:0,marginLeft:3}} onClick={()=>setMs(m=>({...m,offsets:{...m.offsets,[m.wcs]:{...m.offsets[m.wcs],[ax]:0}}}))}>0</button>
                </div>
              ))}
              <button className="btn btn-am full" style={{marginTop:4}} onClick={()=>setMs(m=>({...m,offsets:{...m.offsets,[m.wcs]:{X:0,Y:0,Z:0,B:0,C:0}}}))}>Zero All</button>
              <div className="div"/>
              <div className="sec">Machine Home</div>
              {mach.axes.map(ax=>(
                <div className="frow" key={ax} style={{marginBottom:3,alignItems:"center"}}>
                  <span style={{width:16,fontWeight:700,fontSize:11,flexShrink:0}}>{ax}</span>
                  <input type="number" step={0.001} value={ms.home[ax]||0} onChange={e=>setMs(m=>({...m,home:{...m.home,[ax]:parseFloat(e.target.value)||0}}))}/>
                </div>
              ))}
              <button className="btn btn-gr full" style={{marginTop:4}} onClick={()=>setMs(m=>({...m,pos:{...m.pos,...m.home}}))}>Go to Home</button>
            </>}
            ...
            {leftTab==="jog"&&<>
              ...
              <div style={{display:"flex",gap:3,marginBottom:8,flexWrap:"wrap"}}>
                {mach.axes.map(ax=><button key={ax} className={`btn${jogAxis===ax?" btn-bl":""}`} style={{flex:1,minWidth:32}} onClick={()=>setJogAxis(ax)}>{ax}</button>)}
              </div>
              <div className="lbl">Increment</div>
              <div style={{display:"flex",gap:3,marginBottom:8,flexWrap:"wrap"}}>
                {[0.001,0.01,0.1,1,10].map(s=><button key={s} className={`btn${jogStep===s?" btn-bl":""}`} style={{flex:1,minWidth:36,fontSize:9}} onClick={()=>setJogStep(s)}>{s}</button>)}
              </div>
              <div style={{fontSize:9,color:C.txt3,marginBottom:8}}>Step: <span style={{color:C.txt,fontFamily:"monospace"}}>{jogStep}mm</span> | Axis: <span style={{color:C.blue2,fontFamily:"monospace"}}>{jogAxis}</span></div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
                <button className="btn btn-rd lg" style={{padding:10,textAlign:"center"}} onClick={()=>jog(-1)}>− {jogAxis}</button>
                <button className="btn btn-gr lg" style={{padding:10,textAlign:"center"}} onClick={()=>jog(+1)}>+ {jogAxis}</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:3,marginBottom:10}}>
                {["X","Y","Z"].filter(a=>mach.axes.includes(a)).map(ax=><div key={ax} className="mini"><span className="mini-l">{ax}</span><span className="mini-v">{(jogPos[ax]||0).toFixed(3)}</span></div>)}
              </div>
              <div className="div"/>
              <div className="sec">Feed Override: {feedOvr}%</div>
              <input type="range" min={0} max={200} value={feedOvr} onChange={e=>setFeedOvr(+e.target.value)} style={{width:"100%"}}/>
              <div className="sec">Spindle Override: {rpmOvr}%</div>
              <input type="range" min={50} max={150} value={rpmOvr} onChange={e=>setRpmOvr(+e.target.value)} style={{width:"100%"}}/>
              <div className="div"/>
              <button className="btn btn-am full lg" onClick={()=>setMs(m=>({...m,pos:{...m.home}}))}>⌂ Reference All Axes</button>
            </>}

            {leftTab==="vars"&&<>
              <div className="sec">Macro Variables</div>
              {Object.entries(ms.vars||{}).length?Object.entries(ms.vars).map(([k,v])=><div key={k} className="mvar"><span className="mvar-k">#{k}</span><span className="mvar-v">{typeof v==="number"?v.toFixed(4):v}</span></div>)
                :<div style={{color:C.txt3,fontSize:9}}>No variables set</div>}
              <div className="div"/>
              <div className="sec">System Variables</div>
              {mach.axes.map((ax,i)=><div key={ax} className="mvar"><span className="mvar-k">#{5041+i} {ax}</span><span className="mvar-v">{(ms.pos[ax]||0).toFixed(4)}</span></div>)}
              <div className="mvar"><span className="mvar-k">#_DATE</span><span className="mvar-v">{new Date().toLocaleDateString()}</span></div>
              <div className="mvar"><span className="mvar-k">#_TIME</span><span className="mvar-v">{new Date().toLocaleTimeString()}</span></div>
            </>}
          </div>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          <div className="ctrlbar">
            ...
            <button className={`btn${sbk?" btn-bl":""}`} onClick={()=>setSbk(s=>!s)}>SBK {sbk?"ON":"OFF"}</button>
            <button className="btn" onClick={resetProg}>↺ Reset</button>
            <div className="ctrl-div"/>
            <span style={{fontSize:9,color:C.txt3}}>Speed</span>
            {["max","rt","custom"].map(m=><button key={m} className={`btn${speedMode===m?" btn-bl":""}`} style={{fontSize:9,padding:"3px 7px"}} onClick={()=>setSpeedMode(m)}>{m==="max"?"MAX":m==="rt"?"REAL-TIME":"CUSTOM"}</button>)}
            {speedMode==="custom"&&<><input type="range" min={1} max={5000} value={custSpeed} onChange={e=>setCustSpeed(+e.target.value)} style={{width:70}}/><span style={{fontSize:9,color:C.blue2,minWidth:36}}>{custSpeed}×</span></>}
            <div className="ctrl-div"/>
            <div style={{flex:1}}/>
            {mach.isLathe
              ?<><button className="vp-btn" onClick={()=>setVPView("lathe")}>ZX</button><button className="vp-btn" onClick={()=>setVPView("top")}>Top-ZX</button></>
              :["top","front","side","iso"].map(v=><button key={v} className="vp-btn" onClick={()=>setVPView(v)}>{v.toUpperCase()}</button>)
            }
            <div className="ctrl-div"/>
            {Object.entries(layers).map(([l,on])=>(
              <button key={l} className={`vp-btn${on?" on":""}`} onClick={()=>setLayers(v=>({...v,[l]:!v[l]}))}>{l==="removal"?"Cut":l.charAt(0).toUpperCase()+l.slice(1)}</button>
            ))}
          </div>
          ...
          <div id="vpWrap" ref={vpRef}>
            ...
            <div className="vp-hud">
              ...
              {mach.isLathe&&<><br/>Left-drag: pan · Scroll: zoom · Right-click to finish contour</>}
              {!mach.isLathe&&<><br/>Left-drag: rotate · Right-drag: pan · Scroll: zoom</>}
            </div>
            <div className="vp-legend">
              <span className="leg" style={{background:"#ff6e2e15",color:C.rapid,borderColor:"#ff6e2e30"}}>Rapid</span>
              <span className="leg" style={{background:"#3fb95015",color:C.feed,borderColor:"#3fb95030"}}>Feed</span>
              <span className="leg" style={{background:"#bc8cff15",color:C.arc,borderColor:"#bc8cff30"}}>Arc</span>
              {mach.isLathe&&layers.removal&&<span className="leg" style={{background:"#ff6e2e10",color:C.rapid,borderColor:"#ff6e2e20"}}>Material Removed</span>}
            </div>
          </div>
        ...
        <div className="panel panel-r" style={{minWidth:0}}>
          ...
          {rightTab==="tools"&&<div className="pscroll">
            <div className="sec">Tool Table ({mach.isLathe&&!mach.isMill?"Lathe":"Mill"}{mach.liveTools?" + Live":""})</div>
            {visibleTools.map(([n,t])=>(
              <div key={n} className={`tcard${ms.activeT==n?" on":""}`} onClick={()=>setMs(m=>({...m,activeT:+n,activeH:+n}))}>
                <div className="tcard-h">
                  <span className="tcard-name">T{String(n).padStart(2,"0")} — {t.type}</span>
                  {ms.activeT==n&&<span style={{fontSize:7,background:C.amber,color:"#000",borderRadius:2,padding:"1px 4px",fontWeight:700}}>ACTIVE</span>}
                  {t.cls==="live"&&<span style={{fontSize:7,background:C.teal,color:"#000",borderRadius:2,padding:"1px 4px",fontWeight:700,marginLeft:3}}>LIVE</span>}
                  <button style={{background:"none",border:"none",color:C.txt3,cursor:"pointer",marginLeft:"auto"}} onClick={e=>{e.stopPropagation();setTools(prev=>{const next={...prev};delete next[n];return next;})}}>✕</button>
                </div>
                <div className="tcard-meta">{t.desc} {t.dia?`Ø${t.dia}`:""} TLO:{t.tlo} {t.mat}</div>
              </div>
            ))}
            {visibleTools.length===0&&<div style={{color:C.txt3,fontSize:9,padding:"8px 0"}}>No {mach.isLathe?"lathe":"mill"} tools defined. Add tools below.</div>}
            <div className="div"/>
            <div className="sec">Add / Edit Tool</div>
            <div className="frow">
              <div className="field"><div className="lbl">T#</div><input type="number" value={editTool.n} onChange={e=>setEditTool(t=>({...t,n:+e.target.value}))} style={{width:55}}/></div>
              <div className="field"><div className="lbl">Class</div>
                <select value={editTool.cls||"lathe"} onChange={e=>setEditTool(t=>({...t,cls:e.target.value,type:e.target.value==="mill"?MILL_TOOLS[0]:e.target.value==="live"?LIVE_TOOLS[0]:LATHE_TOOLS[0]}))}>
                  <option value="mill">Mill</option><option value="lathe">Lathe</option><option value="live">Live Tool</option>
                </select>
              </div>
            </div>
            <div className="field"><div className="lbl">Type</div>
              <select value={editTool.type} onChange={e=>setEditTool(t=>({...t,type:e.target.value}))}>
                {(editTool.cls==="mill"?MILL_TOOLS:editTool.cls==="live"?LIVE_TOOLS:LATHE_TOOLS).map(t=><option key={t}>{t}</option>)}
              </select>
            </div>
            <div className="field"><div className="lbl">Description</div><input type="text" value={editTool.desc} onChange={e=>setEditTool(t=>({...t,desc:e.target.value}))}/></div>
            {editTool.cls==="mill"||editTool.cls==="live"?(<>
              <div className="frow"><div className="field"><div className="lbl">Ø dia</div><input type="number" value={editTool.dia} step={0.001} onChange={e=>setEditTool(t=>({...t,dia:+e.target.value}))}/></div><div className="field"><div className="lbl">Corner R</div><input type="number" value={editTool.cr} step={0.001} onChange={e=>setEditTool(t=>({...t,cr:+e.target.value}))}/></div></div>
              <div className="frow"><div className="field"><div className="lbl">TLO (H)</div><input type="number" value={editTool.tlo} step={0.001} onChange={e=>setEditTool(t=>({...t,tlo:+e.target.value}))}/></div><div className="field"><div className="lbl">Flute Len</div><input type="number" value={editTool.lc} step={0.001} onChange={e=>setEditTool(t=>({...t,lc:+e.target.value}))}/></div></div>
              <div className="frow"><div className="field"><div className="lbl">Total Len</div><input type="number" value={editTool.lt} step={0.001} onChange={e=>setEditTool(t=>({...t,lt:+e.target.value}))}/></div><div className="field"><div className="lbl">Shank Ø</div><input type="number" value={editTool.shank} step={0.001} onChange={e=>setEditTool(t=>({...t,shank:+e.target.value}))}/></div></div>
              <div className="frow"><div className="field"><div className="lbl">Holder Ø</div><input type="number" value={editTool.hdia} step={0.001} onChange={e=>setEditTool(t=>({...t,hdia:+e.target.value}))}/></div><div className="field"><div className="lbl">Holder Len</div><input type="number" value={editTool.hlen} step={0.001} onChange={e=>setEditTool(t=>({...t,hlen:+e.target.value}))}/></div></div>
            </>):(<>
              <div className="frow"><div className="field"><div className="lbl">Insert Angle°</div><input type="number" value={editTool.iAngle||80} onChange={e=>setEditTool(t=>({...t,iAngle:+e.target.value}))}/></div><div className="field"><div className="lbl">Corner R</div><input type="number" value={editTool.cr} step={0.001} onChange={e=>setEditTool(t=>({...t,cr:+e.target.value}))}/></div></div>
              <div className="frow"><div className="field"><div className="lbl">TLO</div><input type="number" value={editTool.tlo} step={0.001} onChange={e=>setEditTool(t=>({...t,tlo:+e.target.value}))}/></div><div className="field"><div className="lbl">Shank W</div><input type="number" value={editTool.shank} step={0.001} onChange={e=>setEditTool(t=>({...t,shank:+e.target.value}))}/></div></div>
              {(editTool.type==="Grooving"||editTool.type==="Parting")&&<div className="field"><div className="lbl">Groove Width</div><input type="number" value={editTool.dia} step={0.001} onChange={e=>setEditTool(t=>({...t,dia:+e.target.value}))}/></div>}
            </>)}
            <canvas className="tool-cvs" ref={toolCvsRef}/>
            <button className="btn btn-gr full" onClick={()=>setTools(prev=>({...prev,[editTool.n]:{...editTool}}))}>⊕ Save Tool</button>
          </div>}

          {/* DRAW */}
          {rightTab==="draw"&&<>
            <div className="draw-tb">
              {["select","line","rect","circle","arc","contour"].map(t=>(
                <button key={t} className={`dbtn${drawTool===t?" on":""}`} onClick={()=>{setDrawTool(t);setDrawPts([]);setDrawActive(false);}}>{t}</button>
              ))}
              <div style={{marginLeft:"auto",display:"flex",gap:3}}>
                <button className={`dbtn${snapMode.grid?" on":""}`} onClick={()=>setSnapMode(s=>({...s,grid:!s.grid}))}>SNAP-G</button>
                <button className={`dbtn${snapMode.points?" on":""}`} onClick={()=>setSnapMode(s=>({...s,points:!s.points}))}>SNAP-P</button>
                <button className="dbtn" style={{color:C.red2,borderColor:C.red+"30"}} onClick={()=>setGeoms([])}>CLR</button>
              </div>
            </div>
            <div className="pscroll">
              <div style={{background:C.blueBg,border:`1px solid ${C.blue}30`,borderRadius:4,padding:8,marginBottom:8,fontSize:9,color:C.txt3,lineHeight:1.8}}>
                <b style={{color:C.blue2}}>Draw in the viewport:</b><br/>
                <b>Line:</b> click start → click end<br/>
                <b>Rect:</b> click corner → click opposite corner<br/>
                <b>Circle:</b> click center → click edge (sets radius)<br/>
                <b>Arc:</b> click 3 points — circumscribed arc<br/>
                <b>Contour:</b> click points → right-click to finish<br/>
                Snap: Grid=10mm, Points=existing geometry<br/>
                Esc = cancel current draw
              </div>
              <div className="sec">Coordinate Entry (precise input)</div>
              <div style={{background:C.bg,border:`1px solid ${C.bd}`,borderRadius:4,padding:8,marginBottom:8}}>
                <div className="frow" style={{marginBottom:4}}>
                  <div className="field"><div className="lbl">{mach.isLathe?"Z":"X"}</div><input type="number" id="ci-x" step={0.001} defaultValue={0}/></div>
                  <div className="field"><div className="lbl">{mach.isLathe?"X(dia)":"Y"}</div><input type="number" id="ci-y" step={0.001} defaultValue={0}/></div>
                  {!mach.isLathe&&<div className="field"><div className="lbl">Z</div><input type="number" id="ci-z" step={0.001} defaultValue={0}/></div>}
                </div>
                <button className="btn btn-bl full" onClick={()=>{
                  const x=parseFloat(document.getElementById("ci-x")?.value)||0;
                  const y=parseFloat(document.getElementById("ci-y")?.value)||0;
                  ...
                  if(drawTool==="line"){
                    ...
                    if(!drawActive){setDrawPts([pt]);setDrawActive(true);}
                    else{const r=Math.sqrt((pt.x-drawPts[0].x)**2+(pt.y-drawPts[0].y)**2);setGeoms(g=>[...g,{type:"circle",cx:drawPts[0].x,cy:drawPts[0].y,r}]);setDrawPts([]);setDrawActive(false);}
                  }else if(drawTool==="arc"){
                    const np=[...drawPts,pt];
                    if(np.length<3){setDrawPts(np);setDrawActive(true);}
                    else{
                      const[p1,p2,p3]=np;
                      ...
                      if(Math.abs(D)>0.001){
                        ...
                        const a0=Math.atan2(p1.y-uy,p1.x-ux),a1=Math.atan2(p3.y-uy,p3.x-ux);
                        const amid=Math.atan2(p2.y-uy,p2.x-ux);
                        let da=amid-a0;if(da<0)da+=Math.PI*2;
                        let da1=a1-a0;if(da1<0)da1+=Math.PI*2;
                        setGeoms(g=>[...g,{type:"arc",cx:ux,cy:uy,r,a0,a1,ccw:da<da1}]);
                      }
                      setDrawPts([]);setDrawActive(false);
                    }
                  }else if(drawTool==="rect"){
                    if(!drawActive){setDrawPts([pt]);setDrawActive(true);}
                    else{const p0=drawPts[0];setGeoms(g=>[...g,{type:"rect",x:Math.min(p0.x,pt.x),y:Math.min(p0.y,pt.y),w:Math.abs(pt.x-p0.x),h:Math.abs(pt.y-p0.y)}]);setDrawPts([]);setDrawActive(false);}
                  }else if(drawTool==="contour"){setDrawPts(p=>[...p,pt]);setDrawActive(true);}
                }}>+ Add Point ({drawActive?`${drawPts.length} placed`:"click to start"})</button>
                {drawActive&&<button className="btn full" style={{marginTop:3,fontSize:9}} onClick={()=>{setDrawPts([]);setDrawActive(false);}}>Cancel</button>}
              </div>
              <div className="sec">Toolpath Generation</div>
              <div className="frow">
                <div className="field"><div className="lbl">Depth</div><input type="number" value={geomDepth} step={0.1} onChange={e=>setGeomDepth(+e.target.value)}/></div>
                <div className="field"><div className="lbl">Feed</div><input type="number" value={geomFeed} onChange={e=>setGeomFeed(+e.target.value)}/></div>
              </div>
              <button className="btn btn-gr full lg" onClick={genFromGeom} style={{marginBottom:8}}>⚡ Generate G-Code</button>
              <div className="sec">Objects ({geoms.length})</div>
              {geoms.length===0&&<div style={{color:C.txt3,fontSize:9}}>No geometry yet. Select a draw tool and click in the viewport, or use coordinate entry above.</div>}
              {geoms.map((g,i)=>(
                <div key={i} className={`geom-item${selGeom===i?" on":""}`} onClick={()=>setSelGeom(selGeom===i?null:i)}>
                  <span style={{color:selGeom===i?C.blue2:C.txt,fontWeight:600}}>
                    {g.type} {g.type==="rect"?`${g.w?.toFixed(1)}×${g.h?.toFixed(1)}`:g.type==="circle"?`R${g.r?.toFixed(1)}`:g.type==="arc"?`R${g.r?.toFixed(1)} arc`:g.pts?`${g.pts.length}pts`:""}
                  </span>
                  <button style={{background:"none",border:"none",color:C.txt3,cursor:"pointer",fontSize:11}} onClick={e=>{e.stopPropagation();setGeoms(p=>p.filter((_,j)=>j!==i));if(selGeom===i)setSelGeom(null);}}>✕</button>
                </div>
              ))}
            </div>
          </>}
          ...
          {rightTab==="code"&&(
            <div style={{display:"flex",flexDirection:"column",overflow:"hidden",flex:1}}>
              <div style={{padding:"5px 8px",borderBottom:`1px solid ${C.bd}`,display:"flex",gap:4,alignItems:"center",flexShrink:0}}>
                <button className="btn btn-bl" style={{fontSize:9}} onClick={()=>reload(code)}>⟳ Parse</button>
                <button className="btn" style={{fontSize:9}} onClick={()=>document.getElementById("ncup").click()}>↑ Upload</button>
                <button className="btn" style={{fontSize:9}} onClick={()=>{const b=new Blob([code],{type:"text/plain"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="program.nc";a.click();}}>↓ DL</button>
                <input type="file" id="ncup" accept=".nc,.txt,.cnc,.mpf,.min,.tap" style={{display:"none"}} onChange={e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=ev=>{setCode(ev.target.result);reload(ev.target.result);};r.readAsText(f);e.target.value="";}}/>
                <div style={{flex:1}}/>
                <span style={{fontSize:9,color:C.txt3}}>{blocks.length} blks</span>
              </div>
              <div style={{padding:"4px 8px",borderBottom:`1px solid ${C.bd}`,display:"flex",flexWrap:"wrap",gap:3,flexShrink:0}}>
                {(mach.isLathe?[["G00","G00 X85. Z2."],["G01","G01 Z-80. F0.2"],["G96","G96 S200 M03"],["G97","G97 S800 M03"],["G76","G76 P010060 Q0.1 R0.05"],["T+M","T0303"],["M08","M08"]]
                  :[["G00","G00 X0 Y0"],["G01","G01 X0 F150"],["G02","G02 X0 Y0 R10"],["G03","G03 X0 Y0 R10"],["G81","G81 Z-10. R3. F80"],["T+M","T1 M06\nG43 H1"],["M03","M03 S1500"],["M98","M98 P9001"]]
                ).map(([l,s])=>(
                  <button key={l} style={{background:C.p3,border:`1px solid ${C.bd}`,color:C.txt3,borderRadius:3,padding:"2px 5px",fontSize:8,cursor:"pointer"}}
                    onClick={()=>{const ta=document.getElementById("ced");if(!ta)return;const p=ta.selectionStart;const nv=ta.value.slice(0,p)+"\n"+s+ta.value.slice(p);setCode(nv);ta.selectionStart=ta.selectionEnd=p+s.length+1;ta.focus();}}>{l}</button>
                ))}
              </div>
          ...
          {/* PROGRAMS */}
          {rightTab==="progs"&&<div className="pscroll">
            <div className="sec">Library — {mach.isLathe?mach.liveTools?"Lathe + Live":"Lathe":"Mill"}</div>
            {progLib.map(p=>(
              <div key={p.id} className="tcard" onClick={()=>{setCode(p.code);reload(p.code);setRightTab("code");}}>
                <div className="tcard-h"><span style={{color:C.blue2,fontWeight:700,fontSize:10}}>{p.id}</span><span style={{fontSize:8,color:C.green2}}>Load ▸</span></div>
                <div style={{fontSize:10,fontWeight:600,marginBottom:2}}>{p.name}</div>
            ...
            <div className="frow" style={{marginBottom:6}}>
              ...
              <button className="btn btn-gr" onClick={()=>{const n=document.getElementById("svn")?.value||`Prog ${Date.now()}`;const next=[...savedProgs,{name:n,code,machClass:mach.class,t:Date.now()}];setSavedProgs(next);try{localStorage.setItem("csv4",JSON.stringify(next));}catch{}document.getElementById("svn").value="";}}>Save</button>
            </div>
            {savedProgs.map((p,i)=>(
              <div key={i} className="tcard" style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div><div style={{fontWeight:600,fontSize:10}}>{p.name}</div><div className="tcard-meta">{p.machClass||""} {new Date(p.t).toLocaleDateString()}</div></div>
                <div style={{display:"flex",gap:3}}>
                  <button className="btn btn-gr" style={{fontSize:8,padding:"2px 5px"}} onClick={()=>{setCode(p.code);reload(p.code);}}>Load</button>
                  <button className="btn btn-rd" style={{fontSize:8,padding:"2px 5px"}} onClick={()=>{const next=savedProgs.filter((_,j)=>j!==i);setSavedProgs(next);try{localStorage.setItem("csv4",JSON.stringify(next));}catch{};}}>✕</button>
                </div>
              </div>
            ))}
          </div>}
          ...
          {/* SETUP */}
          {rightTab==="setup"&&<div className="pscroll">
            <div className="sec">Save / Export Setup</div>
            <div style={{fontSize:10,color:C.txt3,lineHeight:1.7,marginBottom:8}}>Exports: program, tools, WCS, stock, fixtures, geometry, machine config, saved programs.</div>
            <button className="btn btn-gr full lg" onClick={exportSetup} style={{marginBottom:6}}>↓ Export .cncsetup</button>
            <div className="div"/>
            <div className="sec">Load Setup</div>
            <div className="filedrop" onClick={()=>document.getElementById("supload").click()}>
              ↑ Drop or click to load .cncsetup
              <input type="file" id="supload" accept=".cncsetup,.json" style={{display:"none"}} onChange={e=>{importSetup(e.target.files[0]);e.target.value="";}}/>
            </div>
            <div className="div"/>
            <div className="sec">STL / Model Import</div>
            <div className="filedrop" onClick={()=>document.getElementById("stlup").click()}>
              ↑ Import STL / DXF / STEP<br/>
              <span style={{fontSize:8,color:C.txt3}}>(Future: edge detection for toolpath generation)</span>
              <input type="file" id="stlup" accept=".stl,.dxf,.step,.stp" style={{display:"none"}} onChange={e=>{const f=e.target.files[0];if(!f)return;alert(`Loaded: ${f.name} (${(f.size/1024).toFixed(1)}KB)\n\nSTL edge extraction coming in next module.`);e.target.value="";}}/>
            </div>
            <div className="div"/>
            <div className="sec">Summary</div>
            <div className="mvar"><span className="mvar-k">Machine</span><span className="mvar-v">{mach.label||"Custom"}</span></div>
            <div className="mvar"><span className="mvar-k">Class</span><span className="mvar-v">{mach.class}</span></div>
            <div className="mvar"><span className="mvar-k">Axes</span><span className="mvar-v">{mach.axes?.join(" ")}</span></div>
            <div className="mvar"><span className="mvar-k">Bed type</span><span className="mvar-v">{mach.bedType}</span></div>
            <div className="mvar"><span className="mvar-k">Live tools</span><span className="mvar-v">{mach.liveTools?"YES":"no"}</span></div>
            <div className="mvar"><span className="mvar-k">Sub-spindle</span><span className="mvar-v">{mach.subSpindle?"YES":"no"}</span></div>
            <div className="mvar"><span className="mvar-k">Blocks</span><span className="mvar-v">{blocks.length}</span></div>
            <div className="mvar"><span className="mvar-k">Tools</span><span className="mvar-v">{Object.keys(tools).length}</span></div>
            <div className="mvar"><span className="mvar-k">Geometry</span><span className="mvar-v">{geoms.length} objects</span></div>
            <div className="mvar"><span className="mvar-k">Fixtures</span><span className="mvar-v">{fixtures.length}</span></div>
          </div>}
        </div>
      </div>
    </div>
