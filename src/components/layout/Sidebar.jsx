...
export default function Sidebar({ onNavigate }) {
  ...
  const activeChapter = params.chapterId ? parseInt(params.chapterId) : null
  ...
  return (
    <nav className="h-full overflow-y-auto py-4">
      ...
      >
        ...
        <div>
          <div className="font-bold text-slate-900 dark:text-slate-100 leading-tight">OpenCalc</div>
          <div className="text-xs text-slate-500 dark:text-slate-400">Interactive Calculus</div>
        </div>
      </Link>

      {CURRICULUM.map((chapter) => {
        const isExpanded = activeChapter === chapter.number || chapter.lessons.length === 0
        return (
          <div key={chapter.id} className="mb-1">
            ...
            {!chapter.comingSoon && chapter.lessons.map((lesson) => {
              const isActive = activeChapter === chapter.number && activeSlug === lesson.slug
