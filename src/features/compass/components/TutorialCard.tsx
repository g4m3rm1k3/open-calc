...
export default function TutorialCard({ data }: { data: TutorialData }) {
  ...
  return (
    <div className="bg-slate-900/80 border border-slate-700 rounded-xl overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2 text-slate-200 font-semibold text-sm">
      ...
      {open && (
        <div className="px-3 pb-3 space-y-3">
          ...
          {data.sections.map((s) => (
            <div key={s.heading}>
              <h4 className="text-xs font-bold text-sky-400 mb-1">{s.heading}</h4>
              {s.prose.map((p, i) => (
                <div key={i} className="text-xs text-slate-400 leading-relaxed mb-1.5 [&_strong]:text-slate-200">
