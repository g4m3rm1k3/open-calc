...
A class in C# bundles data and behaviour together and controls access through the **access modifiers** from the previous lesson. C# adds **properties** — a cleaner alternative to manual getters and setters that look like fields but execute code on access. This lesson covers class creation, constructors, fields, properties, and the `new` keyword.
...
`class Rectangle { ... }` — `public` here is the same access modifier from the previous lesson, now applied to a class's own fields and methods instead of the class declaration itself: `Width`/`Height`/`Area`/`Perimeter` are all reachable from `Program`, a completely different class.
...
`private set` — the property can be read from anywhere (`public`) but only written from within the class (`private set`) — the same public/private vocabulary from the previous lesson, now controlling one accessor independently of the other.

**Out of scope for now:** a real `BankAccount` would reject a negative balance outright rather than silently clamping it to `0` — raising a real error on bad input is covered fully once exceptions have their own lesson.
