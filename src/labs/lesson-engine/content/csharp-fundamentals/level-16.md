...
`System.Collections.Generic` provides strongly-typed collection classes — `List<T>`, `Dictionary<TKey, TValue>`, `HashSet<T>` — that replace the need for raw arrays in most situations. **LINQ** (Language Integrated Query) adds query operators directly to the language, letting you filter, transform, group, and sort any collection with concise, readable syntax — built entirely on the lambda expressions and delegate types from the previous two lessons.
...
`new List<int> { 88, 92, ... }` — creates a `List<int>` with an **initialiser list**. Unlike an array (Level 2), a `List<T>` has no fixed size — it grows automatically.
...
`Where(predicate)` — filters: keeps only elements where the lambda returns `true`. Returns `IEnumerable<T>`. `s => s >= 70` is exactly the lambda syntax from two lessons ago, now passed straight to a real, built-in method instead of stored in a `Func<T, bool>` variable first.
...
Given a list of `(name, score)` tuples — the named-tuple syntax from two lessons ago — write a method `List<string> TopStudents(List<(string Name, int Score)> students, int threshold)` that returns the names of students whose score is at or above the threshold, sorted alphabetically.
