...
console.log(q.items)`,
      explanation: [
        '`Queue` wraps a single array: `this.items`. The FIFO contract — First In, First Out — means the first element added will be the first removed. Line 8 prints `[]`. Unlike a Stack (LIFO, both ends at the tail), a Queue adds at one end and removes from the other.',
...
console.log(q.items)`,
      explanation: [
        '`enqueue(item)` on line 6 calls `this.items.push(item)` — `Array.push` appends to the end. Returning `this` enables chaining: `q.enqueue(\'task-1\').enqueue(\'task-2\').enqueue(\'task-3\')` adds all three in one expression. Line 16 prints `[\'task-1\', \'task-2\', \'task-3\']`. The first item added is at index 0 — the front of the queue.',
      ...
      connections: [{ fromLine: 15, toLine: 6, color: 'emerald', label: 'chained enqueues' }],
    },
    {
...
console.log(q.items)`,
      explanation: [
        '`dequeue()` on line 11 checks for an empty queue first — returning `undefined` rather than crashing. Then it calls `this.items.shift()` — `Array.shift` removes and returns the first element. Line 23 prints `\'task-1\'` (first in). Line 24 prints `\'task-2\'`. Line 25 prints `[\'task-3\']` — the remaining item. FIFO confirmed: insertion order equals removal order.',
      ...
      connections: [{ fromLine: 23, toLine: 11, color: 'emerald', label: 'dequeue → task-1 (first in)' }],
    },
    {
...
console.log(q.size)`,
      explanation: [
        '`peek()` on line 16 returns `this.items[0]` — the front element without removing it. `get size` on line 20 is a getter returning the array length. After two dequeues, `items = [\'task-3\']`. Line 35 prints `\'task-3\'` — the next to be dequeued. Line 36 prints `1` — one item remains. Neither call modifies the queue.',
...
console.log(work.isEmpty())`,
      explanation: [
        '`isEmpty()` on line 24 returns `this.items.length === 0`. Lines 42–44: `work` is loaded with three jobs; the `while` loop dequeues and "processes" each in arrival order — `\'email\'`, `\'resize-image\'`, `\'generate-pdf\'`. Line 45 prints `true` — queue is empty. The drain loop is the canonical pattern for processing a queue to completion.',
      ...
      connections: [{ fromLine: 43, toLine: 24, color: 'violet', label: 'isEmpty checked before each dequeue' }],
    },
    {
...
console.log(pq.dequeue())`,
      explanation: [
        '`PriorityQueue` sorts `items` by descending `priority` after every `enqueue`. After adding all three jobs, `items[0]` is `{ item: \'critical-alert\', priority: 10 }`. Line 59 prints `\'critical-alert\'` (peek). Lines 60–62 dequeue in priority order: `\'critical-alert\'` (10), `\'normal-task\'` (5), `\'low-priority-cleanup\'` (1). The insertion order was 1, 10, 5 — the dequeue order is 10, 5, 1.',
