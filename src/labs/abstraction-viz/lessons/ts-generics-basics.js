...
console.log(getFirstString(['a', 'b', 'c']))  // → 'a'`,
        explanation: [
          'Three functions with identical bodies. Adding Date, User, or any new type requires another function. A bug in the logic must be fixed in three places. This is the DRY violation (Don\'t Repeat Yourself) at the type level. You could use any[] to avoid repetition, but then the return type would be any — you lose all type safety.',
...
console.log(getFirst([]))               // T = never  → returns never | undefined → undefined`,
        explanation: [
          '<T> declares a type parameter — a placeholder the TypeScript compiler fills in differently at each call. T is not a concrete type; it is a variable over types. getFirst<T>(arr: T[]): T | undefined means: for whatever type T turns out to be, take an array of T and return a T or undefined.',
...
console.log(getLast(words))     // → 'gamma'   (T = string)`,
        explanation: [
          'getLast uses the same <T> pattern. Each generic function resolves its own T independently per call. getFirst(numbers) and getLast(numbers) both resolve T = number independently. getFirst(words) and getLast(words) both resolve T = string independently. There is no shared T between the two functions.',
...
console.log(stringBox.get())  // → 'hello'`,
        explanation: [
          'Box<T> is a generic class. T is a type parameter for the whole class — every method and property that uses T within the class uses the same T resolved at construction. new Box(42) infers T = number. numberBox.get() returns number. numberBox.set() requires number. Passing a string to numberBox.set() is a compile error because T was locked to number when numberBox was constructed.',
...
// greetFirst([1, 2, 3])          // ✗ compile error: number has no .name`,
        explanation: [
          'T extends HasName constrains the type parameter: T must be a type that includes at least name: string. Without this constraint, TypeScript forbids firstItem.name because T could be anything — a number, a boolean, an array — none of which have .name. The constraint unlocks the name access while preserving T\'s full type.',
        ...
        connections: [{ fromLine: 22, toLine: 17, color: 'violet', label: 'T must satisfy' }],
        runCode:
`function getFirst(arr) { return arr[0] }
function getLast(arr)  { return arr[arr.length - 1] }
class Box {
...
// getProperty(user, 'email')             // ✗ 'email' is not keyof typeof user`,
        explanation: [
          'getProperty has two type parameters: T (the object type) and K (constrained to be one of T\'s keys via K extends keyof T). The return type T[K] is a mapped type: for the specific key K on type T, it resolves to whatever type that key holds. getProperty(user, \'name\') resolves T[K] = string; getProperty(user, \'age\') resolves T[K] = number. Different return types from the same function depending on which key is passed.',
...
type PublicUser   = Pick<User, 'name' | 'age'> // { name: string; age: number }  — no email`,
        explanation: [
          'TypeScript\'s standard library includes utility types built using the same generics and keyof machinery you just learned. Partial<User> makes all properties optional — useful for PATCH endpoints where only some fields are sent. Readonly<User> makes all properties readonly — useful for props in React. Pick<User, \'name\' | \'age\'> creates a new type with only those keys — useful for public-facing responses that should not expose email.',
...
console.log(getProperty(users[1], 'age'))  // → 30`,
        explanation: [
          'The complete program combines generic functions, bounded type parameters, keyof, and a generic class — all working together without any explicit type annotations at the call sites. TypeScript infers every T from the argument. All return types are precise. All accesses are type-safe. Zero any; zero casts; zero manual type assertions.',
