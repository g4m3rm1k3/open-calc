...
console.log(String(bird))    // → 'Animal(Bird)'`,
        explanation: [
          'Animal is the base class — the most general blueprint in our hierarchy. It captures the concept that every animal has a name, a sound, can speak, and can be converted to a string. Any code that works with an Animal can work with any future subclass.',
...
console.log(dog instanceof Dog)        // → true`,
        explanation: [
          'extends Animal establishes an inheritance relationship: Dog.prototype.__proto__ is set to Animal.prototype. When you call a method on a Dog instance, the lookup walks: dog → Dog.prototype (empty — no methods defined) → Animal.prototype (found speak() and toString()). Dog gets everything for free.',
        ...
        connections: [{ fromLine: 10, toLine: 1, color: 'violet', label: 'extends — chain linked' }],
      },
      {
...
console.log(dog.name)       // → 'Rex'  (set by Animal's constructor via super)`,
        ...
        connections: [{ fromLine: 12, toLine: 2, color: 'pink', label: 'super → Animal constructor' }],
      },
      {
...
// bird.learn              // → undefined — Animal has no learn method`,
        explanation: [
          'learn() is placed on Dog.prototype only — Animal.prototype does not have it. dog.learn(\'sit\') finds learn on Dog.prototype immediately. bird.learn is undefined because Animal.prototype has no learn, and Object.prototype has no learn. The lookup chain for bird terminates without finding it.',
...
// bird.learn              // → undefined — Animal has no learn method`,
        explanation: [
          'Dog defines its own speak() on Dog.prototype. Now dog.speak() finds speak on Dog.prototype immediately — the lookup stops and never reaches Animal.prototype\'s speak. bird.speak() finds nothing on Animal.prototype\'s chain before Dog.prototype (because bird is not a Dog), and uses Animal\'s speak as before.',
...
// super.speak() returned 'Rex says: woof!' — Dog appended its own ending`,
        explanation: [
          'super.speak() explicitly calls Animal\'s speak() with this still bound to dog. It returns \'Rex says: woof!\'. Dog\'s speak then appends \' *wags tail*\'. The chain: dog.speak() → Dog.speak runs → super.speak() → Animal.speak runs with this=dog → returns \'Rex says: woof!\' → Dog appends \' *wags tail*\'.',
        ...
        connections: [{ fromLine: 19, toLine: 3, color: 'pink', label: 'super → Animal.speak' }],
      },
      {
...
console.log(bird.speak())  // → 'Bird says: tweet!'`,
        explanation: [
          'Cat also extends Animal. It is a sibling of Dog — both derive from Animal, neither from each other. Cat has its own constructor (always passes \'meow\' as sound) and its own speak() that formats differently from both Animal\'s and Dog\'s speak. Three classes, three speak() implementations.',
...
// → 'Bird says: tweet!'`,
        explanation: [
          'One forEach. One animal.speak() call per iteration. Three different speak() implementations run. The caller (forEach) has no idea which subtype it is processing — it just calls speak() and the prototype chain dispatches to the correct implementation for each runtime type. This is polymorphism.',
