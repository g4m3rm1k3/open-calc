...
console.log(cpu.execute())`,
      explanation: [
        'Three independent subsystems — `CPU`, `Memory`, `HardDrive` — each with their own API. The caller must know all three interfaces, the correct argument values, and the exact ordering of five operations to boot the computer. This is the problem the Facade solves: **coordination knowledge leaks into every caller** rather than living in one place.',
...
console.log(log.length)`,
      explanation: [
        '`ComputerFacade` establishes the **caller → facade → subsystems dependency chain**: the facade owns all three subsystem instances and `start()` encodes the coordination sequence. The caller writes `computer.start()` — one method, no arguments, no knowledge of subsystem APIs. All five steps happen inside the facade; outside it, the sequence is invisible.',
      ...
      connections: [{ fromLine: 22, toLine: 1, color: 'violet', label: 'start() orchestrates all three subsystems', type: 'calls' }],
    },
    {
...
console.log(log1.length)`,
      explanation: [
        '`start(options)` extends the facade with **named defaults**: an options object lets callers override only what they need, with sensible defaults for everything else. `computer.start()` still reads as a zero-knowledge call. `computer.start({ bootPos: 1024 })` is self-documenting — the caller never touches the subsystem APIs directly. The facade absorbs the configuration complexity.',
...
console.log(postReq.body)`,
      explanation: [
        '`ApiClient` establishes a **configured facade over HTTP mechanics**: URL construction, auth header injection, and body serialisation are all absorbed into the facade. `api.get(\'/users\', { page: 1 })` returns a fully-formed request object without the caller touching `baseUrl`, `headers`, or `JSON.stringify`. The constructor configures the facade once; every method call inherits that configuration.',
      ...
      connections: [{ fromLine: 38, toLine: 29, color: 'violet', label: 'get() delegates to _buildUrl internally', type: 'calls' }],
    },
  ],
}
