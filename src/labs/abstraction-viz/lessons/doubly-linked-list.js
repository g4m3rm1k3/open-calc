...
console.log(b.prev.value)`,
      explanation: [
        'A doubly-linked list node has `next` (pointing forward) AND `prev` (pointing backward). Lines 11–12 manually link `a` and `b`: `a.next = b` and `b.prev = a`. Line 13 prints `10`. Line 14 prints `20` (following `next`). Line 15 prints `10` (following `prev` backwards). Both directions work.',
      ...
      connections: [
        { fromLine: 11, toLine: 4,  color: 'violet', label: 'a.next → b (forward)' },
        { fromLine: 12, toLine: 5,  color: 'indigo', label: 'b.prev → a (backward)' },
      ],
    },
    {
...
console.log(list.tail.prev.value)`,
      explanation: [
        '`pushBack(value)` appends to the tail. If the list is empty, the new node is both head and tail. Otherwise: `node.prev = this.tail` (link backward), `this.tail.next = node` (link forward), `this.tail = node` (advance tail). After pushing 10, 20, 30: head is 10, tail is 30, size is 3. Line 30 prints `10`. Line 31 prints `30`. Line 32 prints `3`. Line 33 prints `20`. Line 34 prints `20`.',
      ...
      connections: [
        { fromLine: 19, toLine: 3,  color: 'violet', label: 'node.prev = old tail (backward link)' },
        { fromLine: 20, toLine: 3,  color: 'indigo', label: 'old tail.next = node (forward link)' },
      ],
    },
    {
...
console.log(list.toArrayReverse())`,
      explanation: [
        '`pushFront` prepends to the head: `node.next = this.head` (link forward to old head), `this.head.prev = node` (link backward from old head to new node), `this.head = node` (advance head). After two `pushFront(5)` and `pushFront(1)`, the list is `1 ↔ 5 ↔ 10 ↔ 20 ↔ 30`. Line 44 prints `[1,5,10,20,30]`. `toArrayReverse()` follows `prev` from tail — line 45 prints `[30,20,10,5,1]`.',
      ...
      connections: [{ fromLine: 17, toLine: 2, color: 'violet', label: 'node.next = old head; head.prev = node' }],
    },
    {
...
console.log(list.size)`,
      explanation: [
        '`popFront` removes head: advance `this.head` to `this.head.next`, null-out the new head\'s `prev`. `popBack` removes tail: retreat `this.tail` to `this.tail.prev`, null-out the new tail\'s `next`. Both are `O(1)`. Line 49 prints `[1,5,10,20,30]`. Line 51 prints `1` (popped from front). Line 52 prints `30` (popped from back). Line 53 prints `[5,10,20]`. Line 54 prints `3`.',
...
console.log(list.size)`,
      explanation: [
        '`deleteNode(node)` removes any node in `O(1)`: `node.prev.next = node.next` (bypass the node going forward), `node.next.prev = node.prev` (bypass the node going backward). Edge cases: if `node` is the head, advance `this.head`; if it\'s the tail, retreat `this.tail`. Line 49 prints `[10,20,30,40,50]`. Line 51 prints `30` (the middle node). After delete: line 53 prints `[10,20,40,50]`. Line 54 prints `4`.',
      ...
      connections: [
        { fromLine: 38, toLine: 2,  color: 'violet', label: 'node.prev.next = node.next (skip forward)' },
        { fromLine: 40, toLine: 2,  color: 'indigo', label: 'node.next.prev = node.prev (skip backward)' },
      ],
    },
  ],
}
