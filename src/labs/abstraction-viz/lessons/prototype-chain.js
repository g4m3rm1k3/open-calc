...
console.log(true)`,
      explanation: [
        '`Object.create(animal)` creates a new empty object whose internal `[[Prototype]]` link points to `animal`. `dog.name = \'Rex\'` sets an own property on `dog`. Line 6 prints `\'Rex\'` — own property, found immediately. Line 7 prints `\'mammal\'` — not on `dog`, engine walks up to `animal` and finds it there. Line 8 prints `true` — confirms the prototype link.',
      ...
      connections: [{ fromLine: 7, toLine: 1, color: 'violet', label: 'dog.type walks chain → animal' }],
    },
    {
...
console.log(puppy.type)`,
      explanation: [
        '`puppy` is created from `dog`, which is created from `animal`. The chain is `puppy → dog → animal → Object.prototype → null`. Line 13 prints `1` — own property. Line 14 prints `\'Rex\'` — one link up (on `dog`). Line 15 prints `\'mammal\'` — two links up (on `animal`). The engine walks until it finds the property or reaches `null`.',
      ...
      connections: [
        { fromLine: 14, toLine: 3,  color: 'violet', label: 'puppy.name → dog' },
        { fromLine: 15, toLine: 1,  color: 'indigo', label: 'puppy.type → animal' },
      ],
    },
    {
...
console.log(animal.type)`,
      explanation: [
        '`puppy.type = \'young mammal\'` on line 17 creates a new own property on `puppy`. Now the chain has `type` on both `puppy` and `animal`. Lookup stops at the first match — `puppy.type` returns `\'young mammal\'`. `dog.type` returns `\'mammal\'` (dog has no own `type`, walks to `animal`). `animal.type` returns `\'mammal\'` unchanged. Shadowing never modifies the prototype.',
      ...
      connections: [{ fromLine: 17, toLine: 1, color: 'violet', label: 'shadow on puppy — animal.type untouched' }],
    },
    {
...
console.log(true)`,
      explanation: [
        '`hasOwnProperty` is inherited from `Object.prototype` — every object gets it through the chain. Line 23 prints `true` — `age` is an own property of `puppy`. Line 24 prints `false` — `name` is on `dog`, not on `puppy`. Line 25 prints `true` — `puppy.type = \'young mammal\'` created an own property on `puppy`. `hasOwnProperty` checks the own property table only — it does not walk the chain.',
...
console.log(cat.speak === bird.speak)`,
      explanation: [
        '`animalProto` is a plain object holding shared methods. `cat` and `bird` are created with it as prototype. `cat.speak()` calls the function with `this = cat`, reading `cat.name = \'Whiskers\'`. `bird.speak()` runs with `this = bird`. Line 44 prints `\'Whiskers says hello\'`. Line 45 prints `\'Tweety says hello\'`. Line 46 prints `true` — both `cat.speak` and `bird.speak` resolve to the same single function.',
      ...
      connections: [{ fromLine: 44, toLine: 28, color: 'violet', label: 'cat.speak → animalProto.speak; this = cat' }],
    },
    {
...
console.log(Object.keys(safeMap))`,
      explanation: [
        '`Object.create(null)` creates an object with no prototype — `Object.getPrototypeOf(safeMap)` returns `null`. Lines 27–29 store values under keys that would be dangerous on a regular object (`\'constructor\'`, `\'hasOwnProperty\'`, `\'__proto__\'`). Line 32 prints `\'value-a\'`. Line 33 prints `\'value-b\'` — there is no inherited `hasOwnProperty` to conflict with. Line 34 prints `[\'constructor\', \'hasOwnProperty\', \'__proto__\']` — all own keys.',
...
console.log(true)`,
      explanation: [
        '`Object.setPrototypeOf(myPet, robotProto)` replaces `myPet`\'s prototype link from `animalProto` to `robotProto` at runtime. Before: `myPet.speak()` returns `\'Rover says hello\'`. After: `myPet.speak()` returns `\'Rover says BEEP BOOP\'`. Line 37 prints `\'Rover says hello\'`. Line 39 prints `\'Rover says BEEP BOOP\'`. Line 40 prints `true`.',
      ...
      connections: [{ fromLine: 38, toLine: 32, color: 'violet', label: 'setPrototypeOf rewires chain to robotProto' }],
    },
  ],
}
