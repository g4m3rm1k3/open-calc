...
```quiz
{"q": "If g(x) = x+1 and f(x) = 2x, what is (f \u2218 g)(3)?", "options": ["7", "8", "6", "5"], "correct": 1, "explanation": "(f \u2218 g)(3) = f(g(3)) = f(4) = 8. Apply g first (3+1=4), then f (2\u00d74=8)."}
```
...
```quiz
{"q": "In general, f \u2218 g equals g \u2218 f:", "options": ["Always", "Never", "Only when f = g", "Not necessarily"], "correct": 3, "explanation": "Composition is not commutative. If f(x)=x+1 and g(x)=2x, then (f\u2218g)(x)=2x+1 but (g\u2218f)(x)=2x+2."}
```
...
```scene
InverseFunctionScene
```


```quiz
{"q": "f\u207b\u00b9 is the inverse of f means:", "options": ["f\u207b\u00b9(x) = 1/f(x)", "f(f\u207b\u00b9(x)) = x and f\u207b\u00b9(f(x)) = x", "f\u207b\u00b9 is the reflection of f", "f\u207b\u00b9(x) = -f(x)"], "correct": 1, "explanation": "The inverse undoes the function: composing f with f\u207b\u00b9 in either order gives the identity."}
```
...
```quiz
{"q": "The graph of f\u207b\u00b9 is obtained from f by:", "options": ["Shifting up by 1", "Reflecting across the x-axis", "Reflecting across y = x", "Rotating 90 degrees"], "correct": 2, "explanation": "Reflecting across y = x swaps x and y. Since (a,b) is on f iff (b,a) is on f\u207b\u00b9, the graphs are mirror images."}
```
