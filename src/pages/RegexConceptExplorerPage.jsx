...
import { REGEX_TOPICS } from '../data/regex-graph.js';
...
import { CATEGORY_STYLES, REGEX_CATEGORY_ORDER } from '../components/concept-explorer/categoryStyles.js';

const STACK_STORAGE_KEY = 'regex-explorer-stack';

export default function RegexConceptExplorerPage({ onClose }) {
  const navigate = useNavigate();
  const topics = REGEX_TOPICS;
  ...
  // Reopen wherever the learner left off, same as the Linear Algebra explorer.
  ...
  const c = CATEGORY_STYLES[current?.category] || CATEGORY_STYLES.history;
      ...
      <aside className="w-72 sm:w-80 shrink-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-r border-slate-200 dark:border-slate-800/60 flex flex-col z-10 relative shadow-xl dark:shadow-2xl transition-colors duration-500">
        <div className={`px-5 py-5 border-b border-slate-200 dark:border-slate-800/60 flex items-center gap-3 bg-gradient-to-br ${c.gradientBg} transition-colors duration-500`}>
          ...
          <div>
            ...
            <p className={`text-[11px] font-bold tracking-wide uppercase mt-0.5 transition-colors duration-500 ${c.text}`}>Regex</p>
          </div>
        </div>
        <div className="flex-1 overflow-hidden">
          <ConceptSearch
            ...
            categoryOrder={REGEX_CATEGORY_ORDER}
          />
        </div>
      </aside>
