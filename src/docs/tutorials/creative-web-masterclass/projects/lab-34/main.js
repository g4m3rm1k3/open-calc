...
// Track whether the hero section is on screen — skip rendering when it's not
...
function animateHero() {
  ...
  // Always reschedule — the loop must stay alive to resume when hero scrolls back in
...
// Skip canvas work when the section is off-screen
let canvasVisible = true;
new IntersectionObserver(function (entries) {
  canvasVisible = entries[0].isIntersecting;
}).observe(canvasSection);

function animateCanvas() {
  if (canvasVisible) {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    canvasParticleList.forEach(function (p) {
      if (mouse.onCanvas) {
