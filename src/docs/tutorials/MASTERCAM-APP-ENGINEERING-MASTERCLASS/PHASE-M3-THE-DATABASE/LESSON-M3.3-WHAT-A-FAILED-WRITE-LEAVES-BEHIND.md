...
Trace ta_number "TA0099" through both units: it never appears in the first (that one's about the general mechanism), but in the second, a single update_ta call with a bad tool_code leaves two real, already-executed ta_changes rows sitting uncommitted - visible to the connection, invisible to a fresh read, and cleaned up only because the test called rollback() itself, standing in for cleanup update_ta doesn't do.
