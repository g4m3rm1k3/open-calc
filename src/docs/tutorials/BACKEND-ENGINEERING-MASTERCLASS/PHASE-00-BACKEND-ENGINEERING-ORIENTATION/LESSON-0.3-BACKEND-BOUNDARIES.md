...
There is no new code in this unit — every file named below already exists. Nothing here gets typed; each is named so it can be opened and read.

### The Updated Project

Not applicable — this unit cites real, existing files as evidence for each named boundary rather than building up a new one.

### Mechanical Walkthrough

Not applicable in the usual sense — six real files/lines are named below as evidence for each boundary, rather than a new syntactic element being enumerated.
...
- **Reference Source:** backend/app/routes/operation_manager.py:815-904 (`_build_export_data`), 907-962 (`_apply_customizations_to_sequences`), 965-1013 (`_build_sequence_export`), 1016-1062 (`_build_operation_export`), and 465-624 (the `generate_nc_file` route itself) — all real, already-investigated findings, cited to file and line in this curriculum's own backend investigation.
- **Files affected:** None
- **Change type:** none
- **Location:** N/A
- **Dependencies:** None beyond the real repository already checked out on disk.
...
Not applicable — this unit cites a real, already-existing violation rather than building anything new.

### Mechanical Walkthrough

Not applicable — see the real citations below in place of a syntactic enumeration.
...
The real, current, honest cost, not a hypothetical one: four real functions — `_build_export_data`, `_apply_customizations_to_sequences`, `_build_sequence_export`, `_build_operation_export` — total 248 lines, about 23% of `operation_manager.py`'s entire file, and constitute a full, real domain service that lives directly in the routes module instead of in `operation_manager_service.py`, the file that exists specifically to hold this kind of logic. The `generate_nc_file` route itself, 160 lines, does real database lookups, real print-based debug logging, real Jinja2 template loading and rendering, real filename derivation, and a real database upsert — all inline, all in one function. The alternative not chosen: moving this logic into the service file that already exists for it. The real cost of not having done so: none of this logic can be tested, reused, or reasoned about without also reading and understanding an HTTP route handler at the same time — the domain layer and the application layer have been fused into one file, and untangling them later (a real, future lesson in this curriculum) has to happen with real, currently-passing behavior still intact.
...
Not applicable under the Verification Rule's own exemption: every real claim here — the exact line ranges, the exact line counts, the exact operations performed inline — is a direct citation to this curriculum's own backend investigation, independently re-verified against the real file this session.
