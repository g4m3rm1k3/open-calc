...
Not applicable under the Verification Rule's own exemption: no execution is required for this unit's actual claim - which boundary each named file belongs to. Every citation below is a direct reference to real, already-existing files, and that classification is established by what the file is, not by anything that has to run. It does not establish anything about those files' runtime behavior, which is not this unit's claim.
...
Not applicable under the Verification Rule's own exemption: no execution is required for this unit's actual claim - that this route mixes transport, domain logic, templating, and persistence in one function, and that `_build_export_data` lives in the routes file instead of the service file. The real code quoted above, read and confirmed verbatim this session, establishes that structural claim directly. It does not establish this route's runtime behavior on a live request, which is not what this unit is claiming.
