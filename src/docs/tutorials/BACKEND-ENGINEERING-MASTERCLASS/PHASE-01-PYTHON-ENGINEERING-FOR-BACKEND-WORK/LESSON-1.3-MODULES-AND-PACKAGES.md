...
**What you will build:** A real, small tool, continuing the same `ast`-based investigation technique already used to find routes, this time extracting a real file's actual module-level import graph mechanically - then a return to this real backend's own `app/__init__.py`, which turns out to contain a real, genuine circular dependency between the package that holds the shared database object and the package that holds the routes needing it, already resolved by deliberately deferring one real import until the moment it's actually needed.
...
The real, honest limitation of choosing `tree.body` over `ast.walk`: this tool would never notice a real import placed deliberately inside a function body, on purpose, rather than one left there by accident - and there's a real, deliberate one waiting in this exact backend's own `app/__init__.py`, investigated next. A tool that only reads a file's own declared top-level imports can tell you what a file depends on at load time; it can't tell you whether a file is hiding a deferred import for a real reason.
...
- `from app.routes import register_routes` — The real line that closes this backend's own circular dependency. `app.routes` (and, inside it, `operation_manager.py`) does `from app import db` - which only works because, by the time this line actually runs (inside a real call to `create_app()`), `db = SQLAlchemy()` already ran, long before, the moment `app` was first imported. Deferring this one import is what makes both real packages able to depend on each other without either one failing to find what it needs.
...
The real alternative not chosen, and why: importing `app.routes` at the very top of `app/__init__.py`, the same way `db` is created at the top - exactly the shape the `broken/` reproduction above actually ran, and actually failed. Its real, captured error - `ImportError: cannot import name 'db' from 'pkg' (consider renaming '...__init__.py' if it has the same name as a library you intended to import)` - was worth actually running rather than assuming: an earlier guess at this exact wording, made from memory before this unit's own lab existed, named a real, genuine Python `ImportError` correctly but got its precise text wrong - the same real failure, run through this same real `subprocess.run` command, produces a different exact message than it does through a plain `sys.path` import, a real reminder that even a message this well-known deserves running, not reciting. Given `operation_manager.py` does `from app import db` at its own top level, and `db` is defined at line 158 - after where a top-level routes import would have to sit to run before `create_app` is even defined - the same real failure would hit this actual backend, not only the small reproduction. Deferring the routes import into `create_app`'s own body - a real, deliberate design choice, not an accident - is the actual real fix already in place, and the `fixed/` reproduction's own real output confirms it actually works, not just that it should. It also draws a real package boundary: `app` owns `db`'s creation and must finish that first; `routes` depends on `app`, never the reverse, and the deferred import is what keeps that direction genuinely one-way instead of circular. The same real file also shows the other concept this lesson names: `_build_export_data`, already investigated, keeps its leading underscore specifically because nothing outside `operation_manager.py`'s own module is meant to import it directly - unlike `generate_nc_file`, exposed as a real route, `_build_export_data` is this module's own private interface, by convention, not by any enforced language rule.
...
The unit above built a tool that reads a file's own declared, top-level import graph; this unit found a real import placed deliberately outside that graph's usual shape - deferred, on purpose, inside a function body - and read closely enough to see exactly why: two real packages that need each other, made to work by controlling when, not just whether, each import actually runs.

## Connect the pieces

One small, real tool extracted `operation_manager.py`'s own top-level import graph, including its real dependency on `app` for `db` - then `app/__init__.py` itself, read closely enough to show that same dependency runs in the other direction too: `app` needs `routes`, `routes` needs `app`. `db = SQLAlchemy()` at module level, before `create_app` is even defined, and two real imports deferred inside `create_app`'s own body, are what let both real packages depend on each other without either one failing to find what it needs.
