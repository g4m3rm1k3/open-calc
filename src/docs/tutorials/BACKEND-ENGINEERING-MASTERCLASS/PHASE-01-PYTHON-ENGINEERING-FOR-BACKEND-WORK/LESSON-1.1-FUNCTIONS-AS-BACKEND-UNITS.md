...
- `existing = NCFile.query.get(nc_id)` — A real ORM lookup by primary key, deciding which of the two real branches below actually runs - the same real `Model.query.get` pattern as `PDMService.download_file`'s own `CAMFile.query.get` call: one primary-key argument in, one real row or `None` back.
