...
for "files that already have `priority`" and a different one for files
that don't. The loop's second iteration is where that one shared
assumption finally meets a real file it doesn't hold for.
