...
object from Lesson 1.1 into something a browser or `curl`
(`concepts/curl-command-line-http-client.md`) can reach — everything
before it (`create_app()`, route registration) only *prepares* the
app; nothing is listening on any real network socket
