...
**Real output**, from this exact request/response shape, run for real this session via `curl`'s own multipart form support (`curl -F "file=@Untitled.TOOLDB" ...` — `-F` is one of the flags `concepts/curl-command-line-http-client.md` covers) — the identical wire format a browser's `FormData` produces, against this project's real `/api/tools/import/preview` route:
```json
