...
Also recognized in: the classic physical gimbal-lock problem in
aircraft/spacecraft inertial navigation (a three-ring gimbal losing a
real degree of freedom at exactly ±90° pitch); a `north`-based compass
heading becoming undefined standing exactly at the geographic pole
(longitude loses meaning); and any Euler-angle-based 3D rotation system
in game engines and CAD software, where the same ±90°-pitch
degeneracy is the standard, named reason quaternions are often used
instead — this project's own epsilon-tilt fix is the cheap, practical
alternative to switching representations entirely.
