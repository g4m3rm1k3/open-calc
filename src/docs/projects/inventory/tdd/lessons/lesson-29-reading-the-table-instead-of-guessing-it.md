...
The same call, `apply_m_data_driven(43, ...)`, run twice with two
different banks — traced against the real dicts and real output above:
...
Same code, same argument (`43`), two different real answers — the branch
taken depends entirely on which `bank` dict was passed in, not on
anything hardcoded in `apply_m_data_driven` itself.
