...
`drawPath(points)` called with this lesson's own real 4-point path
(shown above, from Lesson 6's `compute_path`):
`[{x:0,y:0,z:0}, {x:10,y:20,z:0}, {x:30,y:20,z:0}, {x:30,y:20,z:-5}]`.

```
First call — pathGroup.children.length is 0 (nothing drawn yet):
  while (pathGroup.children.length) { ... }  ← 0, condition false immediately, skipped
  points.length (4) < 2?  → False, continue

  vectors = points.map(p => new THREE.Vector3(p.x, p.y, p.z)):
    p={x:0,y:0,z:0}   → Vector3(0,0,0)
    p={x:10,y:20,z:0} → Vector3(10,20,0)
    p={x:30,y:20,z:0} → Vector3(30,20,0)
    p={x:30,y:20,z:-5}→ Vector3(30,20,-5)
    vectors = [4 real Vector3 objects, same order]

  geometry = new BufferGeometry().setFromPoints(vectors) → one real
    geometry, 4 vertices, in order
  material = new LineBasicMaterial({ color: PATH_COLOR })
  line = new Line(geometry, material)
  pathGroup.add(line) → pathGroup.children.length is now 1
```

A **second** call to `drawPath` (a new program parsed, real points
changed) starts from `pathGroup.children.length === 1` instead of `0`,
so the drain loop actually runs this time:

```
while (pathGroup.children.length) {           ← 1, true
  pathGroup.remove(pathGroup.children[0]);     ← removes the only child
}                                               ← length now 0, condition
                                                  false, loop ends
```
then the same `map`/`geometry`/`material`/`Line`/`add` sequence above
runs again, building a fresh line from the new points — the old one is
