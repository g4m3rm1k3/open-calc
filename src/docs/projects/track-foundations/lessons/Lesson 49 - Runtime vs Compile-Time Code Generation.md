# Lesson 49: Runtime vs. Compile-Time Code Generation

**What you will build:** A real Android contract, read directly —
nothing here compiles with plain `javac`.

**What you need to know first:** Lesson 45's `Class` object.
...
This curriculum has already seen more than one example of "a working
implementation appears with no hand-written class implementing an
interface" — Lesson 24's Retrofit, and Lesson 11/13's own generated `R`
class and Room. These are not all the same mechanism, even though they
can look similar from the outside; recognizing which is which matters
for understanding what's actually inspectable and when the generation
actually happens.
...
Room's annotation processor (a later lesson's own subject) runs during
the build, producing a real, readable generated `.java` file a developer
could open and read. Retrofit's `.create(...)` (Lesson 24) runs when the
app itself executes,
using reflection (Lesson 45's own `Class` object) to build a working
implementation on the spot, with no generated source file ever produced
at all.
...
2. `retrofit.create(CatalogApi.class)` — **(b) reappearing** from Lesson
   24, now examined specifically as runtime generation: `CatalogApi
   .class` (Lesson 45's own `Class` object) is used via reflection to
...
Room's own generated database implementation (a later lesson's own
subject) is produced once, during the build — a real file exists
afterward. Retrofit's
`.create(CatalogApi.class)` (Lesson 24) produces a working
...
1. Identify, from Lesson 19's own `Safe Args` mention (a build-time
