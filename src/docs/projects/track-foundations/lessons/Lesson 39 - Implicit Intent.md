# Lesson 39: Implicit Intent

**What you will build:** A real Android code example, read directly —
nothing here compiles with plain `javac`.

**What you need to know first:** Lesson 19's `Intent`.
...
Lesson 19's `Intent` always named a specific target class —
...
1. `new Intent(MediaStore.ACTION_IMAGE_CAPTURE)` — **(a) first
   appearance** of this constructor shape: unlike Lesson 19's `new
...
2. `startActivity(intent)` — **(b) reappearing** launch call from Lesson
   19; the OS resolution — matching this action string against every
   installed app's own declared intent filters (Lesson 11) — happens
   entirely inside this one call.

### CS Lens

An implicit intent is message passing through a broker (Lesson 19) at
...
`android.media.action.IMAGE_CAPTURE`, the exact mechanism Lesson 11
...
Lesson 19's `Intent` named a specific target class, known at compile
...
capabilities — the same broker-mediated request Lesson 19 established,
...
- [ ] You can state, without looking back at this lesson, the difference
      between the explicit intent from Lesson 19 and this lesson's
      implicit one.
