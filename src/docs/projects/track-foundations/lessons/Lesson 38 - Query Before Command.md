# Lesson 38: Query Before Command
...
true right now" without triggering a side effect.
...
command) in Android's own runtime permission model, `SharedPreferences
.getInt` (query) versus `Editor.putInt` (command) — the same read/write
split recurring across genuinely different systems.
...
checking alone never changes anything.
...
3. Explain, in your own words, why `SharedPreferences.getInt` (Lesson
   35) is a query and `Editor.putInt` is a command, connecting this
