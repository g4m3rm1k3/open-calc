# Lesson 50: Static Analysis vs. Runtime Reflection Blind Spot

**What you will build:** A real Android/build-tooling contract, read
directly — nothing here compiles with plain `javac`.

**What you need to know first:** Lesson 45's `Class` object, Lesson 49's
runtime-vs-compile-time code generation.
...
- **Static analysis vs. runtime reflection blind spot** — a tool that
  ...
  indirectly, by looking up a name as a string at runtime, is invisible to
...
R8 (Lesson 16's own build-time shrinker) removes and renames anything it
...
anywhere in the app's own source — Gson (Lesson 24) and Room (a later
lesson's own subject) only ever reach them through reflection, by looking
up a field's *name* as a string at runtime. R8, reading the source
...
Room and Gson the moment the app actually runs.
...
preserve. `Item.class.getDeclaredField("name")` (Lesson 45's own `Class`
...
2. `Item.class.getDeclaredField("name")` — **(b) reappearing** use of
   Lesson 45's own `Class` object via reflection, now examined
...
shifts to the developer: keep-rule configuration files (Room's and Gson's
own libraries ship these by default) explicitly tell R8 "do not strip or
rename this class or its fields, even though you can't see why they're
needed" — the tool's blind spot is worked around by an explicit,
human-authored exception, not by R8 somehow being taught to see the
reflective call.
...
Lesson 49 already established that Retrofit's `.create(...)` generates a
...
Shipping a Room/Gson-backed app with R8 shrinking enabled and no keep
...
silently returns corrupted data at runtime, the first time Room or Gson
tries to look up a field name that no longer exists under that name — a
failure that never shows up in the IDE or at compile time, only after the
shrunk build actually runs.
