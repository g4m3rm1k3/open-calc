...
`lessons/` (the main curriculum, `cnc-web`/`cnc-service`) teaches computer
science and software engineering ideas *through* building a real app — it
assumes you can already write Python and JavaScript, and never stops to
explain what a `class` is or what a loop does, because that ground was
covered by the time it mattered.

`wpf-lessons/` is different on purpose, per direct instruction: this track
exists to teach **C# and WPF themselves**, as first-class subjects — not
just to arrive at a working app. Where a `lessons/` unit might skip
re-explaining `class` because Python's own `class` was already taught,
this track explains C#'s `class`, C#'s `namespace`, C#'s type system, and
WPF's own markup language from first principles, the way an actual course
should, and the book this is replacing didn't. The same rigor as
`lessons/` applies — every command is really run, every "New Code" step is
real, every claim about what breaks is actually broken and captured — the
difference is *what's assumed known walking in*: nothing about C# or WPF,
even where it resembles something Python/JS-shaped you already know.

This track builds a separate app (`wpf-app/CncWpf`) — not a port of
`cnc-web` yet. It starts as its own small thing so the *language and
framework* are the entire lesson, with no CNC-domain complexity competing
for attention. Whether/when it grows toward the same app the main track
builds is a later, explicit decision — not assumed now.
...
classes), and what compiling/running a program means in general. Nothing
else assumed.
...
`dotnet` is the .NET command-line tool (C#'s rough equivalent of
`python`/`node` — one CLI that can create projects, build them, and run
them). `new` is the subcommand for scaffolding a new project from a
...
- **Location** — new top-level folder `wpf-app/`, alongside the existing
  `cnc-service/`/`cnc-web/`.
- **Dependencies** — a working .NET SDK install (verified: `dotnet
  --version` → `10.0.301`).
...
The SDK-style format's real payoff over the older, fully-explicit project
file style: adding a new `.cs` file to this folder requires *no* project
file edit at all — the build system finds it automatically by convention
...
- `public partial class App : Application` — three ideas at once:
  ...
  can be used from outside this file/project (as opposed to keeping it
  private to this file). `partial` — **(b) hard concept reappearing**,
