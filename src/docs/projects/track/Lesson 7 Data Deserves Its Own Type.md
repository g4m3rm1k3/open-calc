...
**What you need to know first:** Lesson 6 — specifically, `InventoryAdapter`
and `onBindViewHolder`'s dependency on `List<String>`, which this
lesson changes to `List<Item>`. Lesson 5's field/constructor syntax
(from `InventoryViewHolder`) is reused here for a second, unrelated
class.
...
Create a throwaway file, `ParallelListsDemo.java`:
...
- `private final String name;` / `private final String location;` —
  **reappearing** (`private final` field, from Lesson 6's `InventoryAdapter.itemNames`),
  ...
  construction — a name or storage location being edited in place isn't
  a real operation this app needs (renaming would instead mean
  replacing the whole `Item`, a later-lesson concern).
...
- `public Item(String name, int quantity, String location) { ... }` —
  reappearing (constructor, from Lesson 6), new detail worth a clause:
...
You'll shortly need to ask questions like "is this the same item the
user tapped?" or, later, store `Item`s in collections that check for
duplicates. Try it now and see the surprise:
...
System.out.println(a.equals(b));   // also false, right now!
```

Even though `a` and `b` hold identical field values, both checks say
they're different. `==` on objects (as opposed to primitives like `int`,
which you've used since your Java class) compares **reference
identity** — "do these two variables point at the exact same object in
memory?" — not field values. `.equals()`, inherited from Java's root
`Object` class before you override it, defaults to doing exactly the
same reference check `==` does — which is why it also prints `false`
here.
...
- `name.equals(that.name)` / `location.equals(that.location)` —
  **reappearing** (`.equals()` call syntax), applied to `String`, which
  ...
  has always given the sensible answer in your prior Java work, even
  before this lesson explained why.
...
later" trap this lesson's earlier parallel-lists bug was also an
example of.
...
now a `LinearLayout` (same tag you briefly used in the throwaway
...
- `<LinearLayout ... android:orientation="vertical">` — **reappearing**
  (seen briefly in the discarded lab earlier this lesson), now
  permanent: `orientation="vertical"` stacks children top-to-bottom, as
  opposed to `ConstraintLayout`'s relationship-based positioning from
  Lesson 3 — a simpler container appropriate when children just need to
  flow in one direction, not participate in complex constraint
  relationships.
- Two `TextView`s, `android:padding` on the parent instead of each
  child — all reappearing from earlier this lesson and Lesson 3.
...
1. Add a fourth field to `Item`, `String sku`, with a getter but no
   ...
   preview of why later lessons introduce tools (Room annotations, in
   Lesson 13) that reduce this kind of manual repetition.
2. Write a throwaway `main()` method (anywhere temporary, discarded
   ...
   actually grow the set? Confirm your prediction against this lesson's
