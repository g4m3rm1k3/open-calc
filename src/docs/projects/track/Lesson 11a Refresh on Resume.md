...
A value read once, inside `onCreate` (Lesson 2f), and never re-read
...
value, and presses Back, `onCreate` never runs again (Lesson 5b's own
...
1. `protected void onCreate(Bundle savedInstanceState) { ...
   ...
   reappearing** Activity lifecycle method from Lesson 2f, reading the
...
2. `protected void onResume() { ... lowStockThreshold =
   ...
   the second of Lesson 2f's six lifecycle methods, this time given real
...
3. `super.onResume();` — **(b) reappearing** parent-implementation call
   from Lesson 2n, run before this Activity's own additional work, the
   same required pattern already established for `onCreate`.

### CS Lens

This is the Activity lifecycle (Lesson 2f) put to genuine use: `onCreate`
...
`onResume`, reliably fired every time an Activity becomes visible again,
is where potentially-stale state gets re-read. The next lesson looks at
a different distinction entirely — which kinds of state survive at all,
and which vanish the moment a process ends.
...
ever re-read the setting after the screen was first created.
...
2. Explain, in your own words, why `onResume` is a better fit for this
   pattern than `onStart` (also part of Lesson 2f's own lifecycle,
