# Lesson 29a: Interface Segregation Tension
...
1. `interface DataStore { String read(...); void write(...); void
   delete(...); }` — **(b) reappearing** interface shape from Lesson 0q,
...
2. `throw new UnsupportedOperationException("...")` — **(a) first
   ...
   this implementation — distinct from Lesson 0y's own checked
...
3. `try { store.write(...); } catch (UnsupportedOperationException e) {
   ... }` — **(b) reappearing** exception handling from Lesson 0y,
...
redesign the interface into smaller, more specific pieces (a
...
did not. The next lesson shows a real Android component contract with
the same broad-interface shape.
