# Lesson 16a: Read-Only Interface Exposure

**What you will build:** A small, fully runnable, plain Java lab.

**What you need to know first:** Lesson 0k's encapsulation.
...
Lesson 0k's `private` blocks outside code from reaching a field directly
...
1. `interface ReadableCounter { int getValue(); }` — **(b) reappearing**
   interface shape from Lesson 0q, deliberately declaring only the
...
3. `ReadableCounter getCounter() { return counter; }` — **(a) first
   ...
   returned object is a full `Counter`. Lesson 0n's dynamic dispatch
...
This is Lesson 0o's runtime-type-narrowing concept working in reverse:
...
0k's `private` already established, applied here to a returned reference
rather than a field.
...
`CounterHolder.getCounter()` deliberately returns the narrower
`ReadableCounter` type, so `Main` can read the current value but has no
compiler-permitted way to mutate it directly. The next lesson shows
Android's own real, load-bearing use of this exact pattern.
...
2. Attempt `((Counter) counter).increment();` — an explicit cast back to
   ...
   itself doesn't expose `increment()`. Connect this back to Lesson 0o's
