...
- `<argument android:name="item" app:argType="com.yourname.pocketinventory.Item" />`
  — **first appearance.** Declares that reaching `itemDetailFragment`
  *requires* a value named `item`, of a specific type — this is the
...
- `ItemDetailFragmentArgs.fromBundle(getArguments())` — **first
  ...
  destination requires. `getArguments()` — **first appearance** —
