# Lesson 12a: Manual Resource Cleanup
...
1. `new FileReader("notes.txt")` — **(b) reappearing** checked-exception-
   throwing constructor from Lesson 0y, here actually used to open a
...
`try`-with-resources syntax provides similarly), `using` statements in
C#, database connection pools requiring explicit release back to the
pool in virtually every language.
...
their behalf. The next lesson introduces a database — a resource this
same discipline applies to directly.
...
3. Explain, in your own words, why a plain `Dog` object never needs an
