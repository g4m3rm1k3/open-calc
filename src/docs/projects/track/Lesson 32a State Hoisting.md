# Lesson 32a: State Hoisting

**What you will build:** A small, fully runnable, plain Java lab,
isolating the pattern before meeting its real, Compose-specific trigger.
...
Lesson 9a, which always holds its own current text) makes that value
hard for surrounding code to observe or control from outside — reading
it means reaching into the widget itself, and resetting or validating it
means the widget's own internals must expose a way to do so. Some UI
systems deliberately avoid giving a widget any internal value ownership
at all.
...
living one level higher. `Screen.username`, not `StatelessTextField`
itself, is the real, single source of truth (Lesson 21c) for the current
text.
...
1. `interface TextChangedCallback { void onTextChanged(String newText);
   }` — **(b) reappearing** callback interface shape from Lesson 2b.
...
State hoisting is single source of truth (Lesson 21c) applied specifically
...
follows. The next lesson names the broader UI paradigm this pattern is
part of.
...
letting the widget redisplay it.
