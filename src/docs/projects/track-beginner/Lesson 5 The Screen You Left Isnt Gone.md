...
**What you need to know first:** Lesson 2a (fields, objects, `this`),
Lesson 2c (`onCreate`, the Template Method pattern, `Bundle
savedInstanceState` — present since Lesson 1 but unused until now),
...
Add these five overrides (same shape in both files — type it into both
...
- `private int tapCount = 0;` — **reappearing concept** (an instance
  ...
  *inside `MainActivity` specifically* — it lives at the class level,
  outside any single method, keeping its value between separate method
...
- `tapCount++` — **first appearance of the increment operator.**
  ...
  `tapCount += 1` (Python has no `++` at all) or JavaScript's identical
  `tapCount++`.
