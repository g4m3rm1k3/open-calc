...
- **Encapsulation** — bundling related fields behind a controlled
  public interface (getters/setters), so the object's internal
  representation is separated from what outside code is allowed to do
  to it.
- **Making illegal states representable** — a design flaw where the
  type system permits a combination of values that should never
  actually be valid (e.g. a name with no matching score), instead of
  making that combination impossible to construct in the first place.
- **Object identity vs. value equality** — two different questions:
  "are these the same object in memory?" (`==`) versus "do these hold
  the same data?" (`.equals()`) — a class must opt in to the second one
  by overriding `equals()`; it doesn't happen automatically.
- **`hashCode()`** — a method every object has, returning an `int`
  "fingerprint" of its data; hash-based collections use it to locate
  objects efficiently. Java requires that two `.equals()`-equal objects
...
1. `i = 0` — `names.get(0)` now returns `"Bob"`, because removing
   `"Alice"` shifted every later name one position earlier;
   `scores.get(0)` still returns `90`, Alice's original score, since
   `scores` was never modified — prints `"Bob: 90"`, pairing Bob with
   the wrong score.
2. `i = 1` — `names.get(1)` returns `"Carol"` for the same reason;
   `scores.get(1)` returns `75`, which was Bob's original score, since
   `scores` still holds its original three values at their original
   indices — prints `"Carol: 75"`, one misattribution further off than
   the previous line.
...
1. `CoordinateBad a = new CoordinateBad(3, 4);` — runs the
   constructor, which sets `a.x = 3` and `a.y = 4` on a freshly
   allocated object.
2. `CoordinateBad b = new CoordinateBad(3, 4);` — runs the constructor
   again with identical arguments, producing the same field values
   (`b.x = 3`, `b.y = 4`) — but since each `new` call allocates its own
   distinct object, `a` and `b` sit at two different memory locations
   even though every field matches, which is why the equality checks
   below both come back `false`.
