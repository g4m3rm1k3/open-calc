...
- `QAbstractTableModel.flags`, `Qt.ItemFlag` (a second real combinable flag enum, same shape as `Qt.AlignmentFlag`, 2.21), `QAbstractTableModel.setData`, `QAbstractItemModel.dataChanged` signal — introduced Lesson 2.22 (throwaway proof, `lab1_naive_setdata.py`/`lab2_setdata_with_validation.py`); landed for real via `AssetTableModel.flags`/`.setData`
- validation bypass through direct mutation (CS Lens) — a constructor-time check (`__post_init__`) silently never re-runs on plain attribute assignment afterward — Lesson 2.22, proven via a deliberately naive `setData` that let a blank value through
- `dataclasses.replace` (Python standard library), immutable-replacement pattern (CS Lens) — building a brand-new, fully-revalidated instance rather than mutating a field directly, reusing existing `__post_init__` validation with zero duplicated rules — introduced and landed for real Lesson 2.22; `AssetTableModel.setData` uses it to edit `Asset.name`/`.serial_number`/`.category`, catching `InvalidAssetError` (2.15) on a rejected edit
- `QTableView.selectionModel`, `QItemSelectionModel`, `.currentRowChanged` signal (distinct from the more general `currentChanged`), `QModelIndex.isValid` — introduced Lesson 2.23 (throwaway proof, `lab1_selection_model.py`); landed for real: `self.assets_table.selectionModel().currentRowChanged.connect(self.on_current_row_changed)`
- `QAbstractItemView.setSelectionBehavior` (`SelectionBehavior.SelectRows`, real default is `SelectItems`) — a deliberate UX choice, confirmed *not* required for `currentRowChanged` itself to fire — introduced and landed for real Lesson 2.23
- a real, separate detail panel (five `QLabel`s in a new `QFormLayout`, `main.py`) shows `Asset.owner` (1.4) and `Asset.is_retired` (1.2) in the UI for the first time ever — landed for real Lesson 2.23; deliberately shows plain `asset.name`, not `display_name` (1.10, 2.21), since a separate `Status` label already carries retirement — confirmed live-updating with no stale caching
- `QSortFilterProxyModel` (`setSourceModel`, `sort`, `setFilterFixedString`, `setFilterCaseSensitivity` — real default is case-*sensitive*), Decorator pattern (CS Lens) — introduced Lesson 2.24 (throwaway proof, `lab1_proxy_basics.py`); landed for real as `AssetFilterProxyModel` in new file `asset_manager/desktop/asset_filter_proxy_model.py`
- `QSortFilterProxyModel.mapToSource` — translating a (possibly sorted/filtered) proxy index back to the real source model's own index — introduced Lesson 2.24; landed for real inside `on_current_row_changed` (2.23)
- real, confirmed proof: an *unmodified* `QSortFilterProxyModel.setData`, with no override written, already forwards correctly to a real, validating source model via automatic index translation — introduced Lesson 2.24 (throwaway proof, `lab3_proxy_setdata_and_mapping.py`); `AssetFilterProxyModel` deliberately has no `setData` override at all, relying on this
- `QSortFilterProxyModel.filterAcceptsRow` (overridden), `.invalidate()` — real, checked-not-assumed finding: `invalidateFilter`/`invalidateRowsFilter` are both deprecated in this project's own installed PySide6, confirmed via real `DeprecationWarning`s; plain `invalidate()` is correct — introduced Lesson 2.24 (throwaway proof, `lab2_custom_filter_proxy.py`); landed for real combining a category match and a case-insensitive search-text match in one method
- `self.category_box` (2.7) gained a real `"All Categories"` item and its first-ever signal connection (`currentTextChanged`) — landed for real Lesson 2.24, alongside `self.search_box.textChanged` (2.6) now also driving `self.proxy_model.set_search_text`
- `QStyledItemDelegate`, `createEditor`/`setEditorData`/`setModelData`/`updateEditorGeometry`, `setItemDelegateForColumn`, Strategy pattern (CS Lens) — introduced Lesson 2.25 (throwaway proof, `lab1_delegate_basics.py`); landed for real as `CategoryDelegate` in new file `asset_manager/desktop/category_delegate.py`, attached to column `2` (`Category`) via `self.assets_table.setItemDelegateForColumn(2, self.category_delegate)`, replacing `QTableView`'s own default free-text editor with a real `QComboBox` for that column only
- real, confirmed proof: an edit made through `CategoryDelegate.setModelData` reaches the real, underlying `Asset` correctly through `self.proxy_model` (2.24), not `self.assets_model` directly, exercising 2.24's own transparent `setData` write-forwarding through a real delegate for the first time — introduced and landed for real Lesson 2.25
- accepted, honestly-named duplication: `CategoryDelegate.CATEGORIES` and `AssetEditor.category_field`'s own item list (2.9) are two separate, real copies of the identical five category strings, not unified — a deliberate cost named in Lesson 2.25's own SE Lens, matching the blueprint's "mostly throwaway" hedge for that lesson
- single source of truth (CS Lens), redundant state — the same real fact tracked in two separate places, correct only for as long as every writer remembers to update both copies — introduced Lesson 2.26 (throwaway proof, `Radio`/`VolumeDisplay`, stated from confidence per the Verification Rule's Necessity part, not executed); landed for real by deleting `self.current_search_text` (2.6, 2.12) outright, once confirmed dead against `AssetFilterProxyModel.self._search_text` (2.24), the real, sole remaining owner
- `ApplicationState`, Extract Class refactoring / cohesion / Single Responsibility Principle (CS Lens) — introduced and landed for real Lesson 2.26 (throwaway proof, `TodoWidget`/`TodoState`, also stated from confidence, not executed) as a brand-new, plain (non-`QObject`) class in new file `asset_manager/desktop/application_state.py`, holding `self.assets`/`self.validation_errors`; `MainWindow.state = ApplicationState()` replaces the old bare `self.submitted_assets`/`self.validation_errors` attributes, leaving `self.editor` (2.11) as the only real UI-lifetime state left directly on `MainWindow`
- real, confirmed proof: `AssetTableModel`'s own single-reference contract (2.20) still holds one attribute hop further through `self.state` — `window.state.assets is window.assets_model._assets` — confirmed Lesson 2.26
