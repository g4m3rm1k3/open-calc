...
### Taught so far (Level 1, Lessons 1–6 — Level 1 complete)
...
- `struct` (record types)
- `std::vector`, nested brace-initializer lists
- range-based `for`, `const T&` as the standard read-only loop idiom
- `std::cin`, `>>` (stream extraction)
- user-defined functions (return type, parameters, `return`, calling)
- `std::vector::push_back`
- `!` (logical NOT), checking a stream read for success/failure
- header files, `#pragma once`, quote vs. angle-bracket `#include`
- multi-file compilation/linking (declaration vs. definition)
- `class`, `public`/`private`, constructors, member initializer lists,
  `::` scope resolution, `const` member functions
- `std::string::find`/`substr`, `std::stoi` (deserialization)
- string concatenation (`+`), `.c_str()`, `std::remove`/`std::rename`
  (the temp-file-and-rename pattern)
- `continue`
- `throw`/`try`/`catch`, `std::runtime_error`, `.what()`
- `assert` (`<cassert>`), unit testing, `std::endl` (flush on newline)
- `else if`, `break`, command-loop/REPL structure
...
| 2 | ✅ Naming a Record's Shape | `struct`, `std::vector`, range-based `for`, `const T&` | `mydb.cpp` |
| 3 | ✅ Letting the Program Be Told, Not Told-to | `std::cin`/`>>`, user-defined functions, `push_back`, `!` / stream-failure checking | `mydb.cpp` |
| 4 | ✅ Drawing the Line Between Module and User | header files, multi-file compilation, `class`/`public`/`private`, constructors | `storage/student.h`, `storage/table.h`, `storage/table.cpp`, `mydb.cpp` |
| 5 | ✅ Changing Your Mind After You've Already Written It Down | parsing (`find`/`substr`/`stoi`), temp-file-and-rename, `throw`/`try`/`catch` | `storage/table.h`, `storage/table.cpp`, `mydb.cpp` |
| 6 | ✅ Proving It Without Reading the Output Yourself | `assert`/unit testing, `else if`/`break`, command-loop (REPL) structure | `storage/table_test.cpp`, `mydb.cpp` |

**End of Level 1 deliverable — done:** `storage/` module with a frozen,
tested public API, and `mydb` is now a real interactive CLI tool
(`insert`/`select`/`update`/`remove`/`quit`) usable today, independent
of the rest of this curriculum.
...
### Taught so far (Level 2, Lessons 1–2)
- `enum class`
- index-based `for` (`size_t i = 0; i < ...; i++`), `std::string`
  indexing with `[]`
- `<cctype>`: `std::isalpha`, `std::isspace`, `std::isdigit`
- `char` literals (`'x'`), escaped char literals (`'\''`), `&&`, `||`
- `std::string::operator+=`, delimiter-based scanning (vs.
  character-class accumulation)
- `throw`/`std::runtime_error` reused for lexer errors (not just
  storage errors)
...
1. Upload this file **and `API_Reference.md`**.
...
## API documentation as part of Definition of Done

Starting with the lesson that first splits a module out behind its own
header/interface (Level 1, Lesson 4 for `storage/`), any lesson whose
Definition of Done changes a module's *public* signature gets one more
checklist item: **update the matching section of `API_Reference.md`**.
Internals (a new private helper, a changed loop) never touch that file
— only what another module is allowed to call. This is what makes a
brand-new chat able to write, say, Level 2 Lesson 3 knowing exactly how
to call into `storage/` without ever having seen Level 1's lessons.
