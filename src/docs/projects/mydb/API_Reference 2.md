...
**Status:** 🟡 In progress. Lexer stage only so far (Lessons 1–2 of
Level 2) — fully tokenizes `SELECT ... FROM ... WHERE ... = ...` and
`INSERT INTO ... VALUES (...)` shapes. Parser/AST stages not started.

```cpp
enum class TokenType {
    KEYWORD,      // SELECT, FROM, WHERE, INSERT, INTO, VALUES
    IDENTIFIER,   // anything alphabetic that isn't a recognized keyword
    STAR,         // '*'
    SEMICOLON,    // ';'
    NUMBER,       // digits, stored as text (e.g. "42") — not yet a real int
    STRING,       // quoted text, delimiters stripped (e.g. 'Alice' -> "Alice")
    COMMA,        // ','
    EQUALS,       // '='
    LPAREN,       // '('
    RPAREN        // ')'
...
struct Token {
    TokenType type;
    std::string text;   // the exact source text that produced this token
};

class Lexer {
public:
    std::vector<Token> tokenize(const std::string& sql);
    // Converts raw SQL text into a flat sequence of Tokens. Whitespace
    // is consumed and produces no token. Throws std::runtime_error
    // immediately on any character it can't classify (message names
    // the exact character, not yet its position). Known gap: an
    // unterminated string literal (no closing quote) does not error.
};
```

**Tests:** `sql/lexer_test.cpp`, same standalone-executable pattern as
