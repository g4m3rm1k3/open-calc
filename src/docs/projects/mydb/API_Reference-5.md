...
**Status:** 🟡 In progress. Lexer stage only so far (Lesson 1 of
Level 2) — handles exactly one statement shape, `SELECT * FROM
students;`. Parser/AST stages not started.

```cpp
enum class TokenType {
    KEYWORD,      // "SELECT" or "FROM" only, currently
...
public:
    ...
    // is consumed and produces no token. No support yet for: commas,
    // digits/numbers, string literals, WHERE, INSERT, or any keyword
    // besides SELECT/FROM. An unrecognized character (e.g. a digit)
    // is currently silently ignored, not reported as an error — a
    // known gap.
};
```
