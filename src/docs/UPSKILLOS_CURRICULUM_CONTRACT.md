...
| `c` / `cpp` / `csharp` / `java` | — | **Not implemented.** `buildTestHarness` silently falls back to the JavaScript assertion harness for any language it doesn't recognise, which cannot execute C/C++/C#/Java source. Do not write a `test` fence for these languages until an engine harness exists for them. |
