...
import netChangeUrl from '../diagrams/calc-net-change.svg?url';
import displacementDistanceUrl from '../diagrams/calc-displacement-distance.svg?url';
...
export default {
  ...
  intuition: {
    blocks: [
      ...
      { type: 'image', src: netChangeUrl, alt: 'Area between two curves with upper and lower functions labeled', caption: 'Area between curves = ∫[upper − lower] dx — always upper minus lower regardless of sign.' },
      {
        type: 'prose',
        paragraphs: [
      ...
      { type: 'image', src: displacementDistanceUrl, alt: 'Positive and negative velocity areas — displacement subtracts, distance adds', caption: 'Displacement = A₁ − A₂ (signed); total distance = A₁ + A₂ (unsigned). Velocity below zero means moving backward.' },
    ],
    callouts: [
      {
        type: 'theorem',
  ...
  quiz: [
    {
      id: 'app-q1',
      type: 'input',
      text: 'Find the area between \\(f(x) = x^2\\) and \\(g(x) = x\\) on \\([0, 1]\\). (Note: \\(g \\geq f\\) on this interval.)',
      answer: '1/6',
      hints: ['Area = \\(\\int_0^1 (x - x^2)\\,dx\\).', '= \\([x^2/2 - x^3/3]_0^1 = 1/2 - 1/3 = 1/6\\).'],
      reviewSection: 'Math — Area between curves',
    },
    {
      id: 'app-q2',
      type: 'input',
      text: 'Find the intersection points of \\(y = x\\) and \\(y = x^2\\) by solving \\(x = x^2\\). What are the two \\(x\\)-values?',
      answer: '1',
      hints: ['\\(x = x^2 \\Rightarrow x(x-1)=0\\). So \\(x=0\\) or \\(x=1\\). Enter the positive intersection.'],
      reviewSection: 'Math — Finding intersection points',
    },
    {
      id: 'app-q3',
      type: 'input',
      text: 'Find the area enclosed by \\(y = 4 - x^2\\) and \\(y = 0\\) (the \\(x\\)-axis). First find where \\(4-x^2=0\\).',
      answer: '32/3',
      hints: ['Intersections: \\(x = \\pm 2\\).', '\\(\\int_{-2}^{2}(4-x^2)\\,dx = [4x - x^3/3]_{-2}^{2} = (8-8/3)-(-8+8/3) = 32/3\\).'],
      reviewSection: 'Math — Area between a curve and the x-axis',
    },
    {
      id: 'app-q4',
      type: 'choice',
      text: 'When computing the area between curves \\(f(x)\\) and \\(g(x)\\) on \\([a,b]\\) where \\(f(x) \\geq g(x)\\), the formula is:',
      options: [
      ...
      answer: '\\(\\int_a^b f(x)\\,dx - \\int_a^b g(x)\\,dx\\)',
      hints: ['Area = \\(\\int_a^b [f(x) - g(x)]\\,dx\\), which equals \\(\\int f - \\int g\\) by linearity.'],
      reviewSection: 'Math — Area between curves formula',
    },
    {
      id: 'app-q5',
      type: 'input',
      text: 'Find the area between \\(y = 3\\) and \\(y = x\\) on \\([0, 3]\\). (Here \\(3 \\geq x\\) on this interval.)',
      answer: '4.5',
      hints: ['\\(\\int_0^3 (3 - x)\\,dx = [3x - x^2/2]_0^3 = 9 - 9/2 = 9/2\\).'],
      reviewSection: 'Math — Area between curves',
    },
    {
      id: 'app-q6',
      type: 'input',
      text: 'A particle has velocity \\(v(t) = t^2 - 4\\) on \\([0, 3]\\). The displacement (net change in position) equals \\(\\int_0^3 v(t)\\,dt\\). Compute the displacement.',
      answer: '-3',
      hints: ['\\(\\int_0^3(t^2-4)\\,dt = [t^3/3 - 4t]_0^3 = (9-12) - 0 = -3\\).'],
      reviewSection: 'Math — Displacement as integral of velocity',
    },
    {
      id: 'app-q7',
      type: 'input',
      text: 'Find the area between \\(y = \\sqrt{x}\\) and \\(y = x^2\\) on \\([0, 1]\\). (\\(\\sqrt{x} \\geq x^2\\) on \\([0,1]\\).)',
      answer: '1/3',
      hints: ['\\(\\int_0^1(\\sqrt{x} - x^2)\\,dx = [\\frac{2}{3}x^{3/2} - \\frac{x^3}{3}]_0^1 = \\frac{2}{3} - \\frac{1}{3} = \\frac{1}{3}\\).'],
      reviewSection: 'Math — Area between curves',
    },
    {
      id: 'app-q8',
      type: 'choice',
      text: 'If a velocity function is negative on part of \\([a,b]\\) and you want the total distance traveled (not displacement), you should compute:',
      options: [
        '\\(\\int_a^b v(t)\\,dt\\)',
        '\\(\\int_a^b |v(t)|\\,dt\\)',
        '\\(-\\int_a^b v(t)\\,dt\\)',
      ...
      answer: '\\(\\int_a^b |v(t)|\\,dt\\)',
      hints: ['Total distance adds up all motion regardless of direction; displacement can be negative.'],
      reviewSection: 'Intuition — Displacement vs. distance',
    },
    {
      id: 'app-q9',
      type: 'input',
      text: 'Find the area enclosed between \\(y = x + 2\\) and \\(y = x^2\\). First find the intersections: \\(x+2 = x^2\\). The enclosed area is \\(\\int_{-1}^{2}[(x+2)-x^2]\\,dx\\).',
      answer: '4.5',
      hints: ['\\(x^2 - x - 2 = 0 \\Rightarrow (x-2)(x+1)=0\\), so \\(x=-1, 2\\).', '\\(\\int_{-1}^2(x+2-x^2)\\,dx = [x^2/2 + 2x - x^3/3]_{-1}^{2} = (2+4-8/3)-(1/2-2+1/3) = 9/2\\).'],
      reviewSection: 'Math — Area between curves',
    },
    {
      id: 'app-q10',
      type: 'input',
      text: 'A rate of change function is \\(r(t) = 6t\\). The net change from \\(t=1\\) to \\(t=3\\) is \\(\\int_1^3 6t\\,dt\\). Compute this.',
      answer: '24',
      hints: ['\\(\\int_1^3 6t\\,dt = [3t^2]_1^3 = 27 - 3 = 24\\).'],
      reviewSection: 'Math — Net change theorem',
    },
  ],
}
