...
import trigSubTriangleUrl from '../diagrams/calc-trig-sub-triangle.svg?url';
import trigSubCasesUrl from '../diagrams/calc-trig-sub-cases.svg?url';
...
export default {
  ...
  intuition: {
    blocks: [
      ...
      { type: 'image', src: trigSubTriangleUrl, alt: 'Three reference triangles for the three trig substitution cases', caption: 'Three triangles to memorize: √(a²−x²)→x=a sinθ, √(a²+x²)→x=a tanθ, √(x²−a²)→x=a secθ.' },
      {
        type: 'prose',
        paragraphs: [
      ...
      { type: 'image', src: trigSubCasesUrl, alt: 'Decision table: which radical form maps to which trig substitution', caption: 'Choose the substitution based on the radical form under the integrand, then back-substitute using your reference triangle.' },
    ],
    callouts: [
      {
        type: 'strategy',
  ...
  quiz: [
    {
      id: 'trig-sub-q1',
      type: 'choice',
      text: 'To integrate \\(\\int \\sqrt{9-x^2}\\,dx\\), the appropriate trigonometric substitution is:',
      options: ['\\(x = 3\\tan\\theta\\)', '\\(x = 3\\sin\\theta\\)', '\\(x = 3\\sec\\theta\\)', '\\(x = 3\\cos\\theta\\)'],
      answer: '\\(x = 3\\sin\\theta\\)',
      hints: ['For \\(\\sqrt{a^2-x^2}\\), use \\(x = a\\sin\\theta\\) so that \\(a^2-x^2 = a^2\\cos^2\\theta\\).'],
      reviewSection: 'Math — Substitution for √(a²−x²)',
    },
    {
      id: 'trig-sub-q2',
      type: 'choice',
      text: 'To integrate \\(\\int \\frac{1}{\\sqrt{x^2+4}}\\,dx\\), the appropriate substitution is:',
      options: ['\\(x = 2\\sin\\theta\\)', '\\(x = 2\\sec\\theta\\)', '\\(x = 2\\tan\\theta\\)', '\\(x = 2\\cos\\theta\\)'],
      answer: '\\(x = 2\\tan\\theta\\)',
      hints: ['For \\(\\sqrt{a^2+x^2}\\), use \\(x = a\\tan\\theta\\) so that \\(a^2+x^2 = a^2\\sec^2\\theta\\).'],
      reviewSection: 'Math — Substitution for √(a²+x²)',
    },
    {
      id: 'trig-sub-q3',
      type: 'choice',
      text: 'For \\(\\int \\frac{1}{\\sqrt{x^2-25}}\\,dx\\), the appropriate substitution is:',
      options: ['\\(x = 5\\sin\\theta\\)', '\\(x = 5\\tan\\theta\\)', '\\(x = 5\\sec\\theta\\)', '\\(x = 5\\cos\\theta\\)'],
      answer: '\\(x = 5\\sec\\theta\\)',
      hints: ['For \\(\\sqrt{x^2-a^2}\\), use \\(x = a\\sec\\theta\\) so that \\(x^2-a^2 = a^2\\tan^2\\theta\\).'],
      reviewSection: 'Math — Substitution for √(x²−a²)',
    },
    {
      id: 'trig-sub-q4',
      type: 'input',
      text: 'With \\(x = 3\\sin\\theta\\), simplify \\(\\sqrt{9 - x^2}\\).',
      answer: '3*cos(theta)',
      hints: ['\\(9-x^2 = 9-9\\sin^2\\theta = 9\\cos^2\\theta\\), so \\(\\sqrt{9-x^2}=3\\cos\\theta\\).'],
      reviewSection: 'Math — Simplification after substitution',
    },
    {
      id: 'trig-sub-q5',
      type: 'input',
      text: 'Evaluate \\(\\int_0^3 \\sqrt{9-x^2}\\,dx\\) by recognizing it as the area of a quarter-circle of radius 3.',
      answer: '9*pi/4',
      hints: ['A quarter-circle of radius 3 has area \\(\\frac{1}{4}\\pi r^2 = \\frac{9\\pi}{4}\\).'],
      reviewSection: 'Math — Geometric interpretation of trig sub',
    },
    {
      id: 'trig-sub-q6',
      type: 'input',
      text: 'Using \\(x = 2\\tan\\theta\\), find \\(dx\\) in terms of \\(d\\theta\\).',
      answer: '2 / cos(theta)^2',
      hints: ['\\(\\frac{dx}{d\\theta} = 2\\sec^2\\theta\\), so \\(dx = 2\\sec^2\\theta\\,d\\theta\\).'],
      reviewSection: 'Math — Computing dx for trig substitution',
    },
    {
      id: 'trig-sub-q7',
      type: 'choice',
      text: 'After a trigonometric substitution and integration, to convert back to \\(x\\), you use:',
      options: [
        'The original substitution equation directly',
        'A right triangle relating \\(\\theta\\) to \\(x\\)',
        'L\'Hôpital\'s rule',
        'The quadratic formula',
      ],
      answer: 'A right triangle relating \\(\\theta\\) to \\(x\\)',
      hints: ['Draw a right triangle: e.g. for \\(x=a\\sin\\theta\\), opposite = x, hypotenuse = a, adjacent = √(a²−x²).'],
      reviewSection: 'Math — Back-substitution',
    },
    {
      id: 'trig-sub-q8',
      type: 'input',
      text: 'For \\(x = \\sin\\theta\\) (with \\(a=1\\)), what is \\(\\sqrt{1-x^2}\\) in terms of \\(\\theta\\)?',
      answer: 'cos(theta)',
      hints: ['\\(1 - \\sin^2\\theta = \\cos^2\\theta\\), so \\(\\sqrt{1-x^2} = \\cos\\theta\\).'],
      reviewSection: 'Math — Pythagorean simplification',
    },
    {
      id: 'trig-sub-q9',
      type: 'input',
      text: 'After using \\(x = \\tan\\theta\\), the expression \\(1+x^2\\) becomes \\(1 + \\tan^2\\theta =\\) ?',
      answer: '1 / cos(theta)^2',
      hints: ['Pythagorean identity: \\(1 + \\tan^2\\theta = \\sec^2\\theta\\).'],
      reviewSection: 'Math — Pythagorean identity for tan',
    },
    {
      id: 'trig-sub-q10',
      type: 'input',
      text: 'Evaluate \\(\\int_0^1 \\frac{1}{\\sqrt{1-x^2}}\\,dx\\) using \\(x = \\sin\\theta\\). (This equals \\(\\arcsin(1) - \\arcsin(0)\\).)',
      answer: 'pi/2',
      hints: ['\\(\\frac{d}{dx}\\arcsin x = \\frac{1}{\\sqrt{1-x^2}}\\), so the antiderivative is \\(\\arcsin x\\).', '\\(\\arcsin(1) - \\arcsin(0) = \\pi/2 - 0\\).'],
      reviewSection: 'Math — Definite integral via trig sub',
    },
  ],
}
