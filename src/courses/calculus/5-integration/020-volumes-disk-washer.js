  ...
  quiz: [
    {
      id: 'disk-q1',
      type: 'choice',
      text: 'The Disk Method gives the volume of a solid of revolution about the \\(x\\)-axis as:',
      options: [
        '\\(V = \\int_a^b f(x)\\,dx\\)',
      ...
      hints: ['Each cross-section is a disk of radius \\(f(x)\\) and area \\(\\pi[f(x)]^2\\).'],
      reviewSection: 'Math — Disk method formula',
    },
    {
      id: 'disk-q2',
      type: 'input',
      text: 'Find the volume of the solid formed by rotating \\(y = \\sqrt{x}\\) about the \\(x\\)-axis from \\(x=0\\) to \\(x=4\\). Use the disk method.',
      answer: '8*pi',
      hints: ['\\(V = \\pi\\int_0^4 (\\sqrt{x})^2\\,dx = \\pi\\int_0^4 x\\,dx = \\pi[x^2/2]_0^4 = 8\\pi\\).'],
      reviewSection: 'Math — Disk method',
    },
    {
      id: 'disk-q3',
      type: 'input',
      text: 'Find the volume of the solid formed by rotating \\(y = x^2\\) about the \\(x\\)-axis from \\(x=0\\) to \\(x=1\\).',
      answer: 'pi/5',
      hints: ['\\(V = \\pi\\int_0^1 x^4\\,dx = \\pi[x^5/5]_0^1 = \\pi/5\\).'],
      reviewSection: 'Math — Disk method',
    },
    {
      id: 'disk-q4',
      type: 'choice',
      text: 'The Washer Method is used when:',
      options: [
        'The region touches the axis of rotation',
        'There is a hole (gap between the inner and outer boundary and the axis)',
        'You rotate about the \\(y\\)-axis',
        'The function is not integrable',
      ],
      answer: 'There is a hole (gap between the inner and outer boundary and the axis)',
      hints: ['When the region has an inner radius > 0, each cross-section is a washer (annulus), not a full disk.'],
      reviewSection: 'Math — Washer vs. disk',
    },
    {
      id: 'disk-q5',
      type: 'input',
      text: 'Find the volume using the Washer Method for the region between \\(y=\\sqrt{x}\\) (outer) and \\(y=x\\) (inner) from \\(x=0\\) to \\(x=1\\), rotated about the \\(x\\)-axis.',
      answer: 'pi/6',
      hints: ['\\(V = \\pi\\int_0^1[(\\sqrt{x})^2 - x^2]\\,dx = \\pi\\int_0^1(x-x^2)\\,dx = \\pi[x^2/2 - x^3/3]_0^1 = \\pi(1/2-1/3) = \\pi/6\\).'],
      reviewSection: 'Math — Washer method',
    },
    {
      id: 'disk-q6',
      type: 'input',
      text: 'Find the volume of the solid formed by rotating \\(y = 2\\) (a constant) about the \\(x\\)-axis on \\([0, 3]\\). This is a cylinder.',
      answer: '12*pi',
      hints: ['\\(V = \\pi\\int_0^3 4\\,dx = 4\\pi \\cdot 3 = 12\\pi\\).'],
      reviewSection: 'Math — Disk method (constant function = cylinder)',
    },
    {
      id: 'disk-q7',
      type: 'input',
      text: 'The region bounded by \\(y = x\\) and \\(y = x^2\\) (where \\(x \\in [0,1]\\)) is rotated about the \\(x\\)-axis. Set up the Washer integral: \\(V = \\pi\\int_0^1 [x^2 - x^4]\\,dx\\). Evaluate it.',
      answer: '2*pi/15',
      hints: ['\\(\\pi[x^3/3 - x^5/5]_0^1 = \\pi(1/3 - 1/5) = \\pi \\cdot 2/15 = 2\\pi/15\\).'],
      reviewSection: 'Math — Washer method',
    },
    {
      id: 'disk-q8',
      type: 'input',
      text: 'Find the volume obtained by rotating \\(y = \\sin(x)\\) about the \\(x\\)-axis from \\(x=0\\) to \\(x=\\pi\\).',
      answer: 'pi^2/2',
      hints: ['\\(V = \\pi\\int_0^\\pi \\sin^2(x)\\,dx\\).', 'Use \\(\\sin^2 x = (1-\\cos(2x))/2\\): \\(V = \\pi \\cdot \\pi/2 = \\pi^2/2\\).'],
      reviewSection: 'Math — Disk method with trig integral',
    },
    {
      id: 'disk-q9',
      type: 'choice',
      text: 'The Washer Method formula for rotation about the \\(x\\)-axis with outer radius \\(f(x)\\) and inner radius \\(g(x)\\) is:',
      options: [
      ...
      answer: '\\(V = \\pi\\int_a^b [f(x)^2 - g(x)^2]\\,dx\\)',
      hints: ['Each washer has area = \\(\\pi R^2 - \\pi r^2 = \\pi(R^2-r^2)\\), where \\(R=f(x)\\), \\(r=g(x)\\).'],
      reviewSection: 'Math — Washer method formula',
    },
    {
      id: 'disk-q10',
      type: 'input',
      text: 'Find the volume of a sphere of radius \\(r\\) by rotating \\(y = \\sqrt{r^2 - x^2}\\) about the \\(x\\)-axis from \\(-r\\) to \\(r\\). The integral equals \\(\\pi\\int_{-r}^{r}(r^2-x^2)\\,dx\\). For \\(r=1\\), evaluate this.',
      answer: '4*pi/3',
      hints: ['\\(\\pi\\int_{-1}^1(1-x^2)\\,dx = \\pi[x - x^3/3]_{-1}^1 = \\pi[(1-1/3)-(-1+1/3)] = \\pi \\cdot 4/3 = 4\\pi/3\\).'],
      reviewSection: 'Math — Disk method (sphere)',
    },
  ],
}
