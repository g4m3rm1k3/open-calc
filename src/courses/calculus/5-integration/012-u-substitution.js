  ...
  quiz: [
    {
      id: 'u-sub-q1',
      type: 'choice',
      text: 'For \\(\\int 2x\\,(x^2+1)^3\\,dx\\), the best substitution is:',
      options: ['\\(u = x^2+1\\)', '\\(u = x^3\\)', '\\(u = 2x\\)', '\\(u = (x^2+1)^3\\)'],
      answer: '\\(u = x^2+1\\)',
      hints: ['Choose \\(u\\) to be the inner function; then \\(du = 2x\\,dx\\) cancels the \\(2x\\) factor.'],
      reviewSection: 'Math — Choosing u',
    },
    {
      id: 'u-sub-q2',
      type: 'input',
      text: 'Evaluate \\(\\int 2x\\,(x^2+1)^3\\,dx\\) using \\(u = x^2+1\\) (omit \\(+C\\); express in terms of \\(x\\)).',
      answer: '(x^2+1)^4 / 4',
      hints: ['With \\(u=x^2+1\\), \\(du=2x\\,dx\\), so the integral becomes \\(\\int u^3\\,du = u^4/4\\).', 'Substitute back: \\((x^2+1)^4/4\\).'],
      reviewSection: 'Math — u-substitution',
    },
    {
      id: 'u-sub-q3',
      type: 'input',
      text: 'Evaluate \\(\\int e^{3x}\\,dx\\) using \\(u = 3x\\) (omit \\(+C\\)).',
      answer: 'e^(3*x)/3',
      hints: ['\\(du = 3\\,dx\\), so \\(dx = du/3\\).', '\\(\\int e^u \\cdot \\frac{du}{3} = \\frac{1}{3}e^u = \\frac{1}{3}e^{3x}\\).'],
      reviewSection: 'Math — u-substitution with exponential',
    },
    {
      id: 'u-sub-q4',
      type: 'input',
      text: 'Evaluate \\(\\int \\cos(2x)\\,dx\\) using \\(u = 2x\\) (omit \\(+C\\)).',
      answer: 'sin(2*x)/2',
      hints: ['\\(du = 2\\,dx\\), so \\(dx = du/2\\).', '\\(\\int \\cos(u)\\cdot\\frac{du}{2} = \\frac{\\sin u}{2} = \\frac{\\sin(2x)}{2}\\).'],
      reviewSection: 'Math — u-substitution with trig',
    },
    {
      id: 'u-sub-q5',
      type: 'input',
      text: 'Evaluate \\(\\int_0^1 2x\\,e^{x^2}\\,dx\\) using \\(u = x^2\\). Change the limits: when \\(x=0\\), \\(u=0\\); when \\(x=1\\), \\(u=1\\).',
      answer: 'e - 1',
      hints: ['With \\(u=x^2\\), \\(du=2x\\,dx\\), so the integral becomes \\(\\int_0^1 e^u\\,du = e^1 - e^0 = e-1\\).'],
      reviewSection: 'Math — Definite integral with u-substitution',
    },
    {
      id: 'u-sub-q6',
      type: 'input',
      text: 'Evaluate \\(\\int \\frac{x}{x^2+4}\\,dx\\) using \\(u = x^2+4\\) (omit \\(+C\\)).',
      answer: 'log(x^2+4)/2',
      hints: ['\\(du = 2x\\,dx\\), so \\(x\\,dx = du/2\\).', '\\(\\int \\frac{du/2}{u} = \\frac{1}{2}\\ln|u| = \\frac{1}{2}\\ln(x^2+4)\\).'],
      reviewSection: 'Math — u-substitution (logarithm result)',
    },
    {
      id: 'u-sub-q7',
      type: 'input',
      text: 'Evaluate \\(\\int \\sin^4(x)\\cos(x)\\,dx\\) using \\(u = \\sin(x)\\) (omit \\(+C\\)).',
      answer: 'sin(x)^5 / 5',
      hints: ['\\(du = \\cos(x)\\,dx\\), so the integral becomes \\(\\int u^4\\,du = u^5/5\\).', 'Substitute back: \\(\\sin^5(x)/5\\).'],
      reviewSection: 'Math — u-substitution',
    },
    {
      id: 'u-sub-q8',
      type: 'choice',
      text: 'When performing \\(u\\)-substitution on a definite integral, you should:',
      options: [
        'Always convert back to \\(x\\) before evaluating the limits',
        'Change the limits of integration to \\(u\\)-values and evaluate directly',
        'Ignore the limits and add \\(+C\\)',
        'Use the original limits as \\(u\\)-values without change',
      ],
      answer: 'Change the limits of integration to \\(u\\)-values and evaluate directly',
      hints: ['If \\(u = g(x)\\), the new limits are \\(g(a)\\) and \\(g(b)\\).'],
      reviewSection: 'Math — Definite integrals with substitution',
    },
    {
      id: 'u-sub-q9',
      type: 'input',
      text: 'Evaluate \\(\\int_0^{\\pi/2} \\sin(x)\\cos(x)\\,dx\\) using \\(u = \\sin(x)\\). New limits: \\(u(0)=0\\), \\(u(\\pi/2)=1\\).',
      answer: '1/2',
      hints: ['\\(du = \\cos(x)\\,dx\\), so integral = \\(\\int_0^1 u\\,du = [u^2/2]_0^1 = 1/2\\).'],
      reviewSection: 'Math — Definite u-substitution',
    },
    {
      id: 'u-sub-q10',
      type: 'input',
      text: 'Evaluate \\(\\int (3x^2+1)^5 \\cdot 6x\\,dx\\) using \\(u = 3x^2+1\\) (omit \\(+C\\)).',
      answer: '(3*x^2+1)^6 / 6',
      hints: ['\\(du = 6x\\,dx\\), so the integral becomes \\(\\int u^5\\,du = u^6/6\\).', 'Substitute back: \\((3x^2+1)^6/6\\).'],
      reviewSection: 'Math — u-substitution (power)',
    },
  ],
}
