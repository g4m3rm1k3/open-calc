export default {
  ...
  intuition: {
    ...
    visualizations: [
        ...
        mathBridge: 'The function $f(x)=b^x$ satisfies $f(x+1) = b\\cdot f(x)$: every unit increase in $x$ multiplies the output by $b$. When $b>1$: growth. When $b=1$: constant ($b^x=1$ for all $x$). When $0<b<1$: decay. The natural base $e\\approx 2.718$ is highlighted because $\\frac{d}{dx}[e^x]=e^x$ — $e^x$ is its own derivative, which no other base gives. This self-similarity is why $e$ appears in every natural-growth formula.',
        caption: 'Drag the slider to change the base. Base > 1 grows, base = 1 is flat, 0 < base < 1 decays. e ≈ 2.718 is the unique base where slope equals value.',
      },
      {
