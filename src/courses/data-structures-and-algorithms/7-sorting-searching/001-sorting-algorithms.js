  ...
  quiz: [
    {
      id: 'q1',
      type: 'choice',
      text: 'Merge sort is O(n log n) in all cases; quicksort is O(n log n) average but O(n²) worst-case. Why is quicksort often faster in practice despite the worse worst-case?',
      options: [
        'Quicksort has a smaller constant factor — partition is done in-place with minimal data movement, while merge sort allocates O(n) extra space and copies data. For random data, the pivot splits reasonably well. The O(n²) worst case (already sorted data) is avoidable with randomized pivot selection',
        'Quicksort runs on multiple CPU cores by default',
        'Quicksort\'s worst case never occurs with modern processors',
      ],
      correct: 0,
    },
    {
      id: 'q2',
      type: 'choice',
      text: 'Merge sort on 20 elements in reverse order makes far fewer comparisons than bubble sort. What accounts for this gap?',
      options: [
        'Bubble sort compares adjacent pairs: O(n²) comparisons in the worst case. Merge sort divides the array in half at each level (O(log n) levels) and merges in O(n) per level — O(n log n) total. For n=20 reversed: bubble sort ~190 comparisons, merge sort ~100 comparisons',
        'Bubble sort cannot handle reversed arrays without modification',
        'Merge sort uses binary search internally, reducing comparisons',
      ],
      correct: 0,
    },
    {
      id: 'q3',
      type: 'choice',
      text: 'Counting sort achieves O(n + k) where k is the range of values. Why cannot counting sort replace comparison sort in all cases?',
      options: [
        'Counting sort requires Python 3.10 or newer',
        'Counting sort only works on integer (or discretizable) keys within a bounded range. For floating-point numbers, strings, or objects with arbitrary comparison functions, there is no counting bucket to map into. Also, when k >> n (sparse range), the count array is wasteful',
        'Counting sort is always O(k²) making it slower for large inputs',
      ],
      correct: 1,
    },
    {
      id: 'q4',
      type: 'choice',
      text: 'Python\'s built-in `sorted()` uses Timsort, which is O(n log n) worst-case and O(n) for already-sorted data. What property of real-world data does Timsort exploit?',
      options: [
        'Real data is usually stored as floats, which Timsort handles more efficiently',
        'Real data often contains partially sorted sequences ("runs"). Timsort detects and merges these natural runs rather than treating the array as fully random. For already-sorted or nearly-sorted data, the runs span the entire array and merging is trivial — O(n). This makes Timsort adaptive: fast on structured data, still O(n log n) on random data',
        'Timsort avoids comparisons by using element memory addresses',
      ],
      correct: 1,
    },
  ],
