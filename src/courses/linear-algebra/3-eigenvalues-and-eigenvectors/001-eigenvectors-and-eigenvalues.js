export default {
  ...
  intuition: {
    ...
    callouts: [
      {
        type: 'sequencing',
        title: 'Lesson 1 of 7 — Eigenvalues & Eigenvectors',
...
for iter = 1:10
    ...
    if mod(iter, 2) == 0
        fprintf('  iter %2d: x=[%.6f; %.6f], λâ‰ˆ%.6f\\n', iter, x(1), x(2), lambda_est)
    end
end`,
            },
            {
...
fprintf('\\nChatter avoidance: choose spindle speed NOT in [%.0f, %.0f] RPM\\n', ...
    freq_Hz(1)*60*0.9, freq_Hz(end)*60*1.1)`,
      ...
      },
      {
        ...
        mathBridge: 'np.linalg.eig(A) returns (eigenvalues, eigenvectors). Each column of the eigenvector matrix is an eigenvector. Verify: A @ v â‰ˆ λ * v. Sanity check: trace(A) = sum of eigenvalues, det(A) = product of eigenvalues.',
...
`,
  ...
  mentalModel: [
    ...
    'Trace = Σλᵢ, det = Î λᵢ — fast sanity checks.',
  ],
    ...
    questions: [
      {
        ...
        options: ['1', '0', '-1', 'The eigenvalue depends on which eigenvector is used'],
        answer: '1',
        hints: ['$I\\mathbf{v} = 1\\cdot\\mathbf{v}$ for every nonzero vector $\\mathbf{v}$, so $\\lambda = 1$ universally.'],
        reviewSection: 'Intuition tab — eigenvector definition',
      },
      {
        id: 'la3-001-assess-2',
        type: 'choice',
        ...
        options: [
          'Because $(A - \\lambda I)$ must be singular so a nonzero $\\mathbf{v}$ satisfies $(A-\\lambda I)\\mathbf{v} = \\mathbf{0}$',
          'Because eigenvalues must always be zero',
          'Because $\\det(A) = \\lambda$',
          'Because $I = 0$ in linear algebra',
        ],
        answer: 'Because $(A - \\lambda I)$ must be singular so a nonzero $\\mathbf{v}$ satisfies $(A-\\lambda I)\\mathbf{v} = \\mathbf{0}$',
        hints: ['$A\\mathbf{v} = \\lambda\\mathbf{v}$ rearranges to $(A-\\lambda I)\\mathbf{v} = \\mathbf{0}$. For a nonzero $\\mathbf{v}$ to exist, $(A-\\lambda I)$ must be singular, which means $\\det(A-\\lambda I) = 0$.'],
      },
      {
        id: 'la3-001-assess-3',
        type: 'choice',
        text: 'For $A = \\begin{bmatrix}2&1\\\\0&3\\end{bmatrix}$, what are the eigenvalues?',
        options: [
          '$\\lambda_1 = 2$, $\\lambda_2 = 3$ — upper triangular matrix, diagonal entries are eigenvalues',
          '$\\lambda_1 = 5$, $\\lambda_2 = 6$',
          '$\\lambda = 0$ (matrix is singular)',
          '$\\lambda = \\pm\\sqrt{6}$',
        ],
        answer: '$\\lambda_1 = 2$, $\\lambda_2 = 3$ — upper triangular matrix, diagonal entries are eigenvalues',
        hints: ['For a triangular matrix, the eigenvalues are the diagonal entries. Verify: $\\det(A - \\lambda I) = (2-\\lambda)(3-\\lambda) = 0$ gives $\\lambda = 2$ or $\\lambda = 3$.'],
      },
      {
        id: 'la3-001-assess-4',
        type: 'choice',
        text: 'A $3\\times 3$ matrix has eigenvalues $2, -1, 5$. What is $\\det(A)$?',
        options: [
          '$-10$ — product of eigenvalues: $2 \\times (-1) \\times 5 = -10$',
          '$6$ — sum of eigenvalues',
          '$0$ — not enough information',
          '$25$ — largest eigenvalue squared',
        ],
        answer: '$-10$ — product of eigenvalues: $2 \\times (-1) \\times 5 = -10$',
        hints: ['The determinant equals the product of all eigenvalues (including multiplicity). This follows from the characteristic polynomial: $\\det(A-\\lambda I) = (\\lambda_1-\\lambda)(\\lambda_2-\\lambda)(\\lambda_3-\\lambda)$, evaluating at $\\lambda=0$.'],
      },
    ],
  },
