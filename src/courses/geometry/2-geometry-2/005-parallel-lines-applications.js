...
import geoPolygonAngleSumUrl from '../diagrams/geo-polygon-angle-sum.svg?url'
import quizPolygonAngleFindUrl from '../diagrams/quiz-polygon-angle-find.svg?url'
...
function draw(){
  ...
  // Vertex 0 highlight
  ctx.beginPath();ctx.arc(pts[0].x,pts[0].y,8,0,2*Math.PI);ctx.fillStyle=NAVY;ctx.fill();
  ctx.fillStyle=NAVY;ctx.font='bold 12px Georgia';ctx.textAlign='left';ctx.fillText('V (base vertex)',pts[0].x+12,pts[0].y+4);
}
...
function draw(){
  ...
  // Intersection M
  var M=midPt(A,C);
  ctx.beginPath();ctx.arc(M.x,M.y,5,0,2*Math.PI);ctx.fillStyle=RED;ctx.fill();
  ctx.fillStyle=RED;ctx.font='bold 11px Georgia';ctx.textAlign='center';ctx.fillText('M',M.x,M.y-10);
...
export const GEO_2_PARALLEL_DEFAULT = {
    ...
    blocks: [
      {
        type: 'prose',
        paragraphs: [
          '**Polygon interior angle sum = (n−2)×180°.** The proof is elegant: draw diagonals from any one vertex of an n-gon. This splits it into exactly (n−2) non-overlapping triangles. Each triangle contributes 180°. Total: (n−2)×180°. This works for any convex polygon, any n ≥ 3.',
          '**Exterior angle sum = 360°** for any convex polygon — regardless of n. Imagine walking around the polygon: at each vertex you turn by the exterior angle. After one complete loop you\'ve turned exactly 360°. This reasoning works for any convex polygon and shows the exterior angle sum is independent of n.',
        ],
      },
      {
        type: 'image',
        src: geoPolygonAngleSumUrl,
        alt: 'Four polygons (n=3 to 6) each triangulated from one vertex. Diagonal lines show 1, 2, 3, 4 triangles respectively. Formula box: Sum = (n−2)×180°.',
        caption: 'Triangulation proof: draw diagonals from vertex 1 to create (n−2) triangles. Each contributes 180°. Total = (n−2)×180°.',
      },
      {
        type: 'prose',
        paragraphs: [
          '**Parallelogram:** both pairs of opposite sides are parallel. From this single definition, all four key properties follow by drawing one diagonal and applying congruence:',
          '— Opposite sides are equal (proved by AAS + CPCTC on the two triangles)',
          '— Opposite angles are equal (alternate interior angles + angle sum)',
          '— Consecutive angles are supplementary (co-interior angles, sum to 180°)',
          '— Diagonals bisect each other (proved by ASA + CPCTC on the two smaller triangles)',
          'The **converses** all hold too: if any one of these properties is true, the quadrilateral is a parallelogram.',
        ],
      },
      {
        type: 'callout',
        ...
        body: 'Each interior angle of a regular n-gon = (n−2)×180°/n. Triangle (n=3): 60°. Square (n=4): 90°. Pentagon (n=5): 108°. Hexagon (n=6): 120°. Dodecagon (n=12): 150°. As n→∞, each interior angle approaches 180° (the polygon approaches a circle).',
      },
      {
        type: 'viz',
        id: 'ScienceNotebook',
        props: { lesson: LESSON_GEO_2_PARALLEL },
      },
    ],
  },
  ...
  quiz: [
    ...
    },
    {
      id: 'q5',
      type: 'choice',
      diagram: quizPolygonAngleFindUrl,
      diagramAlt: 'Pentagon with four interior angles labelled 100°, 115°, 120°, 110°, and one unknown angle x.',
      text: 'The interior angles of a pentagon (5 sides) sum to (5−2)×180° = 540°. Four angles are 100°, 115°, 120°, and 110°. Find the missing angle x.',
      options: [
        'x = 95°',
        'x = 85°',
        'x = 105°',
    ...
    },
    {
      id: 'q6',
      type: 'choice',
      text: 'A regular decagon (10 sides). What is each interior angle?',
      options: [
        '144°',
        '150°',
        '162°',
    ...
    },
    {
      id: 'q7',
      type: 'choice',
      text: 'The exterior angle sum of an octagon (8 sides) is:',
      options: [
        '1080°',
        '360° — exterior angle sum is always 360° for any convex polygon',
        '45°',
      ],
      correct: 1,
    },
    {
      id: 'q8',
      type: 'choice',
      text: 'In parallelogram ABCD, ∠A = 65°. What is ∠B?',
      options: [
        '65° — opposite angles are equal',
        '115° — consecutive angles are supplementary (co-interior angles, sum to 180°)',
        '90°',
      ],
      correct: 1,
    },
    {
      id: 'q9',
      type: 'choice',
      text: 'Which property of a parallelogram is proved by drawing a diagonal and using ASA congruence?',
      options: [
        'All four sides are equal',
        'Opposite sides are equal — the diagonal creates two triangles; ASA shows they are congruent; CPCTC gives equal opposite sides',
        'All four angles are 90°',
      ],
      correct: 1,
    },
    {
      id: 'q10',
      type: 'choice',
      text: 'A quadrilateral has opposite sides that are equal (AB = CD and BC = DA). Is it necessarily a parallelogram?',
      options: [
        'No — equal opposite sides only mean it\'s a rectangle',
        'Yes — by SSS, the diagonal divides it into two congruent triangles; CPCTC gives equal alternate interior angles; therefore the opposite sides are parallel, making it a parallelogram.',
