...
import geoPerpBisectorUrl from '../diagrams/geo-perp-bisector.svg?url'
import geoAngleBisectorUrl from '../diagrams/geo-angle-bisector.svg?url'
import geoConstructibleUrl from '../diagrams/geo-constructible-numbers.svg?url'
import quizPerpBisectorUrl from '../diagrams/quiz-perp-bisector-find.svg?url'
import quizAngleBisectorUrl from '../diagrams/quiz-angle-bisector-find.svg?url'
...
function draw(){
  ...
  [{p:A,l:'A',ox:-14,oy:16},{p:B,l:'B',ox:8,oy:16}].forEach(function(v){
    ...
    ctx.fillStyle=TEXT;ctx.font='bold 12px Georgia';ctx.textAlign='left';ctx.fillText(v.l,v.p.x+v.ox,v.p.y+v.oy);
  });
  ...
  if(p>0.3){
    ...
    ctx.globalAlpha=1;
    ctx.beginPath();ctx.arc(Pm.x,Pm.y,5,0,2*Math.PI);ctx.fillStyle=AMBER;ctx.fill();
    ctx.beginPath();ctx.arc(Qm.x,Qm.y,5,0,2*Math.PI);ctx.fillStyle=AMBER;ctx.fill();
    ctx.fillStyle=AMBER;ctx.font='bold 12px Georgia';ctx.textAlign='left';
  ...
  ctx.beginPath();ctx.arc(Bv.x,Bv.y,5,0,2*Math.PI);ctx.fillStyle=TEXT;ctx.fill();
  ctx.fillStyle=TEXT;ctx.font='bold 12px Georgia';ctx.textAlign='center';ctx.fillText('B',Bv.x,Bv.y+18);
  ...
  if(p>0.15){
    ...
    ctx.beginPath();ctx.arc(D.x,D.y,5,0,2*Math.PI);ctx.fillStyle=NAVY;ctx.fill();
    ctx.beginPath();ctx.arc(E.x,E.y,5,0,2*Math.PI);ctx.fillStyle=NAVY;ctx.fill();
  ...
  if(p>0.4){
    ...
    ctx.beginPath();ctx.arc(Fb.x,Fb.y,5,0,2*Math.PI);ctx.fillStyle=AMBER;ctx.fill();
    ctx.fillStyle=AMBER;ctx.font='bold 12px Georgia';ctx.textAlign='left';ctx.fillText('F',Fb.x+8,Fb.y+4);
    ctx.globalAlpha=1;
  }
  // Bisector ray
  if(p>0.65){
    ctx.globalAlpha=Math.min(1,(p-0.65)/0.2);
    ctx.strokeStyle=GREEN;ctx.lineWidth=2.5;ctx.setLineDash([8,4]);
...
function draw(){
  ...
  ctx.fillStyle=GREEN;ctx.font='italic 11px Georgia';ctx.textAlign='left';
  ctx.fillText('Constructible with compass + straightedge',38,116);
  ctx.fillStyle=RED;ctx.font='italic 11px Georgia';
...
export default {
    ...
    blocks: [
      {
        type: 'prose',
        paragraphs: [
          'The ancient Greeks imposed a strict constraint on geometric construction: **only two tools**. A **straightedge** (no measurements) draws lines through two points. A **compass** draws circles and transfers distances. No ruler, no protractor, no calculator. These two tools directly implement Euclid\'s first three postulates.',
          'Under these constraints, the Greeks could construct an extraordinary variety of figures — angle bisectors, perpendicular bisectors, square roots, regular polygons. They also identified three problems they could not solve. It took **2,000 years** to prove those three problems genuinely impossible.',
        ],
      },
      {
        type: 'prose',
        paragraphs: [
          '**Perpendicular bisector:** Draw arcs of equal radius from each endpoint A and B. The two intersection points P and Q (one above, one below the segment) are equidistant from both A and B. Connecting P and Q with a straightedge gives the perpendicular bisector. **Proof:** By SSS congruence — each intersection point is equidistant from A and B by equal radii, so the triangles formed are congruent, forcing a right angle at midpoint M.',
        ],
      },
      {
        type: 'image',
        src: geoPerpBisectorUrl,
        alt: 'Perpendicular bisector construction: arcs from A (blue) and B (red) intersect at P and Q; line PQ bisects AB at right angle at midpoint M',
        caption: 'Construction: arcs from A and B with equal radius → intersection P, Q → line PQ is the perpendicular bisector. M bisects AB; ∠PMA = 90°.',
      },
      {
        type: 'prose',
        paragraphs: [
          '**Angle bisector:** Given an angle at vertex V, draw an arc centered at V intersecting both rays at D and E (equal radii). Then draw equal arcs from D and from E; they meet at a point F. Ray VF bisects the angle. **Proof:** Triangles VDF and VEF are congruent by SSS (VD = VE from the first arc; DF = EF from the second arcs; VF shared). Therefore ∠DVF = ∠EVF = θ/2.',
        ],
      },
      {
        type: 'image',
        src: geoAngleBisectorUrl,
        alt: 'Angle bisector construction: arc from V hits rays at D and E; equal arcs from D and E meet at F; VF is the bisector dividing the angle into two equal θ/2 parts',
        caption: 'Construction: arc from V at D, E → equal arcs from D, E → intersection F → bisector VF. Proved by SSS.',
      },
      {
        type: 'prose',
        paragraphs: [
          '**Constructible numbers** are exactly the lengths that can be built from 1 by compass and straightedge: by adding, subtracting, multiplying, dividing, and most importantly **taking square roots**. Any length satisfying a polynomial of degree 2ⁿ over the rationals is constructible. Square roots are always constructible. But cube roots — like ∛2 — require degree 3, which is outside the reach of these tools.',
        ],
      },
      {
        type: 'image',
        src: geoConstructibleUrl,
        alt: 'Two columns: Constructible numbers (√2, √5, φ, standard angles) vs Not Constructible (∛2, π, cos(20°), regular 7-gon)',
        caption: 'Constructible: any length reachable by square roots. Not constructible: cube roots and transcendentals like π.',
      },
      {
        type: 'callout',
        kind: 'important',
        title: 'Impossibility is provable — not just unsolved',
        body: 'The three classical problems are not unsolved — they are **proven impossible**. Wantzel (1837) proved trisection and cube-doubling impossible using Galois theory. Lindemann (1882) proved squaring the circle impossible (π is transcendental). "Impossible" in mathematics means structurally unreachable — no amount of cleverness overcomes an algebraic barrier.',
      },
      {
        type: 'viz',
        id: 'ScienceNotebook',
        props: { lesson: LESSON_GEO_2_2 },
        mathBridge: 'Work through the interactive notebook: (1) step through the perpendicular bisector construction — watch the equal arcs create the bisector and verify SSS at each stage; (2) step through the angle bisector — verify the SSS proof; (3) use the freehand construction tool to attempt the impossible: try to trisect 63° (= 3×21°) and see why the constructions always fail; (4) answer the postulate challenges.',
      },
    ],
  },
  ...
  quiz: [
    ...
    },
    {
      id: 'q5',
      type: 'choice',
      diagram: quizPerpBisectorUrl,
      diagramAlt: 'Segment AB with midpoint M. AM = 3x + 1 and MB = 5x − 7. Perpendicular bisector drawn at M.',
      text: 'Line ℓ is the perpendicular bisector of AB, so M is the midpoint. With AM = 3x + 1 and MB = 5x − 7, find the total length AB.',
      options: [
        'AB = 26',
        'AB = 13',
        'AB = 4',
    ...
    },
    {
      id: 'q6',
      type: 'choice',
      diagram: quizAngleBisectorUrl,
      diagramAlt: 'Angle of 84° at vertex V with bisector VF dividing it into two equal halves labelled x.',
      text: 'Ray VF bisects the 84° angle at V. What is each half-angle x?',
      options: [
        'x = 84°',
        'x = 42°',
        'x = 21°',
    ...
    },
    {
      id: 'q7',
      type: 'choice',
      text: 'Why is a ruler banned from classical construction? After all, a ruler can draw straight lines just like a straightedge.',
      options: [
        'The ruler makes longer lines than the straightedge',
        'A ruler carries measurement marks — using them would allow constructions not derivable from Euclid\'s postulates, which permit only "draw a line through two given points" (no lengths specified)',
        'Rulers were not invented when Euclid was alive',
      ],
      correct: 1,
    },
    {
      id: 'q8',
      type: 'choice',
      text: 'Gauss proved at age 19 that a regular 17-gon is constructible. Which of the following correctly explains why, while a regular 7-gon is not?',
      options: [
        '17 is larger than 7, so the 17-gon fits more accurately',
        '17 is a Fermat prime (17 = 2^(2^2) + 1), so it satisfies the constructibility criterion. 7 is a prime but NOT a Fermat prime — it cannot be expressed as 2^(2^k) + 1 — so the 7-gon requires a degree-3 field extension, unreachable by compass and straightedge.',
        '7-gons have odd sides, which are not constructible',
      ],
      correct: 1,
    },
    {
      id: 'q9',
      type: 'choice',
      text: 'The proof that the perpendicular bisector construction works uses which congruence criterion?',
      options: [
        'SAS — two sides and the included angle',
        'SSS — three sides (the two equal radii from each endpoint, plus the shared segment)',
    ...
    },
    {
      id: 'q10',
      type: 'choice',
      text: 'A student claims: "I trisected a 90° angle with compass and straightedge. I used three equal arcs." Why is this NOT a counterexample to the impossibility of trisection?',
      options: [
        'It is a counterexample — if 90° can be trisected, all angles can',
        '90° is special: one-third of 90° is 30°, which is constructible independently (from an equilateral triangle). The impossibility is for *arbitrary* angles — in particular, 60° cannot be trisected because cos(20°) satisfies an irreducible cubic.',
        'Three equal arcs are not allowed — only two arcs per step',
      ],
      correct: 1,
    },
  ],
