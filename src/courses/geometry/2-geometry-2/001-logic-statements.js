...
function render(){
  ...
  forms.forEach(function(f,i){
    ...
    ctx.fillStyle=NAVY;ctx.font='10px Georgia';ctx.textAlign='center';
    ctx.fillText('If:',x+colW/2,startY+72);
    ctx.fillStyle=TEXT;ctx.font='10px Georgia';
    var n1=wrapText(f.P,x+colW/2,startY+84,colW-12,13);

    ctx.fillStyle=GREEN;ctx.font='10px Georgia';
    ...
    // Equivalence markers
    if(i===0||i===3){
      ctx.fillStyle=PURPLE;ctx.font='bold 9px sans-serif';
      ctx.fillText('EQUIVALENT',x+colW/2,startY+rowH-8);
    }
    if(i===1||i===2){
      ctx.fillStyle=AMBER;ctx.font='bold 9px sans-serif';
      ctx.fillText('EQUIVALENT',x+colW/2,startY+rowH-8);
    }
  });

  // Equivalence bracket between 0 and 3
  ctx.strokeStyle=PURPLE;ctx.lineWidth=1.5;ctx.setLineDash([4,3]);
...
var fwBg=isDark?'#172554':'#eff6ff';
var bkBg=isDark?'#052e16':'#f0fdf4';
var defs=[
  {
    term:'Midpoint',
...
defs.forEach(function(d){
  ...
  fw.style.cssText='padding:8px 10px;background:'+fwBg+';border-radius:7px;border-left:3px solid '+d.color+';';
  ...
  bk.style.cssText='padding:8px 10px;background:'+bkBg+';border-radius:7px;border-left:3px solid '+d.color+';';
