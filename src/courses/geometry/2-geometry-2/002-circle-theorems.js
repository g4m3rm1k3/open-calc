...
function draw(){
  ...
  ctx.fillStyle=NAVY;ctx.fillText('Central angle ∠AOB = '+centralAng.toFixed(1)+'°',14,H-25);
  ctx.fillStyle=GREEN;ctx.fillText('Inscribed angle ∠APB = '+inscribedAng.toFixed(1)+'° (always half the central angle)',14,H-10);

  // Ratio
  var ratio=(centralAng/inscribedAng).toFixed(2);
  ctx.fillStyle=AMBER;ctx.font='bold 14px Georgia';ctx.textAlign='right';
...
function draw(){
  ...
  ctx.fillStyle=TEXT;ctx.font='12px Georgia';ctx.fillText('Drag P',lCx,H-28);
  ctx.fillStyle=NAVY;ctx.font='bold 13px Georgia';ctx.fillText("Thales' Theorem",lCx,28);
  ...
  ctx.fillStyle=PURPLE;ctx.fillText('∠AP₂B = '+ang2+'°',rCx+30,H-14);
  ctx.fillStyle=equal?'#1a3a2a':'#dc2626';ctx.font='bold 13px Georgia';
  ctx.fillText(equal?'Equal! ✓':'Move P₁, P₂ to same arc',rCx,H-42);
  ctx.fillStyle=TEXT;ctx.font='bold 13px Georgia';ctx.fillText('Equal Inscribed Angles',rCx,28);
}
draw();`,
      outputHeight: 380,
    },
...
function draw(){
  ...
  // Circle
  ctx.strokeStyle=BORDER;ctx.lineWidth=1.5;ctx.fillStyle='rgba(59,130,246,0.04)';
  ctx.beginPath();ctx.arc(cx,cy,R,0,2*Math.PI);ctx.fill();ctx.stroke();
  ctx.beginPath();ctx.arc(cx,cy,3,0,2*Math.PI);ctx.fillStyle=TEXT;ctx.fill();
  ctx.fillStyle=TEXT;ctx.font='bold 12px Georgia';ctx.textAlign='center';ctx.fillText('O',cx,cy-10);
...
function draw_invalid(){
  ctx.fillStyle=RED;ctx.font='13px Georgia';ctx.textAlign='center';ctx.fillText('P must be outside the circle',W/2,H/2);
}
draw();`,
      outputHeight: 420,
    },
