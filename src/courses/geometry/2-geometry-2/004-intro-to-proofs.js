...
import geoTwoColumnUrl from '../diagrams/geo-two-column-proof.svg?url'
import geoProofStrategiesUrl from '../diagrams/geo-proof-strategies.svg?url'
...
export default {
    ...
    blocks: [
      {
        type: 'prose',
        paragraphs: [
          '**A proof is a finite sequence of statements** where each follows logically from the given information, a definition, a postulate, or a previously proved theorem. Every single step must have an explicit justification — there are no "obvious" steps in a proof.',
          'The two-column format is the clearest way to write proofs: left column states each claim; right column cites the exact reason. When you get to a QED box (□), every step is accounted for.',
        ],
      },
      {
        type: 'image',
        src: geoTwoColumnUrl,
        alt: 'Two-column proof: Statement column on left, Reason column on right. Five steps from Given through SSS congruence to CPCTC conclusion, ending with QED box.',
        caption: 'Two-column format: every statement pairs with its exact justification. No gaps allowed.',
      },
      {
        type: 'prose',
        paragraphs: [
          '**Diagrams illustrate — they do not prove.** "It looks equal" is never a valid justification. A proof must work for *all* valid configurations, not just the one drawn. Two proofs that look identical on paper may fail for slightly different configurations if the reasoning depends on the diagram.',
          '**Four proof strategies:** the choice depends on the form of the conclusion. If you can see the direct path from given to conclusion, go direct. If the conclusion is a negative or existence claim, contradiction is often cleaner. If the implication is easier to prove in reverse, use the contrapositive. If different cases genuinely behave differently, split and handle each.',
        ],
      },
      {
        type: 'image',
        src: geoProofStrategiesUrl,
        alt: 'Four proof strategy boxes: Direct (assume P, derive Q), Contradiction (assume not-Q, derive absurdity), Contrapositive (prove not-Q implies not-P), Cases (split into exhaustive subcases)',
        caption: 'Choose the strategy based on the form of the claim. Direct is the default; use the others when the path to the conclusion is clearer in a different direction.',
      },
      {
        type: 'callout',
        kind: 'important',
        title: 'The test for a complete proof',
        body: 'A valid proof must convince a logically perfect reader who knows all relevant definitions and theorems but assumes nothing not explicitly stated. Every step needs a reason. Any gap — however small — makes the proof incomplete. CPCTC is always used *after* congruence is established, never before.',
      },
      {
        type: 'viz',
        id: 'ScienceNotebook',
        props: { lesson: LESSON_GEO_2_3 },
        mathBridge: 'Work through the notebook: (1) in the proof verifier, identify which steps are missing justifications; (2) practice building a two-column proof from scratch — the proof-builder cell checks each step; (3) try the common-errors spotter and identify the logical flaw in each faulty proof.',
      },
    ],
  },
  ...
  quiz: [
    ...
    },
    {
      id: 'q5',
      type: 'choice',
      text: 'A proof contains the step: "AB = CD because they look the same in the diagram." Why is this step invalid?',
      options: [
        'Diagrams in geometry are always accurate, so this is actually fine',
        'A proof must cite a specific justification (a given, a definition, a theorem, or an earlier step) — "looks the same" is a visual impression, not a logical justification. The diagram could be drawn inaccurately or might not represent all cases.',
        'The step should say "AB ≅ CD" instead of "AB = CD"',
      ],
      correct: 1,
    },
    {
      id: 'q6',
      type: 'choice',
      text: 'You want to prove: "If △ABC is not equilateral, then at least two sides have different lengths." Which strategy is most natural?',
      options: [
        'Proof by contradiction — assume the conclusion is false (all sides equal) and show this contradicts the hypothesis that △ABC is not equilateral',
        'Proof by cases — consider the cases n = 0, 1, 2',
        'Direct proof — start from "not equilateral" and chain logical steps',
      ],
      correct: 0,
    },
    {
      id: 'q7',
      type: 'choice',
      text: 'In a two-column proof, what is the justification for the statement "AB = AB"?',
      options: [
        'Given',
    ...
    },
    {
      id: 'q8',
      type: 'choice',
      text: 'The Contrapositive of "If ∠A = ∠B, then △ABC is isosceles" is:',
      options: [
        'If △ABC is isosceles, then ∠A = ∠B',
        'If △ABC is NOT isosceles, then ∠A ≠ ∠B',
        'If ∠A ≠ ∠B, then △ABC is NOT isosceles',
      ],
      correct: 2,
    },
    {
      id: 'q9',
      type: 'choice',
      text: 'A student says "I checked 500 triangles and the angle sum was always 180°, so the theorem is proved." What is the correct response?',
      options: [
        'The student is correct — 500 cases is sufficient evidence for a geometric theorem',
        'Checking specific cases is evidence (inductive reasoning) but not proof. A proof must show the statement holds for ALL triangles, including infinitely many not yet checked. One counterexample would disprove the theorem even after 500 confirmations.',
        'The student needs to check at least 1,000 cases to have a valid proof',
      ],
      correct: 1,
    },
    {
      id: 'q10',
      type: 'choice',
      text: 'In a proof, you write: "Therefore ∠1 + ∠2 = 180°." What type of justification must accompany this statement?',
      options: [
        'No justification is needed if the statement is clearly true',
        'A specific theorem, definition, postulate, or earlier proof step that logically forces ∠1 + ∠2 = 180°, such as "Linear Pair Theorem" or "definition of supplementary angles"',
        'The words "obviously" or "clearly"',
      ],
      correct: 1,
    },
  ],
