...
import quizSectorAreaUrl from '../diagrams/quiz-sector-area-find.svg?url'
...
function drawParallelogram(progress){
  ...
  ctx.fillStyle=MUTED;ctx.font='12px Georgia';ctx.textAlign='center';ctx.fillText('h='+h,ox+slant-18,oy-h/2);
...
function drawTriangle(progress){
  ...
  ctx.fillStyle=MUTED;ctx.font='12px Georgia';ctx.textAlign='right';ctx.fillText('h='+h,tx-5,oy-h/2);
...
function drawTrapezoid(progress){
  ...
  ctx.fillStyle=MUTED;ctx.font='12px Georgia';ctx.textAlign='center';
...
function draw(){
  ...
  ctx.strokeStyle=GRID;ctx.lineWidth=1;ctx.beginPath();ctx.roundRect(px,py,pw,H-60,10);ctx.stroke();

  ctx.fillStyle=NAVY;ctx.font='bold 14px Georgia';ctx.textAlign='center';
  ctx.fillText('Arc Length',px+pw/2,py+22);
  ctx.font='13px Georgia';ctx.fillStyle=RED;
  ctx.fillText('s = r·θ',px+pw/2,py+42);
  ctx.fillStyle=TEXT;
  ctx.fillText('= '+r+' × '+theta.toFixed(2),px+pw/2,py+60);
  ctx.fillStyle=NAVY;ctx.font='bold 14px Georgia';
  ctx.fillText('= '+arcLen.toFixed(1)+' units',px+pw/2,py+80);

  ctx.fillStyle=GRID;ctx.fillRect(px+20,py+92,pw-40,1);

  ctx.fillStyle=GREEN;ctx.font='bold 14px Georgia';
  ctx.fillText('Sector Area',px+pw/2,py+114);
  ctx.font='13px Georgia';ctx.fillStyle=GREEN;
  ...
  ctx.fillStyle=GRID;ctx.fillRect(px+20,py+184,pw-40,1);

  ctx.fillStyle=AMBER;ctx.font='12px Georgia';
  ...
  ctx.fillStyle=MUTED;ctx.font='11px Georgia';
  ctx.fillText('This sector = '+fracCirc+'% of circle',px+pw/2,py+255);

  // Center dot
  ctx.beginPath();ctx.arc(cx,cy,4,0,2*Math.PI);ctx.fillStyle=TEXT;ctx.fill();
  ctx.fillStyle=TEXT;ctx.font='12px Georgia';ctx.textAlign='center';ctx.fillText('O',cx,cy+16);
...
function draw(){
  ...
  for(var i=0;i<n;i++){
    ...
    ctx.strokeStyle=NAVY;ctx.lineWidth=0.8;
    ctx.beginPath();ctx.moveTo(lcx,lcy);ctx.arc(lcx,lcy,r,a,a+theta);ctx.closePath();ctx.fill();ctx.stroke();
  }
  ctx.strokeStyle=NAVY;ctx.lineWidth=2;ctx.beginPath();ctx.arc(lcx,lcy,r,0,2*Math.PI);ctx.stroke();
  ...
  ctx.fillStyle=RED;ctx.font='12px Georgia';ctx.textAlign='center';
  ...
  ctx.fillStyle=AMBER;ctx.font='bold 13px Georgia';ctx.textAlign='right';
...
export default {
    ...
    blocks: [
      {
        type: 'prose',
        paragraphs: [
          '**Every polygon area formula traces back to A = b × h** (rectangle). The key is how each shape relates to a rectangle:',
          '**Parallelogram:** shear the rectangle horizontally. Same base, same height, same area. A = bh.',
          '**Triangle:** half a parallelogram (draw the diagonal). A = ½bh.',
          '**Trapezoid:** two triangles, or equivalently a parallelogram with average base (a+b)/2. A = ½(a+b)h.',
          '**Circle:** Archimedes unrolled the circle into sectors arranged into an almost-rectangle of height r and width πr. A = r × πr = πr².',
        ],
      },
      {
        type: 'image',
        src: geoAreaFormulasUrl,
        alt: 'Area formula derivations: rectangle A=bh, parallelogram (shear), triangle (half parallelogram), circle (limit of regular polygon), trapezoid, sector A=½r²θ, Heron\'s formula.',
        caption: 'Every formula is a rearrangement of A = b×h. Height always means perpendicular distance, never slant length.',
      },
      {
        type: 'prose',
        paragraphs: [
          '**Radians are the natural angle unit.** One radian is the angle subtended by an arc equal in length to the radius. In radians, the formulas become perfectly clean: **arc length = rθ**, **sector area = ½r²θ**. In degrees, every formula requires a π/180 conversion factor. Radians eliminate that permanently.',
          '**2π radians = 360°**. One full rotation. π radians = 180°. π/2 radians = 90°.',
        ],
      },
      {
        type: 'callout',
        kind: 'important',
        title: 'Height ≠ side length — the most common area error',
        body: 'In every polygon area formula, "height" means the perpendicular distance between the two parallel sides — the altitude. It is NOT the slant side length. For a parallelogram with a slanted side, the height is measured at right angles to the base. Using the slant side gives an answer that is always too large.',
      },
      {
        type: 'viz',
        id: 'ScienceNotebook',
        props: { lesson: LESSON_GEO_2_4 },
        mathBridge: 'Work through the notebook: (1) drag the parallelogram slider to see how the shear transformation preserves area; (2) the Archimedes unrolling animation — drag the sector-count slider to see the circle approximate a rectangle with dimensions r × πr; (3) the radian efficiency cell — compare the formulas with degrees vs radians and see how the conversion factor disappears.',
      },
    ],
  },
  ...
  quiz: [
    ...
    },
    {
      id: 'q5',
      type: 'choice',
      diagram: quizSectorAreaUrl,
      diagramAlt: 'Circle with radius 6, sector with central angle 2π/3 radians (120°) shaded. Find the sector area x.',
      text: 'A sector has radius r = 6 and central angle θ = 2π/3 radians. Using A = ½r²θ, find the sector area.',
      options: [
        '12π',
        '6π',
        '4π',
    ...
    },
    {
      id: 'q6',
      type: 'choice',
      text: 'A parallelogram has base 8 and a slanted side of length 5. The perpendicular height is 4. What is the area?',
      options: [
        '40 — base × slant side',
        '32 — base × perpendicular height',
        '20 — half of base × height',
      ],
      correct: 1,
    },
    {
      ...
      options: [
        '3π/4',
        '135π/180 = 3π/4 (same answer expressed differently)',
        'Both A and B are correct — they are the same value',
      ],
      correct: 2,
    },
    {
      id: 'q8',
      type: 'choice',
      text: 'A circle with circumference 10π. What is its area?',
      options: [
        '25π',
        '100π',
        '50π',
    ...
    },
    {
      id: 'q9',
      type: 'choice',
      text: 'A trapezoid has parallel sides 5 and 9, and height 6. What is its area?',
      options: [
        '42',
        '84',
        '54',
    ...
    },
    {
      id: 'q10',
      type: 'choice',
      text: 'Archimedes derived A = πr² for a circle by imagining the circle cut into many thin sectors and rearranged. What does the rearranged shape approximate?',
      options: [
        'A square with side πr/2',
        'A rectangle with height r and width πr (= circumference/2), giving area r × πr = πr²',
        'A right triangle with legs r and 2πr',
      ],
      correct: 1,
    },
  ],
