...
export default function RealWorldMVT() {
  const isDark = useIsDark();
  const C = isDark ? darkColors : lightColors;
  const [fnIdx, setFnIdx] = useState(0);
  const PRESETS = [
    { label: 'Temperature', fn: x => 60 + 10 * Math.sin(x), domain: [0, Math.PI], fnTex: '60 + 10\sin(x)' },
    { label: 'Speed', fn: x => 40 + 20 * Math.cos(x), domain: [0, Math.PI], fnTex: '40 + 20\cos(x)' },
    { label: 'Power', fn: x => 100 + 30 * Math.sin(2 * x), domain: [0, Math.PI], fnTex: '100 + 30\sin(2x)' },
  ];
  ...
  return (
    <div style={{ fontFamily: 'var(--font-sans)', padding: '4px 0', maxWidth: 800, margin: '0 auto', color: C.textMain, background: C.bg }}>
      <h2 style={{ fontSize: 26, fontWeight: 700, textAlign: 'center', marginBottom: 8, color: C.textMain }}>Real-World Applications: Mean Value Theorem for Integrals</h2>
      <div style={{ textAlign: 'center', color: C.textMuted, fontSize: 16, marginBottom: 12 }}>
        The theorem guarantees that for any continuous real-world process, there is a moment when the instantaneous value equals the average over the interval.
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 16, alignItems: 'center' }}>
        <label style={{ fontWeight: 500 }}>a (start):
        ...
        <label style={{ fontWeight: 500 }}>b (end):
      ...
      <div style={{ background: C.cyanBg, borderLeft: `3px solid ${C.cyanMain}`, borderRadius: 8, padding: 18, marginTop: 24, color: C.textMain, marginBottom: 16 }}>
        <b style={{ color: C.cyanMain }}>Real-World meaning:</b> If the average temperature over a day is 70°F, there must be a moment when the temperature was exactly 70°F. The same logic applies to speed, power, and any continuous process.
      </div>
    </div>
  );
}
