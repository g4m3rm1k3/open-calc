# Stage 5, Lesson 5.18 — Improper Integrals
...
Lesson 5.16 closed with `sp.limit(impulse_finite, T, sp.oo)` — taking
...
(Lesson 5.13) to infinite limits of integration or to integrands with
...
with a variable, integrate normally, then take a limit (Lesson 5.1).
...
without full evaluation, and finish Lesson 5.16's damped-force impulse
...
- **Limits at infinity** — Lesson 5.1/5.2.
- **FTC, definite integrals** — Lesson 5.13, 5.14.
- **The unfinished impulse calculation** — Lesson 5.16's closing
...
\dfrac{x^{1-p}}{1-p}$ (power rule, Lesson 5.4) — as $x\to\infty$, this
...
**one-sided limit** approaching it — precisely Lesson 5.1's one-sided
...
Theorem's spirit, Lesson 5.2, applied to areas instead of function
...
# e^(-x^2) has no elementary antiderivative (Lesson 5.14), but its
...
# sympy can confirm this directly (using the error function from Lesson 5.14)
...
### Completing Lesson 5.16's Impulse Calculation

Lesson 5.16 computed the total impulse from $F(t)=F_0te^{-kt}$ over a
...
Matches Lesson 5.16's Challenge 3 test exactly (`F0/k**2`) — confirming
...
3. **Evaluation**: `sp.integrate` handles the limit internally,
   producing $F_0/k^2$ directly, matching Lesson 5.16's
...
take a one-sided limit — direct reuse of Lesson 5.1's one-sided limit
machinery.
...
**Completion**: Lesson 5.16's impulse calculation is a genuine,
...
constant-multiple property from Lesson 5.13). By the comparison test,
