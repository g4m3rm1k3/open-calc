# Stage 5, Lesson 5.14 — The Fundamental Theorem of Calculus
...
Lesson 5.13 defined the definite integral as a limit of Riemann
...
**backward** to compute integrals, replacing Lesson 5.13's infinite
summation process with ordinary algebra. It also finally cashes in
Lesson 5.9's Consequence 2 (any two functions with the same derivative
...
Lesson 5.7's position function from a velocity function — undoing
...
of integration (Lesson 5.13's limit of ever-finer sums) to the finite,
...
- **Riemann sums, the definite integral** — Lesson 5.13.
- **MVT's Consequence 2: same derivative implies differ by a
  constant** — Lesson 5.9, the load-bearing fact this theorem relies
  on directly.
- **Differentiation rules for power, trig, exponential functions** —
  Lessons 5.4–5.7, run backward here.
- **Position/velocity/acceleration/jerk chain** — Lesson 5.7, undone
...
**Proof sketch**: by the definition of the derivative (Lesson 5.3),

$$A'(x) = \lim_{h\to0}\frac{A(x+h)-A(x)}{h} = \lim_{h\to0}\frac{1}{h}\int_x^{x+h}f(t)\,dt$$

(using additivity, Lesson 5.13, to isolate the extra sliver of area
...
Theorem for Integrals, a close cousin of Lesson 5.9's MVT, to make
...
$F'(x)=f(x)=A'(x)$ — same derivative — so by **Lesson 5.9's
...
# Compare directly to Lesson 5.13's Riemann sum, which needed 1024
# rectangles to get close to this same number
print(f"Lesson 5.13's midpoint Riemann sum at n=1024 was: 6.66666604")
...
existing precisely because of Lesson 5.9's Consequence 2 (infinitely
...
Lesson 5.12: Kepler's equation $M=E-e\sin E$ has no closed-form
...
numerical methods (Riemann sums, Lesson 5.13; Newton's method, Lesson
...
### Application: Reversing Lesson 5.7's Motion Chain

Lesson 5.7 differentiated position → velocity → acceleration → jerk.
...
t = sp.symbols('t')
v = 3*t**2 - 12*t + 9   # a velocity function (matches Lesson 5.7's example, differentiated)
...
over — the same "signed area" caution from Lesson 5.13 applied to a
...
1. **FTC Part 1**: guarantees an accumulation function (running
   ...
   meaningful at all.
2. **Lesson 5.9's Consequence 2, reused**: every antiderivative of
...
$F$ — the constant cancels, thanks to Lesson 5.9's Consequence 2.
This is *why* `sp.integrate` never sums rectangles: it finds an
antiderivative algebraically and subtracts.

**Antiderivative table**: every Lesson 5.4–5.7 differentiation rule,
reversed.
...
**4. ★** Using FTC Part 1 and the chain rule (Lesson 5.5), find
