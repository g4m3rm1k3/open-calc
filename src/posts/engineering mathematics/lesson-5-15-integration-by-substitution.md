# Stage 5, Lesson 5.15 — Integration by Substitution
...
Lesson 5.14's antiderivative table reversed the *basic* differentiation
rules — power, sine, exponential — but said nothing about reversing
the **chain rule** (Lesson 5.5), which is where most real integrals
...
variable-feed-rate velocity profile — a case where Lesson 5.14's
...
Leibniz's differential notation choices (Lesson 5.3's history): writing
...
- **The chain rule** — Lesson 5.5, reversed directly by this lesson.
- **FTC, antiderivative table** — Lesson 5.14.
- **Recognizing composed expressions in the tree differentiator's
  `'pow'` branch** — Lesson 5.5, the exact pattern this lesson learns
  to spot in reverse.
...
3. Rewrite the entire integral in terms of $u$ only (every $x$ and
   $dx$ replaced).
4. Integrate with respect to $u$ (using Lesson 5.14's table).
...
Lesson 5.5's fix to the tree differentiator's `'pow'` branch: that
...
example, not a plain power-rule reversal Lesson 5.14's table alone
could handle.
...
substitution is needed at all: Lesson 5.14's table alone has no entry
...
2. **Substitution**: $u=t^2$, $du=2t\,dt$, reducing the integral to
   $\frac12\int\cos u\,du$ — Lesson 5.14's basic table now applies
   directly.
3. **Back-substitution and initial condition**: $u\to t^2$, then the
   constant resolved via $s(0)=0$ — exactly Lesson 5.14's motion-chain
...
chatter) often require substitution to integrate — Lesson 5.14's
...
- Programmatic pattern-matching for the substitution signal
  ...
  chain-rule recognition built into Lesson 5.5's tree differentiator,
  now run in reverse
