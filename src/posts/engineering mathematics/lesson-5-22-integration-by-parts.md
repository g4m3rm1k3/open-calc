# Stage 5, Lesson 5.22 — Integration by Parts
...
Lesson 15 reversed the chain rule. This lesson reverses the
**product rule** (Lesson 4) — the tool needed whenever an integrand
...
approximation Lesson 11 introduced in its first, quadratic-order
...
- **The product rule** — Lesson 4, reversed directly here.
- **FTC, basic antiderivatives** — Lesson 14.
...
Start from the product rule (Lesson 4): $(uv)'=u'v+uv'$. Integrate
...
consequence of the power rule, Lesson 4). `integrals` builds the
...
exactly like any other FTC Part 2 evaluation (Lesson 14).
...
improper integral, formally the subject of Lesson 18 — treated here
...
# As T -> infinity (the true total impulse, previewing Lesson 18)
...
as the upper bound grows without bound is precisely what Lesson 18
...
2. **Integration by parts**: produces the boundary term $uv$ plus a
   ...
   immediately handled by Lesson 14's table.
3. **Definite integral**: evaluated over a finite window, matching
   FTC Part 2's evaluation procedure (Lesson 14) exactly.
4. **Forward reference**: taking the finite window's upper bound to
   infinity previews Lesson 18's improper integrals directly, using
   nothing beyond an ordinary limit (Lesson 1) applied to an already-
...
direct, honest preview of Lesson 18's improper integrals via a
