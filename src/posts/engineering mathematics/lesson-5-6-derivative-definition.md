# Stage 5, Lesson 5.6 — The Derivative: Definition, Geometric Meaning, and Notation
...
defined as exactly the limit Lesson 1 built the machinery for:
...
1660s-70s (Lesson 1's history), and their two notations survive
...
Lesson 5, looks almost like ordinary fraction cancellation in his
...
- **Limits, the $\varepsilon$-$\delta$ definition** — Lesson 1.
...
- **Continuity** — Lesson 2, needed for the differentiability
  comparison.
...
taking the limit — exactly the factoring technique from Lesson 1's
opening $0/0$ example, now applied specifically to derivative
computations. At $x=2$: $f'(2)=4$ — and this matches Lesson 1's
`central_difference(f, 2, h)` result for $f(x)=x^3$ conceptually
(that lesson used $x^3$, giving $f'(2)=12$, exactly the "true value"
Lesson 1 compared its floating-point approximations against without
...
Lesson 4 formalizes, essentially instantly. The second block instead
...
notation makes the chain rule (Lesson 5) and implicit
...
Lesson 2), which is exactly continuity's condition.
...
formalized in Lesson 11): expanding $f(x+h)$ and $f(x-h)$ around
...
symbolically (a full Taylor-series proof is Lesson 11's job). This
...
derived properly in Lesson 4"), and the central-difference number
...
2. **Algebraic derivation**: expand, cancel the problematic $h$,
   take the limit — the same technique from Lesson 1's opening
...
plus the limit laws (Lesson 2), not as a separately-assumed fact.

<details><summary>Answer</summary>
$$\frac{d}{dx}[cf(x)] = \lim_{h\to0}\frac{cf(x+h)-cf(x)}{h} = \lim_{h\to0}c\cdot\frac{f(x+h)-f(x)}{h}$$
By the constant-multiple limit law (Lesson 2), a constant factor
can be pulled outside the limit:
$$= c\lim_{h\to0}\frac{f(x+h)-f(x)}{h} = c\cdot f'(x) \qquad\blacksquare$$
This is genuinely representative of how *all* of Lesson 4's
differentiation rules will be derived: each one reduces, via algebra,
to the raw limit definition plus Lesson 2's limit laws — nothing
