# Stage 5, Lesson 5.17 — Newton's Method: Root Finding by Linearisation
...
Lesson 2's bisection algorithm found roots reliably but slowly —
...
**derivative**. At each guess, follow the tangent line (Lesson 8's
...
5A by putting nearly every tool built since Lesson 3 to direct,
...
- **Bisection, IVT** — Lesson 2, the algorithm this one upgrades.
- **Linear approximation / tangent lines** — Lesson 8, the direct
...
Given a guess $x_n$ near a root of $f$, Lesson 8's tangent-line
approximation is:

$$L(x) = f(x_n) + f'(x_n)(x-x_n)$$

Instead of using $L$ to estimate $f$ near $x_n$ (Lesson 8's job),
...
Bisection's error roughly **halves** every iteration (Lesson 2's
...
"error ratio at each step" way Lesson 3 measured central
...
change is bracketed (Lesson 2's IVT guarantee), Newton's method has
...
1. **Newton's method, derived**: follow the tangent line to
   $f(E)=E-e\sin E-M$ down to zero — exactly Lesson 8's linear
...
tracing, the same technique from Lesson 3.
...
bound established in Lesson 9: $|f(x)-L(x)|\le\frac{M}{2}(x-a)^2$.
...
— precisely Lesson 9's bound, applied with $a=x_n$ and evaluated
near $r$:
...
consequence of Lesson 9's linear-approximation error bound, applied
