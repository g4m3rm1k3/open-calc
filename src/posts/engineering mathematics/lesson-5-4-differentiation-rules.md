# Stage 5, Lesson 5.4 — Differentiation Rules: Power, Sum, Product, Quotient
...
Lesson 5.3 computed every derivative from the raw limit definition —
...
that same limit definition and Lesson 5.2's limit laws, so that
...
- **Binomial expansion pattern** — Lesson 5.3's Extension problem
...
By the sum limit law (Lesson 5.2), this splits into two separate
...
hence continuous, Lesson 5.3), the first fraction $\to g'(x)$, the
...
integers) or implicit differentiation (Lesson 5.5, for rational
...
def differentiate(expr):
    ...
    if kind == 'pow':
        ...
        if base != ('var',):
            raise NotImplementedError("Only x^n supported directly (no chain rule yet -- Lesson 5.5)")
        n = exponent[1]
