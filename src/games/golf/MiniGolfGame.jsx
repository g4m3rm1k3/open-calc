...
/* Mobile / narrow-screen layout — side panels were a fixed 500px combined
   width that simply didn't fit a phone. Stack the canvas above scrollable
   control strips instead of trying to cram three columns side by side. */
@media (max-width: 860px) {
  .golf-wrap .g-topbar { flex-wrap: wrap; padding: 8px 10px; gap: 8px; }
  .golf-wrap .level-pills { overflow-x: auto; max-width: 100%; -webkit-overflow-scrolling: touch; }
  .golf-wrap .g-main { flex-direction: column; overflow-y: auto; }
  .golf-wrap .g-canvas-wrap { flex: none; height: 46vh; min-height: 240px; order: -1; }
  .golf-wrap .g-left, .golf-wrap .g-right {
...
export default function MiniGolfGame({ params = {}, height: rootHeight = 640, onClose }) {
  ...
  useEffect(() => {
    ...
    function onResize() {
      ...
      renderer.setSize(W, H);
      // Camera distance was a fixed (0,26,30) regardless of hole size or window
      // shape, so on a wide/short canvas the green only filled a small box in
      // the middle with empty fog on every side. Refit on every resize too.
      fitCameraToLevel(LEVELS[currentLevel]);
    }

    // Frame the current hole's green so it fills the viewport instead of
    // floating tiny in the middle of it — distance scales with hole size and
    // with the canvas's current aspect ratio, not just a fixed camera spot.
    function fitCameraToLevel(lv) {
      const wrap = q('canvas-wrap');
      if (!wrap || !camera || !lv) return;
      const aspect = Math.max(0.35, wrap.clientWidth / Math.max(1, wrap.clientHeight));
      const gSize = lv.greenSize || { w: 40, d: 20 };
      const halfExtent = Math.max(gSize.w, gSize.d) / 2 * 1.25; // padding so edge walls aren't clipped
      const vFov = camera.fov * Math.PI / 180;
      const distForHeight = halfExtent / Math.tan(vFov / 2);
      const distForWidth  = halfExtent / (Math.tan(vFov / 2) * aspect);
      const dist = Math.max(distForHeight, distForWidth, 16);
      const elevAngle = Math.PI / 4.2; // same elevated 3/4 angle the scene always used
    ...
    function resolveCollisions(oldPos, newPos, vel) {
      ...
      for (let mesh of obstacles3d) {
        ...
        if (wd.wallType === 'spinner') {
          ...
          const ax1  = wd.cx + cosA*wd.armLen, az1 = wd.cz - sinA*wd.armLen;
          const ax2  = wd.cx - cosA*wd.armLen, az2 = wd.cz + sinA*wd.armLen;
          const adx  = ax2-ax1, adz = az2-az1, al2 = adx*adx + adz*adz;

          // Swept check: a fast putt can move further in one frame than the arm
          // is wide, so testing only the end-of-frame position let the ball
          // tunnel straight through. Sample along the path from oldPos to p and
          // stop at the first sample that's inside the arm.
          const SUBSTEPS = 6;
          let hitDd = Infinity, hitCpx = 0, hitCpz = 0, hitSx = p.x, hitSz = p.z;
          for (let s = 0; s <= SUBSTEPS; s++) {
            const st = s / SUBSTEPS;
            const sx = oldPos.x + (p.x - oldPos.x) * st;
            const sz = oldPos.z + (p.z - oldPos.z) * st;
            const t   = Math.max(0, Math.min(1, ((sx-ax1)*adx + (sz-az1)*adz) / al2));
            const cpx = ax1+t*adx, cpz = az1+t*adz;
            const dd  = Math.sqrt((sx-cpx)*(sx-cpx) + (sz-cpz)*(sz-cpz));
            if (dd < BALL_R + wd.armW/2) { hitDd = dd; hitCpx = cpx; hitCpz = cpz; hitSx = sx; hitSz = sz; break; }
          }
          if (hitDd < Infinity) {
            const dd  = hitDd > 0.001 ? hitDd : 0.001;
            const nx2 = (hitSx - hitCpx) / dd, nz2 = (hitSz - hitCpz) / dd;
            const pen = BALL_R + wd.armW/2 - dd;
            p.x = hitSx + nx2*pen; p.z = hitSz + nz2*pen;
    ...
    function updateStats() {
      ...
      const windMag = lv.wind.length();
      q('sv-wind').textContent  = windMag > 0.05 ? windMag.toFixed(1) + ' m/s' : 'none';
    }
    ...
    function goLevel(idx) {
      ...
      buildLevelPills();
      buildLevel(idx);
      // Sync the friction slider to this hole's intended surface — without this
      // every hole simulated whatever friction the player last left the slider at,
      // so holes designed to feel icier/sandier (e.g. the loop at μ=0.09) never did.
      q('s-friction').value = LEVELS[idx].friction;
      updateStats(); // refresh wind/speed/etc readouts immediately so a windy hole shows it before the first putt
    ...
    // renderer.setSize() pins the canvas to an inline pixel width/height, so the
    // canvas stays stuck at whatever size it was when the lab booted unless
    // something re-measures it. A browser `resize` event only fires when the
    // actual window changes — it never fires when this lab is opened in a small
    // floating window and then maximized (that's a layout/CSS change, not a
    // window resize), so the canvas was staying pinned to the tiny windowed
    // size even after maximizing. A ResizeObserver catches that case too.
    const resizeObserver = new ResizeObserver(() => onResize());
    resizeObserver.observe(q('canvas-wrap'));
  ...
  return (
    ...
    >
      ...
      <GameHelp
        title="Mini Golf — Physics Putting"
        sections={[
          { heading: 'What you’re learning', body: 'Each of the 9 holes isolates one physics idea — friction (F=μmg), momentum & reflection off banks, centripetal force on the spinner and loop, projectile motion off ramps, and superposition of forces in the crosswind hole. Watch the colored force arrows and the live physics panel on the right as you putt.' },
          { heading: 'How to play', body: 'Drag on the green to orbit the camera. Use the Aim angle, Power, and Loft sliders to set up your shot, then press PUTT (or hit Space). The ball rolls under friction and gravity, bounces off banks and obstacles, and reacts to wind on some holes.' },
          { heading: 'Goal', body: 'Sink the ball in as few strokes as possible and try to beat par on each hole. Your best stroke count and completion status are tracked per hole in the pills along the top.' },
        ]}
      />
        ...
        <div className="g-main">
          ...
          <div className="g-right">
            ...
            <div className="stat-grid">
              ...
              <div className="stat-card"><div className="sk">Wind</div><div className="sv" style={{color:'#9b5de5'}} id="mg-sv-wind">none</div></div>
            </div>
