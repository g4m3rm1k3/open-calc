...
class ChannelState {
  ...
  vars: Map<number | string, number>;
  localVars: Map<number | string, number>[];
  rVars: Map<number | string, number>;
  callStack: CallFrame[];
  pointer: number;
  blocks: ParsedBlock[];
  ...
  constructor(channelDef: ChannelDef, machDef: MachineDefinition) {
  ...
  clone(): ChannelState {
    const c = new ChannelState(
      { id: this.id, label: this.label, tag: this.tag ?? undefined },
      this.machDef,
    );
    Object.assign(
      c,
...
export class GCodeParser {
  ...
  constructor(machDef: MachineDefinition) {
    this.machDef = machDef;
    this.dialect = machDef.dialect || "fanuc";
    this.syntax = machDef.syntax || {};
  }

  parse(src: string): Map<string, ParsedBlock[]> {
  ...
  _parseLine(raw: string, lineIndex: number): ParsedBlock | null {
    const t = raw.trim();
  ...
  _extractWords(str: string): ParsedWords {
  ...
  _detectWaitCode(words: ParsedWords, str: string): WaitCode | null {
    ...
    for (const [code, def] of Object.entries(this.machDef.waitCodes)) {
      ...
      if (code.startsWith("M")) {
        ...
        if (mWords.includes(mVal)) {
          const pq = words.P ?? words.Q;
          return { code, def, syncId: pq != null ? String(pq) : null };
        }
      }
...
class ExpressionEvaluator {
  ...
  resolve(val: number | string | null): number | null {
  ...
  _evalExpr(expr: string | number): number {
    ...
    e = e.replace(/#\[([^\]]+)\]/g, (_, name) => {
      return String(this.ch.vars.get(`_NAME_${name.toUpperCase()}`) ?? 0);
    });
    ...
    do {
      prev = e;
      e = e.replace(/#(\d+)/g, (_, n) => {
        return String(this.ch.vars.get(parseInt(n)) ?? 0);
      });
      safety++;
    ...
    e = e.replace(/\bR(\d+)\b/gi, (_, n) => {
      return String(this.ch.rVars.get(parseInt(n)) ?? 0);
    });

    // Step 4: Fanuc trig (degrees) — now safe since vars are substituted
    e = e.replace(/SIN\[([^\]]+)\]/gi, (_, x) =>
      String(Math.sin((this._evalExpr(x) * Math.PI) / 180)),
    );
    e = e.replace(/COS\[([^\]]+)\]/gi, (_, x) =>
      String(Math.cos((this._evalExpr(x) * Math.PI) / 180)),
    );
    e = e.replace(/TAN\[([^\]]+)\]/gi, (_, x) =>
      String(Math.tan((this._evalExpr(x) * Math.PI) / 180)),
    );
    e = e.replace(
      /ATAN\[([^\]]+)\]/gi,
      (_, x) => String((Math.atan(this._evalExpr(x)) * 180) / Math.PI),
    );
    e = e.replace(/SQRT\[([^\]]+)\]/gi, (_, x) => String(Math.sqrt(this._evalExpr(x))));
    e = e.replace(/ABS\[([^\]]+)\]/gi, (_, x) => String(Math.abs(this._evalExpr(x))));
    e = e.replace(/ROUND\[([^\]]+)\]/gi, (_, x) =>
      String(Math.round(this._evalExpr(x))),
    );
    e = e.replace(/FIX\[([^\]]+)\]/gi, (_, x) => String(Math.trunc(this._evalExpr(x))));
    e = e.replace(/FUP\[([^\]]+)\]/gi, (_, x) => String(Math.ceil(this._evalExpr(x))));
...
type PathRecorder = { pathPoints: PathPoint[]; stats: EngineStats };

export class CNCEngine {
  machDef: MachineDefinition;
  channels: ChannelState[];
  parser: GCodeParser;
  programs: Map<string, ParsedBlock[]>;
  syncPoints: Map<string, Set<number>>;
  pathPoints: PathPoint[];
  stats: EngineStats;
  backplotPathPoints: PathPoint[];
  backplotStats: EngineStats;
  evalors: ExpressionEvaluator[];
  toolTable: Record<string, ToolDefinition>;
  toolUnits: string;
  _maxSteps: number;
  ...
  setToolTable(toolTable: Record<string, ToolDefinition> = {}, units = "mm"): void {
    this.toolTable =
      toolTable && typeof toolTable === "object" ? toolTable : {};
    this.toolUnits = units === "inch" ? "inch" : "mm";
  }

  _selectParsedEntryBlocks(parsed: Map<string, ParsedBlock[]>, sourceName = "MAIN"): ParsedBlock[] {
  ...
  loadPrograms(sources: string | Record<string, string>): void {
  ...
  _getMainBlocks() {
    const mainKey =
      [...this.programs.keys()].find(
        (k) =>
          (k === "MAIN" && (this.programs.get(k)?.length ?? 0) > 0) ||
          k.startsWith("O0000") ||
          k.match(/^O\d{1,4}$/),
  ...
  _splitChannelBlocks(mainBlocks: Map<string, ParsedBlock[]>): void {
  ...
  stepChannel(channelId = 0): StepResult {
  ...
  _executeBlock(ch: ChannelState, ev: ExpressionEvaluator, pathOnly: boolean, recorder: PathRecorder | null = null): void {
  ...
  _applyGCode(ch: ChannelState, ev: ExpressionEvaluator, g: number, w: ParsedWords, b: ParsedBlock): void {
  ...
  _applyMCode(ch: ChannelState, ev: ExpressionEvaluator, m: number, w: ParsedWords, pathOnly: boolean, recorder: PathRecorder | null = null): void {
  ...
  _applyMotion(ch: ChannelState, ev: ExpressionEvaluator, w: ParsedWords, b: ParsedBlock, pathOnly: boolean, recorder: PathRecorder | null = null): void {
  ...
  static prerun(src: string | Record<string, string>, machDefId = "fanuc_mill"): PrerunResult {
...
export const TOOL_TEMPLATES: Record<string, Record<string, ToolDefinition | Record<string, ToolDefinition>>> = {
  // ── Mill cutting tools ────────────────────────────────────────────────────
  mill: {
    end_mill_4fl: {
      cls: "mill",
...
export function getHolderProfile(holderKey: string): ToolDefinition | null {
  const holders = TOOL_TEMPLATES.holders as Record<string, ToolDefinition>;
  return holders[holderKey] || null;
}
