// Rebuild src/courses from scratch. Clean 2-level structure: course/chapter/files.
// Run once. Stop.
...
const DEST = path.join(ROOT, 'src/courses')
...
function copyLessons(srcDir, destDir) {
  ...
  console.log(`  ${destDir.replace(DEST+'/', '')}: ${files.length} files`)
  return files.length
}
...
// --- Grouped courses: each src folder = one chapter ---
function grouped(course, chapters) {
  console.log(`\n${course}/`)
  chapters.forEach(([src, slug], i) => copyLessons(path.join(SRC, src), path.join(DEST, course, `${i+1}-${slug}`)))
}
...
// --- Multi-chapter: lessons split by chapter field ---
function multi(course, src, map) {
  ...
  for (const [folder, flist] of Object.entries(buckets)) {
    ...
    fs.mkdirSync(destDir, { recursive: true })
    for (const f of flist) fs.copyFileSync(path.join(srcDir, f), path.join(destDir, f))
...
single('cli-0',          'cli-0',           'command-line')
...
single('dp-1',           'dp-1',            'foundations')
single('gcode-parser-1', 'gcode-parser-1',  'math-and-geometry-foundations')
single('javascript-1',   'javascript-1',    'javascript-core-foundations')
single('logic-0',        'logic-0',         'boolean-logic-and-digital-fundamentals')
single('nosql-1',        'nosql-1',         'nosql-databases')
single('plc-0',          'plc-0',           'plc-programming-fundamentals')
single('tetris-1',       'tetris-1',        'build-tetris')

// ============================================================
// MULTI-CHAPTER COURSES (split by chapter field)
// ============================================================
